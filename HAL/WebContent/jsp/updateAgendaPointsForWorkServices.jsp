...
function checkField(updateAgendaPointsForWorkServices)  {
  if (updateAgendaPointsForWorkServices.<%=REMARKS %>.value=="") {
    alert("please enter remarks");
     document.updateAgendaPointsForWorkServices.<%=REMARKS %>.focus();
     document.updateAgendaPointsForWorkServices.<%=REMARKS %>.select();
  return false;
  }
}
...
if( text=="R")
{
document.updateAgendaPointsForWorkServices.<%=REMARKS %>.style.display="";
document.updateAgendaPointsForWorkServices.<%=REMARKS %>.focus();
...
{
document.updateAgendaPointsForWorkServices.<%=REMARKS %>.style.display="none";
...
<DIV class="Clear"></DIV>
<FORM name="updateAgendaPointsForWorkServices" action="" method=post
	enctype="multipart/form-data">
<h6>Update Agenda Points For Work Services</h6>
<div class="Clear"></div>
<Label class="common">Agenda No.<span> *</span></Label> <Label
	class="value"><%=agendaList.get(0).getAgendaNo()%></Label> <INPUT
	type=hidden value="<%=agendaList.get(0).getAgendaNo()%>"
	name="<%=AGENDA_NO%>">
...
<Label>Agenda Date<span> *</span></Label> <input tabindex="1"
	...
	onClick="setdate('<%=date%>',document.updateAgendaPointsForWorkServices.<%= AGENDA_DATE%>,event);"
	class="calender" /> <Label>Agenda Time<span> *</span></Label> <input
	tabindex="1" type="text" maxlength="8" name="<%=AGENDA_TIME %>"
	value="<%=agendaList.get(0).getAgendaTime() %>"
	validate="Agenda Time,string,yes"
	onblur="checkTimeAgendaPoints('updateAgendaPointsForWorkServices','agendaTime');">
<INPUT type=hidden value="<%=userName%>" name="<%=CHANGED_BY%>">

<Label>Agenda Type<span> *</span> </Label> <input tabindex="1"
...
<label>Agenda Place<span> *</span></Label> <textarea class="large"
	name="<%=AGENDA_PLACE %>" onkeyup="chkLength(this,30);"
...
<label>Agenda Summary</Label> <textarea tabindex="1" class="large"
	onkeyup="chkLength(this,400);" name="<%=AGENDA_SUMMARY %>"
	validate="Agenda Summary,String,yes"><%=agendaList.get(0).getAgendaSummary() %></textarea>

<DIV class="Clear"></DIV>
<%if(agendaList.get(0).getStatus().equalsIgnoreCase("A")){%> <label>Agenda
Accepted</label> <input tabindex="1" type="radio" class="radio"
	name="<%=AGENDA_ACCEPTED %>" value="A" CHECKED
	onclick="makeChoice(this);" /> <label>Agenda Not Accepted</label> <input
	type="radio" tabindex="1" class="radio" name="<%=AGENDA_ACCEPTED %>"
	value="R" onclick="makeChoice(this);" /> <%}else{ %> <label>Agenda
Accepted</label> <input tabindex="1" type="radio" class="radio"
	name="<%=AGENDA_ACCEPTED %>" value="A" onclick="makeChoice(this);" /> <label>Agenda
Not Accepted</label> <input tabindex="1" type="radio" class="radio"
	name="<%=AGENDA_ACCEPTED %>" value="R" CHECKED
	onclick="makeChoice(this);" /> <%} %> <Label>Remarks </Label> <input
	tabindex="1" type="text" id="remarksId" maxlength="50"
	name="<%=REMARKS %>" value="<%=agendaList.get(0).getAgenaRemarks() %>"
	onblur="checkField(this.form)" />
<div class="Clear"></div>
<label class="large">Location of Prev Uploaded File </Label> <Label
	class="valueNoWidth"><%=agendaList.get(0).getResoursePath()%></Label>

<DIV class="Clear"></DIV>
<Label class="large">Agenda Summary Upload</Label> <input tabindex="1"
	type="file" id="uploadFile" name="<%=UPLOAD_FILENAME %>"
...
<div class="bottom"><INPUT class=button id="updatebutton"
	accessKey=a
	onClick="submitForm('updateAgendaPointsForWorkServices','updateAgendaPointsForWorkServices?method=editUpdateAgendaPointsForWorkServices','jsImport');"
