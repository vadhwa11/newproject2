...
<%System.out.println("medExamId ====>"+medExamObj.getId()); %>
<%--<div class="Block">
...
<input tabindex="1" type="text"   id="" name="<%=AUTHORITY_OF_BOARD %>"maxlength="15" value="<%=medExamObj.getAuthority() %>"
	onKeyUp="limitText(this,100);"  />

 <% }else{%>
<input tabindex="1" type="text"  id="" name="<%=AUTHORITY_OF_BOARD %>" maxlength="15" value="IAP 4303(4th Ed)"	onKeyUp="limitText(this,100);"  />

 <% }%>

 <label>Place  </label>
 <% if(medExamObj.getPlace()!=null){%>
<input tabindex="1" type="text" maxlength="50" class="auto" size="30" id="" name="<%=PLACE %>" value="<%=medExamObj.getPlace() %>"
	onKeyUp="limitText(this,20);"  />

 <% }else{%>
<input tabindex="1" type="text" maxlength="50" class="auto" size="30" id="" name="<%=PLACE %>"
	onKeyUp="limitText(this,20);"  />
...
<input	tabindex="1" name="<%=DATE_DISCHARGE %>" class="" value="<%=HMSUtil.convertDateToStringWithoutTime(medExamObj.getDateDischarge())%>"
	validate="DOB,date,no" maxlength="10"
	onKeyUp="mask(this.value,this,'2,5','/');" />

 <% }else{%>
<input	tabindex="1" name="<%=DATE_DISCHARGE %>" class=""
...
<div class="clear"></div>
  ...
  <input	tabindex="1" name="<%=DATE_OF_RELEASE %>" class="" value="<%=HMSUtil.convertDateToStringWithoutTime(medExamObj.getDateRelease())%>"
	validate="DOB,date,no" maxlength="10"
	onKeyUp="mask(this.value,this,'2,5','/');" />

 <% }else{%>
  <input	tabindex="1" name="<%=DATE_OF_RELEASE %>" class="" value="<%=date %>"
	validate="DOB,date,no" maxlength="10"
	onKeyUp="mask(this.value,this,'2,5','/');" />
 <% }%>
<%--   <img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
...
<label>Name  </label>
  <input	type="text" value="<%= visit.getHin().getSFirstName() %>" name="<%=FULL_NAME%>"	tabindex="2" maxlength="100" readonly="readonly"/>
...
<input	tabindex="1" name="<%=DATE_OF_BIRTH %>"  value="<%=date %>"
 ...
 <% }else{%>
 <input	type="text"  name="<%=TRADE%>"	tabindex="2"  maxlength="20" />
 ...
 <input	type="text" maxlength="20"  value="<%=visit.getHin().getServiceYears() %>" name="<%=TOTAL_SERVICE%>"	tabindex="2" />

 <% }else{%>
<input	type="text" maxlength="20"  value="" name="<%=TOTAL_SERVICE%>"	tabindex="2" />
...
<label>Permanent Address  </label>
 ...
 <input	type="text"  name="<%=PAST_MEDICAL_HISTORY%>"	tabindex="2"  value="<%=medExamObj.getPastmedicalhistory() %>" maxlength="200"/>
  <% }else{%>
<input	type="text"  name="<%=PAST_MEDICAL_HISTORY%>"	tabindex="2" maxlength="200" />
 <% }%>
...
<td width="10%"><input size="11" tabindex="1" readonly="readonly" type="text"	name="<%=TO+inc1 %>" maxlength="10" value="<%=HMSUtil.convertDateToStringWithoutTime(setMedicalExam.getAddressto()) %>"/></td>
<% }else{%>
<td width="10%"><input size="11" tabindex="1" readonly="readonly" type="text"	name="<%=TO+inc1 %>" maxlength="10" />
</td>
<% }%>
<td>
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
...
<td width="10%"><input tabindex="1" type="text"	name="<%=P_NO+inc1 %>" maxlength="5" value="<%=setMedicalExam.getPno() %>"/></td>
<% }else{%>
<td width="10%"><input tabindex="1" type="text"	name="<%=P_NO+inc1 %>" maxlength="5" /></td>
<% }%>
...
<%
	...
	//System.out.println("this is else jsp");
inc1=1;%>
<tr>
...
<td width="10%">
<input tabindex="1" type="text" readonly="readonly"	name="<%=FROM+inc1 %>" maxlength="10" size="11" />
</td>
<td>
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender"
...
<td width="10%"><input size="11" tabindex="1" type="text" readonly="readonly"	name="<%=TO+inc1 %>" maxlength="10" />
</td>
<td>
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender"
...
<textarea rows="" cols="50" class="auto" maxlength="100"   	name="particularOfPreviousService" class="large" onkeyup="chkLength(this,150);" 
...
for(MasMedicalExaminationDetail setMedicalExam:medExamObj.getMasmedicaldetail()){
	...
	if(setMedicalExam.getParticular()!=null && setMedicalExam.getParticular().equalsIgnoreCase("particular")){
		...
		/*if(setMedicalExam.getSystemDiagnosis()!=null){
			icdName = setMedicalExam.getSystemDiagnosis().getSystemDiagnosisName()+ "["+ setMedicalExam.getSystemDiagnosis().getId()+ "]";

		}*/
		if(setMedicalExam.getMasIcd()!=null){
			icdName = setMedicalExam.getMasIcd().getIcdName()+ "["+ setMedicalExam.getMasIcd().getId()+ "]";
		...
		String principle="";
		  ...
		  if(setMedicalExam.getPrincipal() !=null){
			  principle=setMedicalExam.getPrincipal();
		  }
		  int icdId=0;
			/*if(setMedicalExam.getSystemDiagnosis()!=null){
...
<td width="10%"><input tabindex="1" type="text"	name="<%=ILLNESS+inc11 %>" maxlength="100" value="<%=setMedicalExam.getIllness() %>"/></td>
  <% }else{%>

<td width="10%"><input tabindex="1" type="text"	name="<%=ILLNESS+inc11 %>" maxlength="100" /></td>
 <% }%> --%>
 <td>
<input type="text" tabindex="1"	value="<%=icdName%>" id="<%=PRINCIPAL+inc11%>"  name="<%=PRINCIPAL+inc11%>"	class="auto"  size="30" />
<div id="ac2updatex2"	style="display: none;" class="autocomplete"></div>
<script type="text/javascript" language="javascript" charset="utf-8">
		 <%--new Ajax.Autocompleter('<%=PRINCIPAL+inc11%>','ac2updatex2','opd?method=autoCompleteForSystemDiagnosis',{parameters:'requiredField=<%=PRINCIPAL+inc11%>'}); --%>
		 new Ajax.Autocompleter('<%=PRINCIPAL+inc11%>','ac2updatex2','opd?method=getICDForIdList',{parameters:'requiredField=<%=PRINCIPAL+inc11%>'});
...
<input  type="text"	tabindex="1" name="<%=APPROXIMATE_DATE+inc11 %>" id="approximatedate" size="11"	validate="DOB,String,no" maxlength="30" />
 ...
 <!-- changed made by anshu starts for disease,wound or injury -->
  ...
  <td width="20%">
  <img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender" onclick="setdate('',medicalBoardExaminationReleaseDischarge.<%=APPROXIMATE_DATE+inc11%>,event);" />
  </td>
...
<%} %>
  ...
  <img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender"
	onclick="setdate('',medicalBoardExaminationReleaseDischarge.<%=APPROXIMATE_DATE2+inc11 %>,event);" />
  </td>
...
<td width="10%"><input tabindex="1" type="text"	name="<%=PRINCIPAL+inc11 %>" maxlength="10" /></td>
<td width="10%">
...
<%if(setMedicalExam.getParticulardate() != null){ %>
<td width="10%">
<input name="pDate<%=j %>" onkeyup="mask(this.value,this,'2,5','/');" value="<%=HMSUtil.convertDateToStringTypeDateOnly(setMedicalExam.getParticulardate()) %>" id="pDate<%=j %>" maxlength="10" validate="First Started on,date,no" size="11"  tabindex="1">
</td>
<%}else{ %>
<td width="10%">
...
<div class="Block">
<label>Signature of Witness</label> <input type="text"  name="" id="" value="" />
...
if(medExamObj.getRemarksTeath()!=null){%>
 ...
 <% }else{%>
 <textarea rows="" cols="60" maxlength="300"	name="<%=DENTAL_REMARKS %>" class="auto" 
 onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" 
 onkeyup="chkLength(this,299);"></textarea>
 <% }%>
...
<table border="0" align="center" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<th scope="col">Upload/View File</th>
		<th scope="col">Add</th>
...
<input tabindex="1" type="text"  size="10" id="height" class="auto"	name="<%=HEIGHT_WITHOUT_SHOOSE %>" onkeyup="isNumber1(this)" value="<%=medExamObj.getHeight() %>"
	maxlength="6" onblur="checkForWiegth(this.value,id);calculateBMI();" /><label class="unit2">cm</label>

 <% }else{%>
<input tabindex="1" type="text" size="10"  id="height"	class="auto"	name="<%=HEIGHT_WITHOUT_SHOOSE %>" onkeyup="isNumber1(this)"
	maxlength="6" onblur="checkForWiegth(this.value,id);calculateBMI();"  /><label class="unit2">cm</label>
...
<input tabindex="1" type="text"  id="weight" class="auto" size="10" name="<%=ACTUAL_WEIGHT %>" maxlength="6" value="<%=medExamObj.getActualweight() %>"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);calculateBMI();" /><label class="unit">kg</label>

 <% }else{%>
<input tabindex="1" type="text"  id="weight" class="auto" size="10"	name="<%=ACTUAL_WEIGHT %>" maxlength="6"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);calculateBMI();" /><label class="unit">kg</label>
...
<input tabindex="1" type="text" size="10" id="<%=IDEAL_WEIGHT %>" name="<%=IDEAL_WEIGHT %>" class="auto"	maxlength="3" value="<%=medExamObj.getIdealweight() %>"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);" /><label class="unit">kg</label>

 <% }else{%>
<input tabindex="1" type="text" size="10" id="<%=IDEAL_WEIGHT %>" name="<%=IDEAL_WEIGHT %>" class="auto"	maxlength="3"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);" /><label class="unit">kg</label>
...
<input tabindex="1" type="text" size="10" id="<%=OVER_WEIGHT %>" name="<%=OVER_WEIGHT %>" class="auto"	maxlength="3" value="<%=medExamObj.getOverweight() %>"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);" /><label class="unit2">%</label>
...
<input tabindex="1" type="text"  id="<%=WAIST %>" name="<%=WAIST %>" maxlength="4" value="<%=medExamObj.getWaist() %>"
	onKeyUp="limitText(this,6);" onblur="calculateWHR();" class="auto" size="10"  /><label class="unit2">cm</label>
...
<input tabindex="1" type="text" maxlength="20"  id="WHR" name="WHR" maxlength="4" value="<%=medExamObj.getWhr() %>"
	onKeyUp="limitText(this,6);" class="auto" size="10" /><input class="transparent" size="6">

 <% }else{%>
<input tabindex="1" type="text" maxlength="20"  id="WHR" name="WHR" maxlength="4"
	onKeyUp="limitText(this,6);" class="auto" size="10"  /><input class="transparent" size="6">
...
<input tabindex="1" type="text"  id="" name="<%=THICKNESS %>" maxlength="5" value="<%=medExamObj.getSignfoldthickness() %>"
	onKeyUp="limitText(this,6);" class="auto" size="10"  /><input class="transparent" size="6">

 <% }else{%>
<input tabindex="1" type="text" id="" name="<%=THICKNESS %>" maxlength="5"
...
<input tabindex="1" type="text" maxlength="18" 	name="<%= ARTERIAL_WALLS%>" class="auto" size="18"   value="<%=medExamObj.getArterialWalls() %>"/>
 <% }else{%>
 <input tabindex="1" type="text" maxlength="18"  value="Normal"	name="<%= ARTERIAL_WALLS%>" class="auto" size="18"  />
 <% }%>
...
<input tabindex="1" type="text" maxlength="50" 	name="<%= HEART_SIZE%>" class="auto" size="27"  value="<%=medExamObj.getHeartSize() %>"/>
 <% }else{%>
<input tabindex="1" type="text" maxlength="50" 	name="<%= HEART_SIZE%>" class="auto" size="27" value="Normal" />
 <% }%>


 <label>Sounds</label>
 <% if(medExamObj.getSounds()!=null){%>
 <input tabindex="1" type="text" maxlength="100"  name="<%= SOUND%>" class="auto"	size="27"  value="<%=medExamObj.getSounds() %>"/>

 <% }else{%>
 <input tabindex="1" type="text" maxlength="100"  name="<%= SOUND%>" class="auto"	size="27"  value="Normal" />
 <% }%>
<label>Rhythm</label>
<% if(medExamObj.getRhythm()!=null){%>
 <input tabindex="1" type="text" maxlength="18"  name="<%= RHYTHM%>" class="auto"	size="18"  value="<%=medExamObj.getRhythm() %>"/>

 <% }else{%>
 <input tabindex="1" type="text" maxlength="18"  name="<%= RHYTHM%>" class="auto"	size="18"  value="Regular" />
 <% }%>
...
<div class="Block">
<div class="clear"></div>
<label class=""> Respiratory System</label>
<% if(medExamObj.getRespiratorySystem()!=null){%>
  <input tabindex="1" type="text" maxlength="100" 	name="<%=RESPIRATORY_SYSTEM %>" class="auto" size="120" maxlength="100" value="<%=medExamObj.getRespiratorySystem() %>"/>
   <% }else{%>
  <input tabindex="1" type="text" maxlength="100" 	name="<%=RESPIRATORY_SYSTEM %>" class="auto" size="120" maxlength="100" />
 <% }%>
...
<input tabindex="1" type="text" maxlength="50" 	name="<%=HIGHER_MENTAL_FUNCTION %>" class="" size="20" maxlength="20" value="<%=medExamObj.getHigherMentalFunction() %>"/>
   <% }else{%>
<input tabindex="1" type="text" maxlength="50" 	name="<%=HIGHER_MENTAL_FUNCTION %>" class="" size="20" maxlength="20" value="Normal"/>
 <% }%>

<label > Speech</label>
<% if(medExamObj.getSpeech()!=null){%>
<input tabindex="1" type="text" maxlength="50" 	name="<%=SPEECH %>" class="" size="20" maxlength="20" value="<%=medExamObj.getSpeech() %>"/>
   <% }else{%>
<input tabindex="1" type="text" maxlength="50" 	name="<%=SPEECH %>" class="" size="20" maxlength="20" value="Normal"/>
 <% }%>


<label > Reflexes</label>
<% if(medExamObj.getReflexes()!=null){%>
<input tabindex="1" type="text" maxlength="50" 	name="<%=REFLEXES %>" class="" size="20" maxlength="20" value="<%=medExamObj.getReflexes() %>"/>
   <% }else{%>
<input tabindex="1" type="text" maxlength="50" 	name="<%=REFLEXES %>" class="" size="20" maxlength="20" value="Normal"/>
 <% }%>
...
<input tabindex="1" type="text" maxlength="6" 	name="<%=TREMORS %>" class="" size="20" maxlength="20" value="<%=medExamObj.getTremors() %>"/>
   <% }else{%>
<input tabindex="1" type="text" maxlength="6" 	name="<%=TREMORS %>" class="" size="20" maxlength="20" />
 <% }%>
...
<table width="50%" border="0" cellspacing="0" cellpadding="0">
	...
	<td>
		...
		<input tabindex="1" type="text" maxlength="4" class="auto" size="10" id="hrfw" name="<%=HEARING_R_F_W %>" value="<%=medExamObj.getEarHearingRfw()   %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%}else{ %>
		<input tabindex="1" value="600" type="text" maxlength="4" class="auto" size="10"  id="hrfw" name="<%=HEARING_R_F_W %>" onkeyup="isNumber1(this)"	maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%} %>
		cm
	</td>
	...
	<td>
		...
		<input tabindex="1" type="text" maxlength="4"class="auto" size="10"  id="hlfw" value="<%=medExamObj.getEarHearingLfw()  %>" name="<%=HEARING_L_F_W%>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%}else{ %>
		<input tabindex="1" value="600" type="text" maxlength="4" class="auto" size="10"  id="hlfw" name="<%=HEARING_L_F_W %>" onkeyup="isNumber1(this)"	maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%} %>
		cm
	</td>
	...
	<td>
		...
		<input tabindex="1" type="text" maxlength="4" class="auto" size="10"  id="bothfw"	name="<%=HEARING_BOTH_FW %>" value="<%= medExamObj.getEarHearingBothFw() %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%}else{ %>
		<input tabindex="1" value="600" type="text" maxlength="4" class="auto" size="10"  id="bothfw"	name="<%=HEARING_BOTH_FW %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%} %>
		cm
	</td>
	...
	<td>
		<% if(medExamObj.getHearingRcv() != null){ %>
 		<input tabindex="1" type="text" maxlength="4" class="auto" size="10"  id="hrcv" name="<%=HEARING_R_C_V %>" value="<%= medExamObj.getHearingRcv() %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%}else{ %>
		<input tabindex="1" value="600" type="text" maxlength="4" class="auto" size="10"  id="hrcv" name="<%=HEARING_R_C_V %>" onkeyup="isNumber1(this)"	maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%} %>
		cm
	</td>
	...
	<td>
	  ...
	  <input tabindex="1" type="text" maxlength="4" class="auto" size="10"  id="hlcv" name="<%=HEARING_L_C_V %>" value="<%= medExamObj.getHearingLcv() %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
	  <%}else{ %>
	    <input tabindex="1" value="600" type="text" maxlength="4" class="auto" size="10"  id="hlcv" name="<%=HEARING_L_C_V %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
	  <%} %>
	  cm
	...
	<td>
		 <% if(medExamObj.getHearingBothCv() != null){ %>
	 	 <input tabindex="1" type="text" maxlength="4" class="auto" size="10"  id="bothcv" name="<%=HEARING_BOTH_CV %>" value="<%= medExamObj.getHearingBothCv()  %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%}else{ %>
 		<input tabindex="1" value="600" type="text" maxlength="4" class="auto" size="10"  id="bothcv" name="<%=HEARING_BOTH_CV %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
		<%} %>
		cm
	</td>
	</tr>
	...
	<td>
	  	...
	  	<input tabindex="1" type="text" class="auto" size="19" name="<%=MOBILITYL %>" maxlength="20"  value="<%= medExamObj.getMobilityL()  %>"  />
		<%}else{ %>
 		<input tabindex="1" value="Normal" type="text" class="auto" size="19" name="<%=MOBILITYL %>"  maxlength="20"  />
		<%} %>
	</td>

	<td>
		  <% if(medExamObj.getMobilityR() != null){ %>
	  	  <input tabindex="1" type="text" class="auto" size="19" name="<%=MOBILITYR %>" maxlength="20"  value="<%= medExamObj.getMobilityR()  %>"  />
		  <%}else{ %>
 		  <input tabindex="1" value="Normal" type="text" class="auto" size="19" name="<%=MOBILITYR %>"  maxlength="20"  />
		  <%} %>
	</td>
	<td></td>
	</tr>
...
<label>Nose, Throat &amp; Sinuses</label>
	  ...
	  <input tabindex="1" type="text" maxlength="20" class="auto" size="19" id="bothcv" name="<%=NOSE_THROAT %>" value="<%= medExamObj.getNosethroat()  %>"  />
<%}else{ %>
 <input tabindex="1" type="text"  class="auto" size="19" id="bothcv" name="<%=NOSE_THROAT %>"  maxlength="20"  />
<%} %>

<label>Audiometry Record </label>
	  	 <% if(medExamObj.getAudiometryRecord() != null){ %>
	  <input tabindex="1" type="text" maxlength="20" class="auto" size="19"  id="bothcv" name="<%=AUDIOMETRY_RECORD %>" value="<%= medExamObj.getAudiometryRecord()  %>"  />
<%}else{ %>
 <input tabindex="1" type="text" class="auto" size="19"  id="bothcv" name="<%=AUDIOMETRY_RECORD %>"  maxlength="20"  />
<%} %>
<input name="Send" type="button"  class="buttonBig" value="Upload/ View" onClick="javascript:FileUploadWindow();" />
...
<input tabindex="1" type="text" maxlength="20" 	name="<%=MENSTRUAL_HISTORY %>" class="auto" size="20" maxlength="50" value="<%=medExamObj.getMenstrualHistory() %>"/>
   <% }else{%>
<input tabindex="1" type="text" maxlength="20" 	name="<%=MENSTRUAL_HISTORY %>" class="auto" size="20" maxlength="50" />
 <% }%>
...
<input tabindex="1" type="text"  	name="<%=NO_OF_PREGNANCY %>" class="auto" size="20" maxlength="2" value="<%=medExamObj.getNoOfPregnancies() %>"/>
   <% }else{%>
<input tabindex="1" type="text"  	name="<%=NO_OF_PREGNANCY %>" class="auto" size="20" maxlength="2" />
 <% }%>

<div class="clear"></div>
<label >No of Abortions</label>
<% if(medExamObj.getNoOfAbortions()!=null){%>
<input tabindex="1" type="text" 	name="<%=NO_OF_ABORTION %>" class="auto" size="20" maxlength="2" value="<%=medExamObj.getNoOfAbortions() %>"/>
   <% }else{%>
<input tabindex="1" type="text"  	name="<%=NO_OF_ABORTION %>" class="auto" size="20" maxlength="2" />
 <% }%>
<label >No. of Children</label>
<% if(medExamObj.getNoOfChildren()!=null){%>
<input tabindex="1" type="text" 	name="<%=NO_OF_CHILDREN %>" class="" size="20" maxlength="2" value="<%=medExamObj.getNoOfChildren() %>"/>
   <% }else{%>
<input tabindex="1" type="text"  	name="<%=NO_OF_CHILDREN %>" class="" size="20" maxlength="2" />
 <% }%>
...
<input tabindex="1" type="text" maxlength="20"  class="calDate" readonly="readonly " value="<%=HMSUtil.convertDateToStringWithoutTime(medExamObj.getLastConfinementDate()) %>"	name="<%=DATE_OF_LASTCONFINEMENT %>" maxlength="10" onKeyUp="mask(this.value,this,'2,5','/');" validate="Dental Date,date,no" />
   <% }else{%>
<input tabindex="1" type="text" maxlength="20"  class="calDate" readonly="readonly "	name="<%=DATE_OF_LASTCONFINEMENT %>" maxlength="10" onKeyUp="mask(this.value,this,'2,5','/');" validate="Dental Date,date,no" />
 <% }%>
...
<input tabindex="1" type="text"  	name="<%=VAGINAL_DISCHARGE %>" class="auto" size="20" maxlength="20" value="<%=medExamObj.getVaginalDischarge() %>"/>
   <% }else{%>
<input tabindex="1" type="text"  	name="<%=VAGINAL_DISCHARGE %>" class="auto" size="20" maxlength="20" />
 <% }%>
...
<input tabindex="1" type="text"  	name="<%=PROLAPSE %>" class="" size="20" maxlength="50" value="<%=medExamObj.getProlapse() %>"/>
   <% }else{%>
<input tabindex="1" type="text" 	name="<%=PROLAPSE %>" class="" size="20" maxlength="50" />
 <% }%>


<label >USG Abdomen</label>
<% if(medExamObj.getUsgAbdomen()!=null){%>
<input tabindex="1" type="text"  	name="<%=USG_ABORTION %>" class="auto" size="20" maxlength="200" value="<%=medExamObj.getUsgAbdomen() %>"/>
   <% }else{%>
<input tabindex="1" type="text" 	name="<%=USG_ABORTION %>" class="auto" size="20" maxlength="200" />
 <% }%>
...
<textarea rows="" cols="91" class="auto"   	name="commutationOfPensionRemarks" class="large" onkeyup="chkLength(this,100);" ><%=medExamObj.getCommandRemarks() %></textarea>
 <% }else{%>
<textarea rows="" cols="91" class="auto"  	name="commutationOfPensionRemarks" class="large" onkeyup="chkLength(this,100);" ></textarea>
 <% }%>
...
<input type="hidden" name="<%=MED_CAT_REC %>" id="medcatrec" value="<%=medcatrec %>"/>
...
<input tabindex="1" type="text" id="signidBy" name="<%= SIGNED_BY %>"  size="20" maxlength="100" value="<%=signedByAA %>" readonly="readonly" validate="Signed By,string,no"/>
<%--
<label >Send To</label>
<%
String sendTo="";
if(medExamObj.getMedicalExamType().equalsIgnoreCase("Med. Exam On Release/Discharge(AFMSF-18)"))
{
	sendTo="Co/Stn Cdr";
}
...
<input tabindex="1" type="text"  maxlength="100"  id="remarks"	name="<%=MEDICIN_REMARKS %>" class="Auto" size="20"  value="<%=medExamObj.getRemarks() %>"
/>
   <% }else{%>
<input tabindex="1"  type="text"  maxlength="100" id="remarks"	name="<%=MEDICIN_REMARKS %>" class="Auto" size="20" value=""/>
 <% }%>
...
{ %>
<input tabindex="1" type="text" name="aaFinalObservation" readonly="readonly" size="20" maxlength="100" value="<%=medExamObj.getAprovAuthFinalObservation() %>"/>
<%}else{ %>
<input tabindex="1" type="text" name="aaFinalObservation" readonly="readonly" size="20" maxlength="100" />

<%} %>
<label>Remark</label>
<textarea cols="20" rows="2" tabindex="2" maxlength="100" name="aaRemarks" onkeyup="chkLength(this,100);" tabindex="1" readonly="readonly"
onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" ><%=medExamObj.getApprovAuthRemarks() %></textarea>
<label >Signed By</label>
<% if(medExamObj.getApprovAuthSignedBy()!=null){ %>
<input tabindex="1" type="text" name="aaSignedBy" value="<%=medExamObj.getApprovAuthSignedBy()%>"  readonly="readonly" size="20" maxlength="100"/>
<%}else{ %>
<input tabindex="1" type="text" name="aaSignedBy" value=""  readonly="readonly" size="20" maxlength="100"/>
...
<div class="division"></div>
<div class="clear"></div>
<input tabindex="1" name="Button"	type="button" class="button" value="Update"	onClick="submitForm('medicalBoardExaminationReleaseDischarge','medicalExam?method=updateMedicalExamEntry&Labresult=<%=Labresult %>');" />
...
<input tabindex="1" name="Button"	type="button" class="buttonBig2" value="Initiate New Medical Exam"	onClick="submitForm('medicalBoardExaminationReleaseDischarge','medicalExam?method=initiateNewMedicalExam');" />
...
function jsViewFileUploadWindowGynaecology()
{
	  ...
	  var url="/hms/hms/medicalExam?method=viewUploadDocumentsDetails&hinId=<%=medExamObj.getHin().getId()%>&visitId=<%=medExamObj.getVisit().getId()%>&masExam_Id=<%=medExamObj.getId()%>&folderName=gynaecology";
  ...
  function showHideInvestigationTemplateCombo(valueOfTemplate){
		...
		if(checkTemplateId(valueOfTemplate)){
		  	...
		  	document.getElementById("copyPrevInvestigationTemplateDiv").style.display='none';
				...
				submitProtoAjaxWithDivName('medicalBoardExaminationReleaseDischarge','/hms/hms/opd?method=showGridForInvestigation&flag=Exam','gridview');
...
//	  new Ajax.Autocompleter('icdId'+iteration,'ac2updatex2','opd?method=autoCompleteForSystemDiagnosis',{parameters:'requiredField=icdId'+iteration});
 ...
 function addRow(){
	...
	{
		   var url="/hms/hms/medicalExam?method=viewUploadDocumentsDetails&hinId=<%=medExamObj.getHin().getId()%>&visitId=<%=medExamObj.getVisit().getId()%>&masExam_Id=<%=medExamObj.getId()%>&folderName=hearing";
 ...
 function submitForInvestigationPrintOut()
 {
 	...
 	if(orderNo!=null && orderNo!="" && hinId!=null && hinId!=""){
 		submitFormForButton('medicalBoardExaminationReleaseDischarge','/hms/hms/investigation?method=printResultValidationLab&parent='+orderNo+'&hinId='+hinId+'&flag='+flag);
 		}
 }
