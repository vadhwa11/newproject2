...
<%
	...
	System.out.println("box inside jspxxx" + box);
%>
...
function GoPage() {	
	var pgno = parseInt(dailyRoutineAddForm.gopage.value);
	var totalPages = parseInt(dailyRoutineAddForm.totalPages.value);
	if (pgno < 1 || pgno > totalPages)
	{
	alert('Invalid Page No!.....');
	return;
	}  
	dailyRoutineAddForm.pageno.value = pgno; 
	dailyRoutineAddForm.method="post";
	submitForm('dailyRoutineAddForm','hrRelated?method=getGridDataForDailyRoutineEmployeeAdd');
}

function goNext()
{
 var pgno = parseInt(dailyRoutineAddForm.pageno.value)+1;
 
 if (pgno > dailyRoutineAddForm.totalPages.value)
 {
...
{
	if (dailyRoutineAddForm.<%=EMPLOYEE_TO_BE_ADDED%>.length)
	{
			 for(var i = 0; i < dailyRoutineAddForm.<%=EMPLOYEE_TO_BE_ADDED%>.length; i++)
			 {
			  if (dailyRoutineAddForm.<%=EMPLOYEE_TO_BE_ADDED%>[i].checked == true)
             		return true;
			 }
	}
	else
	{
		if (dailyRoutineAddForm.<%=EMPLOYEE_TO_BE_ADDED%>.checked == true)
			return true;
	}
	return false;
}
...
function jsAdd()
{
		if (validateButton())
		{
			...
			{
				submitForm('dailyRoutineAddForm','hrRelated?method=AddDailyRoutineEntry');
	...
	function jsClose()
	{
	  window.opener.location.href = "hrRelated?method=showDailyRoutineRecords&toDate="+dailyRoutineAddForm.toDate.value+"&fromDate="+dailyRoutineAddForm.fromDate.value+"&dutyId="+dailyRoutineAddForm.dutyId.value;
	...
	function jsSearch()
	{
		if(dailyRoutineAddForm.searchServiceNo.value!="" || dailyRoutineAddForm.searchName.value!="" )
		{
			dailyRoutineAddForm.method="post";
			submitForm('dailyRoutineAddForm','hrRelated?method=searchDailyRoutineEntry');
		}
		else
    ...
    {
    	dailyRoutineAddForm.method="post";
 		submitForm('dailyRoutineAddForm','hrRelated?method=getDailyRoutineEmployeeLastDutyDetails&empData='+empData);
    }
...
<h6>Employee Addition for Detailing of Daily Routine, Scott & Sick
transfer duty entry</h6>
<DIV class="Clear"></DIV>
<form name="dailyRoutineAddForm"><input type="hidden"
...
<table align="center" width="100%" colspan="7" cellpadding="0"
	...
	<tbody>
		...
		<tr>
			...
			<td><a
				href="javascript:getLastDutyDetails(<%=masEmployee.getId()%>)"><%=msg%></a></td>
...
<table align="center" width="100%" colspan="3" cellpadding="0"
	...
	<tbody>
		<%
			    ...
			    for(HrDailyRoutineDutyEntry hrDailyRoutineDutyEntry :dutyEntryList){ 
			   
			    %>
		<tr>
			<td><%=HMSUtil.convertDateToStringWithoutTime(hrDailyRoutineDutyEntry.getDutyDate()) %></td>
			<td><%=hrDailyRoutineDutyEntry.getDutyTime().getDutyFromTime()%>-<%=hrDailyRoutineDutyEntry.getDutyTime().getDutyToTime()%></td>
		</tr>
		<%}
