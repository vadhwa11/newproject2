...
<form name="medExamSearch" method="post">
<div class="Block">
...
<input
	...
	class="calender"
	onClick="setdate('<%=currentDate %>',document.medExamSearch.<%=FROM_DATE%>,event)" />
...
<label> To Date<span>*</span> </label> <input type="text" id="ToDateId"
	...
	onClick="setdate('<%=currentDate %>',document.medExamSearch.<%=TO_DATE%>,event)" />
<div class="Clear"></div>
<label>Rank</label>
...
<input type="button" name="Search" value="Search" class="button" onClick="submitForm('medExamSearch','medicalExam?method=searchMedicalExamPerAuthority');" />
<div class="Clear"></div>
</div>
</form>
...
<div class="Clear"></div>
<form name="medicalExamPerAuthourity" method="post"
	action="">
   ...
   data_arr[<%= counter%>] = new Array();
      ...
      <%if(masexam.getMedicalExamType()!=null){%>
		...
		*/
		...
		{
			if(masexam.getPriority()==9)
			{
				ma_status="Rejected By Perusing II";
			}else
			{
				ma_status="New";
			}
...
%>
formName = "medicalExamPerAuthourity"
	start = 0
...
<%--
<input type="text" class="signPriority1" readonly="readonly" >
<label class="valueAutoPriority">Rejected By Perusing Authority(AIR HQ/AFRO)</label> --%>
