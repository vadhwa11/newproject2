...
<div id="hinDiv"><input type="hidden"
	...
	onblur="if(checkHinNo()){submitProtoAjax('opdPatientIssue','stores?method=showOPDPatientNIPApprovalGrid');}" />
</div>
<%} %>
...
<%}else{ %> <label>Employee No.</label> <input type="text" id="serviceNo"
	...
	onblur="getHinNo('opdPatientIssue','stores?method=getHinForPatientIssue');submitProtoAjax('opdPatientIssue','stores?method=showOPDPatientNIPApprovalGrid');"
...
<%}else{%>
		<% if (hinId !=0) { %>
			...
			submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showOPDPatientNIPApprovalGrid');
			
		<% 
		//System.out.println("(hinId !=0) ");
		}else if(patient.getId()!=null){ %>
		var prescriptionId ='<%=prescriptionId%>';
				submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showOPDPatientNIPApprovalGrid&prescriptionId='+prescriptionId);
...
/* function populateItem(val,inc){
	
 	if(val != "")
	{
	    var index1 = val.lastIndexOf(")");
	    var indexForItemId=index1;
...
function checkForPurchase(val,a,inc)
{
	  ...
	  if(pvms !="")
	  {
		  ...
		  ajaxFunctionForAutoCompleteInLPOGeneral('opdPatientIssue','opd?method=fillItemsInGrid&pvmsNo=' +  pvms , inc);
...
function fillPrescribeBy() 
{
	...
	if(document.getElementById('stockDetails')!=null)
		{
		trlength = document.getElementById('stockDetails').rows.length;
		}
		...
		if(saveFlag){
			fillItemsInGrid();submitForm('opdPatientIssue','stores?method=submitOPDPatientNIPDetails');
		}
}
