...
<table border="0" align="center" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<%int inc=1; %> <input type="text" value="" tabindex="1"
			...
			onblur="if(validateSurgeryForAutoComplete(this.value,'<%=inc %>')){submitProtoAjaxWithDivName('inpatientSurgeryRequisition','/hms/hms/fwc?method=fillChargeCode&rowVal=1','chargeCodeVal');}" />
		<div id="ac2update2"
			style="display: none; font-weight: normal; border: 1px solid black; padding-right: 10px; background-color: white;"></div>
		<script type="text/javascript" language="javascript" charset="utf-8">
			
			  new Ajax.Autocompleter('chargeCodeName1','ac2update2','fwc?method=getChargeCodeListForAutoComplete&deptId=<%=departmentId%>',{parameters:'requiredField=chargeCodeName1'});
			</script></td>
...
<div class="paddLeft300"><input name="Submit" type="button"
	...
	onclick="if(checkSubmitSurgery()){submitForm('inpatientSurgeryRequisition','fwc?method=submitSurgeryRequisitionDetailsForInpatient&orderNo=<%=orderNo %>');}" /></div>
</div>
