	...
	if(map.get("max") != null){
	 ...
	 if(map.get("message") != null)
	 {
		...
		for(StorePoHeader header: poHeader)
		{
			...
			yearId = (header.getFinancialYear()!=null?header.getFinancialYear().getId():0);
			...
			vendorId = (header.getSupplier()!=null?header.getSupplier().getId():0);	
		    vendorName = (header.getSupplier()!=null?header.getSupplier().getSupplierName():"");	
		    quotationId =(header.getQuotation()!=null?header.getQuotation().getId():0);
		    quotationNo =(header.getQuotation()!=null?header.getQuotation().getRequestNo():"");
		    stockistId= (header.getStockist()!=null?header.getStockist().getId():0);	
...
%>
 ...
 <div class="titleBg">
<h2>View/Update Purchase Order</h2>
</div>
...
<div class="clear" style="padding-top:10px;"></div>
	...
	<input	type="text" name="poNo" readonly="readonly" validate="PO Number,String,yes" tabindex=1 value="<%=PONo%>" id="poNo"/>
...
<select name ="ddlRequestYear" id="ddlRequestYear" onchange="getEnquiryList()">
	<%
		if(financialYearList.size()>0)
		{
			...
			for(Object[] fyear : financialYearList)
			{
				if(Integer.parseInt(fyear[0].toString()) == yearId)
				{
					%>
					<option value="<%=fyear[0]%>" selected="selected"><%=fyear[1]%></option>
				<%
				}
...
<input  type="text" class="calDate"  id="poDate" name="poDate" placeholder="DD/MM/YYYY" validate="Due Date,string,yes" value="<%=PODate%>" onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10" style="width: 120px"/>
	
<label> Delivery Date <span>*</span></label>
<input  type="text" class="calDate"  id="deliveryDate" name="deliveryDate" placeholder="DD/MM/YYYY" validate="Due Date,string,yes" value="<%=deliveryDate%>" onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10" style="width: 120px"/>
	...
	<select	class="" name="quotationNo" id="quotationNo" validate="Quotation No,String,yes" tabindex=1 onchange="getVendorDetails();getEnquiryDetails();">
		<option value=<%=quotationNo%>><%=quotationNo%></option>
	...
	<option value="<%=stockistId%>"><%=stockistName%></option>
	<%-- <%
...
<input	name="ReferenceNo" id="ReferenceNo" validate="ReferenceNo,String,yes" value="<%=referenceNo%>" tabindex=1>	
	
<label> MPR Approval Date <span>*</span></label>
<input  type="text" class="calDate"  id="mprApprovalDate" name="mprApprovalDate" value="<%=mprApprovalDate%>" placeholder="DD/MM/YYYY" validate="MPR Approval Date,string,yes"  onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10"/>

<label> Proposal No<span>*</span> </label>
<input	name="ProposalNo" id="ProposalNo" validate="Proposal No,String,yes" tabindex=1 value="<%=ProposalNo%>">

<div class="clear"></div>
<label> Proposal Date <span>*</span></label>
<input  type="text" class="calDate"  id="ProposalDate" name="ProposalDate" value="<%=ProposalDate%>" placeholder="DD/MM/YYYY" validate="Proposal Date,string,yes"  onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10"/>

<label> Proposal Approval Date <span>*</span></label>
<input  type="text" class="calDate"  id="ProposalApprovalDate" name="ProposalApprovalDate" value="<%=ProposalApprovalDate%>" placeholder="DD/MM/YYYY" validate="Proposal Approval Date,string,yes"  onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10"/>

<label> Tax Details<span>*</span> </label>
<input	name="TaxDetails" id="TaxDetails" validate="Tax Details,String,yes" tabindex=1 value="<%=TaxDetails%>">
	<div class="clear"></div>
	<label> Delivery Schedule<span>*</span> </label>
	<textarea size ="20" class="large"  id="DeliverySchedule" class="medium3"  name="DeliverySchedule"  validate="Remarks,string,no" value=""   tabindex=1 style="width: 250px; height: 50px;" ><%=DeliverySchedule%></textarea>
	
	<label> Payment Terms<span>*</span> </label>
	<textarea size ="20" class="large"  id="PaymentTerms" class="medium3"  name="PaymentTerms"  validate="Remarks,string,no" value=""   tabindex=1 style="width: 250px; height: 50px;" ><%=PaymentTerms%></textarea>
 </div>  
...
<div class="clear"></div>	
	...
	<input type="button" name="add" id="addbutton" value="HAL COPY" class="button" onClick="submitForm('createPurchaseOrder','/hms/hms/stores?method=generateHALPo&poId=<%=poHeaderId %>');"  tabindex=1 />
<input type="button" name="add" id="addbutton" value="Vendor PO COPY" class="button" onClick="submitForm('createPurchaseOrder','/hms/hms/stores?method=generatePo&poId=<%=poHeaderId %>');"  tabindex=1 />
...
<script type="text/javascript" src="/hms/jsp/js/jquery.datePicker.js"></script>
	...
	{		
		...
		/* 	if($j("#ddlRequestYear").val() == 0)
			...
			if(quotationNo !=0)
				{
					submitProtoAjaxWithDivName('createPurchaseOrder','/hms/hms/stores?method=getQuotationDetailsforPOSaved&quotationNo='+quotationNo+'&headerId='+<%=poHeaderId%>,'divPOList');
				}
