...
<table id="MRGrid">
 	...
 	int counter =1;
		...
		<td id="SRNO"><input size="2" id="tdSRNO" type="text" value="<%out.print(counter); %>" readonly /></td>
...
<div class="clear"></div>
	...
	{
		%>
			<input type="button" name="Save"  class="button" value="Save" onclick="saveMR('MR','stores?method=updateSavedMR&mrHeaderId=<%out.print(mrHeaderId); %>&appId=A1523');"/>
			<input type="button" name="Submit"  class="button" value="Submit" onclick="submitMR('MR','stores?method=updateSavedMR&mrHeaderId=<%out.print(mrHeaderId); %>&appId=A1523');"/>
		<%
	}
