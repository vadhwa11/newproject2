	...
	if(map.get("max") != null){
	 ...
	 if(map.get("message") != null)
	 {
		...
		for(StoreGrnM header: grnHeader)
		{
		    ...
		    InspectedBy = (header.getInspectedBy()!=null?header.getInspectedBy().getEmployee().getFirstName():"");
		    InspectedDate = (header.getInspectedDate()!=null?HMSUtil.changeDateToddMMyyyy(header.getInspectedDate()):"");
...
%>
 ...
 <div class="titleBg">
<h2>Approval Of Receiving Report</h2>
</div>
...
<form name="RRApproval" method="post">
...
<input	type="text" name="Invoice" value="<%=InvoiceAmt%>" readonly="readonly"  tabindex=1  readonly/>
	...
	<input  type="text"   id="RR" name="RR"  value="<%out.print(InspectedBy); %>"   readonly/>
	<label> Inspected Date  </label>
	<input  type="text"   id="RR" name="RR"  value="<%out.print(InspectedDate); %>"   readonly/>
...
<table id="IndentGrid">
     ...
     <input type="hidden" name="grnDetailId" tabindex="1" value="<%out.print(detailsList.getId()); %>" /></td>
      ...
      <td>
      <input type="text"  class="smcaption"  name="txtShortageQty<%out.print(detailsList.getId()); %>" id="txtShortageQty<%out.print(detailsList.getId()); %>" tabindex="1" validate="Shortage Qty ,String,no" size="10" value="<%out.print(detailsList.getShortage()!=null?detailsList.getShortage():0.0); %>" onblur ="calculateQty(<%out.print(detailsList.getId()); %>);" onkeypress ="return isNumber(event);" <% if(detailsList.getSurplus().doubleValue()>0){%>readonly<%} %> />
      </td>
      <td>
      <input type="text"  class="smcaption"  name="txtSurplusQty<%out.print(detailsList.getId()); %>" id="txtSurplusQty<%out.print(detailsList.getId()); %>" tabindex="1" validate="Surplus Qty ,String,no" size="10" value="<%out.print(detailsList.getSurplus()!=null?detailsList.getSurplus():0.0); %>" onblur ="calculateQty(<%out.print(detailsList.getId()); %>);" onkeypress ="return isNumber(event);" <% if(detailsList.getShortage().doubleValue()>0){%>readonly<%} %> />
      </td>
      <td>
      <input type="text"  class="smcaption"  name="txtRejectedQty<%out.print(detailsList.getId()); %>" id="txtRejectedQty<%out.print(detailsList.getId()); %>" tabindex="1" validate="Rejected Qty ,String,no" size="10" value="<%out.print(detailsList.getRejectedQty()!=null?detailsList.getRejectedQty():0.0); %>" readonly  />
      </td>
      <td>
      <input type="text"  class="smcaption"  name="txtAcceptedQty<%out.print(detailsList.getId()); %>" id="txtAcceptedQty<%out.print(detailsList.getId()); %>" tabindex="1" validate="Accepted Qty ,String,no" size="10" value="<%out.print(detailsList.getAcceptedQty()!=null?detailsList.getAcceptedQty():0.0); %>" readonly />
      </td>
      <td>
...
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Submit" class="button" onClick="submitForm('RRApproval','/hms/hms/stores?method=submitRRApprovalDetails');"  tabindex=1 />
	...
	<div class="division"></div>
		...
		function calculateQty(inc)
		{
					...
					var AdvisedQty = document.getElementById('txtAdvisedQty'+inc).value
					AdvisedQty =AdvisedQty *1;
					
					var RejectedQty = document.getElementById('txtRejectedQty'+inc).value
					RejectedQty= RejectedQty *1;
					var ShortQty = document.getElementById('txtShortageQty'+inc).value
					ShortQty= ShortQty *1;
					var OverQty = document.getElementById('txtSurplusQty'+inc).value
					OverQty= OverQty *1;
					
					if(ShortQty>0)
						{
							document.getElementById('txtReceivedQty'+inc).value = AdvisedQty-ShortQty;
							var ReceivedQty = document.getElementById('txtReceivedQty'+inc).value
							ReceivedQty =ReceivedQty *1;
							
							document.getElementById('txtAcceptedQty'+inc).value = ReceivedQty-RejectedQty;
						}
					if(OverQty>0)
					{
						document.getElementById('txtReceivedQty'+inc).value = AdvisedQty+OverQty;
						var ReceivedQty = document.getElementById('txtReceivedQty'+inc).value
						ReceivedQty =ReceivedQty *1;
