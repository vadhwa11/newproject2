...
<tr>
	...
	<td>
		<input type="text" value="" size="30"  validate="Item Name,string,yes" id="<%=nameItem+inc%>" onblur="if(fillSrNo(<%=inc+temp %>)){checkForIndentToDepot(this.value, '<%=nameItem%>','<%=inc+temp %>');}" name="<%=nameItem%>" tabindex=1 />
		<div id="ac6update" class="autocomplete" style="display: none;"></div>
		<script type="text/javascript" language="javascript" charset="utf-8">
	...
	<td><input type="text" value="" size="20" tabindex=1 validate="Batch No,string,yes" name="<%=BATCH%>" id="<%=batchNoVarTemp+inc%>" onblur="fillValuesBalance(<%=temp+inc%>);checkBatchNo(this.value,'<%=temp+inc%>');"  maxlength="10" tabindex=1 />
	...
	<td><input type="text" size="8" value="" name="<%=QTY_BALANCE_TEMP%>" validate="Qty,string,yes" maxlength="9" tabindex="1" id="<%=qtyVarTemp+inc%>" validate="Qty,float,no" onblur="fillValuesBalance(<%=inc%>);calculateAmount(<%=inc %>);" tabindex=1 />
	 <input type="hidden" value="0" name="<%=QTY_BALANCE%>" tabindex="2" id="<%=qtyVar+inc%>" /> </td>
	 
	<td><input type="text" size="8" value="" name="<%=UNIT_RATE_BALANCE_TEMP%>" validate="Unit Rate,string,yes" maxlength="9" tabindex="1" id="<%=unitRateVarTemp+inc%>" validate="unitRateBalanceTemp,float,no" onblur="fillValuesBalance(<%=inc%>);calculateAmount(<%=inc %>);checkBeforeSubmit(<%=inc %>)" />
...
<%m++;} %>
	...
	function addRow(){
		  ...
		  e4.setAttribute('validate','Manufacturer No,string,yes');
		  e4.type = 'text';
		  ...
		  var e41 = document.createElement('input');
		  e41.name='BrandName';
		  e41.setAttribute('validate','Brand Name,string,yes');
		  e41.type = 'text';
		  ...
		  e5.size='20';
		  e5.name='<%=BATCH%>';
		  e5.setAttribute('validate','Batch No,string,yes');
		  ...
		  e81.setAttribute('validate','Qty,string,yes');
		  ...
		  e9.setAttribute('validate','Unit Rate,string,yes');
 		...
 		function validateGridData() {
 			...
 			for (var inc = 0; inc < count; inc++) {
 				...
 				if (batchNoVarTemp == "" && itemName != "") {
 					alert("pls check the batch No in row " + inc);
 					document.getElementById("batchNoVarTemp" + inc).focus();
 					return false;

 				}
 				/* if (manufactureDateVarTemp == "" && itemName != "") {
 					alert("pls check the Manufature date in row " + inc);
