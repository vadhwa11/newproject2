...
<div class="mesg"
	style="width: 100%; height: 23px; font-weight: bold; color: red;">
...
<div class="mesg"
	style="width: 100%; height: 23px; font-weight: bold; color: red;">
<%=message.toUpperCase() %> <% if(appSetupList.size() > 0){ %>
...
<label class="common">Selected Department:</label> <input type="hidden"
...
<%		}
    ...
    %> <label class="large">Selected Appointment Date:</label> <input
...
<div class="Clear"></div>
<% if(appSetupList.size() > 0){ %> <label class="common"
	style="width: auto; float: none; color: red;"> APPOINTMENT
AVAILABLE ON DAYS : </label> <% }else{ %> <label class="common"
	style="width: auto; float: none; color: red;"> NO APPOINTMENT
SETUP FOR THIS DEPARTMENT </label> <% } %> <label class="large"
	style="color: red; width: 100%; float: none;"> <%if(appSetupList.size() == 1){ %>
...
<div class="tableHolderAuto" style="width: 1100px;">
<table border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<th colspan="5" scope="col"><span
			style="color: #FFF; padding-left: 25px; float: left;">Relation</span><span
	...
	<tr>
		...
		<td><input type="text" size=8 name="<%=SERVICE_NO%>"
			id="serviceNo<%=inc %>" MAXLENGTH="20" disabled="disabled"
			onblur="if(this.value!='')openPopUp(<%=inc %>);" /></td>
		<td><input type="text" size=16 id="rank<%=inc %>" name="rank"
			disabled="disabled" value="" MAXLENGTH="30" validate="" /></td>
		...
		<td><input type="text" size=10 id="hinNo<%=inc %>"
			...
			onblur="if(this.value!='')submitProtoAjaxWithDivName('patientAppointments','appointment?method=showListBasedonHinNo&inc=<%=inc%>&hinNo='+this.value,'testDiv<%=inc %>');" />
		</td>

		<td colspan="5" id="testDiv<%=inc %>"><select
			id="relationId<%=inc%>" name="<%=RELATION_ID%>" disabled="disabled">
			<option value="0">Select</option>
			<%for(MasRelation relation : relationList){ %>
			<option value="<%=relation.getId()%>"><%=relation.getRelationName()%></option>
			<%} %>
		</select> <input type="text" size=16 id="patientName<%=inc %>"
			name="<%=PATIENT_NAME%>" disabled="disabled" MAXLENGTH="30"
			validate="Patient Name,fullName,no" /> <input type="text" size=10
			id="mobileNo<%=inc %>" name="<%=MOBILE_NO%>" disabled="disabled"
			MAXLENGTH="10" validate="Mobile No.,int,no" /> <select
			id="sex<%=inc%>" name="<%=SEX%>" disabled="disabled">
			<option value="M">M</option>
			<option value="F">F</option>
		</select> <input type="text" size=1 id="age<%=inc%>" name="<%=AGE%>"
			MAXLENGTH="2" disabled="disabled" /> <select id="ageUnit<%=inc%>"
			name="<%=AGEUNIT%>" disabled="disabled">
			<option value="0">Select</option>
