...
<%
	...
	if(document.getElementById('detailId').value =="Admission"){
		...
		submitProtoAjax('patientAndAdDetailsGrid','/hms/hms/enquiry?method=getAllEnquiry1&opOrString='+document.getElementById('opOrString').value+'&detailId='+document.getElementById('detailId').value+'&id='+document.getElementById('opOrId').value+'&hinId='+document.getElementById('hinId').value)
	}
	function displayAdNo(id,opOrString,extPara){
		...
		if(opOrString =="0"){
				...
				obj.length++;
				obj.options[obj.length-1].value="Prescription"
				obj.options[obj.length-1].text="Prescription";
				obj.length++;
				obj.options[obj.length-1].value="Diagnostics"
				obj.options[obj.length-1].text="Diagnostics";
			submitProtoAjax('patientAndAdDetailsGrid','/hms/hms/enquiry?method=getAdVisitDetails&id='+document.getElementById('opOrId').value);
		}else{
			...
			obj =document.getElementById("detailId"); 
				...
				submitProtoAjax('patientAndAdDetailsGrid','/hms/hms/enquiry?method=getAllEnquiry1&opOrString='+document.getElementById('opOrString').value+'&detailId=Visit'+'&id='+document.getElementById('opOrId').value)
		}
	}
...
<div id="status1">
<select name="<%=ADMISSION_STATUS %>"	id="detailId" onchange="getAdVisitDetails();">
	<option value="">Select</option>
</select>
</div>
<br />
<div style="overflow: auto; width: 99%; height: 270px; padding: 0px 4px 0px 4px;" id="testDiv"></div>
</div>
</form>
<div class="Clear"></div>
</div>
