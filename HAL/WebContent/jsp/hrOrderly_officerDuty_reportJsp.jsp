...
<h6>Officers Duty Entry Report</h6>
<div class="Clear"></div>
...
From Date</label> <input type="text" id="fromDateId" name="<%=FROM_DATE %>"
	...
	onClick="setdate('<%=currentDate%>',document.guardReport.<%=FROM_DATE%>,event);" />
...
<label><span>*</span> To Date</label> <input type="text" id="ToDateId"
	...
	onClick="setdate('<%=currentDate%>',document.guardReport.<%=TO_DATE%>,event);" />

</div>
<div class="Clear"></div>
<input type="button" name="OK" value="OK" class="cmnButton"
	onClick="submitForm('guardReport','hrOrderly?method=generateOfficerReport');" />
