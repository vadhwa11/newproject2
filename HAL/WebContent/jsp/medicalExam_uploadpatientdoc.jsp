...
<input type="hidden" name="hin_no" id="hin_no" value="<%=patient.getHinNo() %>"  validate="hin_no,netachar,no"/>
<%} %>
...
<!--Main div starts-->
 ...
 <input id="visitId" name="visitId" type="hidden" value="<%=visitId%>" readonly="readonly" validate="visitId,netachar,no"/>
  <input type="hidden" name="date" size="5" value="<%=date%>"  validate="date,date,no"/> 
...
<%%>
<div class="Clear"></div>
<input type="button" class="buttonBig" name="impbutton" value="Attach"
	onClick="jsImport()" /> 
<input type="button"
		...
		if (document.getElementById('attachment'+i).value!="")
		{
			...
			if(filename.lastIndexOf(".doc")==-1){
				...
				document.getElementById('attachment'+i).value="";
					...
					if( validateMetaCharacters(hinId) && validateMetaCharacters(masExam_Id) && validateMetaCharacters(visit_id) && validateMetaCharacters(hin_no) && validateMetaCharacters(folderName) &&  validateMetaCharacters(inc)){
					submitForm('appointmentUpload','medicalExam?method=submitUploadDocuments'+fileNames +'&hin_id='+ hin_id+'&hin_no='+hin_no+'&masExam_Id='+masExam_Id+'&folderName='+folderName+'&visitId='+visit_id+'&uploadCount='+inc);
					}
...
function viewPatientDocuments()
{
	...
	folderName='<%=folderName%>';
	document.appointmentUpload.method="post";
	if( validateMetaCharacters(hin_id) && validateMetaCharacters(masExam_Id) && validateMetaCharacters(visit_id) && validateMetaCharacters(folderName)){
   submitForm('appointmentUpload','medicalExam?method=viewUploadDocumentsDetails&hinId='+hin_id+'&visitId='+visit_id+'&masExam_Id='+masExam_Id+'&folderName='+folderName);
	}   	
}
