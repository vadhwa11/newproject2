...
<label> Validated By <span>*</span></label> 
<%

Users user = (Users)session.getAttribute("users");
Integer empId =user.getEmployee().getId();
for (MasEmployee  obj : employeeList){
//	if(obj.getEmpCategory() != null &&  obj.getEmpCategory().getEmpCategoryCode().equals(empCategoryCodeForDoctor) ){
	if(empId.equals(obj.getId())){
%>
<label class="value"><%=obj.getRank().getRankName()+" "+obj.getFirstName()+" "+(obj.getMiddleName()!=null?obj.getMiddleName():"")+" "+(obj.getLastName()!=null?obj.getLastName():"")%></label>
<%}
//	}
	}%>
	<input	type="hidden" name="<%=RESULT_VALIDATED_BY %>" value="<%=empId%>" />
...
<table id="chargeDetails" width="100%" cellpadding="0" cellspacing="0">
	...
	<%	}else{
		...
		<tr>
			...
			<input	name="<%=RESULT_SINGLE_VALUE %>" tabindex="1" onkeyup="submitAllExceptEnter(this,event,'','<%=index+1 %>');" id="<%=RESULT_SINGLE_VALUE %><%=index+1 %>" value="<%=result %>" />
			<% }else{ %> 
			<input name="<%=RESULT_SINGLE_VALUE %>" tabindex="1" onkeyup="submitAllExceptEnter(this,event,'','<%=index+1 %>');"	id="<%=RESULT_SINGLE_VALUE %><%=index+1 %>" class="highlight"	value="<%=result %>" /> 
			<% }
		}else{%> 
...
<input type="button" class="button" value="Submit"	onclick="if(inputValidate()){submitForm('sampleCollection','investigation?method=submitResultValidationForAllInvestigationTypeLab')};" /> 
