...
<%@page import="jkt.hms.masters.business.MasCompany"%><script type="text/javascript" language="javascript" src="/hms/jsp/js/proto.js"></script>
...
function openPopupWindow(id)
{
  if(id != ""){
    ...
    alert("Please first select the one Niv No.!!! ");
  }  
}
...
<%
	...
	if(map.get("itemNo")!=null){
		itemNo=map.get("itemNo").toString();
	}
%>
<%
			  ...
			  <label>NIV No.</label>
				...
				<label>Section Code:</label>
				   ...
				   <select name="sectionCode" id="sectionCode" onchange="" tabindex=1>
                    ...
                    <input type="button" name="search" value="Search" class="button" onclick="submitForm('search','pharmacy?method=searchItemNIV')" tabindex=1  />
                    <input type="reset" name ="add" id="reset" value ="Generate Report" class="buttonBig" onclick="submitForm('search','pharmacy?method=generateMedicineMasterCIVReport');" accesskey="r"  tabindex=1 />
 		            <%--- Report Button   --%>  
	 ...
	 <input type="hidden" name="<%= HOSPITAL_ID %>" value = "<%=hospitalId%>">
	  ...
	  <input type="hidden" name="<%=JSP_NAME %>" value = "item">
		<div class="clear"></div>	
				...
				<select id="sectionId" name="sectionId" class="large2" validate="Section Name,string,yes"  tabindex=1>
				<%}else{ %>
				<select id="sectionId" name="sectionId" class="large2" validate="Section Name,string,yes" tabindex=1>
				<% }%>
				<option value="0">Select</option>
				  <%
				  if(storeSectionList!=null){	
				...
				<select name="<%= ITEM_TYPE_ID %>" validate="Item Type,string,yes"  onchange="checkForNivItem(this.value)" tabindex=1>
				  ...
				  for (MasItemType masItemTypecode : itemTypeList) {
				  		if(masItemTypecode.getItemTypeName().equalsIgnoreCase("NIV")){
				  %>
		  	...
		  	<label> NIV No.<span>*</span> </label>
			<input type="text" id="pvmsNo" name="<%= CODE%>" value="<%=itemNo%>" validate="NIV No.,string,yes"  onblur="checkPvmsForAlreadyExist();"  MAXLENGTH="20" tabindex=1  />
			...
			<input type="text" id="nomenclatureToCheck" name="<%= SEARCH_NAME%>" value="" validate="Nomenclature,string,yes"  MAXLENGTH="250" class="large2" tabindex=1  onblur="showSimilarMedicineNames(this.value);"/>
		 ...
		 <label>Strength </label>  
			...
			if(brandList!=null){
				  ...
				  %>
		  	  	...
		  	  	<div style="display:none;">	
				<label class="smallAuto"> <a  href="#" onclick="openPopupWindow(item.<%=COMMON_ID%>.value);">Add Brand</a></label>
				</div>
					<label>Group </label>
			...
			<label>Source of supply</label>
			 ...
			 <input type="hidden" name="<%=BRAND_GENERIC%>"  value="B" tabindex=1>
			 <div class="clear"></div>
			  <label>Remarks</label>
			...
			<input type="radio" class="radio" name="pac" value="y"> 
			<font class="wardspan">PAC</font>
			<input type="radio" class="radio" name="non_pac" value="n">
...
-->		    
  	...
  	<div class="clear"></div>
		...
		<div class="clear"></div>
			...
			<input type="text" name="<%= COST_PRICE%>" value="" validate="Cost(Rs).,string,no"  MAXLENGTH="10" tabindex=1 />
			
			<label>Self Life (in months) </label>
			<input type="text" name="<%= SHELF_LIFE%>" value="" validate="Shelf Life,string,no"  MAXLENGTH="10" tabindex=1 />
			...
			<label>Expiry </label>
			 <select name="<%=EXPIRY%>" onkeypress="return submitenter(this,event,'pharmacy?method=addItemNIV','checkPvmsNo')"  tabindex=1>
			  <option value="0">Select</option>
...
<div class="clear"></div>
<div id="edited"></div>
	<input type="button" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('item','pharmacy?method=addItemNIV','checkPvmsNo','checkFieldForAmpersant')" accesskey="a" tabindex=1 style="display:inline"/>
	<input type="button" name="edit" id="editbutton" value="Update" class="button" onClick="submitForm('item','pharmacy?method=editItemNIV&flag='+this.value,'checkPvmsNo','checkFieldForAmpersant')" accesskey="u" tabindex=1 style="display:none;"/>
	<input type="button" name="Delete" id="deletebutton" value="Activate" class="button" onClick="submitForm('item','pharmacy?method=deleteItemNIV&flag='+this.value)" accesskey="d" tabindex=1/>		
...
function checkPvmsNo(){
	...
	if(sectionId == pvmsNo && pvmsNo1 != "" || pvmsNo == "NI"){
		...
		alert("Please Enter valid NIV No. starting with Section Code")
	}
}
...
data_header[43][0] = "Status"
data_header[43][1] = "data";
data_header[43][2] = 0;
data_header[43][3] = "<%=STATUS %>";
...
data_arr[<%= counter%>][44] = "Active"
<%}else{%>
data_arr[<%= counter%>][44] = "InActive"
<%}%>
