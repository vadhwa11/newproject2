...
for (var i=0;i < document.sampleCollection.elements.length;i++)
 ...
 function goBack(){
 	window.location.href='/hms/hms/investigation?method=showPendingForResultValidationLabJsp';
 }  
...
<label class="value"><%=user.getEmployee().getFirstName() %></label>
	...
	<input	type="hidden" name="<%=RESULT_VALIDATED_BY %>" id="<%=RESULT_VALIDATED_BY %>" value="<%=empId%>" />
		    ...
		    try{
		    	if(dgDetail.getInvestigation().getMinNormalValue() != null	&& dgDetail.getInvestigation().getMaxNormalValue() != null
		    		...
		    		if(dgDetail.getResult() != null && !dgDetail.getResult().equals("")){
		    			...
		    			String finalResult="";
		    			/* System.out.println("dgDetail.getResult()"+dgDetail.getResult()); */
...
<input type="button" class="button" value="Submit" id="submitButton"	onclick="if(inputValidate()){submitForm('sampleCollection','investigation?method=submitResultValidationForAllInvestigationTypeLab')};" /> 
