...
%>
<div class="titleBg"><h2>Non Drug Master</h2></div>
<div id="searcharea">
<div id="searchbar">
...
<label>Mat. Code</label>
 <input type="radio" class="radioAuto" name="<%=SELECTED_RADIO  %>"   value="1" checked="unchecked" onclick="checkCriteria();"/>
			    ...
			    <label>Nomenclature</label>
		            ...
		            <div id="txt">
					 <input type="text" id="searchField" name="<%= SEARCH_FIELD%>" value=""  validate="Code,string,no"   MAXLENGTH="15" tabindex=1 onkeypress="return submitenter(this,event,'pharmacy?method=searchNonDrug')" />
					 </div>
					 
					 <input	type="hidden" name="colCode" value="pvms_no"/>
<input	type="hidden" name="colName" value="nomenclature"/>
                    <input type="button" name="search" value="Search" class="button"   onclick="submitForm('search','pharmacy?method=searchNonDrug','checkSearch')" tabindex=1  />
                    ...
                    <input type="hidden" name="add" onclick="submitForm('search','pharmacy?method=generatePVMSExcel');" accesskey="r"  tabindex=1  value="Export To Excel" class="buttonBig">
 		            ...
 		            <%--- Report Button   --%>  
 		            <input type="hidden" name="<%=JASPER_FILE_NAME%>" value="Mas_non_drug_list"> 
...
{
%> <h4><a href="pharmacy?method=showNonDrugJsp">Show All Records</a></h4> <%
 	}
 	}
...
statusTd = 16;
	  ...
	  <input type="hidden" name="<%=JSP_NAME %>" value = "nonDrug">
		<div class="Clear"></div>	
...
<div class="division"></div>
	...
	<input type="button" name="add" id="addbutton" value="Add"	class="button" onclick="submitForm('item','pharmacy?method=addNonDrug')" accesskey="a" tabindex=1 style="display: inline" />
<input	type="button" name="edit" id="editbutton" value="Update"	style="display: none" class="button" onclick="submitForm('item','pharmacy?method=editNonDrug&flag='+this.value)"	accesskey="u" tabindex=1 />
	
	<input type="button" name="Delete" id="deletebutton" value="Activate" class="button" onClick="submitForm('item','pharmacy?method=deleteNonDrug&flag='+this.value)" accesskey="d" tabindex=1/>		
		<input type="reset" name="Reset" id="reset"	value="Reset" class="button" onclick="submitFormForButton('item','pharmacy?method=showNonDrugJsp)" accesskey="d"	tabindex=1 accesskey="r" />
...
data_header[14][3] = "maxTemperature";
 --%>
data_header[10] = new Array;
data_header[10][0] = "uomQty"
data_header[10][1] = "hide";
data_header[10][2] = 0;
data_header[10][3] = "uomQty";

data_header[11] = new Array;
data_header[11][0] = "dispensingUnit"
data_header[11][1] = "hide";
data_header[11][2] = 0;
data_header[11][3] = "dispensingUnit";
...
<%}%> --%>
	...
	data_arr[<%= counter%>][11] = "<%= masStoreItem.getADispQty() %>"
		<%}else{%>
		data_arr[<%= counter%>][11] = "";
		<%}%>
	<% if(masStoreItem.getDispUnit() != null){%>
	data_arr[<%= counter%>][12] = "<%= masStoreItem.getDispUnit() %>"
		<%}else{%>
data_arr[<%= counter%>][12] = "";
<%}%>
...
data_arr[<%= counter%>][13] = "<%= masStoreItem.getLastChgBy().getFirstName() %>"
<%}else{
	%>
	data_arr[<%= counter%>][13] = "<%=userName%>";
<%}	%>
