...
<div class="Clear"></div>
<label>Name:</label> <%if(pName!= null){ %>
...
<div class="Clear"></div>
<label>Rank:</label> <%if(visitObj.getHin().getRank()!= null){ %>
<label class="value"><%=visitObj.getHin().getRank().getRankName() %></label>
<%}else{ %> <label class="value">-</label> <%} %> 
<label>Age:</label> <%if(visitObj.getHin().getAge()!= null){ %> <label
	class="value"><%=HMSUtil.calculateAge(visitObj.getHin().getAge() , visitObj.getHin().getRegDate(), visitObj.getHin().getDateOfBirth()) %></label> <%}else{ %> <label
	class="value">-</label> <%} %>
<label>Unit:</label> <%if(visitObj.getHin().getUnit()!= null){ %> <label
	class="value"><%=visitObj.getHin().getUnit().getUnitName() %></label> <%}else{ %>
<label class="value">-</label> <%} %> 
<div class="Clear"></div>
<label>Trade/AF/Navy/Army:</label> <%if(visitObj.getHin().getTradeName()!= null){ %> <label
	class="value"><%=visitObj.getHin().getUnit().getUnitName() %></label> <%}else{ %>
<label class="value">-</label> <%} %> 
...
<form name="opdPatientVisitFromOPDScreen" method="post" action="">
...
<script type="text/javascript" language="javascript">
		...
		%>
			...
			data_arr[<%= i%>][0] ="<%=visit.getId()%>@<%=maxVisitIdList.get(0)%>@<%=minVisitIdList.get(0)%>@<%=visitId%>@<%=visitNoForJsp%>"
		...
		formName = "opdPatientVisitFromOPDScreen"
...
<input name="Back" type="button" alt="Back" value="Back" class="button"
	onclick="submitForm('opdPatientVisitFromOPDScreen','opd?method=showOPDMainJsp&visitId=<%=visitId%>&deptId=<%=deptId%>');"
	align="right" /></div>
<script type="text/javascript">
<!--
...
No Previous Visit For Patient </label> <input name="Back" type="button"
	alt="Back" value="Back" class="cmnButton" style="float: none;"
