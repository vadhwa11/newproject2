...
<%} 
	...
	for(ReferralPatientDetails referralPatientDetails: referralPatientDetailsList)
	{
		...
		subject = referralPatientDetails.getSubject();
		note = referralPatientDetails.getReferralNote();
		validity_period =referralPatientDetails.getLetterValidityPeriod();
		...
		else
		{
		 ...
		 if(extentionCnt!=0)
		 {
			 ...
			 <div class="Clear"></div>
			 <h4>Extension No : <%=referralPatientDetails.getReferralNo()%></h4>
...
<div class="Clear"></div>
<label> Note:</label>
<input type="text" class = "large"  validate="Note,String,no" maxlength="500" value="<%=note %>" name ="referral_note" id ="referral_note" readOnly = "true" style="width:477px;"/>
...
<%}%>
		 ...
		 <label> This letter is valid for </label><input type="text" value="<%=validity_period %>" class = "small" name ="validity_period" id="validity_period" validate="Validity Period,int,no" maxlength="5" readOnly = "true"/>Days
		 <div class="Clear"></div>
		 <label> Subject:</label><input type="text"  class="large" value="<%=subject %>" name ="subject" id ="subject" validate="Subject,String,no" maxlength="250" readOnly = "true" style="width:477px;"/>
...
<div class="Clear"></div>
        <label>Doctor Note</label>
         <textarea class = "large" id="doctorRemarks" name = "doctorRemarks" onkeyup="auto_grow(this)" maxlength="500"></textarea>
         
		 <div class="Clear"></div>
<label>No. of Days<span></span></label> <input id="referdays" name="referdays" type="text" maxlength="3" validate="No. of Days,int,no"/>
<label >Treatment Type <span>*</span></label> 
									<select	id="referral_treatment_type" name="referral_treatment_type" validate="Treatment Type,String,yes">										
...
<input type="hidden" name="referralBillingId<%=divCountBilling%>" id="referralBillingId<%=divCountBilling%>" value="<%=referralPatientBilling.getId()%>"/>
    <label>Bill No</label><input type="text"  maxlength="15"  value='<%=referralPatientBilling.getBillNo()!=null?referralPatientBilling.getTotalBillAmt():""%>'  readOnly="readOnly"/>
...
<div class="Clear"></div>
    <label>Total Bill Amount<span>*</span></label><input type="text" validate="Empanelled Bill,int,yes" maxlength="15" name ="impanelled_bill<%=divCountBilling%>" id ="impanelled_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getTotalBillAmt()!=null?referralPatientBilling.getTotalBillAmt():""%>'  readOnly="readOnly"/>

<label>Approved Bill Amount<span>*</span></label><input type="text" validate="HAL Bill,int,yes" maxlength="15" name ="hal_bill<%=divCountBilling%>" id ="hal_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getAdminBillAmt()!=null?referralPatientBilling.getAdminBillAmt():""%>'  readOnly="readOnly"/>
...
<select id="referralNoId<%=divCountBilling%>" name="referralNoId<%=divCountBilling%>"  validate="Referral Number,string,yes"  readOnly="readOnly">
	...
	<label>Admin Remarks</label><textarea validate="Remarks,string,no" maxlength="500" name ="admin_remarks<%=divCountBilling%>" id ="admin_remarks<%=divCountBilling%>" readOnly="readOnly" /><%=referralPatientBilling.getAdminRemarks()!=null?referralPatientBilling.getAdminRemarks():""%></textarea>
  ...
  <% ++divCountBilling;
	...
	{
		 %>
	<label>Hr Remarks</label><textarea validate="Remarks,string,no" maxlength="500"  readOnly="readOnly"/><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks():""%></textarea>
    <label>Finance Remarks</label><textarea validate="Remarks,string,no" maxlength="500"  readOnly="readOnly" /><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getFinanceRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getFinanceRemarks():""%></textarea>
    
    <label>GM Remarks</label><textarea validate="Remarks,string,no" maxlength="500"  readOnly="readOnly" /><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getGmRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getGmRemarks():""%></textarea>
...
<input type="hidden" name ="divCountBilling" id="divCountBilling" value=""/>
...
<input type="button" class="button" value="Submit " align="left"
	onClick="submitForm('generateReferralLetterPage','referral?method=submitReferralExtension');" />
<!-- <input type="button" class="button" value="Reject " align="left" /> -->
<input type="button" class="button" value="Back" align="left"
	onClick="submitFormForButton('generateReferralLetterPage','referral?method=waitingListForReferralExtension');" />
