 ...
 for(i=1;i<=42;i++){
  ...
  if(x!='&nbsp;'){
   ...
   datelink=dd+'/'+mm+'/'+oD.getFullYear()
		   	...
		   	}else if(document.admissionByHin){
		   	...
		   	}else if(document.internalDemand){
   			   	if(document.internalDemand.toDate == dateobj1){
   			   		tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.calculateDays();window.close();'
		   		}else{
		   		    tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		   		}
		   }else if(document.investigationAppointments){
   			   	if(document.investigationAppointments.appointmentDate == dateobj1){
   			   		tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.submitForm(\'investigationAppointments\',\'appointment?method=showAppointmentInvestigationWise\',\'isApptGrCurrentDate\',\'checkForHoliday\',\'checkEquipment\');window.close();'
		   		}else{
		   		    tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		   		}
		   }else if(document.purchaseGrid){
		       document.getElementById('soDate').focus();
		       tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		   }
		   else if(document.personnelEntry){
		      if(document.personnelEntry.appointmentDate == dateobj1){
		        	tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		      }else{
		      	 tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		      } 	
		   }
		   else if(document.updatePersonnelEntry){
		      if(document.updatePersonnelEntry.appointmentDate == dateobj1){
		        	tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		      }else{
		      	    tmp='window.opener.dateobj1.value=\''+datelink+'\';window.close();'
		      } 	
		   }/*else if(document.opdMain)
			{  
				   
					tmp='window.opener.dateobj1.value=\''+datelink+'\';window.opener.checkDate1(\''+datelink+'\',\''+dateobj1.id+'\');window.close();'
...
function openWindowForCalender(text,posy,posx){
 ...
 var styleStr ="top="+ posy + ", left= " + posx + ", hotkeys=yes,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbar=no,width=200,height=305";
 msgWindow = window.open("","msg", styleStr);
 var calhed='<head><title>Select a Date</title><link href="/hms/jsp/css/style.css" rel="stylesheet" type="text/css" /></head>' 
 var calender='<body onunload="setTimeout(\'resetDates()\',100)"><div class="calender"><table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td><table width="100%"><tr><td><input name="back" type="button" class="buttoncell" id="btnchar2" value="&lt;&lt;" onclick="window.opener.prevmonth()" /></td><td align="center"><select name="year" class="Dropdown" id="size8" onchange="window.opener.changeyear(this.value)">'
 ...
 for(aa=(serverdateobj.getFullYear()-120);aa<=(serverdateobj.getFullYear()+20);aa++){
