 ...
 *
 * $Id: wysiwyg.js,v 1.2 2010/06/30 12:13:01 grijesh Exp $
 * $Revision: 1.2 $
 *
...
var WYSIWYG = {
	...
	Settings: function() {
		...
		// Images Directory
		this.ImagesDir = "/hms/jsp/images/wysiwyg/";
		
		// Popups Directory
		this.PopupsDir = "popups/";
		
		// CSS Directory File
		this.CSSFile = "/hms/jsp/css/wysiwyg.css";		
		...
		this.addToolbarElement = function(element, toolbar, position) {
			...
			/*if(this.Toolbar[toolbar-0] == null) {
				this.Toolbar[toolbar-0] = new Array();
			}
			this.Toolbar[toolbar-0].splice(position-0, 0, element);	*/		
		};
	...
	_display: function(n, settings) {
		...
		var iframe = '<table cellpadding="0" cellspacing="0" border="0" style="width:100%; height:' + currentHeight + ';" class="tableTextareaEditor"><tr><td valign="top">\n'
	...
	_generate: function(n, settings) {
	    ...
	    editor += '<table border="0" cellpadding="0" cellspacing="0" class="tableTextareaEditor" id="wysiwyg_table_' + n + '" style="width:100%;">';
		...
		for (var j = 0; j < this.config[n].Toolbar.length;j++) { 
			if(this.config[n].Toolbar[j] && this.config[n].Toolbar[j].length > 0) {
	    		...
	    		editor += '<tr><td style="width:6px;"></td>';
				    	...
				    	if(typeof (this.config[n].DropDowns[id]) != "undefined") {
							...
							if(buttonObj) {
								...
								// View Source button
									...
									editor += '<span id="HTMLMode' + n + '"><img src="' + buttonImage +  '" border="0" unselectable="on" title="' + buttonTitle + '" id="' + buttonID + '" class="buttonEditor"  onclick="WYSIWYG.execCommand(\'' + n + '\', \'' + buttonID + '\');" unselectable="on" width="20" height="20"></span>';
									editor += '<span id="textMode' + n + '"><img src="' + this.config[n].ImagesDir + 'view_text.gif" border="0" unselectable="on" title="viewText" id="ViewText" class="buttonEditor"  onclick="WYSIWYG.execCommand(\'' + n + '\',\'ViewText\');" unselectable="on"  width="20" height="20"></span>';
							        editor += '</td>';
						        }
								else {
									editor += '<td style="width: 22px;">';
									editor += '<img src="' + buttonImage + '" border=0 unselectable="on" title="' + buttonTitle + '" id="' + buttonID + '" class="buttonEditor"  onclick="WYSIWYG.execCommand(\'' + n + '\', \'' + buttonID + '\');" unselectable="on" width="20" height="20">';
									editor += '</td>';
								}
							}
						}
		...
		// Create iframe which will be used for rich text editing
		editor += '<iframe frameborder="0" id="wysiwyg' + n + '" class="iframeText" style="width:100%;"></iframe>\n'
	    + '</td></tr>';
	    // Status bar HTML code
	  if(this.config[n].StatusBarDisabled) {
	...
	execCommand: function(n, cmd, value) {
		...
		switch(cmd) {
			...
			case "Save":
			 ...
			 if(form.name=="chargeCode"){
			   var appendedHtml=chargeCode.appendedHtml.value;
			   chargeCode.browse.value="";
			  
			  var updatedtemplate= chargeCode.test2.value;
			  updatedtemplate=appendedHtml+updatedtemplate+"</body></html>";
			  
			  
			submitForm('chargeCode','investigation?method=getFileName&test2='+updatedtemplate+'&appendedHtml='+appendedHtml+'');
			}
			if(form.name=="sampleCollection"){
			
				  var updatedtemplate= sampleCollection.test2.value;
				// alert(updatedtemplate);
				var appendedHtml=sampleCollection.appendedHtml.value;
				//alert(appendedHtml);
				 updatedtemplate=appendedHtml+updatedtemplate+"</body></html>";
			  
				 form.browse.value="";
				  var hinNo=form.hinNo.value;
				  var inpatientId=form.inpatientId.value;
				 var sampleCollectionDetailId= form.sampleCollectionDetailId.value;
				 var departmentId=document.getElementById("departmentId").value;
				// alert(departmentId);
				obj = eval('document.'+form.name)
		obj.action = obj.action = "/hms/hms/investigation?method=getFileName&departmentId="+departmentId+"&sampleCollectionDetailId="+sampleCollectionDetailId+"&hinNo="+hinNo+"&formName="+form.name+"&appendedHtml="+appendedHtml+"&test2="+updatedtemplate+"" ;
		obj.submit();
		
			
			}
			if(form.name=="resultValidationEntry"){
			
				  var updatedtemplate= resultValidationEntry.test2.value;
				  	var appendedHtml=resultValidationEntry.appendedHtml.value;
				 updatedtemplate=appendedHtml+updatedtemplate+"</body></html>";
				// alert(updatedtemplate);
				  var hinNo=form.hinNo.value;
				  var inpatientId=form.inpatientId.value;
				...
				obj = eval('document.'+form.name)
		obj.action = obj.action = "/hms/hms/investigation?method=getFileName&diagnosisNo="+diagnosisNo+"&resultId="+resultId+"&hinId="+hinId+"&departmentId="+departmentId+"&subChargeCodeId="+subChargeCodeId+"&inpatientId="+inpatientId+"&hinNo="+hinNo+"&formName="+form.name+"&appendedHtml="+appendedHtml+"&test2="+updatedtemplate+"" ;
		obj.submit();
