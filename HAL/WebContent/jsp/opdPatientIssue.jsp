...
<div id="hinDiv"><input type="hidden"
	...
	onblur="if(checkHinNo()){submitProtoAjax('opdPatientIssue','stores?method=showOPDPatientIssueGridByHin');}" />
</div>
<%} %>
...
<%}else{ %> <label>Employee No.</label> <input type="text" id="serviceNo"
	...
	onblur="getHinNo('opdPatientIssue','stores?method=getHinForPatientIssue');submitProtoAjax('opdPatientIssue','stores?method=showOPDPatientIssueGridByHin');"
	onFocus="opdPatientIssue.reset();" /> <%} %> --%>
	
	
<label>Employee No.</label> <input type="text" id="serviceNo"
	name="serviceNo" value="<%=patient.getServiceNo() %>" tabindex="1" readonly="readonly"/>
...
if(patient.getPLastName() != null){
	...
	name="p_name" value="<%=p_name %>" tabindex="1" readonly"/> <%if(patient.getRelation() != null){%>
...
}%> <label>Employee Name</label> <input type="text" id="S_name" name="S_name"
...
String doc_name="";
if(prescriptionList.size()>0){	
if(prescriptionList.get(0).getVisit() != null && prescriptionList.get(0).getVisit().getDoctor()!=null){
	doc_name=prescriptionList.get(0).getVisit().getDoctor().getFirstName()+" "+(prescriptionList.get(0).getVisit().getDoctor().getLastName()!=null?prescriptionList.get(0).getVisit().getDoctor().getLastName():"");
}
}
%>
...
<%}else{%>
		<% if (hinId !=0) { %>
			...
			submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showOPDPatientIssueGridByHin');
			
		<% 
		//System.out.println("(hinId !=0) ");
		}else if(patient.getId()!=null){ %>
		var prescriptionId ='<%=prescriptionId%>';
				submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showOPDPatientIssueGridByHin&prescriptionId='+prescriptionId);
...
*/
 ...
 function jsDisplay()
	...
	function generateRowPrescription(rowVal)
	 {
		...
		/* for(var z=1; z<=listSize; z++)
			...
			{ */
				if(document.getElementById("qtyIssued"+rowVal)!=null && document.getElementById("qtyIssued"+rowVal).value.trim() != "")
				{
					...
					/* previousAddedRow = z; */
					issueQty = document.getElementById("qtyIssued"+rowVal).value;
					prescribeQty = document.getElementById("qtyPrescription"+rowVal).value;
				}
		...
		icdArray[2]="BMW";
		  ...
		  e02.type = 'hidden';		  
		  e02.name='itemType'+iteration;
		  e02.id='itemType'+iteration;
		  e02.readOnly=true;
		  e02.value='mat';
		  ...
		  e1.type = 'text';
		  e1.name='patientPrescriptionDtId'+iteration;
		...
		/*   if(document.getElementById('qtyIssued'+rowVal).value=="")
		  {	  
		    ...
		    */
		    ...
		    document.getElementById("qtyPrescription"+rowVal).value=issueQty;
		 ...
		 // e39.setAttribute("validate","lpQty,string,yes")
		  ...
		  var cellRight401 = row.insertCell(14);
		  var e401 = document.createElement('input');
		  e401.type = 'text';		  
		  e401.id = 'doctorName' + iteration;
		  e401.value=document.getElementById('doctorName'+rowVal).value;		  
		  cellRight401.appendChild(e401);
		  
		  var cellRight40 = row.insertCell(15);
		  ...
		  var cellRight41 = row.insertCell(16);
	...
	function removeRowPrescription(element, rowNum)
	{
		 ...
		 var presQty = parseInt(document.getElementById('qtyPrescription'+rowNum).value);
	  var lastRow = tbl.rows.length;
	  if (lastRow > 2){
	  	...
	  	for(var i=listsize; i>=1; i--)
	  		...
	  		if(document.getElementById('qtyIssued'+i) != null && document.getElementById('itemId'+i).value == itemId)
	  			...
	  			document.getElementById('qtyPrescription'+i).value = parseInt(document.getElementById('qtyPrescription'+i).value)+presQty;
	  			break;
	  			}
	  		}
	...
	function checkQtyValue()
	{
		...
		{
			if(document.getElementById('qtyIssued'+i)!=null && document.getElementById('qtyIssued'+i).value=="")
			{		
...
function fillPrescribeBy() 
{
	...
	if(stockFlag)
	{
		...
		if(saveFlag){
			fillItemsInGrid();submitForm('opdPatientIssue','stores?method=submitOPDPatientStockDetailsByHin');
		}
	}
}
	  ...
	  function cheackForBatch(rowVal){
		   ...
		   if( document.getElementById('batchNo'+rowVal).value=='0')
			...
			/* 	if(document.getElementById("batchNo"+rowVal).value==document.getElementById("batchNo"+i).value)
				...
				if(rowVal != i && (document.getElementById('button'+rowVal)==null || document.getElementById('button'+rowVal).value!="add"))
				{
