...
<%
	...
	if(map.get("createReport")!=null){
		createReport=(Boolean)map.get("createReport");
	}
	
	String temprelation=patientsReg.substring(patientsReg.length()-3, patientsReg.length());
	String relation="NA";
...
<div class="popupbg">
	...
	<table class="divmain"  style="width: 1004px;">
		<tr>
			<td align="right" class="historytd">Name : <b><%=patientsName %> &nbsp;&nbsp;</b>Reg.No :&nbsp;<b><%=patientsReg%></b>&nbsp;&nbsp;Relation : <b><%=relation %></b></b>&nbsp;&nbsp;Gender : <b><%=patientsSex %></b></td> 
		</tr>
		<tr>
		...
		<div class="divmain"> 
			<table style="width: 1000px;"  class="small">
				...
				<tr id="<%=patientPacsReportList.get(i).getReportId()%>" style="background: lightgoldenrodyellow;">
					...
					<td align="right" class="tdstyle"><%if(patientPacsReportList.get(i).getFileStatus().equalsIgnoreCase("Y")){ %><a href="/hms/hms/pacs?method=downloadReportFile&reportId=<%=patientPacsReportList.get(i).getReportId()%>&patientName=<%=patientsName%>"><b>Download Report</b></a><%} %></td> 
...
function validateReportforms()
{
	window.onbeforeunload = null;
	 if (confirm("Are You Sure To Save Report?") == true) {
			...
			var action="pacs?method=submitPatientReport&reportStatus="+reportStatus+"&patientId="+patientId+"&patientReg="+<%=patientsReg%>+"&createReport=y";
			errorMsg="";
			ob1 = true;
			ob2 = true;
			ob3 = true;
...
function submitProtoAjaxForTemplate(){	
	...
	if(tempname=="" || tempname==" "){
		...
		var action="pacs?method=getTemplateForRepors&tempname="+tempname+"&patientId="+<%=patientsId%>+"&createReport=y";
		errorMsg = "";
		ob1 = true;
		ob2 = true;
		ob3 = true;
 ...
 {	  
	 ...
	 if (confirm("Are You Sure To Delete Report?") == true) {
		 var formName="patientReports" 
				var action="pacs?method=deleteReport&reportId="+reportId+"&patientId="+patientId+"&createReport=y";
				errorMsg = "";
				ob1 = true;
				ob2 = true;
				ob3 = true;
 ...
 { 
	window.onbeforeunload = null;
	var formName="patientReports" 
	var action="pacs?method=editReport&reportId="+reportId+"&patientId="+patientId+"&createReport=y";
	errorMsg = "";
	ob1 = true;
	ob2 = true;
	ob3 = true;
