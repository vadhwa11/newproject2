...
<input type="button" name="Submit" id="addbutton"
	onclick="if(checkBlankSearch()){submitForm('patientSearch','/hms/hms/mediClaim?method=showPatientDetailsForGeneralTracking');}"
...
<form name="searchGeneralClaim" method="post" action=""><script
...
<script type="text/javascript" language="javascript">
	...
	%>
			...
			<%
					...
					%>
			data_arr[<%= counter%>][5] = "<%=contigentMedicalBillHd.getHin().getPFirstName()%> <%=middleName%> <%=lastName%>"
		<%
		 counter++;
		    	}
		}
		%>		
    formName = "searchGeneralClaim"
	start = 0
