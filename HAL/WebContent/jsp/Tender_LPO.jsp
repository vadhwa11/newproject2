...
if (TenderLPOForm.<%=TENDER_NO%>.value == "")
	{
	alert("Please select Tender No!.....");
	//TenderLPOForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value = "";
	return;
	}
TenderLPOForm.method="post";
submitForm('TenderLPOForm','tender?method=getTenderSupplierListForLPO&groupId='+TenderLPOForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value);
//submitForm('TenderLPOForm','tender?method=getTenderGroupListForLPO&tenderId='+TenderLPOForm.<%=TENDER_NO%>.value);
}

function onChangeGroup()
{
if (TenderLPOForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value == "")
	{
	alert("Please select Group!.....");
	TenderLPOForm.<%=TENDER_NO%>.value = "";
	return;
	}

TenderLPOForm.method="post";
submitForm('TenderLPOForm','tender?method=getTenderGroupListForLPO&groupId='+TenderLPOForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value);
//submitForm('TenderLPOForm','tender?method=getTenderSupplierListForLPO');
}

function openPopupWindow(supplier_id,amount)
{
 var tender_id = TenderLPOForm.<%=TENDER_NO%>.value;
 var group_id = TenderLPOForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value;
 var dept_id = TenderLPOForm.deptId.value;
...
<div class="Clear"></div>
<div class="blockTitle">Tender - SO For L1</div>
