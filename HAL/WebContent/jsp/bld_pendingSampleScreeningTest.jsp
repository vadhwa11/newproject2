...
<h6>Pending For Blood Grouping & Cross Matching</h6>
<div class="Clear"></div>
...
<input type="button" name="submit" id="addbutton"
	onclick="submitForm('patientSearch','/hms/hms/bloodBank?method=searchPatientForBloodSampleScreening','check()');"
	value="Search" class="CmnButton" accesskey="a" /> <jsp:include
	page="searchResultBlock.jsp" />
<div class="division"></div>
<div id="searchresults" tabindex="2">
<div id="searchtable" tabindex="2"></div>
<form name="pendingBloodSampleScreeningTest" method="post" action="">
<input type="hidden" name="<%=CHANGED_BY %>" value="<%=userName%>" /> <input
	type="hidden" name="<%=CHANGED_DATE %>" value="<%=date%>" /> <input
	type="hidden" name="<%=CHANGED_TIME %>" value="<%=time%>" /> <script
	type="text/javascript">
	  formFields = [
			[0, "sampleId", "id"],[1,"orderBy"],[2,"servNo"],[3,"sPersonName"],[4,"serviceType"], [5,"hin"], [6,"patName"], [7,"rank"]];
	  statusTd = 7;
	</script></form>
</div>
</div>
...
data_header[0][0] = "Order By"
data_header[0][1] = "data";
data_header[0][2] = "7%";
data_header[0][3] = "orderBy"
...
data_header[2][0] = "ServicePerson Name"
data_header[2][1] = "data";
data_header[2][2] = "15%";
data_header[2][3] = "sPersonName";
	...
	<% 	
		for(BloodSampleCollection sampleCollection: patientDetailList){
			...
			%>
					...
					data_arr[<%= counter%>][1] = "<%= sampleCollection.getRequest().getDepartment().getDepartmentName()%>"
...
%>
    formName = "pendingBloodSampleScreeningTest"
	start = 0
