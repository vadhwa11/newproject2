...
formFields = [
		[0, "<%= RequestConstants.HIN_ID%>", "id"],[1, "<%= RequestConstants.RADIO_FOR_TABLE%>"], [2,"<%=RequestConstants.AD_NO%>"], [3,"<%= RequestConstants.AD_DATE %>"], [4,"<%= RequestConstants.DISCHARGE_DATE%>"],[5,"<%=RequestConstants.HOSPITAL_NAME%>"],[6,"<%=RequestConstants.DIAGNOSIS_ID%>"],[7,"<%=RequestConstants.DISPOSAL_ID %>"]]; 
 statusTd =13;
</script></div>
<div id="edited"></div>
...
<script	type="text/javascript" language="javascript">
		...
		data_header[1][0] = "SI No. "
		data_header[1][1] = "data";
		data_header[1][2] = "5%";
		data_header[1][3] = "SI No."

		data_header[2] = new Array;
		data_header[2][0] = "A&D No."
		data_header[2][1] = "data";
		data_header[2][2] = "8%";
		data_header[2][3] = "<%=RequestConstants.AD_NO%>"
		...
		data_header[5][0] = "Hospital/SMC"
		data_header[5][1] = "data";
		data_header[5][2] = "5%";
		data_header[5][3] = "<%=RequestConstants.HOSPITAL_NAME%>";
		  
		data_header[6] = new Array;
		data_header[6][0] = "Diagnosis"
		data_header[6][1] = "data";
		data_header[6][2] = "6%";
		data_header[6][3] = "<%=RequestConstants.DIAGNOSIS_ID%>";
		
		data_header[7] = new Array;
		data_header[7][0] = "Disposal"
		data_header[7][1] = "data";
		data_header[7][2] = "5%";
		data_header[7][3] = "<%=RequestConstants.DISPOSAL_ID%>";
		...
		data_arr[<%= i%>][2] ="<%=sino%>"
			...
			data_arr[<%= i%>][3] = "<%=discharge.getInpatient().getAdNo()%>"
					
			<%
				if(discharge.getInpatient().getDateOfAddmission()!=null)
				{
			%>
			data_arr[<%= i%>][4] = "<%=discharge.getInpatient().getDateOfAddmission()%>"
			<%
				}else{
			%>
			data_arr[<%= i%>][4] = ""
			<%
			   ...
			   {
			%>
			data_arr[<%= i%>][5]="<%=discharge.getDateOfDischarge()%>"
			<%
			   }else{
			%>
			data_arr[<%= i%>][5]=""
			<%
			   ...
			   {
			%>
			data_arr[<%= i%>][6] = "<%=discharge.getHospital().getHospitalName()%>"
			<%
			   }else{
			%> 
			data_arr[<%= i%>][6] = ""
			<%
			   ...
			   {
			%>
			data_arr[<%= i%>][7] = "<%=discharge.getWorkingDiagnosis()%>"
			<%
			   }else{
			%>
			data_arr[<%= i%>][7] = ""
			<%
			   ...
			   {
			%>
			data_arr[<%= i%>][8] = "<%=discharge.getDisposal().getDisposalName()%>"
			<%
			   }else{
			%>
			data_arr[<%= i%>][8] = ""
...
<script type="text/javascript">	
	function submitFormForDischargeSummaryReport(){
		...
		if(flag == false){
			...
			submitForm('opd_previousVisitForHospitality','/hms/hms/fwc?method=showHospitalizedDischargeSummaryReport');
			checkTargetForNo();
		}
