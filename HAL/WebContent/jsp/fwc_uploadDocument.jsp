...
<input type="hidden" name="<%=CHANGED_TIME %>" value="<%=time%>" class="textbox_size20" readonly="readonly" tabindex=3 />
  <input type="hidden" name="fwcFlag" id="fwcFlag" size="5" value="<%=fwcFlag%>" />

<div class="Clear"></div>
<div class="division">
<input type="hidden" name="flag" value="yes" id="flag"></div>
...
<input type="button" class="button" name="impbutton" value="Attach"
	onClick="jsImport(<%=visitId %>,'<%=fwcFlag %>')" /> 
<input type="button"
	class="button" name="impbutton" value="View Documents"
	onClick="viewPatientDocuments('<%=fwcFlag %>');" /> 	
<%
if(visitId!=0){
%>
<input name="Back" type="button"
	...
	<%-- onclick="submitForm('appointmentUpload','fwc?method=showAntCardJsp&visitId=<%=visitId%>&deptId=<%=departmentId%>');" --%>
	align="right" />
<%} %>
<div class="Clear"></div>
...
function jsImport(visitId1,fwcFlag)
{
	...
	submitForm('appointmentUpload','fwc?method=submitFwcUploadDocuments'+fileNames + '&<%=CHANGED_BY%>=' + changed_by + '&<%=CHANGED_DATE%>=' + changed_date + '&<%=CHANGED_TIME%>=' + changed_time + '&hospitalId=' + hospitalId + '&deptId=' + deptId+'&visitId='+visitId1+'&uploadCount='+inc+'&fwcFlag='+fwcFlag);
	...
	function viewPatientDocuments(fwcFlag)
	{
		var num;
		...
		if(document.getElementById("visitId") && document.getElementById("hinNo")){
		 ...
		 submitForm('appointmentUpload','fwc?method=viewFwcDocument&flag1=viewDocuments&fieldValue='+num+'&hin_no='+hin_no+'&visitId='+visitId+'&fwcFlag='+fwcFlag,'mywindow');
