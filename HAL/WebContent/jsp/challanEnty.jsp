...
function removeRow(idName,rowLen,obj)
{
...
<script language="javascript">
	...
	{
	alert('Pl. Select a .xls file to upload!.....');
	return;
	}
...
<input type="file" name="<%=UPLOAD_FILENAME %>" id="exlfileNameId"  style="height: 24px; width:250px; margin-right: 20PX " class="browse">
...
%>
	...
	<div class="clear"></div>
	<label class="auto">Amount Pending for Proforma B Entry</label>
...
<input type="button" name="details" id="details" align="right" class="buttonBig2" onblur="" value="Import Supply Order" onclick="getsoitems();" />
<input type="hidden" name="details" id="details" align="right" class="button" onblur="" value="Import Medcine" onclick="submitForm('grnGrid','stores?method=showLPImportGrnJsp');" />
	...
	<label >SO Date<span>*</span></label>
 	<input type="text" name="<%=LOANIN_DATE%>"	value=""  id="soIdDate" class="" readonly="readonly" validate="So Date, String ,No" MAXLENGTH="30" />
	 ...
	 <a href="javascript:setdate('<%=currentDate%>',document.loanGrid.receivedDate,true)">
		...
		<%if(storeloanObj != null){ %> 
		<input type="text" 	name="<%=CHALLAN_NO %>" id="challanNo" class="auto" size="20"	value="<%=storeloanObj.getChallanNo() %>" validate="Challan No, String ,No" MAXLENGTH="20"/  > 
		<%}else{ %> 
		<input type="text"  name="<%= CHALLAN_NO %>" value="" class="auto" size="20" id="challanNo" validate="Challan No, String ,No" tabindex=1 	MAXLENGTH="20" /> 
		<%} %>
...
<input	type="checkbox"  name="partiallyReceive" id="partiallyReceive" value="y"	 MAXLENGTH="30" />
...
<input type="button" name="sss" align="right" id="submitForDisable" class="button" value="Submit"	onclick="submitChallanForm('loanGrid','stores?method=submitLoanIn');" />
...
function submitChallanForm(form,url)
{
	if(!document.getElementById("partiallyReceive").checked)
		{
			if(confirm("Are you sure that you have received complete supply order if no then Please check the checkbox  'Received Partially' "))
			{
				submitForm(form,url);
			}
			else
