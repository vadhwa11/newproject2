...
<%@page import="jkt.hms.masters.business.Visit"%><script type="text/javascript" language="javascript"src="/hms/jsp/js/proto.js"></script>
...
<%
		...
		AviCa34 aviCa34Obj = new AviCa34();
		
		if(aviCa34List.size() > 0){
			aviCa34Obj = aviCa34List.get(0);
		}
		...
		}System.out.println("templateList--JSp--->"+templateList.size());
...
<div class="clear"></div>
<div class="titleBg">
<h2>Civil Aviation Med Exam(MA)</h2> </div>
...
<div class="Block">
<%int avExamId=0;
	if(medicalExamMaMoObj !=null && medicalExamMaMoObj.getId() !=null){ %>
<input type="hidden" name="visitId" value="<%=aviCa34Obj.getVisit().getId() %>"/>
<input type="hidden" name="hinId" value="<%=aviCa34Obj.getHin().getId() %>"/>
<input type="hidden" name="visitNo" value="<%=aviCa34Obj.getVisit().getVisitNo() %>"/>
<input type="hidden" name="avica34Id" value="<%=aviCa34Obj.getId() %>"/>

	<%avExamId=medicalExamMaMoObj.getId();
%>
<input type="hidden" name="medExamMaMoId" value="<%=avExamId %>"/><%}else{ %>
...
<%int medExamMaMoId=0;
if(medicalExamMaMoObj !=null && medicalExamMaMoObj.getId() !=null){ 
medExamMaMoId=medicalExamMaMoObj.getId();} %>
<label>Designation  </label> 
<input type="text" value="" readonly="readonly" name="<%=DESIGNATION%>"	tabindex="2" maxlength="20"/>
<label>Name  </label> 
<%  
	String fullName="";
	fullName=aviCa34Obj.getFirstName();
	if(aviCa34Obj.getLastName() !=null){
		fullName=fullName+" "+aviCa34Obj.getLastName() ;
	}
	String licenceApplied="";
	String licenceHeld="";
	String licenceNo="";
	if(aviCa34Obj.getLicenceNo() !=null){
		licenceNo=aviCa34Obj.getLicenceNo();
	}
	if(aviCa34Obj.getTypeOfLicenceApplied() !=null){
	licenceApplied=aviCa34Obj.getTypeOfLicenceApplied().getCaLicenceName();
	}
	if(aviCa34Obj.getLicenceHeld() !=null){
	licenceHeld=aviCa34Obj.getLicenceHeld().getCaLicenceName();
	}
%>
...
<label>Age  </label> 
	<%
	String age="";
		if(aviCa34Obj.getAge() !=null){
			age=aviCa34Obj.getAge();}%>
<input	type="text" readonly="readonly" maxlength="20"  value="<%=age%>" name="<%=AGE %>"	tabindex="2" />

<label>Gender</label> 
<input	type="text" readonly="readonly" value="<%=aviCa34Obj.getSex().getAdministrativeSexName() %>" 
...
<h4>Investigations'S<a href="javascript:animatedcollapse.toggle('slide5')"></a></h4>
<div id="slide5">
<div class="Block">
...
onClick="submitForm('civilAviationMedExam','aviationMedicine?method=updateAviationMA&data=farwarded&Labresult=<%=Labresult.trim() %>');" />
...
function fileUploadWindowInvestigation(rowVal)
{
	var avExamId='<%=avExamId%>';
 	...
 	var invest_id = val.substring(index1,index2);
 		var url="/hms/hms/aviationMedicine?method=displayFileUploadInvestigation&hinId=<%=visit.getHin().getId()%>&hinNo=<%=visit.getHin().getHinNo() %>&invest_id="+invest_id+"&avExamId=<%=avExamId%>";
...
function showHideInvestigationTemplateCombo(valueOfTemplate){
	...
	if(validateMetaCharacters(aviationFlag)){
	submitProtoAjaxWithDivName('civilAviationMedExam','/hms/hms/opd?method=showGridForInvestigationMedicalExam&flag='+aviationFlag,'gridview');
	  }
	}
}
	...
	alert("avExamId-"+avExamId);
 	...
 	}else
 	{
	   var folderName='hearing';
		var url="/hms/hms/aviationMedicine?method=displayFileUpload&hinId=<%=visit.getHin().getId()%>&visitId=<%=visit.getId()%>&hinNo=<%=visit.getHin().getHinNo()%>&folder="+folderName+"&avExamId=<%=avExamId%>";
