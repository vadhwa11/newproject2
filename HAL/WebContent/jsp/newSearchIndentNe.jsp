...
<%	
	Map map = new HashMap();
		...
		List<StoreIssueT> searchList = new ArrayList<StoreIssueT>();
		...
		System.out.println("searchIndentList  in JSP "+searchIndentList.size());
	}
...
<script type="text/javascript" language="javascript">
		...
		//Iterator itr=objectList.iterator();
		          int  counter=0;
		          //while(itr.hasNext())
		        	  for(StoreIndentM sim : searchIndentList)
		           {           
		             ...
		             data_arr[<%= counter%>][0] = <%= ""+sim.getId()%>
		             data_arr[<%= counter%>][1] = '<input type="radio"  name="parent" value="<%= ""+sim.getIndentNo()%>" id="radio" />'    							 
		             data_arr[<%= counter%>][2] = "<%=sim.getIndentNo()%>"
		             data_arr[<%= counter%>][3] = "<%=HMSUtil.changeDateToddMMyyyy((Date)sim.getIndentDate())%>"
		             data_arr[<%= counter%>][4] = '<input type="hidden"  name="parent1" value="<%= ""+sim.getId()%>" id="parent1" />'
...
<!-- <input type="button" name="Modify" type="submit" value="Modify" 	class="button" onClick="if(requiredFields()){submitForm('searchIndent','stores?method=searchIndentDepot');}"> -->
 ...
 var deptId='24';
 	...
 	for(var i = 0; i < document.getElementsByName('parent').length; i++){
			  if(document.getElementsByName('parent')[i].checked == true)
              {
                ...
                submitForm('searchIndent','neStores?method=printIndentTODepoNe&indentNo='+document.getElementsByName('parent')[i].value+"&deptId="+deptId);
                return true;
			  }		
  		}
...
function requiredFields(){
	 for(var i = 0; i < document.getElementsByName('parent').length; i++){
				  if(document.getElementsByName('parent')[i].checked == true)
	              {
					  ...
					  submitForm('searchIndent','stores?method=searchIndentDepot&indentNoForSearch='+document.getElementsByName('parent1')[i].value);
	                return true;
				  }		
	  		}
	  		alert("Please select the Indent  No. !!")
