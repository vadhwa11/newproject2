...
<script language="javascript">
	...
	function checkForGrn(val,inc)
	{
		...
		//    if (src=='l')
		    	ajaxFunctionForAutoCompleteInGrn1('grnGrid','stores?method=fillItemsForGrn1&requiredField=' + pvms + "&poId=" + poId , inc);
		  //  else
	  ...
	  try {
		...
		//	obj2 = document.getElementById(dispenseType); 
		     ...
		     //   var expiry = item.getElementsByTagName("expiry")[0];
		        ...
		        document.getElementById('nameItem'+rowVal).value=itemName.childNodes[0].nodeValue
...
<script language="javascript">
	function jsImportPro()
	{


	submitForm('grnGrid','stores?method=showLPGrnForExlJsp');
...
function addRow(idName,rowLen) {
	  ...
	  e1.onblur=function(){if(fillSrNo(iteration)){checkForGrn(document.getElementById('codeItem'+iteration).value,iteration)}};
	  
	  //e1.onblur = function(){fillItem(document.getElementById('codeItem'+rowLen).value, iteration)}
...
<%
		...
		List<MasStoreItem> itemList = new ArrayList<MasStoreItem>();
				...
				if(map.get("storeProformaNoList")!=null){
					storeProformaNoList=(ArrayList)map.get("storeProformaNoList");
				}
...
<label>From Date</label> <input type="text" name="<%= FROM_DATE %>"
	class="date" maxlength="30" onkeyup="mask(this.value,this,'2,5','/');" onblur="validateExpDate(this,'fromDate');" id="fromDate"  tabindex=1 /> <img id="calendar"
...
<label>To Date</label> <input type="text" name="<%= TO_DATE %>"
	class="date" maxlength="30" onkeyup="mask(this.value,this,'2,5','/');" onblur="validateExpDate(this,'toDate');" id="toDate"  tabindex=1 /> <img id="calendar"
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" tabindex="1"
	onClick="setdate('<%=currentDate%>',document.createGrn.<%=TO_DATE%>,event)" />
<div class="clear"></div>
<label>CRV/RV No.</label> <input type="text" name="<%=GRN_NO%>" value=""
	tabindex=1 MAXLENGTH="100" id="civNo" />
<div id="ac3update" style="display: none;" class="autocomplete"></div>
<script type="text/javascript" language="javascript" charset="utf-8">
			  new Ajax.Autocompleter('civNo','ac3update','stores?method=getGrnNoListForAutoComplete',{parameters:'requiredField=<%=GRN_NO%>'});
			</script>

<input type="hidden" name="supplyType"  id="supplyType" value="l">
...
<select name="<%=SOURCE_OF_SUPPLY%>" id="sourceCombo" tabindex=1
 ...
 --><label>
   CRV/RV No.<span>*</span></label> <% if(storeGrnObj != null){%> <input
...
CRV/RV Date<span>*</span></label> <input type="text" class="date" name="<%=GRN_DATE%>" id="grnDate"
	value="<%=currentDate %>" MAXLENGTH="10" onkeyup="mask(this.value,this,'2,5','/');" tabindex=1 onblur="validateExpDate(this,'grnDate');"/>
...
<label>Date Received</label> <input type="text"
	class="date" name="<%=RECEIVED_DATE%>" value="<%=currentDate %>" id="receivedDate"
	MAXLENGTH="10" onkeyup="mask(this.value,this,'2,5','/');" tabindex=1 onblur="validateExpDate(this,'receivedDate');"/> <img id="calendar"
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	class="calender" validate="Pick a date" tabindex="1"
	onClick="setdate('<%=currentDate%>',document.grnGrid.<%=RECEIVED_DATE%>,event)" />
<!--  <label>Invoice No.</label>-->
 <%if(storeGrnObj != null){ %>
  <input type="hidden" name="<%=INVOICE_NO %>" id="<%=INVOICE_NO%>" value="<%=storeGrnObj.getInvoiceNo()%>" validate="Invoice No, String, no" maxlength="30" tabindex=1 />
	 <%}else{ %>
<input type="hidden" name="<%=INVOICE_NO %>" id="<%=INVOICE_NO%>" value="" validate="Invoice No,String,no" MAXLENGTH="30" tabindex=1 /> <%} %>
	<!--   <label>Invoice Date</label>--> 
<input type="hidden" class="date" name="<%=INVOICE_DATE%>" readonly id="invoiceDate" value="<%=currentDate %>" maxlength="30" tabindex=1 />
...
<input type="text" name="invoiceAmt" value="">
<label>Unpacked-Checked By<span>*</span></label> <select
	name="<%= EMPLOYEE_ID %>"    tabindex=1 >
	<option value="">Select</option>
	<%
	String firstName="";
	String lastName="";
...
<input type="button" name="sss" align="right" class="button"
	value="Submit"
	onclick="if(checkForCRVGrid() && testForGrn()&& checkSave()&& checkForSubmit() && checkInvoiceNo()){submitForm('grnGrid','stores?method=submitGrnwithprint');}" />
