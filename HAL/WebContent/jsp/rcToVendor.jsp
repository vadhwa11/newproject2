...
<h4><%=message %></h4>
<div class="titleBg">
<h2>Creation of RC to Vendor</h2>
</div>
<div class="clear"></div>
...
<div class="clear"></div>
<input class="buttonSm" name="Button2" value="Search" onclick="removeValidation();submitForm('search','/hms/hms/stores?method=rcToVendor');" type="button" />
...
<table>
		<thead><tr><th>S.No.</th><th>MAT Code</th><th>Nomenclature</th><th>RC Quantity</th><th>From</th></tr></thead>
		<tbody>
		<%
		String status="N/A";
		...
		%>
			...
			<td><%=rc[1] %></td>
			<td><%=rc[2]%></td>
			<td><input type="hidden" name="requestedQty<%=count %>" id="requestedQty<%=count %>" value="<%=rc[3]%>"/><%=rc[3]%></td>
			<td><%=rc[4]%></td>
			</tr>
		<%count++;} %>
		</tbody>
