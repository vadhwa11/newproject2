...
<input type="button" name="search" value="Search" class="button"	onclick="submitForm('search','inPatientMaster?method=searchBodyPart','checkSearch')"	tabindex=1 />
 <%--- Report Button   --%> <input type="button"	name="Report" value="Generate Report Based on Search" class="button"	onClick="submitForm('search','hospital?method=generateReportForHospitalRelatedMasters');"	accesskey="g" tabindex=1 />
  <input type="hidden"	name="<%=JASPER_FILE_NAME%>" value="Mas_body_part">
  </div>
</form>
...
<br />
<form name="bodyPart" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasBodyPart"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="BodyPartName">
<input type="hidden" name="title" value="BodyPart"> <input
	type="hidden" name="<%=JSP_NAME %>" value="bodyPart"> <input
	type="hidden" name="pojoPropertyCode" value="BodyPartCode"> <br>
...
<label><span>*</span> BodyPart Code </label> 
	...
	<input id="codeId" type="text"	name="<%= CODE%>" value="" validate="BodyPart Code,string,yes"	class="textbox_size20" MAXLENGTH="8"  tabindex=1>
	<label><span>*</span> BodyPart Name </label> 
	<input type="text" name="<%= SEARCH_NAME %>" value=""	validate="BodyPart Name,string,yes" class="textbox_size20"	MAXLENGTH="30" tabindex=1	onkeypress="return submitenter(this,event,'inPatientMaster?method=addBodyPart')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masBodyPart.getLastChgBy() %>"
