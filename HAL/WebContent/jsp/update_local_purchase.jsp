...
<%
			for (MasItemCategory masItemCategoryL :masItemCategory ) {
				try{
				System.out.println("this is JSP before po object"+masItemCategoryL.getId().toString());
				System.out.println("this is before po object"+poHeaderObj.getCategory().toString());
				if(poHeaderObj.getCategory().toString().equals(masItemCategoryL.getId().toString())){
					System.out.println("this is po object"+poHeaderObj.getCategory().toString());
%>
...
%>
	...
	<input	type="text"  name="telephoneNo"  value="<%=poHeaderObj.getTelephoneNo()%>"  validate="telephoneNo ,String,no" tabindex=1 />
	<%}else{%>
		<input	type="text"  name="telephoneNo"  value=""  validate="telephoneNo ,String,no" tabindex=1 />
	<%}%>
	...
	<textarea value="<%=poHeaderObj.getRemarks()%>" name="<%=REMARKS %>" validate="Remarks ,String,no" tabindex=1 onpaste="return checkOnPaste(this)" oninput="return checkMaxLengthMoz(this)" onkeydown="return checkMaxLength(this)" onkeyup="finalCheck(this)" maxlength="200"/>
	...
	<thead>
		<tr>
		  ...
		  <th width="6%">Packaging</th>
		   <th width="7%">Ordered </br>Quantity(OQ)</th>
		...
		<!-- -  
	      ...
	      <th width="5%">MRP per</br>Packaging</th>
	      <th width="5%">Disc(%)</th>
	      <th width="5%">Tax(%)</th>
    ...
    %>
     ...
     <%-- 
      ...
      <%} %>
			...
			<td width="10%">
			<input type="text" class="medcaption" value="<%=storePoDetail.getLsoQty()==null?"0":storePoDetail.getLsoQty() %>"  size="5" name="" id="<%=actualqtyin %>" validate="ActualQuantity,float,no" onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);" tabindex="1" maxlength="9" /> 
...
<td width="10%">
    <input type="text" 
	  ...
	  <option value="y">Yes</option>
       ...
       -->
	    ...
	    //	 document.purchaseGrid.noOfRows.value=<%//=inc-((pageNo-1)*10)-1%>;
	    	 ...
	    	 document.purchaseGrid.noOfRows.value=<%=inc-1%>
	    	 </script> <%
		...
		System.out.println("this is inc in jsp"+inc);
		System.out.println("this is incTem"+incTemp2);
     ...
     <%-- 
      ...
      <td width="10%">
       <input type="text" value="0" class="medcaption" name="" id="<%=actualqtyin %>"  validate="ActualQuantity,float,no" onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);" tabindex="1" maxlength="9" size="5"/>
      ...
      <td width="10%">
      <input type="text"  name="mdq" id="<%=mdq%>" onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);" validate="Minimum Dispensable Qty (MDQ),num,no" value="1" tabindex="1" maxlength="9" size="8"/>
      </td>
      <td width="10%">
      <input type="text" value="0"  name="" id="<%=quantityInVarTemp %>" readonly validate="Quantity,float,no" onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);" tabindex="1" maxlength="9" size="5"/>	
	 <input type="hidden" value="0"  name="<%=QUANTITY %>" id="<%=quantityInVar%>" />
      ...
      -->
      <td width="10%">
      <input type="text"  value="0" name="mrp"  id="<%=mrp%>"  onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);" validate="Tax,float,no" maxlength="15" tabindex="1" size="5"/>
      </td>
      <td width="3%">
      <input type="text"  value="0" name="<%=DISCOUNT_PERCENTAGE%>"  id="<%=discountVarTemp%>" onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);"  validate="Discount,float,no" maxlength="6" tabindex="1" size="4"/>
      <input type="hidden" class="medcaption" value="0" name="<%=DISCOUNT_AMOUNT%>" id="<%=discountAmount%>"/>
      </td>
 
      <td width="10%">
      <input type="text" class="medcaption" value="0" name="<%=TAX_PERCENT %>"  id="<%=taxVarTemp%>" onblur="gridCalculationLocalSupplyOrderAdd(<%=inc%>);" validate="Tax,float,no" maxlength="15" tabindex="1" size="4"/>
...
<input type="button" name="update" align="right" class="button" value="Update" onclick="if(checkForSubmit() ){submitForm('purchaseGrid','purchaseOrder?method=updatePurchaseOrder');}" />
<input type="button" name="delete" align="right" class="button"
	value="Delete"
	onclick="submitForm('purchaseGrid','purchaseOrder?method=deletePurchaseOrderItem');" />
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
</form>
