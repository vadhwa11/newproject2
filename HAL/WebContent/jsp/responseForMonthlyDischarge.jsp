...
<%@page import="jkt.hms.masters.business.MonthlyDischargeDt"%>
...
if(map.get("monthlyDisList")!=null){
	monthlyDisList = (List<MonthlyDischargeDt>)map.get("monthlyDisList");
}

int monthlyDisHdId= 0;
if(monthlyDisList.size() > 0){
	for(MonthlyDischargeDt obj : monthlyDisList){
		monthlyDisHdId = obj.getMonthlyDisHd().getId();
		break;
	}
}
%>
<input type="hidden" name="monthlyDisHdId" value="<%=monthlyDisHdId %>"/>
<div class="cmntable">
...
<thead>
	<tr>
		...
		int i=1;
			if(monthlyDisList.size() > 0){
				for(MonthlyDischargeDt moSickAdmDetails : monthlyDisList){
	...
	<%--<td><input type="text" id="doa<%=i %>" name="doa<%=i %>" readonly="readonly" size="8" value="<%=(moSickAdmDetails.getDateOfAdmission()!=null?HMSUtil.convertDateToStringWithoutTime(moSickAdmDetails.getDateOfAdmission()):"") %>" onblur="getNoOfDays('<%=i %>')"/></td>
		...
		<td><input type="text" id="dod<%=i %>" name="dod<%=i %>" readonly="readonly" size="8" value="<%=(moSickAdmDetails.getDateOfDischarge()!=null?HMSUtil.convertDateToStringWithoutTime(moSickAdmDetails.getDateOfDischarge()):"") %>""/></td>
		<td><img id="calendar" src="/hms/jsp/images/cal.gif" width="16"	height="16" border="0" class="calender"	onClick="setdate('',document.monthlySick.dod<%=i %>,event)" />
		<input type="hidden" name="monthlyDisDtId<%=i %>" value="<%=moSickAdmDetails.getId()%>" id="monthlyDisDtId<%=i %>">
	...
	<%--<td><input type="text" id="noOfDays<%=i %>" name="noOfDays<%=i %>"  size="2" value="<%=(moSickAdmDetails.getNoOfDays()!=null?moSickAdmDetails.getNoOfDays():"") %>"/> 	</td>--%>
	...
	<%--<td><input type="text" name="remarks<%=i %>" maxlength="200" value="<%=(moSickAdmDetails.getRemarks()!=null?moSickAdmDetails.getRemarks():"") %>"/></td> --%>	
		...
		<% i++;
			...
			}else if(dischargeList.size() > 0){
				for(Object[] obj : dischargeList){
		%>
		<tr>
		<td><input type="text" name="serviceNo<%=i %>" readonly="readonly" value="<%=(obj[6]!=null?obj[6]:"") %>" size="7"/></td>
		<td><input type="text" name="adNo<%=i %>" readonly="readonly" value="<%=(obj[12]!=null?obj[12]:"") %>" size="7"/></td>
	...
	<%--<td><input type="text" id="doa<%=i %>" name="doa<%=i %>" readonly="readonly" size="8" value="<%=(obj[7]!=null?HMSUtil.convertDateToStringWithoutTime((Date)obj[7]):"") %>" onblur="getNoOfDays('<%=i %>')"/></td>
		...
		<td><input type="text" id="dod<%=i %>" name="dod<%=i %>" readonly="readonly" size="8" value="<%=(obj[8]!=null?HMSUtil.convertDateToStringWithoutTime((Date)obj[8]):"") %>" /></td>
		<td><img id="calendar" src="/hms/jsp/images/cal.gif" width="16"	height="16" border="0" class="calender"	onClick="setdate('',document.monthlySick.dod<%=i %>,event)" />
		<input type="hidden" name="monthlyAdmDtId<%=i %>" value="0" id="monthlyAdmDtId<%=i %>">
		<input type="hidden" name="hinId<%=i %>" value="<%=obj[11]%>" id="hinId<%=i %>">
		<input type="hidden" name="inpatientId<%=i %>" value="<%=obj[0]%>" id="inpatientId<%=i %>">
		</td>
	<%--<td><input type="text" id="noOfDays<%=i %>"  name="noOfDays<%=i %>" readonly="readonly" size="2" value="<%=(obj[12]!=null?Integer.parseInt(obj[12].toString())+1:"") %>"/> </td> --%>	
...
<input type="button" name="save" value="submit" onclick="submitForm('monthlySick','/hms/hms/mis?method=submitMonthlySickDischarge');"/>
<input type="button" name="print" value="Print" onclick="submitForm('monthlySick','/hms/hms/mis?method=searchMonthlySickDischargeReport');"/>
