...
<h6> Application For medical Advance</h6>
<form name="medicalAdvance" target="_blank" action="" method="post">
...
<input 	type="text" class="calDate" id="fromDate" name="<%=FROM_DATE %>" value="" validate="From Date,date,no" MAXLENGTH="10" tabindex="1" readonly="readonly"/> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.medicalAdvance.<%=FROM_DATE%>,event)" />
	...
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.medicalAdvance.<%=TO_DATE%>,event)" />
...
<div class="bottom"><input type="button" name="OK" value="OK" class="button"
	onClick="if(testFromToDate())if(checkBlankSearch())submitForm('medicalAdvance','/hms/hms/mediClaim?method=printApplicationMedicalAdReport','validateFromToDate');" />
...
<script type="text/javascript">
	...
	obj1 = eval(document.medicalAdvance.fromDate)
	obj2 = eval(document.medicalAdvance.toDate)
