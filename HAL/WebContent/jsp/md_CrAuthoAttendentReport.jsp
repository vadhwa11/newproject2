...
<h6> Certificate Of Authorized Attendant</h6>
<form name="certifiedAutho" target="_blank" action="" method="post">
...
<input 	type="text" class="calDate" id="fromDate" name="<%=FROM_DATE %>" value="<%=currentDate %>" validate="From Date,date,yes" MAXLENGTH="10" tabindex="1" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.certifiedAutho.<%=FROM_DATE%>,event)" />
	...
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.certifiedAutho.<%=TO_DATE%>,event)" />
	
</div>
<div class="Clear"></div>
<div class="bottom"><input type="button" name="OK" value="OK" class="button"
	onClick="submitForm('certifiedAutho','/hms/hms/mediClaim?method=printContingentBillRemReport','validateFromToDate');" />
...
<script type="text/javascript">
	...
	obj1 = eval(document.certifiedAutho.fromDate)
	obj2 = eval(document.certifiedAutho.toDate)
