...
<div class="titleBg"><h2>Pending List For  Voucher Approval</h2></div>
<div class="clear"></div>
<form name="voucherApproval" method="post" action="">
<div class=Block>
...
$j(document).ready(function()
		{
		getpendingListForVoucherApproval('ALL');
	
		});
		
function getpendingListForVoucherApproval(MODE)
{
	var data;
	...
	var url = "account?method=pendingListForVoucherApproval";
	var bClickable = true;
	...
	function makeTable(jsonData)
	{
		...
		for(var i=0;i<jsonData.length;i++)
			{
			htmlTable = htmlTable+"<tr id='"+jsonData[i].Id+"' onclick=executeEvent("+jsonData[i].Id+",'"+jsonData[i].VoucherType+"')>";
	...
	function executeEvent(Id,VoucherType)
	{
		var locationURL = "account?method=showApprovalJspForVoucher&headerId="+Id+"&VoucherType="+VoucherType;
		submitFormForButton('voucherApproval',locationURL);
	} 
...
{
	nPageNo = pageNo;	
	getpendingListForVoucherApproval('FILTER');
}
