...
<%
		...
		if(patientMap.get("patientDetailList") != null){
			patientDetailList= (List<BloodSampleScreeningHeader>)patientMap.get("patientDetailList");
		}
...
<label> Service No.</label> <input type="text" name="<%=SERVICE_NO %>"
	...
	name="<%=HIN_NO %>" value="" MAXLENGTH="50" /> <label>A&D No.</label>
<input type="text" name="<%=AD_NO %>" value="" MAXLENGTH="50" /> <label>Order
...
<form name="pendingBloodIssue" method="post" action=""><input
	type="hidden" name="<%=CHANGED_BY %>" value="<%=userName%>" /> <input
	type="hidden" name="<%=CHANGED_DATE %>" value="<%=date%>" /> <input
	type="hidden" name="<%=CHANGED_TIME %>" value="<%=time%>" /> <script
	type="text/javascript">
	  formFields = [
			[0, "screeningId", "id"],[1,"orderNo"],[2,"orderDate"],[3,"orderTime"],[4,"servNo"],[5,"sPersonName"],[6,"serviceType"], [7,"hin"], [8,"patName"], [9,"rank"]];
	  statusTd = 9;
	</script></form>
</div>
</div>
...
data_header[3][2] = "15%";
data_header[3][3] = "servNo";

data_header[4] = new Array;
data_header[4][0] = "ServicePerson Name"
...
data_header[8][2] = "10%";
data_header[8][3] = "rank";
data_arr = new Array();
	<%
	    int  counter=0;
		if (patientDetailList != null && patientDetailList.size() > 0){ %>
	<% 	
		for(BloodSampleScreeningHeader bloodSampleScreeningHeader: patientDetailList){
			Patient patient = bloodSampleScreeningHeader.getHin();
			%>
			  		data_arr[<%= counter%>] = new Array();
					data_arr[<%= counter%>][0] = <%= bloodSampleScreeningHeader.getId()%>
					data_arr[<%= counter%>][1] = "<%=bloodSampleScreeningHeader.getSampleCollection().getRequest().getOrderNo()%>"
					data_arr[<%= counter%>][2] = "<%=HMSUtil.convertDateToStringWithoutTime(bloodSampleScreeningHeader.getSampleCollection().getRequest().getOrderDate())%>"
					data_arr[<%= counter%>][3] = "<%=bloodSampleScreeningHeader.getSampleCollection().getRequest().getOrderTime()%>"
					
					<%if(patient != null && patient.getServiceNo()!= null){%>
					...
					<%}else{%>
					 ...
					 <%   
					if(patient != null && patient.getSFirstName() != null  && !(patient.getSFirstName().equals(""))){
					...
					data_arr[<%= counter%>][8] = "<%=patient.getPFirstName()%> <%=patient.getPLastName()%>"
					data_arr[<%= counter%>][9] = "<%=patient.getRank().getRankName()%>"
						<%}else{%>
						data_arr[<%= counter%>][5] = ""
						data_arr[<%= counter%>][6] = ""
					data_arr[<%= counter%>][7] = ""
