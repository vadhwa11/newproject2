...
<%
	...
	if(map.get("FlagFOrMedExamMa")!= null)
	{
		...
		System.out.println("FlagFOrMedExamMa====************>>>>"+FlagFOrMedExamMa);
	}
...
<form name="dentalPatientPrevVisitForViewScreen" method="post" action="">
...
<script	type="text/javascript" language="javascript">
		...
		%>
			...
			data_arr[<%= i%>][1] = '<input type="radio" class="radiogrid" name="parent" value="<%=visit.getHin().getServiceNo()%>@<%=visit.getVisitNo()%>@<%=visit.getHin().getHinNo()%>@<%=visit.getId()%>" id="parent" onclick="fillVisitNo(this)"/>'
...
<script type="text/javascript">	
	function submitFormForPrescriptionReport(){
		...
		submitForm('dentalPatientPrevVisitForViewScreen','dental?method=showPatientMedicalDentalCaseSheetReport');
	...
	function showPreviousVisit(hinId,deptId,visitNo,token){
       ...
       document.dentalPatientPrevVisitForViewScreen.action="/hms/hms/dental?method=getPatientOpdDentalDetails&flag=current&hinId="+hinId+"&deptId="+deptId+"&visitNumber="+visitNo+"&token="+token+"&link=<%=link%>";
      
       document.dentalPatientPrevVisitForViewScreen.submit();
