...
<form name="movementSearchIn" action="" method="post"><script
...
Name</label> <input type="text" name="<%=S_LAST_NAME %>" id="<%=S_LAST_NAME %>"
	...
	name="add_other_person" id="add_other_person"
	value="Add Person From Other Units" style="width: 200px;"
	class="cmnButton"
	onClick="window.open('hrOrderly?method=showPersonnelFromOtherUnitJsp')" />
<div class="Clear"></div>
<div class="Clear"></div>
</div>
</form>
...
<input type="button" name="submit" id="addbutton"
	onclick="submitForm('movementSearchIn','/hms/hms/hrOrderly?method=searchRecordsForMovementIn');"
...
<form name="movementIn" method="post" action=""><script
	type="text/javascript">
formFields = [
[0, "<%= EMPLOYEE_ID%>", "id"],[1,"<%=SERVICE_NO%>"],[2,"<%=RANK_ID%>"],[3,"<%=S_FIRST_NAME%>"],[4,"employee_person","code"]];
statusTd = 4;
</script></form>
</div>
...
data_header[3][0] = "employee_person"
data_header[3][1] = "hide";
data_header[3][2] = "10%";
data_header[3][3] = "employee_person";
...
data_arr[<%= counter%>][3] = "-"
<%}%>
<%if(emp.getEmployeeCode() != null && !emp.getEmployeeCode().equals("")){%>
