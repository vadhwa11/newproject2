...
<script
	...
	function check(){
		//alert("sss");
		if(document.patientSearch.orderNo.value=="")
		{
		...
		{
			submitForm('patientSearch','/hms/hms/lab?method=searchPatientForBackUp&flag=DIAG');
		}
	}
	
</script>

<div class="titleBg"><h2>Backup Facility of Sample Collection by Order No</h2></div>
<div class="Clear"></div>
...
<label> Order No (For Backup Facility)</label> <input type="text" validate ="Order no,string,yes" name="orderNo" id="orderNo" value="" MAXLENGTH="20"  onkeypress="return searchKeyPress(event);"/>
...
<form name="pendingSampleCollection" method="post" action=""></form>
<input type="hidden" name="<%=CHANGED_BY %>" value="<%=userName%>" /> 
<input type="hidden" name="<%=CHANGED_DATE %>" value="<%=date%>" /> 
<input type="hidden" name="<%=CHANGED_TIME %>" value="<%=time%>" /> 
<script	type="text/javascript">
	  formFields = [
			[0, "orderId", "id"],[1,"orderNo"],[2,"orderDate"],[3,"servNo"],[4,"patName"], [5,"relation"], [6,"rank"], [7,"sPersonName"], [8,"age"], [9,"sex"], [10,"unit"], [11,"pType"],[12,"priority"],[13,"Doctor"]];
	  statusTd = 11;
	</script>
	</div>
</form>
...
data_arr = new Array();
	...
	<% 
		...
		for(DgOrderhd dgOrderhd: patientDetailList){
			...
			//System.out.println("dgOrderhd----------"+dgOrderhd.getId());
				
			if(flag.equals("lab") && status.equalsIgnoreCase("p")){
			 ...
			 <%   try{
					if(patient.getSFirstName() != null  && !(patient.getSFirstName().equals(""))){
						...
						data_arr[<%= counter%>][11] = "<%=dgOrderhd.getDepartment()!=null?dgOrderhd.getDepartment().getDepartmentName():"" %> "
							data_arr[<%= counter%>][13] = "<%=dgOrderhd.getPrescribedBy()!=null?dgOrderhd.getPrescribedBy().getFirstName():"" %> "
