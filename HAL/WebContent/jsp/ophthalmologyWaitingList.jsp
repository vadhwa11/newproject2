...
<form name="opdOphthalPatientList" method="post" action="">
<div class="clear"></div>
...
<div class="clear"></div>
<div class="floatRight"><label class="auto">Total Waiting:</label> <label class="valueAuto"><%=patientList.size()%></label></div>
<div class="clear"></div>
<div class="paddingTop5"></div>
<div class="clear"></div>
<div class="Block">
...
</select>  --%><label> Employee No.</label> <input type="text" name="<%=SERVICE_NO%>"
...
</select>--%> <input type="button" name="search" value="Search"
	onClick="submitForm('opdOphthalPatientList','opd?method=showOphthalmologyWaitingList')"
...
<script type="text/javascript" language="javascript">
	...
	if(visit.getVisitStatus().equalsIgnoreCase("w")  || visit.getVisitStatus().equalsIgnoreCase("r") || visit.getVisitStatus().equalsIgnoreCase("c") )
	{
...
data_arr[<%= i%>][3] = " <%=visit.getSession().getId()==1?"M-":visit.getSession().getId()==2?"E-":""%>  <%=visit.getTokenNo()==0?"R":visit.getTokenNo()%>"
	...
	formName = "opdOphthalPatientList"
