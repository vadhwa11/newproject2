...
<!-- 			<th width="10%">Barcode</th> -->
	...
	<%
		for(PatientPrescriptionDetails prescriptionDetails : presList){
			if(prescriptionDetails.getItemStopStatus()!=null && prescriptionDetails.getItemStopStatus().equalsIgnoreCase("y"))
				continue;
		int qtyIssued = 0;
...
<%}%>
			...
			BatchList = (List<Object[]>) mapItem.get(prescriptionDetails.getItem().getId());
							
			%>
			<select name="batchNo<%=i%>" id="batchNo<%=i%>"  class="small3" onchange="if(cheackForBatch(<%=i%>)){getDataForBarBatchStockId(this.value,<%=i%>)}" >
			...
			boolean batchflag = true;
				for (Object[] listA: BatchList) {
					if(batchflag)
					{
						batchflag=false;	
						%>
						<option selected="selected" value="<%=listA[0]%>" ><%=listA[1]%></option>
						<%
					}
					else
					{
		...
		<script type="text/javascript">
			...
			<input type="hidden" name="list<%=i%>" id="list<%=i%>" size="10" readonly="readonly" value="<%=BatchList.size()%>" />
		...
		%>
			<input type="text"  size="6" name="qtyIssued<%=i%>" id="qtyIssued<%=i%>" value="<%=prescriptionDetails.getTotal() %>"  onblur="checkQty(<%=i%>)"/>
		<%}else{ %>
        ...
        <td><input type="text"  readonly="readonly" id="doctorName<%=i%>" value='<%=prescriptionDetails.getPrescription().getEmp()!=null?prescriptionDetails.getPrescription().getEmp().getFirstName():""%>' /></td>
		...
		<td width="8%"><input name="Button" type="button" id="button<%=i%>" class="buttonAdd" value="add" onclick="generateRowPrescription(<%=i %>);" /></td>
		<%}%>
		
		<%if(deptId!=24){%> 
		
		<td width="8%"><input name="Button" type="button"  id="button<%=i%>" class="buttonAdd" value="add"  onclick="if(generateRowPrescription(<%=i %>)){document.getElementById('qtyIssued'+<%=i %>).readOnly=true;}" /></td>
		<%}%>
...
<tr>
		...
		<td><input type="text"  readonly="readonly" value='<%=prescriptionDetails.getPrescription().getEmp()!=null?prescriptionDetails.getPrescription().getEmp().getFirstName():""%>' /></td>
		<td ><input type="text" size="10" readonly="readonly" value='<%=prescriptionDetails.getRemarks()!=null?prescriptionDetails.getRemarks():""%>' name="remark<%=i%>"  id="remark<%=i%>"  /></td>
		<input type="hidden" name="qtyPending<%=i%>" id="qtyPending<%=i %>"	value="<%=qtyPending %>" />
...
<input type="button" name="Submit"	value="Submit" class="button" onclick="fillPrescribeBy();" />
