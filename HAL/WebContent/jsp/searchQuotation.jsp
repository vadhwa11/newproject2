 ...
 * searchQuotation.jsp  
 * Purpose of the JSP -  This is for Search for Quotation.
 * @author  Deepali
 * @author  Mansi
...
<%
	...
	Map map = new HashMap();
		...
		try{
			gridQuotationHeaderList=(List)map.get("gridQuotationHeaderList");
		...
		<table width="20%" align="right" border="0" cellpadding="0"
			...
			<tr>
				...
				<td width="10%" background="/hms/jsp/images/toolbuttBack.gif"><input
					...
					class="toolbutton"
					onClick="submitForm('searchQuotation','stores?method=quotationModifyJsp');"></td>
...
<table cellpadding="4" cellspacing="1" border="0">
	...
	<tr>
		...
		<label class="bodytextB_blue">Quotation No:</label> <select
			...
			onClick="submitForm('createPo','stores?method=searchQuotation');" />
		</td>
	</tr>
...
<script type="text/javascript" language="javascript">
	...
	formFields = [
			[0, "<%= RequestConstants.QUOTATION_ID%>", "id"],[1, "<%= RequestConstants.QUOTATION_REQUEST_NO%>"], [2,"<%= RequestConstants.QUOTATION_DATE%>"], [3,"<%= RequestConstants.QUOTATION_TYPE %>"], [4,"<%= RequestConstants.STORE_SUPPLIER_ID %>"],[5,"<%=RequestConstants.STATUS%>"] ];
	 statusTd =5;
...
<script type="text/javascript" language="javascript">
		...
		data_header[1][0] = "QUOTATION No"
		data_header[1][1] = "data";
		data_header[1][2] = "15%";
		data_header[1][3] = "<%= RequestConstants.QUOTATION_REQUEST_NO%>"
		
		data_header[2] = new Array;
		data_header[2][0] = "QUOTATION Date"
		data_header[2][1] = "data";
		data_header[2][2] = "15%";
		data_header[2][3] = "<%= RequestConstants.QUOTATION_DATE %>";
		
		data_header[3] = new Array;
		data_header[3][0] = "QUOTATION TYPE"
		data_header[3][1] = "data";
		data_header[3][2] = "15%";
		data_header[3][3] = "<%=RequestConstants.QUOTATION_TYPE %>";
		
		data_header[4] = new Array;
		data_header[4][0] = "SUPPLIER"
		data_header[4][1] = "data";
		data_header[4][2] = "15%";
		data_header[4][3] = "<%=RequestConstants.STORE_SUPPLIER_ID %>";
		...
		Iterator itrrr=gridQuotationHeaderList.iterator();
		          ...
		          while(itrrr.hasNext())
		           {           
		        	  StoreQuotationReceiptM  storeQuotationM = (StoreQuotationReceiptM)itrrr.next(); 
		             %>
		
			data_arr[<%= counter%>] = new Array();
			
			data_arr[<%= counter%>][0] = <%= storeQuotationM.getId()%>
			data_arr[<%= counter%>][1] = '<input type="radio"  name="parent" value="<%= storeQuotationM.getId()%>" id="radio" />'
			data_arr[<%= counter%>][2] = "<%=storeQuotationM.getQuotationNo()%>"
			data_arr[<%= counter%>][3]="<%= storeQuotationM.getQuotationDate()%>"
			 data_arr[<%= counter%>][4]="<%= storeQuotationM.getQuotationType()%>"
			 data_arr[<%= counter%>][5]="<%= storeQuotationM.getSupplier().getSupplierName()%>"
             	
				<%if(storeQuotationM.getStatus().equals("y")){%>
