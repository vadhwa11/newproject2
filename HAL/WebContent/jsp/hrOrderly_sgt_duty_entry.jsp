...
function add()
{
	...
	{
		var url="/hms/hms/hrOrderly?method=showOrderlyDutyAddJsp&entryNo="+guardDuty.entryNo.value+"&entryDate="+guardDuty.entryDate.value+"&dutyId="+guardDuty.dutyId.value+"&toDate="+guardDuty.toDate.value+"&fromDate="+guardDuty.fromDate.value;
...
function GoPage() {	
	...
	submitForm('guardDuty','hrOrderly?method=getGridDataForOrderlyEmployee');
}
...
function jsUpdate()
{
		...
		{
		guardDuty.method="post";
		submitForm('guardDuty','hrOrderly?method=updateOrderlyDutyEntry','checkForNextDutyDate');
		}
		else
...
function checkDutyStartEndDate(){
var msg="";
var currentDate=new Date();
var startdate=document.getElementById('fromDate').value;
var enddate=document.getElementById('toDate').value;
if(startdate!=""){
var startdate1=new Date(startdate.substring(6),(startdate.substring(3,5)-1),startdate.substring(0,2));
if(currentDate>startdate1){
msg=msg+"startdate should be greater than current date \n";
document.getElementById('fromDate').value="";
}
}
if(enddate!=""){
if(startdate!=""){
var enddate1=new Date(enddate.substring(6),(enddate.substring(3,5)-1),enddate.substring(0,2));
if(enddate1<startdate1){
msg= msg+"enddate should be greater than startdate \n";
document.getElementById('toDate').value="";}
}else{
msg=msg+"enter start date first \n";
document.getElementById('toDate').value="";
}
}
if(msg!="")
...
function jsDisplay() {
	...
	submitForm('searchPanel','hrOrderly?method=searchOrderlyDutyData&searchEntryNo='+searchPanel.searchEntryNo.value+'&searchEntryDate='+searchPanel.searchEntryDate.value);
}  
...
<table cellpadding="4" cellspacing="1" border="0">
	...
	<tr>
		<td class="vbmenu_option" title="nohilite"><label
			...
			name="searchEntryNo" id="searchEntryNo" value="<%=entryNo %>" /> <!-- code 1 -->
		<DIV class="Clear"></DIV>
		<label>Entry Date</label> <input type="text" id="searchEntryDate"
			name="searchEntryDate" class="textbox_Date" value="" MAXLENGTH="30"
...
<form name="guardDuty" action="" method="post">
<h6>Orderly SGT Duty Entry(CPL & Below)</h6>
<div class="Clear"></div>

<div class="blockFrame"><label class="medium">Entry No.</label> <input
	name="entryNo" type="text" class="calDate" id="entryNo"
	value="<%=entryNo%>" readonly="readonly" /> <label class="medium">Entry
Date</label> <input name="entryDate" type="text" class="calDate" id="entryDate"
...
<input type="hidden" name="dutyId" id="dutyId" value="<%=dutyId%>" /> <label
	...
	id="fromDate" class="calDate" onfocus="checkDutyStartEndDate();"
	name="<%=FROM_DATE %>" value="<%=box.get(FROM_DATE)%>" MAXLENGTH="30"
	readonly="readonly" /> <img src="/hms/jsp/images/cal.gif" width="16"
	height="16" border="0" validate="Pick a date"
	onClick="setdate('<%=currentDate%>',document.guardDuty.<%=FROM_DATE%>,event);" />

<%
		String flag= "false";
	%> <label class="noWidth">Duty End Date</label> <input type="text"
	id="toDate" class="calDate" onfocus="checkDutyStartEndDate();"
	name="<%=TO_DATE %>" value="<%=box.get(TO_DATE)%>" MAXLENGTH="30"
	readonly="readonly" /> <img src="/hms/jsp/images/cal.gif" width="16"
	height="16" border="0" validate="Pick a date"
	onClick="setdate('<%=currentDate%>',document.guardDuty.<%=TO_DATE%>,event);" />
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%
			    	int iFirstRow=0;
			    for(HrOrderlySgtDutyEntry hrorderlyGuardDutyEntry:dutyEntryList)
			    { 
			    	iFirstRow++;
			    %>
		<tr>
			<td width="10%"><input type="text"
				value="<%=hrorderlyGuardDutyEntry.getEmp().getServiceNo()%>"
				name="pvms" readonly="readonly" /></td>
			<td width="40%"><input type="text"
				value="<%=hrorderlyGuardDutyEntry.getEmp().getRank().getRankName()%>"
				name="nomenclature" readonly="readonly" /></td>
			<td width="12%"><input type="text"
				value="<%=hrorderlyGuardDutyEntry.getEmp().getFirstName()%>"
				name="au" readonly="readonly" /></td>
			<td width="40%"><input type="text"
				value="<%=HMSUtil.convertDateToStringWithoutTime(hrorderlyGuardDutyEntry.getDutyDate())%>"
				name="brand" readonly="readonly" /></td>
			<td><input type="text" value="" name="nextDutyDate"
				onfocus="checkForNextDutyDate();" id="nextDutyDate<%=iFirstRow%>"
				readonly="readonly" /> <img src="/hms/jsp/images/cal.gif" width="16"
				height="16" border="0" validate="Pick a date" tabindex="1"
			...
			<td width="40%"><Select name="dutyTime"
				...
				<%for(HrDutyTimeMaster hrDutyTimeMaster:dutyMasterList){ 
							if(hrDutyTimeMaster.getId()==hrorderlyGuardDutyEntry.getDutyTime().getId()){
					%>
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%
			    	int iFirstRow=0;
			    for(HrOrderlySgtDutyEntry hrorderlyGuardDutyEntry:searchGuardDutyList)
			    { 
			    	iFirstRow++;
			    %>
		<tr>
...
<div class="bottom"><input type="button" class="button"
	...
	onClick="if (validateButton()){submitForm('guardDuty','hrOrderly?method=deleteOrderlyDutyEntry');}else{alert('Pl. Select Employee to delete.');}"
