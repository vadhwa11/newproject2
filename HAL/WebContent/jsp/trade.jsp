	  ...
	  <div id="searcharea">
			   ...
			   <form name="search" method="post" action="">
			    <div class="Block">
					 ...
					 <input type="text" id="searchField" name="<%= SEARCH_NAME%>" value=""  validate="Trade Name,string,no"   MAXLENGTH="8" tabindex=1  onkeypress="return submitenter(this,event,'personnel?method=searchTrade')"/>
                    <input type="button" name="search" value="Search" class="button" onclick="submitForm('search','personnel?method=searchTrade','checkSearch')" tabindex=1  />
<%--- Report Button   --%>  
<input type="button" name="Report" value="Generate Report Based on Search" class="buttonBig3" onClick="submitForm('search','hospital?method=generateReportForGeneralMasters');" accesskey="g" tabindex=1/>
                    <input type="hidden" name="<%=JASPER_FILE_NAME%>" value="Mas_trade">
		     </form>
<div class="Clear"></div>
...
<div class="Clear"></div>
   ...
   <h4> <a href="personnel?method=showTradeJsp">Show All Records</a></h4>
	<%
			}
		 }
  ...
  <input type="hidden" name="<%= POJO_NAME %>" value = "MasTrade">
  <input type="hidden" name="<%=POJO_PROPERTY_NAME %>" value = "TradeName">
  <input type="hidden" name="title" value = "Trade">
  <input type="hidden" name="<%=JSP_NAME %>" value = "trade">
  <input type="hidden" name="pojoPropertyName" value = "TradeName">
			<div class="Clear"></div>
	  		
 	<div class="Block">

	  	<label > Trade Name <span>*</span> </label>
		<input type="text" name="<%= SEARCH_NAME%>" value="" validate="Trade Name,string,yes"  MAXLENGTH="30"/ tabindex=1 >
  			<script>
				document.trade.<%=SEARCH_NAME%>.focus();
			</script>
			
			<label > Service Type <span>*</span> </label>
			<select name="<%= SERVICE_TYPE_ID %>" validate="Service Type,string,yes" tabindex=1 onkeypress="return submitenter(this,event,'personnel?method=addTrade')">
...
<div class="Clear"></div>
			...
			<div class="Clear"></div>
			<input type="button" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('trade','personnel?method=addTrade');" accesskey="a" tabindex=1/>
			<input type="button" name="edit" id="editbutton" value="Update" class="button" onClick="submitForm('trade','personnel?method=editTrade')" accesskey="u" tabindex=1 />
			<input type="button" name="Delete" id="deletebutton" value="Activate" class="button" onClick="submitForm('trade','personnel?method=deleteTrade&flag='+this.value)" accesskey="d" tabindex=1/>		
...
<%
		Iterator itrGridServiceTypeList=gridServiceTypeList.iterator();
		 while(itrGridServiceTypeList.hasNext())
			...
			<%}
            }catch(Exception e){System.out.println("ssssssss "+e);}}%>
