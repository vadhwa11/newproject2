...
<label class="value"> <%=referralPatientDetails.getWard()!=null?referralPatientDetails.getWard().getWardName():""%></label>
...
<div class="Clear"></div>
<label>HR Remarks</label><textarea validate="Remarks,string,no" maxlength="500" name ="admin_remarks" id ="admin_remarks" readOnly="true"/><%=finance_remarks%></textarea>
<label>Remarks</label><textarea validate="Remarks,string,no" maxlength="500" name ="gm_remarks" id ="gm_remarks" /></textarea>
<div class="Clear"></div>
...
<div class="Block" id="billingList">
  <label><font color="red">Deduction From Salary</font></label><input type="text" style="margin-right: 0px;" validate="Deduction From Salary,string,nos" maxlength="19" name ="deductionFS" id ="deductionFS" readOnly="readonly" value=<%=deductionFS!=null?deductionFS:0%> />
  <div class="Clear"></div>
<% 
...
<input type="hidden" name="referralBillingId<%=divCountBilling%>" id="referralBillingId<%=divCountBilling%>" value="<%=referralPatientBilling.getId()%>"/>
    <label>Bill No</label><input type="text"  maxlength="15"  value='<%=referralPatientBilling.getBillNo()!=null?referralPatientBilling.getBillNo():""%>'  readOnly="readOnly"/>
<label>Bill Date</label><input type="text"  maxlength="15"  value='<%=referralPatientBilling.getBillDate()!=null?HMSUtil.convertDateToStringTypeDateOnly(referralPatientBilling.getBillDate()):""%>'  readOnly="readOnly"/>
<label>Cheque No</label><input type="text"  maxlength="15"  value='<%=referralPatientBilling.getCheckNo()!=null?referralPatientBilling.getCheckNo():""%>'  readOnly="readOnly"/>
<div class="Clear"></div>
<label>Check Date</label><input type="text"  maxlength="15"  value='<%=referralPatientBilling.getCheckDate()!=null?HMSUtil.convertDateToStringTypeDateOnly(referralPatientBilling.getCheckDate()):""%>'  readOnly="readOnly"/>

    <label>Total Bill Amount</label><input type="text" validate="Empanelled Bill,int,yes" maxlength="15" name ="impanelled_bill<%=divCountBilling%>" id ="impanelled_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getTotalBillAmt()!=null?referralPatientBilling.getTotalBillAmt():""%>'  readOnly="readOnly"/>

<label>Approved Bill Amount</label><input type="text" validate="HAL Bill,int,yes" maxlength="15" name ="hal_bill<%=divCountBilling%>" id ="hal_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getAdminBillAmt()!=null?referralPatientBilling.getAdminBillAmt():""%>'  readOnly="readOnly"/>
...
<label>TDS</label><input type="text" validate="TDS,int,no" maxlength="15" name ="tds<%=divCountBilling%>" id ="tds<%=divCountBilling%>" value='<%=referralPatientBilling.getTaxAmount()!=null?referralPatientBilling.getTaxAmount():""%>'  readOnly="readOnly"/>

<label>Net Amount</label><input type="text" validate="Net Amount,int,no" maxlength="15" name ="net_amount<%=divCountBilling%>" id ="net_amount<%=divCountBilling%>" value='<%=referralPatientBilling.getNetAmount()!=null?referralPatientBilling.getNetAmount():""%>'  readOnly="readOnly"/>

<div class="Clear"></div>
<input type="hidden" id="referralNoId<%=divCountBilling%>" name="referralNoId<%=divCountBilling%>"  value="<%=referralPatientBilling.getReferralDetails().getId()%>"  readOnly="readOnly">
...
<select id="referralNoId<%=divCountBilling%>" name="referralNoId<%=divCountBilling%>"  validate="Referral Number,string,yes"  readOnly="readOnly">
	...
	<label>Admin Remarks</label><textarea class="large" onkeyup="auto_grow(this)" validate="Remarks,string,no" maxlength="500" name ="admin_remarks<%=divCountBilling%>" id ="admin_remarks<%=divCountBilling%>" readOnly="readOnly" /><%=referralPatientBilling.getAdminRemarks()!=null?referralPatientBilling.getAdminRemarks():""%></textarea>
  ...
  <% ++divCountBilling;
	...
	{
		 %>
		 <label>Covernote Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getCovernoteDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getCovernoteDate()):""%></label>
		 <label>Covernote No</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getCovernoteNumber()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getCovernoteNumber():""%></label>
		<div class="clear"></div>  
	<label>Hr Approval Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrApprovalDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrApprovalDate()):""%></label></textarea>	 
	<label>Hr Remarks</label><textarea class="large" onkeyup="auto_grow(this)" validate="Remarks,string,no" maxlength="500"  readOnly="readOnly"/><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks():""%></textarea>
	<div class="clear"></div> 
	 <label>Notesheet Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getNoteSheetDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getNoteSheetDate()):""%></label>
     <label>Notesheet No</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getNoteSheetNo()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getNoteSheetNo():""%></label>
	<div class="clear"></div> 
	<label>Finance Approval Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getFinanceApprovalDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getFinanceApprovalDate()):""%></label></textarea>
    <label>Finance Remarks</label><textarea class="large" onkeyup="auto_grow(this)" validate="Remarks,string,no" maxlength="500"  readOnly="readOnly" /><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getFinanceRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getFinanceRemarks():""%></textarea>
    <div class="clear"></div>     
	 <label>Dispatch Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getDispatchDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getDispatchDate()):""%></label>
     <label>Dispatch No</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getDispatchNumber()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getDispatchNumber():""%></label>
	<div class="clear"></div> 
    
	 <label>Consolidation of Bill Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getConsolidateDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getConsolidateDate()):""%></label>
     <label>Consolidation of Bill No</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getConsolidateNumber()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getConsolidateNumber():""%></label>
      <div class="clear"></div>     
	 <label>Bill Upload Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getCompleteBillDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getCompleteBillDate()):""%></label>
...
<div class="Block">
	...
	<input type="button" name="yes" value="Print Covering Letter"	class="button"	onclick="submitFormForButton('generateReferralLetterPage','/hms/hms/referral?method=showReferralReport&flag=covering_letter&referral_patient_header_id=<%=referralPatientHeaderId%>');" />
...
<div class="Block">
 <%
	 ...
	 if(map.get("deducted").toString().equals("n"))
    	{
    	%>
    <input type="button" name="yes" value="Deducted" class="button"	onclick="submitFormForButton('generateReferralLetterPage','/hms/hms/referral?method=submitDeduction&referralHeaderId=<%=referralPatientHeaderId%>');"  />
