...
function add()
{
	...
	{
		var url="/hms/hms/hrOrderly?method=showRangFiringDutyAddJsp&entryNo="+guardDuty.entryNo.value+"&entryDate="+guardDuty.entryDate.value+"&dutyId="+guardDuty.dutyId.value+"&toDate="+guardDuty.toDate.value+"&fromDate="+guardDuty.fromDate.value;
...
function GoPage() {	
	...
	submitForm('guardDuty','hrOrderly?method=getGridDataForRangFiringEmployee');
}
...
function jsUpdate()
{
		...
		{
		guardDuty.method="post";
		submitForm('guardDuty','hrOrderly?method=updateRangFiringDutyEntry','checkForNextDutyDate');
		}
		else
...
function jsDisplay() {
	...
	submitForm('searchPanel','hrOrderly?method=searchRangFiringDutyData&searchEntryNo='+searchPanel.searchEntryNo.value+'&searchEntryDate='+searchPanel.searchEntryDate.value);
}  
...
<%
	...
	List<HrRangeFiringDutyEntry> searchGuardDutyList = new ArrayList<HrRangeFiringDutyEntry>();
...
<h6>RangFiring Duty Entry(CPL & Below)</h6>
<div class="Clear"></div>
...
<input type="hidden" name="dutyId" id="dutyId" value="<%=dutyId%>" /> <label
	...
	onfocus="checkDutyStartEndDate();" value="<%=box.get(FROM_DATE)%>"
...
<%
	...
	id="toDate" class="calDate" name="<%=TO_DATE %>"
	onfocus="checkDutyStartEndDate();" value="<%=box.get(TO_DATE)%>"
	MAXLENGTH="30" readonly="readonly" /> <img
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%
			    	int iFirstRow=0;
			    for(HrRangeFiringDutyEntry hrorderlyOfficerDutyEntry:dutyEntryList)
			    { 
			    	iFirstRow++;
			    %>
		<tr>
			<td width="10%"><input type="text"
				value="<%=hrorderlyOfficerDutyEntry.getEmp().getServiceNo()%>"
				name="pvms" readonly="readonly" /></td>
			<td width="40%"><input type="text"
				value="<%=hrorderlyOfficerDutyEntry.getEmp().getRank().getRankName()%>"
				name="nomenclature" readonly="readonly" /></td>
			<td width="12%"><input type="text"
				value="<%=hrorderlyOfficerDutyEntry.getEmp().getFirstName()%>"
				name="au" readonly="readonly" /></td>
			<td width="40%"><input type="text"
				value="<%=HMSUtil.convertDateToStringWithoutTime(hrorderlyOfficerDutyEntry.getDutyDate())%>"
			...
			<td width="40%"><Select name="dutyTime"
				...
				<%for(HrDutyTimeMaster hrDutyTimeMaster:dutyMasterList){ 
							if(hrDutyTimeMaster.getId()==hrorderlyOfficerDutyEntry.getDutyTime().getId()){
					%>
			...
			<td><input type="text" name="typeStatus" id="typeStatus"
				value="<%=hrorderlyOfficerDutyEntry.getTypeStatus()%>"
				readonly="readonly" /></td>
			<td><input type="checkbox" name="dutyToBeUpdated" class="radio"
				value="<%=hrorderlyOfficerDutyEntry.getId()%>"></td>
			<td><input type="hidden"
				value="<%=hrorderlyOfficerDutyEntry.getId()%>" name="guardDutyId" /></td>
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%
			    	int iFirstRow=0;
			    for(HrRangeFiringDutyEntry hrorderlyOfficerDutyEntry:searchGuardDutyList)
			    { 
			    	iFirstRow++;
			    %>
		<tr>
...
<div class="bottom"><input type="button" class="button"
	...
	onClick="if (validateButton()){submitForm('guardDuty','hrOrderly?method=deleteOfficerDutyEntry');}else{alert('Pl. Select Employee to delete.');}"
