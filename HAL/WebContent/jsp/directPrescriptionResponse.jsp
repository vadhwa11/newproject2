...
<%	}
		...
		if(map.get("dependentId") != null){
			dependentId=(Integer)map.get("dependentId");
		}
		...
		if(patientList.size()>0){
			flag = "Block";
			System.out.println("patientList.get(0).getPFirstName()"+patientList.get(0).getPFirstName());
			...
			System.out.println("Age="+patientList.get(0).getDateOfBirth());
			...
			if(patientList.get(0).getId()!=null)
			{
				...
				hinNo =patientList.get(0).getHinNo();
				 ...
				 bloodGroup = patientList.get(0).getBloodGroup()!=null?patientList.get(0).getBloodGroup().getBloodGroupName():"";
				 allergy = patientList.get(0).getBloodGroup()!=null?patientList.get(0).getDrugAllergies():"";
			}
...
<input type="text" id="serviceNo" name="<%=SERVICE_NO%>" value="<%=serviceNo %>" readonly="readonly"	 MAXLENGTH="30"	 />
...
<label class="value"> <%=bloodGroup!=null?bloodGroup:"" %> </label>
<div class="clear"></div>

<label> allergy </label>
<input type="text" name="allergies" maxlength="500" value="<%=allergy!=null?allergy:"" %>" style="width:478px;"> 
</div>

<!-- <div class="photoImageDiv"><img width="105" height="112" src="/hms/hms/personnel?method=displayImageEmployeeDependent&amp;employeeDependentId=2589"></div> -->
	...
	%>		
			<img
				src="/hms/hms/personnel?method=displayImageEmployeeDependent&amp;employeeDependentId=<%=dependentId%>"
				width="105" height="112">
		</div>
		<%}else{
		
		String patientTypeNameForHAL = HMSUtil.getProperties("adt.properties", "patientTypeNameForHAL");
		
		  if(patientList.size()>0 && patientList.get(0).getPatientType()!=null && patientList.get(0).getPatientType().getPatientTypeName().equalsIgnoreCase(patientTypeNameForHAL))
		  {
		%>	
		  <img
				src="/hms/hms/personnel?method=displayImage&amp;employeeId=<%=patientList.get(0).getEmployee().getId()%>"
				width="105" height="112">  
		 <% }
		  else
		  {
...
<label>Working Diagnosis</label>
	<textarea id="initialDiagnosis" tabindex="1"  name="initialDiagnosis" maxlength="500" style="width: 649px"></textarea>		
...
<input	name="prescriptionImportButton1" tabindex="1" type="button"	value="Import New" class="button"	onclick="getListForTreatment('treatmentDiv');" /> 
			...
			<label>Template</label>
			<div id="treatmentDiv">
				<select name="templateIdFAC" id="templateIdFAC" tabindex="1">
...
<div id="testDiv1">
	...
	<table border="0" align="center" cellpadding="0" cellspacing="0" id="nomenclatureGrid" style="border: 0;">
	<tr>
...
<!-- 		<th scope="col">Route</th> -->
	...
	<tr>
	    ...
	    <input type="text" value="" tabindex="1" id="nomenclature1" size="50"  name="nomenclature1" onblur="checkForAlreadyIssuedPrescription(this.value,'1','0');populatePVMS(this.value,'1');checkItem(1);displayAu(this.value,'1','5555');checkForPurchase(this.value, '1');"  />
...
//			  new Ajax.Autocompleter('nomenclature1','ac2update1','opd?method=getItemListForAutoCompleteItem',{parameters:'requiredField=nomenclature1'});
			   new Ajax.Autocompleter('nomenclature1','ac2update1','purchaseOrder?method=getItemListForPurchaseOrderForFAC',{parameters:'requiredField=nomenclature1'});
			</script>
		</td>
	...
	<!-- 	<td><select name="itemConversionId1" id="itemConversionId1"
		...
		<td><input type="text" name="noOfDays1" tabindex="1" id="noOfDays1" onblur="fillValue('1')"  size="5"	maxlength="3" validate="No of Days,num,no" /></td>
			<td>	<input type="text" name="total1" tabindex="1" id="total1" validate="total,num,no" size="5" onblur="treatmentTotalAlert(this.value,1)" /></td>
	<!-- 	<td><input type="text" name="route1" tabindex="1" id="route1" value=""  size="5" maxlength="20"	 validate="Route,string,no" />
		
		</td> -->
	
		<td><input type="text" name="remarks1" tabindex="1" id="remarks1" size="10" maxlength="15" placeholder="1-1-1"/></td>
		...
		<td><input type="button" name="delete" value="" class="buttonDel" onclick="removeRow('nomenclatureGrid','hdb',this);" tabindex="1" /></td>
	</tr>
...
<div class="division"></div>
	<input type="button" name="Submit" value="Submit" class="button" onClick="if(validateFrequency() && validateICD()){submitForm('facPriscription','opd?method=saveDirectPrescription');}" />
	<input type="reset" name="Reset" value="Cancel" class="button" onclick="submitFormForButton('facPriscription','opd?method=showPatientDirectPrescription');" accesskey="r" />
<div class="clear"></div>
<div class="division"></div>
</div>
<div class="clear"></div>
...
/* 	   var cellRight14 = row.insertCell(3);
	...
	//  var cellRightSel = row.insertCell(2);
	 ...
	 // e8.setAttribute('onClick', 'addRow();'); 
	  ...
	  e9.onclick = function(){removeRow("nomenclatureGrid","hdb",this);};  
