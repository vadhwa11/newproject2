...
function checkForPurchase(val,a,inc)
{
	  ...
	  if(pvms !="")
	    ajaxFunctionForAutoCompleteInLPOGeneral('purchaseGrid','neStores?method=fillItemsForNonLpo&pvmsNo=' +  pvms , inc);
...
<input type="text" name="<%=PO_DATE%>" class="date" id="soDate" value="<%=currentDate %>" class="" readonly="readonly" style="border: 1px solid #7f9db7;" validate="S.O. Date,dateOfAdmission,yes"  MAXLENGTH="30"  />
...
<%}%>
 <img  id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" class="calender" tabindex="1"
	...
	<%}else{ %>
	<input	type="text"  name="references" value="TENDER INQUIRY NO. 01/2012-13/12/MS/PRICE AGREEMENT DATED 03 MARCH 12."  validate="references,String,no" tabindex=1 />
	<%}%> 
...
%>
	...
	<input	type="text"  name="telephoneNo"   validate="Contact No,phone,no" tabindex=1 maxlength="12"/>
	...
	<textarea readOnly name="<%=REMARKS %>" validate="Remarks ,String,no" tabindex=1 onpaste="return checkOnPaste(this)" oninput="return checkMaxLengthMoz(this)" onkeydown="return checkMaxLength(this)" onKeyUp="chkLength(this,200);"/>
	<%=poHeaderObj.getRemarks().trim()%></textarea> 
	<%}else{%>
...
<%---
      <td width="10%">
       <input type="text" value="0" class="medcaption" name="" id="<%=actualqtyin %>"  size="5" validate="ActualQuantity,float,no" onblur="gridCalculationNonLSOAdd(<%=inc%>);" tabindex="1" maxlength="9"/>
      ...
      <td width="10%">
      <input type="text" class="medcaption" name="mdq" id="<%=mdq%>" onblur="gridCalculationNonLSOAdd(<%=inc%>);" size="7" validate="Minimum Dispensable Qty (MDQ),num,no" value="1" tabindex="1" maxlength="9"/>
      </td>--%>
      <td width="10%">
      <input type="text" value="0" class="medcaption" name="" id="<%=quantityInVarTemp %>"  size="5"  validate="Quantity,float,no" onblur="gridCalculationNonLSOAdd(<%=inc%>);" tabindex="1" maxlength="9" />	
	 <input type="hidden" value="0" class="medcaption" name="<%=QUANTITY %>" id="<%=quantityInVar%>" />
      ...
      <td width="10%">
      <input type="text" class="medcaption" value="0" name="mrp"  id="<%=mrp%>"  size="5" onblur="gridCalculationNonLSOAdd(<%=inc%>);" validate="Tax,float,no" maxlength="15" tabindex="1" size="5"/>
      </td> 
      <td width="3%">
      <input type="text" class="medcaption" value="0" name="<%=DISCOUNT_PERCENTAGE%>" size="4" id="<%=discountVarTemp%>" onblur="gridCalculationNonLSOAdd(<%=inc%>);"  validate="Discount,float,no" maxlength="6" tabindex="1" size="5"/>
      <input type="hidden" class="medcaption" value="0" name="<%=DISCOUNT_AMOUNT%>" id="<%=discountAmount%>"/>
      </td>
      <td width="10%">
      <input type="text" class="medcaption" value="0" name="<%=TAX_PERCENT %>" size="4" id="<%=taxVarTemp%>" onblur="gridCalculationNonLSOAdd(<%=inc%>);" validate="Tax,float,no" maxlength="15" tabindex="1"/>
...
for (MasManufacturer manufacturer : manufacturerList) {
     			...
     			manufatArr[<%=manuCnt%>]= new Array();
     			manufatArr[<%=manuCnt%>][0] = "<%=manufacturer.getId()%>";
     			manufatArr[<%=manuCnt%>][1] = "<%=manufacturer.getManufacturerName()%>";
            </script> <%
++manuCnt;
}
           %>
...
<div class="clear"></div>
	...
	<input type="button" name="sss"  class="button" value="Submit" onclick="if(checkForSubmit() && checkSaveRem() ){submitForm('purchaseGrid','neStores?method=submitNonLocalSupply');}"/>
	<!--<input type="button" class="button" value="Next"  onclick="if(checkForNext10() && checkForLocalPurchaseGrid() && checkSaveRem()){submitForm('purchaseGrid','purchaseOrder?method=submitPurchaseOrder&buttonFlag=next');}" align="right" />
		...
		<script type="text/javascript">
		 function addRow(idName) 
		 {
				 ...
				 var newdiv = document.createElement('div');
				  ...
				  new Ajax.Autocompleter('nameItem'+iteration,'ac2update','neStores?method=getItemListForPurchaseOrder',{parameters:'requiredField=nameItem'});
...
--%>
				var cellRight6 = row.insertCell(++el);
				  ...
				  for(var i = 0;i<manufatArr.length;i++ ){
					     e6.options[i+1] = new Option(manufatArr[i][1],manufatArr[i][0]);
					      }
			...
			<%--- var cellRight7 = row.insertCell(++el);
				  ...
				  e7.onblur=function(){gridCalculationNonLSOAdd(iteration)}; 
				  e7.size='5';
				...
				e13.size='7';
				e13.value='1';
				e13.onblur = function(){gridCalculationNonLSOAdd(iteration)};
				...
				cellRight13.appendChild(e13);--%>
				  ...
				  e7.name='';
			      e7.value='0';
			      e7.id = 'quantityInVarTemp'+iteration;
			      e7.setAttribute('tabindex', 1);
			      e7.setAttribute('maxlength', 9);
			      e7.onblur=function(){gridCalculationNonLSOAdd(iteration)}; 
			      e7.size='5';
				  ...
				  e8.type = 'hidden';
				  e8.name='<%=QUANTITY %>';
				  e8.id = 'quantityInVar'+iteration;
				  e8.setAttribute('tabindex', 1);
				  e8.setAttribute('maxlength', 9);
				  e8.onblur=function(){gridCalculationNonLSOAdd(iteration)}; 
				  e8.size='5';
				  cellRight7.appendChild(e8);
				...
				e9.size='5';
				e9.value='0';
				e9.onblur = function(){gridCalculationNonLSOAdd(iteration)};
				...
				var cellRight10 = row.insertCell(++el);
				var e10 = document.createElement('input');
				e10.type = 'text';
				e10.name='<%=DISCOUNT_PERCENTAGE%>';
				e10.id = 'discountVarTemp'+iteration;
				e10.size='4';
				e10.value='0';
				e10.setAttribute('tabindex', 1);
				e10.onblur = function(){gridCalculationNonLSOAdd(iteration)};
				cellRight10.appendChild(ee16);
				...
				e11.type = 'text';
				e11.name='<%=TAX_PERCENT%>';
				e11.id = 'taxVarTemp'+iteration;
				e11.size='4';
				e11.value='0';
				e11.setAttribute('tabindex', 1);
				e11.onblur=function(){gridCalculationNonLSOAdd(iteration)};
				cellRight11.appendChild(ee17);
				...
				e13.type = 'text';
				e13.name='';
				e13.id = 'amountVarTemp'+iteration;
				e13.size='8'
				e13.value='0'
				e13.setAttribute('tabindex', 1);
				cellRight13.appendChild(e23);
				cellRight13.appendChild(e13);
				  ...
				  e14.onclick= function(){addRow('localSupply')};
