...
<%
	...
	if(map.get("storeSupplierList") != null){
		storeSupplierList = (List<MasStoreSupplier>)map.get("storeSupplierList");
		}
	int deptId = 0;
...
<select id="supplierId" name="supplierId">
			...
			<%
				         		if(storeSupplierList != null){ 	
				         			for (Iterator iter = storeSupplierList.iterator(); iter.hasNext();) {
				         				MasStoreSupplier masStoreSupplier = (MasStoreSupplier) iter.next();
				         %>
...
<input type="button" name="add" id="addbutton" value="Print" class="button" onClick="submitForm('nisDetails','/hms/hms/stores?method=generateVendorReturnRegReport');"  tabindex=1 />

<input type="reset" name="Reset" id="reset" value="Reset" class="button" onClick="submitFormForButton('nisDetails','/hms/hms/stores?method=showVendorReturnRegJsp');" accesskey="r" /> 
