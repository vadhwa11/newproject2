...
function showReport()
{
  var poNo;
 	...
 	for(var i = 0; i < document.getElementsByName('parent').length; i++){
			  if(document.getElementsByName('parent')[i].checked == true)
              {
                submitForm('poMain','purchaseOrder?method=printLocalSupplyOrder&po_id='+document.getElementsByName('parent')[i].value);
                return true;
			  }		
  		}
  		alert("Please select the Local Supply Order!!!")
...
%>
		...
		<td width="144" valign="middle" title="nohilite"><input
			...
			onClick="submitForm('poMain','purchaseOrder?method=searchPO');" /> 
