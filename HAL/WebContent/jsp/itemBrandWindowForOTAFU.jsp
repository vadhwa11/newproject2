...
<br />
<h2 align="left" class="style1">Issue to Other than Air Force Units</h2>
<title>Issue to Other than AF Units</title>
<div id="contentspace">
...
<div
	style="overflow: auto; width: 80%; height: 350px; border: 1px solid #232323;">
  ...
  %>
		<tr>
			...
			<td width="13%"><input type="text"
				...
				readonly="readonly"
				value="<%=masStoreItem.getItemConversion().getPurchaseUnit().getUnitName()%>" />
			</td>
			<td width="10%"><input type="text" readonly="readonly"
				class="medcaption1" value="<%=object[2] %>" /> <input type="hidden"
	 	...
	 	%>
			...
			<td width="13%"><input value="<%=""+object[4]%>"
				id="<%=stockIn%>" class="medcaption" readonly="readonly" /></td>
			<td width="13%"><input type="text"
			...
			<input type="hidden" name="<%=RequestConstants.COST_PRICE%>"
				value="<%=object[5].toString()%>" class="medcaption"
				id="<%=costPrice%>" />
...
<input id="save" property="save" type="button" name="save" value="Save"
	class="button"
	onclick="if(submitFormSOC('itemBrandForm','')&&setTotalQty(<%=inc%>,<%=rowVal%>)){submitForm('itemBrandForm','/hms/hms/stores?method=addBrandDetailsIssueToOTAFU');}" />
<%}else{ %>
<h2><font id="error">Records not Found</font></h2>
<br />
<%} %> <input type="button" name="cancel" id="addbutton" value="Cancel"
	class="button" onClick="cancelForm();" accesskey="a" tabindex=1 /> <br />
</form>
</div>
</div>