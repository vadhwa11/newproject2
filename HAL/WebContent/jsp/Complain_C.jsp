...
<form name="complainC" method="post" action="">
...
onClick="setdate('<%=FROM_DATE%>',document.complainC.<%=FROM_DATE%>,event)" />

<label>To Date</label>
<input type="text" name="<%=TO_DATE %>" class="date" value="<%=currentDate %>" validate="To Date,date,yes"
readonly="readonly" MAXLENGTH="30" tabindex="1" />
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date"
onClick="setdate('<%=TO_DATE%>',document.complainC.<%=TO_DATE%>,event)" />
...
onClick="submitForm('complainC','user?method=searchComplainC');" accesskey="a" tabindex="1" />
</div>
...
<tr onclick="submitProtoAjax('complainC','/hms/hms/user?method=getComplainCData&compainId=<%=complain.getId() %>')">
