...
<div class="clear" style="padding-top: 10px;"></div>
		...
		<div id="divSearchResult">
			...
			<table  class="tblSearchResultsHeader"
				cellspacing="0" cellpadding="0" border="0">
				<tr>
                     <th id="th1" style='width: 10px;'>Select</th>
					<th id="th1" style='width: 10px;'>Dept No/Emp. No</th>
		...
		<div class="block">
				...
				<input type="button" name="yes" value="Submit"	class="button"	onclick="if(confirm('Are you sure want to generate!')){validateCheckbox()}" />
				<input type="button" name="yes" value="Revert to HR Division"	class="button"	onclick="if(confirm('Are you sure want to Revert!')){validateCheckboxForRevert()}" />
...
<script language="javascript">
	...
	function GetPatientList(MODE) {
		...
		if (MODE == 'ALL') {
			...
			var data = "PN=" + nPageNo + "&impHospitalId=" + impHospitalId+"&empNo="+empNo+"&rankCategoryId="+rankCategoryId;
		}
		var url = "referral?method=getInvoiceWaitingListForNoteSheet";
	...
	function makeTable(jsonData) {
		...
		for (i = 0; i < jsonData.length; i++) {
			...
			htmlTable = htmlTable + "<td >" + jsonData[i].deptNo +"/"+ jsonData[i].employeeNo
					+ "</td>";
		...
		if (jsonData.length == 0) {
			htmlTable = htmlTable
					+ "<tr><td colspan='11'><img src='/hms/jsp/images/dot.gif' width='1' height='200'/></td></tr><input type = 'hidden' name='checkboxCount' id='checkboxCount' value='"+jsonData.length+"'/>";
		}
		else
			{
	...
	/* function executeClickEvent(Id)
	 ...
	 window.location="referral?method=generateInvoicePageForGM&opdId="+Id;
	 } */
...
/*
	...
	function validateCheckbox() {
		...
		else
		{
			...
			submitForm('getReferralWaitingList','/hms/hms/referral?method=generateNoteSheetLetter&flag=Divisional_approval_letter');
	...
	function validateCheckboxForRevert() {
