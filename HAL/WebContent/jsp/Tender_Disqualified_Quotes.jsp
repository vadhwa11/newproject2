...
function showReport()
{
if (DisqualifiedQuotesForm.<%=TENDER_NO%>.value=="" || DisqualifiedQuotesForm.<%=TENDER_SUPPLIER_GROUP_ID %>.value=="")
{
alert('Pl. Check Your Inputs (Tender No & Group)');
return;
}

DisqualifiedQuotesForm.method="post";
submitForm('DisqualifiedQuotesForm','tender?method=generateTenderDisqualifiedQuotesReport');
}

function onChangeTender()
{
//DisqualifiedQuotesForm.<%=TENDER_SUPPLIER_GROUP_ID%>.options.length=1;
DisqualifiedQuotesForm.method="post";
submitForm('DisqualifiedQuotesForm','tender?method=showTenderForDisqualified&groupId='+DisqualifiedQuotesForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value);
}
</script>

<form name="DisqualifiedQuotesForm"><input type="hidden"
...
<h6>Tender - Disqualified Quotes</h6>
<div class="Clear"></div>
<div class="blockTitle">Disqualified Quotes</div>
