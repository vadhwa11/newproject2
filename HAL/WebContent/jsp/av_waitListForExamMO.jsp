...
<%@page import="jkt.hms.masters.business.AvMedicalExamMaMo"%><script type="text/javascript" language="javascript"	src="/hms/jsp/js/common.js"></script>
...
<%
	...
	if (map.get("aviationMaList") != null) {
		aviationMaList = (List) map.get("aviationMaList");
	}
...
<input type="text"	class="calDate" id="appointmentDate" name="<%=APPOINTMENT_DATE %>"	value="<%=currentDate %>" readonly="readonly" MAXLENGTH="30" /> 
...
<input type="text" name="<%=LICENCE_NO %>" value="" MAXLENGTH="20" id="licenceNo"/>
...
<div class="clear"></div>
<form name="waitingListForAviationMO" method="post" 	action="">

<script type="text/javascript">
	formFields = [ [0,"medExamMaMoId","id"],[1,"visitId"],[2,"appointmentDate"], [3,"name"], [4,"<%=LICENCE_NO%>"], [5,"<%= RequestConstants.TYPE_OF_LICENCE %>"],[6,"licenceHeld"]];
	 statusTd = 1;
	</script>
   </form></div>
<script type="text/javascript">
...
<%
int  counter=0;
if (aviationMaList != null && aviationMaList.size() > 0) { 
for(AvMedicalExamMaMo aviCa34 : aviationMaList){
%>
  	   data_arr[<%= counter%>] = new Array();   
       data_arr[<%= counter%>][0] = '<%= aviCa34.getId()%>'
   	   data_arr[<%= counter%>][1] = '<%=aviCa34.getVisit().getId()%>'
 		<%if(aviCa34.getMaDate() !=null){%>
       data_arr[<%= counter%>][2] = '<%=HMSUtil.changeDateToddMMyyyy(aviCa34.getMaDate())%>'; 
	 <%}else{%>data_arr[<%= counter%>][2] ="";
	 <%}%>
	 <%String name="";
		 name=aviCa34.getAviCa34().getFirstName();
		 if(aviCa34.getAviCa34().getLastName() !=null){
	 	 name=name+" "+aviCa34.getAviCa34().getLastName(); 
	  }
		 String licenceType="";
		 String licenceHeld="";
		 String licenceNo="";
		 if(aviCa34.getAviCa34().getLicenceNo() !=null){
			 licenceNo=aviCa34.getAviCa34().getLicenceNo();}
		 if(aviCa34.getAviCa34().getTypeOfLicenceApplied() !=null){
		   licenceType=aviCa34.getAviCa34().getTypeOfLicenceApplied().getCaLicenceName();}
		 if(aviCa34.getAviCa34().getLicenceHeld() !=null){
			 licenceHeld=aviCa34.getAviCa34().getLicenceHeld().getCaLicenceName();}
		 String examDueDate="";
		 if(aviCa34.getAviCa34().getRenewalDueDate() !=null){
		 examDueDate=HMSUtil.changeDateToddMMyyyy(aviCa34.getAviCa34().getRenewalDueDate());
		 }
...
%>
formName = "waitingListForAviationMO"
	start = 0
