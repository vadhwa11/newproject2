...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<%-- <tr
		...
		<tr id="selfRow"
			onclick="if(checkStatusEmp('<%=emp.getEmployeeStatus().getEmpStatusName()%>','<%=emp.getEmployeeStatus().getEmpStatusCode()%>', '<%=emp.getContractDate()%>', '<%=emp.getExtensionDate()%>')){displayPatientInfoHAL(<%=selfRelationId%>);changerel('<%=srEmployeeListGrid.get(0).getGender().getAdministrativeSexName()%>',<%=selfRelationId%>, '/hms/hms/personnel?method=displayImage&amp;employeeId=<%=emp.getId()%>');}">
		<td><%= patientName%></td>	
