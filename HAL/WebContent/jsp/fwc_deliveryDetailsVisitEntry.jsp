...
<input type="text" name="<%=SERVICE_NO %>" id="serviceNo" value="<%= patient.getServiceNo()%>" validate="Service No.,metachar,yes" onblur="submitProtoAjaxWithDivName('fwcVisitEntryDirect','/hms/hms/fwc?method=getDeliveryDetailsPatientData','hinDiv');" tabindex="1" />
<%-- 
<label>HIN <span>*</span></label>--%> 
<div id="hinDiv">
<input type="hidden" name="<%=HIN_NO %>" id="hinId" value="<%=patient.getHinNo() %>" tabindex="1" /> 
<input type="hidden" name="hinId" id="hinId" value="<%=patient.getId() %>" tabindex="1" />
</div>
...
<%
		...
		%>
		<label class="value"><%= patient.getOccupation()!=null?patient.getOccupation().getOccupationName():""%></label>	
		<%} else{ %>
		<label class="value">-</label>	
		<%} %>
		...
		%>
		<label class="value"><%=patient.getAllergyDetails()  %></label>	
		<%} else{ %>
		<label class="value">-</label>	
		<%} %>
