...
<h6>Orderly SGT Duty Entry Report</h6>

<form name="orderlyReport" target="_blank" method="post" action="">
...
From Date</label> <input type="text" id="fromDateId" name="<%=FROM_DATE %>"
	...
	onClick="setdate('<%=currentDate%>',document.orderlyReport.<%=FROM_DATE%>,event);" />

<label class="bodytextB"><span>*</span> To Date</label> <input
	type="text" id="ToDateId" name="<%=TO_DATE %>"
	value="<%=currentDate %>" class="textbox_date" readonly="readonly"
	MAXLENGTH="30" /> <img src="/hms/jsp/images/cal.gif" width="16"
	height="16" border="0" validate="Pick a date"
	onClick="setdate('<%=currentDate%>',document.orderlyReport.<%=TO_DATE%>,event);" />

</div>
<div class="Clear"></div>
<input type="button" name="OK" value="OK" class="cmnButton"
	onClick="submitForm('orderlyReport','hrRelated?method=generateOrderlyReport');" />
