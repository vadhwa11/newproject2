...
<form name="otherPersonUpdate" action="" method="post"><script
...
if(map.get("searchOtherPersonList") != null){
	searchOtherPersonList= (List<MovementInOtherPerson>)map.get("searchOtherPersonList");
	}
...
<input type="button" name="search" id="search"
	onclick="submitForm('otherPersonUpdate','/hms/hms/hrOrderly?method=searchOtherPersonUpdate');"
...
<%
	int  counter=0;
    if(searchOtherPersonList.size()>0){
	for (MovementInOtherPerson masEmp : searchOtherPersonList) {
%>
...
<%if(masEmp.getCategory() != null){
            ...
            if(masEmp.getCategory() != null){
			if(masEmp.getCategory().getId().equals(masEmpCategory.getId()) && masEmpCategory.getStatus().equals("y")){
			 %>
				data_arr[<%= counter%>][6] = "<%=masEmpCategory.getEmpCategoryName()%>"
			<%}else if(masEmp.getCategory().getId().equals(masEmpCategory.getId()) && masEmpCategory.getStatus().equals("n")){%>
 ...
 <% if(masEmp.getPresentUnit() != null){
		 ...
		 while(itrGridUnitList.hasNext())
            {try{
			 ...
			 if(masEmp.getPresentUnit().getId().equals(unitGrid.getId()) && unitGrid.getStatus().equals("y")){%>
