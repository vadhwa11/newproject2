...
<form name="searchHroEntry" action="" method="post"><script
...
if(map.get("searchHROList") != null)
{
	searchHROList = (List<HroEntry>)map.get("searchHROList");
}
...
<label>Entry No</label> <input type="text" name="<%=ENTRY_NO %>"
	...
	onClick="setdate('<%=date %>',document.searchHroEntry.<%=DATE%>,event)" />
<div class="Clear"></div>
...
<input type="button" name="submit" id="addbutton"
	onclick="submitForm('searchHroEntry','/hms/hms/hrOrderly?method=searchHroEntry');"
...
<form name="updateHROEntry" method="post" action=""><script
...
<% if (searchHROList != null && searchHROList.size() > 0) { 
%>
<% 	for(HroEntry proposal : searchHROList){
%>
...
data_arr[<%= counter%>][1] = "-"
<%}%>
<%if(proposal.getHroDate() != null){%>
data_arr[<%= counter%>][2] = "<%=proposal.getHroDate()%>"
<%}else{%>
data_arr[<%= counter%>][2] = "-"
<%}%>
