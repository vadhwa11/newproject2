...
<%
	...
	LibBookStockTakingHd bookStockTakingHd=new LibBookStockTakingHd();
	...
	if (request.getAttribute("map") != null) {
		...
		if (map.get("bookStockHdList")!=null)
		{
			bookStockHdList = (List<LibBookStockTakingHd>)map.get("bookStockHdList");
		}
		if (map.get("bookStockDtList")!=null)
		{
			bookStockDtList = (List<LibBookStockTakingDt>)map.get("bookStockDtList");
		}
		...
		if(bookStockHdList != null) {
			bookStockTakingHd = (LibBookStockTakingHd) bookStockHdList.get(0);
		}
		System.out.println("bookStockHdList in Jsp  :"+bookStockHdList.size());
%>
<form name="blockStockTaking" method="post" action="">
<div id="contentHolder">
<h6>Books stock Taking Entry</h6>
<div class="Clear"></div>
<div class="blockFrame" id="testDiv"><input id="libHeaderId"
	type="hidden" name="libHeaderId" value="<%=libHeaderId %>" /> <label>Stocking
Taking No.</label> <input id="orderNoId" type=hidden name="<%=STOCK_NO %>"
	value="<%=bookStockTakingHd.getStockTakingNo() %>"
	title="Stock Taking No" /> <label class="value"><%=bookStockTakingHd.getStockTakingNo() %></label>

<label> Date</label> <%if(bookStockTakingHd.getStockTakingNo() !=null){ %>
<input type="text" class="calDate" id="stockDateId" name="<%=DATE %>"
	value="<%=HMSUtil.convertDateToStringWithoutTime(bookStockTakingHd.getSockTakinDate()) %>"
	readonly="readonly" MAXLENGTH="30" validate="Date,date,yes" /> <%}else{ %>
<input type="text" class="calDate" id="stockDateId" name="<%=DATE %>"
	value="<%=currentDate %>" readonly="readonly" MAXLENGTH="30"
	validate="Date,date,yes" /> <%} %> <img src="/hms/jsp/images/cal.gif"
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"
	...
	<tbody>
		<%
if (bookStockDtList== null||bookStockDtList.size()==0) { %>
		<tr>
    	...
    	//   for(LibBookStock libBookStock:stockList){
    		///   String status="n";
    		//   if(savedDtList!=null&&savedDtList.size()>0){
    		...
    		//	}}   
    			 ...
    			 if(bookStockDtList!= null && bookStockDtList.size()>0){
    				 for(LibBookStockTakingDt bookStockTakingDt:bookStockDtList){
    		   %>
		<tr>
			<td><input type="text" size="2" id="srNo<%=cnt %>"
				value="<%=temp %>" name="<%=SR_NO%>" readonly="readonly" /></td>
			<input type="hidden" name="bookStockId"
				value="<%=bookStockTakingDt.getBookSock().getId() %>" />
			<input type="hidden" name="bookStockDtId"
				value="<%=bookStockTakingDt.getId() %>" />
			<td><%=bookStockTakingDt.getBookSock().getBook().getBookName()%></td>
			<td><%= bookStockTakingDt.getBookSock().getBook().getBookNo()%></td>
			<td><%=bookStockTakingDt.getBookSock().getBook().getYearPublication() %></td>
			<td><%= bookStockTakingDt.getBookSock().getOpenBalanceQty()%></td>
			<td><%= bookStockTakingDt.getBookSock().getCost()%></td>
			<td><%= HMSUtil.convertDateToStringWithoutTime(bookStockTakingDt.getBookSock().getBook().getPurchaseDate())%></td>
			<td><%=bookStockTakingDt.getBookSock().getClosingStockQty() %></td>
			<td>
			<%if(bookStockTakingDt.getStrikeCharge().equalsIgnoreCase("y")) {%> <input
				id="checkId<%=cnt %>" name="<%=CHARGE %>" type="checkbox" value="y"
				checked="checked" class="check" /> <%}else {%> <input
				id="checkId<%=cnt %>" name="<%=CHARGE %>" type="checkbox" value="n"
				class="check" /> <%} %> <input type="hidden"
				name="tempForCheackBoxes" id="tempForCheackBoxes<%=cnt %>" value="n" />
			<input type="hidden" name="stockList" id="stockList"
				value="<%=stockList.size() %>" /></td>
		</tr>
		<%
    	   temp++;
    	   cnt++;
