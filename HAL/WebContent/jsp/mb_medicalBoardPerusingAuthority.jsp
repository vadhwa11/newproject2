...
<%
	...
	if (map.get("patientDetailAppAuthList") != null) {
		patientDetailAppAuthList = (List) map.get("patientDetailAppAuthList");
	}
...
<form name="medBoardSearch" method="post">
<div class="Block">
...
<input
	...
	class="calender"
	onClick="setdate('<%=currentDate %>',document.medBoardSearch.<%=FROM_DATE%>,event)" />
...
<label> To Date<span>*</span> </label> <input type="text" id="ToDateId"
	...
	onClick="setdate('<%=currentDate %>',document.medBoardSearch.<%=TO_DATE%>,event)" />
<div class="Clear"></div>
<label>Rank</label>
...
<input type="button" name="Search" value="Search" class="button" onClick="submitForm('medBoardSearch','medicalBoard?method=searchMedicalBoardPerAuthority');" />
<div class="Clear"></div>
</div>
</form>

<div class="clear paddingTop15"></div>
<form name="medicalBoardPerusingAuthority" method="post" action="">
<div class="clear"></div>
...
data_header[7][2] = "10%";
data_header[7][3] = "dateMB";

data_header[8] = new Array;
data_header[8][0] = "Place of MB"
data_header[8][1] = "data";
data_header[8][2] = "10%";
data_header[8][3] = "placeMB";
...
<%
		int  counter=0;
		if (patientDetailAppAuthList != null && patientDetailAppAuthList.size() > 0) {
		for(MasMedicalExaminationReportOnEntry masexam : patientDetailAppAuthList){
		%>
		...
		<%if(masexam.getMedicalCategoryRecomended()!=null){%>
		data_arr[<%= counter%>][11] = "<%=masexam.getMedicalCategoryRecomended().getCategories()%>"
		<%}else{%>
		data_arr[<%= counter%>][11] = ""
		<%}%>
<%
...
%>
formName = "medicalBoardPerusingAuthority"
start = 0
