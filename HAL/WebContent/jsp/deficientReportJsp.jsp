...
<%@page import="jkt.hms.masters.business.MasTrade"%><script type="text/javascript" src="/hms/jsp/js/autoComplete1.js"></script>
...
%>
  ...
  {
		var strValue = document.deficientReport.<%=FROM_DATE%>.value;
		...
		strValue = document.deficientReport.<%=TO_DATE%>.value;
...
%>
<form name="deficientReport" method="post">
<input type="hidden" name="hospitalId" value="<%=hospitalId %>"/>
<div class="titleBg">
<h2>Deficient AFMSF-1</h2>
</div>
<div class="clear"></div>
<div class="Block">
<label> From Date <span>*</span> </label> 
<input type="text" name="<%= FROM_DATE %>" value="<%=currentDate%>" class="date" maxlength="30" tabindex=1 /> 
<img id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"	validate="Pick a date"	onclick="setdate('<%=currentDate%>',document.deficientReport.<%= FROM_DATE%>,event);" />
<label> To Date <span>*</span></label> <input type="text" name="<%= TO_DATE %>" value="<%=currentDate%>" class="date"	maxlength="30" tabindex=1 /> 
<img id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onclick="setdate('<%=currentDate%>',document.deficientReport.<%= TO_DATE%>,true);" />
...
<input type="button" class="button" value="Ok"
	onclick="if(validateDates()){submitForm('deficientReport','/hms/hms/mis?method=generateDefeicientReport');}" />
<input type="button" class="button" value="Cancel" onclick="submitForm('deficientReport','/hms/hms/mis?method=showDefeicientReportJsp');"/>
