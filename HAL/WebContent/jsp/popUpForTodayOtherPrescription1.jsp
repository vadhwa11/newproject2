...
function stopMedicine(val){
	...
	if(validateStop(val)){
		...
		submitForm('currentMedication','opd?method=stopCurrentMedication&hinId=<%out.print(hinIdparam);%>&bufferingFlag=y&backFlag=OPD');
	}
	}
		...
		/* 		  var x3 = window.opener.document.createElement('Select');
			...
			/* 	  var x4 = window.opener.document.createElement('Select');
				...
				/*   var x10 = window.opener.document.createElement('input');
				  ...
				  x11.size='3';
				  x11.value = eval('document.'+formname+'.stock' + index + '.value');
				  x11.setAttribute('tabindex','1');
				  c12.appendChild(x11);
				 ...
				 if(window.opener.document.getElementById('nomenclaturehdb')!=null)
				{
				 ...
				 else if(window.opener.document.getElementById('hdb')!=null)
				{
				  var x12 = window.opener.document.getElementById('hdb');
				  x12.value=len;
				}
...
List<Integer> repeatCheckboxItemId = new ArrayList();

/* 	HashMap<Integer,Integer> ItemIdKey = null;
	int itemId=0;
	if(ppdList.size()>0)
	{   ItemIdKey =new HashMap<Integer,Integer>();
		for(PatientPrescriptionDetails ppd:ppdList){
			itemId = ppd.getItem().getId();
			if(ItemIdKey.containsKey(itemId))
			{
			   ItemIdKey.put(itemId,ItemIdKey.get(itemId)+1);
			}
			else
				ItemIdKey.put(itemId, 1);
		}
	} */
...
for(PatientPrescriptionDetails ppd:ppdList){ 
	...
	if(stockList.size() > 0){
		  ...
		  for(Object[] obj : stockList){
			  if(ppd.getItem().getId() == Integer.parseInt(obj[1].toString())){
...
<input type="hidden" tabindex="1" id="itemClassCode<%=i%>" name="itemClassCode<%=i%>" validate="itemClassCode,string,no" value='<%=ppd.getItem().getItemClass()!=null ? (ppd.getItem().getItemClass().getId()!=0?ppd.getItem().getItemClass().getItemClassCode():""):""%>'/>
...
<%}%>
<td>
<input type="hidden" name="stock<%=i%>" id="stock<%=i%>" size="6" tabindex="1" readonly="readonly" value="<%=stock%>"/>
<%=stock%></td>
...
Medicine stopped on <%=ppd.getItemStopDate()!=null?HMSUtil.changeDateToddMMyyyy(ppd.getItemStopDate()):"-"%> by <%=ppd.getItemStopDate()!=null?doctor:"-"%>
  <%}else if( backFlag.equals("OPD") && ppd.getPrescription().getStatus().equalsIgnoreCase("I")){ %>
...
//if(!HMSUtil.alreadyPrescribedMedicine(ppd))
	//System.out.println(currentPrescribedIdList.contains(ppd.getId()) +" id="+currentPrescribedIdList.get)
		...
		System.out.println("item="+ppd.getItem().getId() +" " + ppd.getPrescription().getStatus() +" "+ !currentPrescribedItemIdList.contains(ppd.getItem().getId()) +" "+!repeatCheckboxItemId.contains(ppd.getItem().getId()));
	 if(currentNIPPrescriptionIdList.contains(ppd.getId()) && ppd.getPrescription().getPrescriptionDate().compareTo(nipValidatedDate) <0)
	{
		out.println("NIP");
	}
/* else  if(!currentPrescribedItemIdList.contains(ppd.getItem().getId()) && !repeatCheckboxItemId.contains(ppd.getItem().getId()) && ItemIdKey.get(itemId)==1 ) */

else  if(!ppd.getPrescription().getStatus().equalsIgnoreCase("p") && !currentPrescribedItemIdList.contains(ppd.getItem().getId()) && !repeatCheckboxItemId.contains(ppd.getItem().getId()))
{
	repeatCheckboxItemId.add(ppd.getItem().getId());
