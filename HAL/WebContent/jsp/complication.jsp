...
<div class="Block">
<label>Complication Code</label><input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /> <label >Complication
Name</label> <input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO %>" value="2" />  <input type="text" id="searchField" name="<%= SEARCH_FIELD%>"
	value="" validate="Complication Code,string,no" MAXLENGTH="8"
	tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=searchComplication')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchComplication','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_complication"></div></form>
</div>
<div class="Clear"></div>
</div>
<div class="Clear"></div>
...
<form name="complication" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasComplication">
<input type="hidden" name="<%=POJO_PROPERTY_NAME %>"
	value="ComplicationName"> <input type="hidden" name="title"
	value="Complication"> <input type="hidden"
	name="<%=JSP_NAME %>" value="complication"> <input
...
<div class="Clear"></div>
<div class="Block">
<label >  Complication Code<span>*</span> </label> <input id="codeId" type="text"
	name="<%= CODE%>" value="" validate="Complication Code,string,yes"
	 MAXLENGTH="8" / tabindex=1> <label>Complication
Name<span>*</span></label> <input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="Complication Name,string,yes" 
	MAXLENGTH="30" / tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addComplication')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masComplication.getLastChgBy() %>"
