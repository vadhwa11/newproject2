...
<h6>Marriage Accommodation Registration For Airmen</h6>
<div class="Clear"></div>
...
<%
	...
	%> <label><span>*</span> Registration No.</label> <label class="value"><%=registrationNo %></label>
<input type=hidden name="<%=REGISTRATION_NO %>"
	id="<%=REGISTRATION_NO %>" value="<%=registrationNo %>" /> <label>
<span>*</span> Registration Date </label> <input type="text" id="regDateId"
	name="<%=REGISTRATION_DATE %>" value="<%=date %>" readonly="readonly"
	tabindex="1" /> <label><span>*</span> Registration Time</label> <input
	type="text" value="<%=time %>" name="<%=TIME %>" id="<%=TIME %>"
...
<%
	...
	<%}} %>
</select> <label><span>*</span> Service No.</label> <% if(emp.getServiceNo() != null){ %>
...
<%} %> <script>
		document.regAirmen.<%=SERVICE_NO%>.focus();
	</script> <%String name="";
	...
	%> <label><span>*</span> Service Person</label> <%if(name != null){ %> <input
	...
	readonly="readonly" /> <%}else{ %> <input typ="text"
	name="<%=SERVICE_PERSON_NAME %>" id="<%=SERVICE_PERSON_NAME %>"
	value="<%=name %>" validate="Service Person Name,string,yes"
...
<label><span>*</span> Date of AME</label> <input type="text"
	class="calDate" id="ameDateId" name="<%=DATE_OF_AME %>"
	value="<%=date %>" readonly="readonly" MAXLENGTH="30" tabindex="1" /> <img
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
...
<div class="bottom"><input type="button" class="button"
	value="Submit"
	onclick="if(checkMarriageDate() && checkAntiSeniorityDate()){submitForm('regAirmen','accom?method=submitRegForAirmen');}"
