...
<table width="98%" colspan="7" id="grnDetails" 	border="0" cellpadding="0" cellspacing="0">
	   ...
	   <input type="hidden" name="gridSize" id="gridSize" value="<%=gridLoanInTList.size()%>"/>
		...
		<input type="text"	value="<%=storeLoaninT.getItem().getPvmsNo() %>" name="<%=ITEM_CODE %>" size="8" readonly="readonly" id="<%=codeItem+""+inc%>" />
		<%}else{ %>
		<input type="text" value=" " name="<%=ITEM_CODE %>" size="8" readonly="readonly" id="<%=codeItem+""+inc%>" />
		<%} %>
		...
		<td width="10%">
		<input type="text"	value="<%=storeLoaninT.getItem().getItemConversion().getItemUnitName() %>"	 readonly="readonly" name="<%=AV%>"	id="<%=idAu+""+inc%>" />
		</td>
		<td>
		<input type="text" name="barCodeNo" id=""<%=barCodeId+""+inc%>" value="" tabindex=""/>
		</td>

		<td>
		<input type="text" name="BRAND_GEN" id=""<%="BrandGen"+""+inc%>" value="<%=storeLoaninT.getItem().getBrandedGeneric()%>" tabindex=""/>
		</td>
		...
		<td width="10%">
		<input type="text"	value=""	name="<%=BATCH_NO %>"  tabindex="3"	id="<%=batchNo+""+inc%>" tabindex="1"/>
		</td>
<td width="40%">
...
<td width="40%">
			...
			<%} %>
				...
				name="mdq"  id="<%=mdq+""+inc%>" tabindex="2" onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=inc%>);"/>
			</td>
		...
		%>
			<!--<td width="10%"><input type="text" value=""
				...
				id="<%=TAX_AMT_MDQ +""+inc%>" />------>
			<td width="10%">
			<input type="text"  name="<%=TAX_PERCENT %>" value="" tabindex="1" id="<%=taxVar+""+inc %>" onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=inc%>);"  validate="Tax,float,no"/>
			<input type="hidden"  value="" name="taxAmount" id="<%=taxAmount+""+inc%>" />
			</td>
			<%} else { %>
			<td width="10%"><input type="text"
				name="<%=TAX_PERCENT %>" tabindex="1" id="<%=taxVar+""+inc%>"
				onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=inc%>);" validate="Tax,float,no" />
				 <input
				type="hidden"  value="" name="taxAmount"
				id="<%=taxAmount+""+inc%>" /></td>
			<%}%>
			<td width="10%">
			<input type="text" value="" name="<%= COST_PRICE %>"	id="<%=costPrice+""+inc%>" onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=inc%>);"/>
			</td>
			<td width="10%">
...
<!-- -
		   ...
		   <td>
				<input type="button" name="delete" value="" class="buttonDel" onclick="removeRow('grnDetails');" tabindex="1" />
		...
		<tr>
			...
			<input type="text"	value="<%=storePoDetail.getItem().getPvmsNo() %>" tabindex="1"	onblur="autocompleteBasedOnPvms(this.value,'<%=inc%>');" name="<%=ITEM_CODE %>" size="8" id="<%=codeItem+""+inc%>" />
			<%}else{ %>
			<input	type="text" value=" "  name="<%=ITEM_CODE %>" size="8"	tabindex="1" id="<%=codeItem+""+inc%>" />
			<%} %>
		   ...
		   -->
			...
			<td width="10%">
			<input type="text"	value="<%=storePoDetail.getItem().getItemConversion().getItemUnitName() %>"	tabindex="1"  readonly="readonly" name="<%=AV%>" id="<%=idAu+""+inc%>" tabindex="1"/>
			</td>
<td>
<input type="text" name="barCodeNo" id=""<%=barCodeId+""+inc%>" value="" tabindex="1"/>
</td>
			<%
		...
		<td>
		<input type="text" name="BRAND_GEN" id=""<%="BrandGen"+""+inc%>" value="<%=storePoDetail.getItem().getBrandedGeneric()%>" tabindex=""/>
		</td>
			...
			<td width="10%">
			<input type="text" value="" name="<%=BATCH_NO %>"	onblur="fillSrNo('<%=inc%>');calculationInCRV();" tabindex="1"	id="<%=batchNo+""+inc%>" tabindex="1"/>
			</td>
			<!--
	       <td width="10%">
 	       ...
 	       -->
			<td width="40%">
			<input type="text" value=""	name="<%=MANUFACTURING_DATE%>" size="8" id="<%=manufacturingDate+""+inc%>" MAXLENGTH="30" tabindex="1" readonly="readonly" />
			</td><td><img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" OnClick="setdate('<%=currentDate%>',document.getElementById('<%=manufacturingDate+""+inc%>'),event);" />
			</td>
			<td width="40%">
			<input type="text" value=""	name="<%=EXPIRY_DATE%>" size="8" id="<%=expiryDate+""+inc %>" MAXLENGTH="30" tabindex="1" readonly="readonly" />
			</td><td><img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate%>',document.getElementById('<%=expiryDate+""+inc%>'),event);" />
			</td>
			<td width="10%">
			<input type="text"	value="<%=storePoDetail.getQuantityOrdered() %>" name="<%=QUANTITY_RECEIVED %>" size="8" onblur="calculationInCRV();" tabindex="1" id="<%=quanRec+""+inc%>" />
				...
				name="mdq" readonly="readonly" id="<%=mdq+""+inc%>" tabindex="2" />
			</td>
			<!--
			<td width="10%">
...
<!-- -
			<td width="10%"><input type="text"
				value="<%=storePoDetail.getExciseDuty()==null?"0":storePoDetail.getExciseDuty()%>"
				 name="ed" onblur="calculationInCRV();"
				readonly="readonly" id="<%=ed+""+inc%>" tabindex="2" /></td>
...
-->
			...
			<td width="10%">
			<input type="text"  name="<%=TAX_PERCENT %>" value="<%=storePoDetail.getTaxPercent()%>" tabindex="1" id="<%=taxVar+""+inc%>" onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=rowLenSize%>);" validate="Tax,float,no"/>
			...
			%>
			<input type="text" value="<%=storePoDetail.getUnitRate()%>" name="<%= COST_PRICE %>"	id="<%=costPrice+""+inc%>"  onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=rowLenSize%>);"/>
			</td>
			<td width="10%">
			<input type="text" value="<%=storePoDetail.getAmount()%>" 	name="<%=AMOUNT %>" tabindex="1" readonly="readonly"	id="<%=amtVar+""+inc%>"    onblur="calculationInCRV();"  onfocus="calculateTotalCrvAmount(<%=rowLenSize%>);"/>
