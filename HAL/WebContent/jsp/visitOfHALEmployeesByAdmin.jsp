...
window.open(url,'windowRef','left=0,top=0,width=600,height=400,scrollbars = yes');
    ...
    function validateTokenDiv()
    {
    	...
    	for(var i=1; i<=divCount; i++)
        	...
        	var doctorId = document.getElementById("doctorId"+i).value;
        	var sesId = document.getElementById("sesId"+i).value;
        	...
        	{
        		alert("Please Select Department, Doctor And Session For Row "+i);
        	}
    		}
...
<%-- <input	id="serviceNoId" class="auto" size="8" type="text"	name="<%=SERVICE_NO %>" value="" title="Enter Service No"   validate="Employee No,metachar,yes" maxlength="20"  onKeyPress="return isNumber(event)"
...
<input	id="serviceNoId" class="auto" size="8" type="text"	name="<%=SERVICE_NO %>" value="" title="Enter Employee No"   validate="Employee No,string,yes" maxlength="20" 
onblur="if(this.value.trim()!=''){validateServiceNoHAL(this.value,'visitEmployee');submitProtoAjaxNew('visitEmployee','/hms/hms/registration?method=getServiceNoDetailsForRegHAL&serviceNo='+this.value,'depenedentDiv');}" />
<input id="bloodGroupId" name="bloodGroupId" type="hidden"   maxlength="8" validate="Pin Code,metachar,no"/>
</div>
<input type="hidden" value="y"  id="visitByAdmin" name="visitByAdmin"/>
<input type="hidden" value="<%=patientTypeNameForHAL%>"  id="patientTypeName" name="patientTypeName"/>
<input type="hidden" value="<%=selfRelationName %>" id="selfRelationName" name="selfRelationName"/>
...
<div class="clear"></div>
<label>HIN</label>
<select id="regHinId" name="regHinId"    onchange="if(validateMetaCharacters(this.value) && this.value!='0'){displayPatientInfo(this.value)}else{getHin();}"> 
<option value="0">Select</option>
</select>
...
<select	id="rankId" name="<%=RANK_ID%>" validate="Rank,metachar,no"   onchange="">
	<option value="0">Select</option>
	<%
...
<input id="sFNameId" type="text" name="<%=S_FIRST_NAME %>" value=""  	alidate="Patient First Name,string,no" MAXLENGTH="15"	onblur="fillPatientName(this);" /> 

<label>Middle Name</label> 
<input	id="sMNameId" type="text" name="<%=S_MIDDLE_NAME%>" value=""	  validate="Patient Middle Name,string,no"	MAXLENGTH="15" onchange="fillPatientName(this);" />

<label>Last Name</label> 
<input id="sLNameId" type="text"	name="<%=S_LAST_NAME %>" value=""  	validate="Patient Last Name,string,no" MAXLENGTH="15"	onchange="fillPatientName(this);" />
...
<select name="<%=SR_SEX %>" id="srSexId" validate="Service Person Gender,metachar,yes"  	onchange="fillPatientName(this);">
	<%
...
<label>DOB</label> 
<input type="text" id="srdobId" name="<%=SR_DOB %>"	  value="" validate="Date of Birth,date,no" MAXLENGTH="30" class="calDate" onkeyup="mask(this.value,this,'2,5','/');"		onblur="validateExpDate(this,'srdobId');" /> 
...
<select name="srMaritalStatus" id="srmrstatus"	validate="Marital Status of service Person,metachar,no"  >
	<option value="0">Select</option>
	<%
...
<input type="text" id="srMarriageDateId"	name="srMarriageDate"   value=""	onkeyup="mask(this.value,this,'2,5','/');"		onblur="validateExpDate(this,'srMarriageDateId'); " validate="Marriage Date,date,no" MAXLENGTH="30" class="calDate" /> 
<img id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16"	border="0" validate="Pick a date" class="calender"	onClick="setdate('<%=currentDate %>',document.visitEmployee.srMarriageDate,event)" />


<label > Religion</label>
<select id="religionId" name="<%=RELIGION_ID %>" validate="Religion,metachar,no"   >
<option value="0">Select</option>
<%
...
<input id="phoneNo" name="<%=TELEPHONE_NO %>" type="text"   maxlength="11" onblur="fillPatientName(this)"  validate="Telephone No.(Off.),metachar,no" /> 
<label>Mobile No.</label> 
<input id="mobileNo" name="<%=MOBILE_NO %>" type="text"  	maxlength="11" onblur="fillPatientName(this)"  validate="Mobile No.,metachar,no" />


<label>Telephone No.(Res.)</label> 
<input id="phoneNoRes" name="phoneNoRes" type="text"   maxlength="11"  validate="Telephone No.(Res.),metachar,no"/> 
<div class="clear"></div>
<label>Other Contact No.</label> 
<textarea cols="20" rows="2"   maxlength="60"validate="Other Contact No.,metachar,no" name="otherContactNo" onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" onKeyUp="finalCheck(this)" ></textarea>
<div class="clear"></div>
...
<textarea name="<%=ADDRESS %>" id="sraddr"	cols="20" rows="2"   validate="Address,string,no" maxlength="500"
	...
	<label>State</label> 
<select id="srstateId" name="<%=STATE%>"	validate="State,metachar,no"  	onChange="populateDistrict(this.value,'registration','srcityId')">
	<option value="0">Select</option>
	<%
...
<label>City</label> 
<select name="<%=DISTRICT%>" validate="City,string,no" id="srcityId"   onblur="fillPatientName(this)">
	<option value="0">Select</option>
	<%
		for(Object[] masDistrict : districtList){
	%>
...
<label>City</label> 
<select name="permCityId" validate="City,string,no" id="perCityId"   onchange="populateStateForCity(this.value);">
	<option value="0">Select</option>
	<%
		for(Object[] masDistrict : districtList){
	%>
...
<input id="telephoneNoPerm" name="telephoneNoPerm" type="text"   maxlength="11" validate="Telephone No.,metachar,no"/> 
<label>Police Station</label> 
<input id="policeStation" name="policeStation" type="text"   maxlength="15" validate="Police Station,metachar,no"/> 
<label>Pin Code</label> 
<input id="pinCode" name="pinCode" type="text"   maxlength="8" validate="Pin Code,metachar,no"/> 
<div class="clear"></div>
<label>Identification Mark 1</label> 
<textarea cols="20" rows="2"   maxlength="50" validate="Identification Mark 1,metachar,no" name="identificationMark1" id="identificationMark1" onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" onKeyUp="finalCheck(this)"></textarea>

<label>Identification Mark 2</label> 
<textarea cols="20" rows="2"   maxlength="50"validate="Identification Mark 2,metachar,no" name="identificationMark2" id="identificationMark2" onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" onKeyUp="finalCheck(this)"></textarea>
...
<div class="clear"></div>
<label>  Name</label>
<input type="text" name="<%=SR_NEXT_OF_KIN_NAME%>" 	validate="NOK1 Name,string,no" id="srnokNameId" maxlength="30"/>
...
<div class="clear"></div>
<label> Address</label> 
<textarea name="<%=SR_NEXT_OF_KIN_ADDRESS %>" id="srnextOfKinAdd"	maxlength="500" cols="20" rows="2"   validate="Nok1 Address,string,no"
	></textarea>
<label>Police Station</label> 
<input id="srnok1PoliceStation" name="srnok1PoliceStation" type="text"   maxlength="15" /> 
<label>Pin Code</label> 
<input id="srnok1PinCode" name="srnok1PinCode" type="text"   maxlength="8" /> 
<div class="clear"></div>
	
<h4> NOK 2</h4>
<div class="clear"></div>
<label>  Name</label>
<input type="text" name="srnok2Name" validate="NOK2 Name,string,no" id="srnok2NameId" maxlength="30"/>
...
<div class="clear"></div>
<label> Address</label> 
<textarea name="srnok2Address" id="srnok2Add"	cols="20" rows="2" maxlength="500"   validate="Nok2 Address,string,no"
	></textarea>
<label>Police Station</label> 
<input id="srnok2PoliceStation" name="srnok2PoliceStation" type="text"   maxlength="15" validate="Police Station,metachar,no"/> 
<label>Pin Code</label> 
<input id="srnok2PinCode" name="srnok2PinCode" type="text"   maxlength="8" validate="Pin Code,metachar,no"/> 
<div class="clear"></div>
</div><!-- End Block 3-->
<div class="clear"></div>
...
<select name="srfamilyHistory" id="srfamilyHistory"   multiple="multiple" class="list" onblur="openOtherField(this.value,this.id);">
 <option value="0">Select</option>
<%
...
<label> Allergy</label> 
<textarea name="srAllergies" id="srAllergies" value=""	validate="Allergy,string,no" maxlength="100" onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" onKeyUp="finalCheck(this)">
</textarea>
...
<label>First Name<span>*</span></label> 
<input type="text"	id="pFirstNameId" name="<%=P_FIRST_NAME %>" value=""  	title="Patient First Name"	validate="Patient First Name,string,yes" MAXLENGTH="15" />
<label>Middle Name</label> 
<input type="text" id="pMiddleNameId"	name="<%=P_MIDDLE_NAME%>" value=""  	validate="Patient Middle Name,string,no" MAXLENGTH="15" /> 

<div class="clear"></div>
<label>Last Name</label> 
<input type="text" id="pLastNameId" name="<%=P_LAST_NAME %>"	value="" validate="Patient Last Name,string,no" MAXLENGTH="15"	  /> 
...
<input type="text" id="dobId"	name="<%=DATE_OF_BIRTH %>"   value=""	onkeyup="mask(this.value,this,'2,5','/');"		onblur="if(this.value!=''){if(validateExpDateHAL(this,'dobId')){setAge(this.value, 'ageId');}}" validate="Date of Birth,date,yes" MAXLENGTH="10" class="calDate" /> 
<img id="calendar" src="/hms/jsp/images/cal.gif" width="16" height="16"	border="0" validate="Pick a date" class="calender"	onClick="setdate('<%=currentDate %>',document.visitEmployee.<%=DATE_OF_BIRTH%>,event)" />
...
for(MasEmployeeDependent dependent :med)
{
	...
	<div class="photoImageDiv"><img src="/hms/hms/personnel?method=displayImageEmployeeDependent&amp;employeeDependentId=<%=dependent.getId()%>" width="105" height="112"></div>  
	  <%break; }
	  %>
	  <div class="photoImageDiv"><img src="/hms/jsp/images/photo_icon.png" width="105" height="112"></div>
	  <%
...
<div class="clear"></div>
<label>  Name</label>
<input type="text" name="<%=NEXT_OF_KIN_NAME%>" value=""	validate="NOK1 Name,string,no" id="nokNameId" maxlength="30"/>
...
<label> Contact No.</label> 
<input	type="text" name="<%=NEXT_OF_KIN_PHONE_NO%>" value="" id="nok1Phone"	validate="NOK1 Phone No,phone,no" maxlength="11" />
<div class="clear"></div>
<label> Address</label> 
<textarea name="<%=NEXT_OF_KIN_ADDRESS %>" id="nextOfKinAdd"	cols="20" rows="2"   validate="Nok1 Address,string,no" maxlength="500"
	></textarea>
<label>Police Station</label> 
<input id="nok1PoliceStation" name="nok1PoliceStation" type="text"   maxlength="15" validate="Police Station,metachar,no"/> 
<label>Pin Code</label> 
<input id="nok1PinCode" name="nok1PinCode" type="text"   maxlength="8" validate="Pin Code,int,no"/> 
<div class="clear"></div>
	
<h4> NOK 2</h4>
<div class="clear"></div>
<label>  Name</label>
<input type="text" name="nok2Name" value=""	validate="NOK2 Name,string,no" id="nok2NameId" maxlength="30"/>
...
<label> Contact No.</label> 
<input	type="text" name="nok2ContactNo" value="" id="nok2Contact"	validate="NOK2 Contact No,phone,no" maxlength="11" />
<div class="clear"></div>
<label> Address</label> 
<textarea name="nok2Address" id="nok2Add"	cols="20" rows="2"   validate="Nok2 Address,string,no" maxlength="500"
	></textarea>
<label>Police Station</label> 
<input id="nok2PoliceStation" name="nok2PoliceStation" type="text"   maxlength="15" validate="Police Station,metachar,no"/> 
<label>Pin Code</label> 
<input id="nok2PinCode" name="nok2PinCode" type="text"   maxlength="8" validate="Pin Code,int,no"/> 
<div class="clear"></div>
</div><!-- End Block 3-->
...
<h4>Medical Details</h4>
<label>Blood Group</label> 
<select name="<%=BLOOD_GROUP_ID %>"	id="bldGrp" validate="Blood Group,string,no"  >	
<option value="0">Select</option>
	<%
	 for(Object[]  masBloodGroup : bloodGroupList){
	%>
...
<label>Family History</label> 
<select name="familyHistory" id="familyHistory"   multiple="multiple" class="list" onblur="openOtherField(this.value,this.id);">
 <option value="0">Select</option>
<%
	if(familyHistoryList.size() > 0){
		for(Object[] familyHistory : familyHistoryList){
...
<label> Allergy</label> 
<textarea name="allergies" id="allergies" value=""	validate="Allergy,string,no" maxlength="100" onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" onKeyUp="finalCheck(this)">
</textarea>
...
<div class="Block" id="doctorNSessionList">
<div id="div1">
...
<select id="deptId1" name="<%=DEPARTMENT_ID%>1" onchange="submitProtoAjaxNew('visitEmployee','/hms/hms/appointment?method=getDoctorAndSessionDetails&divcount=1&<%=DEPARTMENT_ID%>1='+this.value,'doctorNSession1');">
			<option value="0">Select</option>
			<%
		...
		<script type="text/javascript">	var	departmentArray= new Array();
	<%
		    		MasDepartment  department = new MasDepartment();

				     for (int d = 0; d < departmentList.size(); d++) {
				    	 department = (MasDepartment) departmentList.get(d);
	     			 %> 

	     			departmentArray[<%=d%>]= new Array();
	     			departmentArray[<%=d%>][0] = "<%=department.getId()%>";
	     			departmentArray[<%=d%>][1] = "<%=department.getDepartmentName()%>";
	     			 <% }%>    </script>
		
		
		
		<div id="doctorNSession1">
			<label>Doctor List<span>*</span> </label>
		<select id="doctorId1" name="consultingDoctor1">
		<option value="0"></option>
		</select>
		
		<label>Session<span>*</span> </label>
		<select id="sesId1" name="sessionId1">
		<option value="0"></option>
		</select>
		 <label  class="medium"> Priority</label>
            <select name="priority1" id="priority1" class="smallest"   style="width: 65px;">
	...
	<input name="Button" type="button" class="buttonAdd" value="" onclick="addDiv();"   /> 			
	<input type="button" name="delete" value="" class="buttonDel" onclick="removeDiv();"   />
...
<input type=hidden value="0" name="consultingDocId" id="consultingDocId"/>
<div class="clear"></div>
...
<input name="Print" type="button" value="Show Token" target="_blank" style="width:147px;" class="button" onClick="validateTokenDiv()">
 <input type="button" name="Submit11" value="Submit"   class="button"	onClick="if(checkPatientStatus() && checkNameSpaces() ){submitRegistration()}" />
<input type="reset" name="Reset" value="Reset" class="button"   onClick="submitFormForButton('visitEmployee','/hms/hms/registration?method=showEmployeeVisitJsp');" accesskey="r" />
...
<input	type="button" name="Submit" value="Save &amp; Admission"  	class="buttonbig"
	onClick="if(displayWarning()){submitForm('registration','/hms/hms/registration?method=submitPatientInformation');}" />
<input type="reset" name="Reset" value="Reset" class="button" 	  onClick="{clearHin();}" accesskey="r" />
</div>
...
function submitRegistration(){
	...
	for(var i=1; i<=divCount; i++)
		{
		if(document.getElementById("tokenNoId"+i) !=null)
		{
		if(isNaN(document.getElementById("tokenNoId"+i).value))
			{
			 alert(document.getElementById("tokenNoId"+i).value);
			 submitFlag = false;
			}
		else
			{
			for(var j=1; j<=divCount; j++)
				...
				if(i!=j)
					{
					if(document.getElementById("deptId"+i).value == document.getElementById("deptId"+j).value &&  document.getElementById("sesId"+i).value == document.getElementById("sesId"+j).value)
					{
						alert("Department and Session can't be same.")
						submitFlag = false;
						break;
					}
					}
	...
	if(submitFlag)
		{
		submitForm('visitEmployee','/hms/hms/registration?method=submitPatientInformationEmployeeVisit&flag='+flag,'checkTargetForNo');
		}
...
/* function addDiv(){
	  ...
	  var newdiv = document.createElement('div');
    	newdiv.setAttribute('id', 'div'+(divCount+1));
    	
    	newdiv.innerHTML = document.getElementById("div1").innerHTML;
	  document.getElementById("doctorNSessionList").appendChild(newdiv)
	  var newDepartment = document.createElement('div'+(divCount+1)).childNodes[0];
      var newDoctor =	document.getElementById("doctorNSessionList"); 
	  var newSession = document.getElementById("doctorNSessionList");
		  .setAttribute("onchange", "submitProtoAjaxWithDivName('visitEmployee','/hms/hms/appointment?method=getDoctorAndSessionDetails&divcount=1','doctorNSession1');");
	...
	function addDiv(){
		  ...
		  var divList = document.getElementById('doctorNSessionList');		 
		  var divCount = parseInt(document.getElementById("divCount").value);
		  ...
		  e2.name='departmentId'+iteration;		  
		  e2.id='deptId'+iteration;	 
		  
		  e2.options[0] = new Option('Select', '0');
		  e2.onchange=function(){submitProtoAjaxNew('visitEmployee','/hms/hms/appointment?method=getDoctorAndSessionDetails&divcount='+iteration+'&departmentId'+iteration+'='+this.value,'doctorNSession'+iteration);}
		   ...
		   subdiv.setAttribute('id', 'doctorNSession'+iteration);
		   newdiv.appendChild(subdiv);
		   
		   var e3 = document.createElement('label');
			  e3.innerHTML = "Doctor List<span>*<span>";
			  subdiv.appendChild(e3);
			  
			  var e4 = document.createElement('Select');
			  e4.name='employeeId'+iteration;		  
			  e4.id='doctorId'+iteration;
			  subdiv.appendChild(e4);
			  
			  var e5 = document.createElement('label');
			  e5.innerHTML = "Session<span>*<span>";
			  subdiv.appendChild(e5);
			  
			  var e6 = document.createElement('Select');
			  e6.name='sessionId'+iteration;		  
			  e6.id='sesId'+iteration;
			  subdiv.appendChild(e6);
			  
			  var e7 = document.createElement('label');
			  e7.innerHTML ="Priority<span>*<span>";
			  e7.setAttribute("class", "medium");
			  subdiv.appendChild(e7);
			  
			  var e8 = document.createElement('Select');
			  e8.name='priority'+iteration;		  
			  e8.id='priority'+iteration;
			  e8.innerHTML = document.getElementById('priority1').innerHTML;
			  e8.setAttribute("class", "smallest");
			  e8.setAttribute("style", "width: 65px;");
			  subdiv.appendChild(e8);
			  
			  var subsubdiv = document.createElement('div');
			  subsubdiv.setAttribute('id', 'displayToken'+iteration);
			  subdiv.appendChild(subsubdiv);
			  
			  var e9 = document.createElement('input');
			  e9.type = 'button';
			  e9.name='Button';
			  e9.onclick=function(){addDiv();}
			  e9.setAttribute("class", "buttonAdd");			 
			  newdiv.appendChild(e9);
			  
			  var e10 = document.createElement('input');
			  e10.type = 'button';
			  e10.name='delete';
			  e10.onclick=function(){removeDiv();}
			  e10.setAttribute("class", "buttonDel");			 
...
function checksex(row)
{
	var sex = document.getElementById("sex").value;
	var element = document.getElementById("deptId"+row);
	...
	if(sex == 'Male' && text == <%=departmentNameToRestrictMalePatient.trim()%>)
		...
		element.value='0';
		document.getElementById("displayToken"+row).innerHTML ="";
		return false;
		}
