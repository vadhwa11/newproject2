...
<form name="injectionReport"  method="post" action="">
<div class="titleBg"><h2>Injection Administration Register</h2></div>
<div class="Clear"></div>
...
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" class="calender"
	onClick="setdate('<%=currentDate%>',document.injectionReport.<%=FROM_DATE%>,event)" />

<label> To <span>*</span> </label>
 <input type="text" id="ToDateId"name="<%=TO_DATE %>" value="<%=currentDate %>" class="date"	readonly="readonly" MAXLENGTH="30" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender"	onClick="setdate('<%=currentDate%>',document.injectionReport.<%=TO_DATE%>,event)" />
<div class="Clear"></div>	
<label>Rank</label> 
...
<label class="medium">&nbsp;</label>
<input type="text" name="injectionName" id="injectionName" value="" validate="Injection Name,string,no" onblur="if(this.value!=''){getItemId(this.value,1);}" size="60"/>
<div id="ac2update1" style=" display:none; " class="autocomplete"></div>
<script type="text/javascript" language="javascript" charset="utf-8">
  new Ajax.Autocompleter('injectionName','ac2update1','registration?method=getInjectionListForAutoComplete',{parameters:'requiredField=injectionName'});
</script>
	<div class="Clear"></div>
</div>
<div class="Clear"></div>
<input type="button" name="OK" value="OK" class="button"
	onClick="if(datevalidation()){submitForm('injectionReport','/hms/hms/registration?method=showPrintInjectionRegisterReport');}" />
<input type="button" name="PRINT" value="PRINT" class="button"
	onClick="if(datevalidation()){submitForm('injectionReport','/hms/hms/registration?method=printInjectionRegisterReport');}" />
...
<script>
function openGraph(){
	var url = "registration?method=showInjectionRegisterGraph&"+getNameAndData('injectionReport');
