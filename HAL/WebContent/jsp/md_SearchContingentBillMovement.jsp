...
<%
		...
		if(patientMap.get("patientList") != null){
			patientList= (List<MdCardicContingentBillHd>)patientMap.get("patientList");
		}
		String message ="";
...
<input type="button" name="Submit" id="addbutton"
	onclick="if(checkBlankSearch()){submitForm('patientSearch','/hms/hms/mediClaim?method=showPatientDetailsForBillMovement');}"
...
<form name="searchBillMovement" method="post" action=""><script
...
<script type="text/javascript" language="javascript">
	...
	<% 	for(MdCardicContingentBillHd cardicContingentBillHd : patientList){
	%>
	  		data_arr[<%= counter%>] = new Array();
			data_arr[<%= counter%>][0] = <%= cardicContingentBillHd.getId()%>
			data_arr[<%= counter%>][1] = "<%=cardicContingentBillHd.getHin().getServiceNo()%>"
			data_arr[<%= counter%>][2] = "<%=cardicContingentBillHd.getHin().getHinNo()%>"
			<%
					String middleName = "";
					String lastName = "";
					if(cardicContingentBillHd.getHin().getPMiddleName() != null){
						middleName = cardicContingentBillHd.getHin().getPMiddleName();
					}
					if(cardicContingentBillHd.getHin().getPLastName() != null){
						lastName = cardicContingentBillHd.getHin().getPLastName();
					}
					
					%>
			data_arr[<%= counter%>][3] = "<%=cardicContingentBillHd.getHin().getPFirstName()%> <%=middleName%> <%=lastName%>"
			data_arr[<%= counter%>][4] = "<%=cardicContingentBillHd.getHin().getRank().getRankName()%> "
			<%
			if(cardicContingentBillHd.getHin().getSFirstName() != null  && !(cardicContingentBillHd.getHin().getSFirstName().equals(""))){
				
				String sMiddleName = "";
				String sLastName = "";
				if(cardicContingentBillHd.getHin().getSMiddleName() != null){
					sMiddleName = cardicContingentBillHd.getHin().getSMiddleName();
				}
				if(cardicContingentBillHd.getHin().getSLastName() != null){
					sLastName = cardicContingentBillHd.getHin().getSLastName();
				}
				String sName = cardicContingentBillHd.getHin().getSFirstName()+" "+sMiddleName+" "+sLastName;
			%>
