...
<label class="medium">Service No.</label> <input type="text"
...
<label class="medium">Patient First Name:</label> <input type="text"
	...
	MAXLENGTH="15" /> <label class="medium">Patient Last Name:</label> <input
	type="text" name="<%=P_LAST_NAME%>" value="" class="textbox_size20"
	style="width: 110px;" MAXLENGTH="15" /></div>

</form>
<br />

<input type="button" name="Submit" id="addbutton"
	onclick="submitForm('search','/hms/hms/ot?method=showPatientDetailsForPreAnesthesia');"
...
<form name="patientSearchForPreAnesthesia" method="post" action="">
<script type="text/javascript">
	formFields = [
			[0, "<%= HIN_ID%>", "id"], [1,"servNo"], [2,"hin"], [3,"patName"], [4,"serviceType"], [5,"sPersonName"], [6,"rank"], [7,"unit"], [8,"PacDate"]];
	 statusTd = 7;
	</script>
</div>
</form>
...
<script type="text/javascript" language="javascript">
	...
	data_header[7][0] = "PAC Date"
	data_header[7][1] = "data";
	data_header[7][2] = "10%";
	data_header[7][3] = "pacDate"
	...
	<% 	for(OtPreAnesthesiaDetails inpatient : patientList){
	%>
			...
			%>
				...
				<%}
				if(inpatient.getPacDate() != null){
				%>
				data_arr[<%= counter%>][8] = "<%=HMSUtil.changeDateToddMMyyyy(inpatient.getPacDate())%> "
				<%}else{%>
				data_arr[<%= counter%>][8] = ""
    ...
    formName = "patientSearchForPreAnesthesia"
