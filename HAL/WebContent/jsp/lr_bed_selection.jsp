...
<!--
	function setBedId(bedId,bedNo,bedNoFieldId, bedIdFieldId) {
		window.opener.jsSetBedId(bedId, bedIdFieldId);
		window.opener.document.getElementById(bedNoFieldId).value=bedNo;
...
<%
	...
	if (request.getAttribute("map") != null) 
	{
		...
		if (map.get("bedNoFieldId")!=null)
			bedNoFieldId = (String)map.get("bedNoFieldId");
		
		if (map.get("bedIdFieldId")!=null)
			bedIdFieldId = (String)map.get("bedIdFieldId");
	}
...
if (masBedList != null && masBedList.size() > 0) { %>
	...
	<tr>
		<td><input type="hidden" id="roomTypeId"
			...
			type="button" name="Submit11" id="" tabindex="1"
			value="<%=masBed.getBedNo()%>"
			onselect="javascript:setBedId('<%=masBed.getId()%>','<%=masBed.getBedNo()%>');"
			onclick="javascript:setBedId('<%=masBed.getId()%>','<%=masBed.getBedNo()%>', '<%=bedNoFieldId%>','<%=bedIdFieldId%>');" /></td>
