...
<%
	...
	if(map.get("masStoreSupplierList") != null)
	 {
		masStoreSupplierList =  (List<MasStoreSupplier>)map.get("masStoreSupplierList");	  		 
	...
	if(map.get("max") != null){
	 ...
	 if(map.get("message") != null)
	 {
		...
		for(StoreQuotationRequestM header: quotationHeader)
		{
			...
			savedYearId = (header.getYear()!=null?header.getYear().getId():0);
			System.out.println("savedYearId"+savedYearId);
			quotationNo = (header.getRequestNo()!=null?header.getRequestNo():"");
			MPRId = (header.getMprHeader()!=null?header.getMprHeader().getId():0);
...
<select name ="ddlRequestYear" id="ddlRequestYear">
	...
	<%
		if(financialYearList.size()>0)
		{
			...
			{
				if(Integer.parseInt(year[0].toString()) == savedYearId)
				{
					%>
					<option  value="<%=year[0]%>" selected="selected"><%=year[1]%></option>
				<%
				}
...
<input  type="text" class="calDate"  id="enquiryDate" name="enquiryDate" placeholder="DD/MM/YYYY" validate="Enquiry Date,string,yes" value="<%=quotationDate%>" onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10" style="width: 120px"/>
	
<label> Due Date <span>*</span></label>
<input  type="text" class="calDate"  id="dueDate" name="dueDate" placeholder="DD/MM/YYYY" validate="Due Date,string,yes" value="<%=dueDate%>" onkeyup="mask(this.value,this,'2,5','/');"onblur="validateExpDate(this,'mprDate');" maxlength="10" style="width: 120px"/>
	...
	<select	class="" name="mprNo" id="mprNo" validate="MPR No,String,yes" tabindex=1  >
		<option value="<%=MPRId%>">"<%=MPRNo%>"</option>
...
<div class="clear"></div>	
	...
	<input type="button" name="add" id="addbutton" value="Vendor COPY" class="button" onClick="submitForm('EnquiryforQuotation','/hms/hms/stores?method=generatevendorQuotationReport&headerId=<%=quotationHeaderId %>');"  tabindex=1 />
<input type="button" name="add" id="addbutton" value="HAL COPY" class="button" onClick="submitForm('EnquiryforQuotation','/hms/hms/stores?method=generatevendorQuotationReportHAL&headerId=<%=quotationHeaderId %>');"  tabindex=1 />
...
<script type="text/javascript" src="/hms/jsp/js/jquery.datePicker.js"></script>
	...
	/* 	selectItems(); */
		...
		{
			<%-- var matCodeCommaSeperated = <%=matCodeCommaSeperated%>; --%>
			
			$j('[id="chk"]').each(function(j)
					{
	...
	/* 	function checkYear()
	 ...
	 function populateSavedState()
	 {
		...
		var m_select = document.getElementById("supplier");
		<%
				for(MasStoreSupplier mss :masStoreSupplierList)
				{
					%>
				    var option = document.createElement("option");
				    option.value = '<%=mss.getId()%>';
				    option.text = '<%=mss.getSupplierName()%>';
