...
<%-- <select id="deptId" name="<%=DEPARTMENT_ID%>" onchange="submitProtoAjaxWithDivName('appSetup','/hms/hms/appointment?method=getLoginDoctorDetailsByDepartment','doctorNSessionList');"> --%>
...
<div class="clear"></div>
<div class="Block">
<label>Start Time<span>*</span> </label> <input type="text" name="startTime" id="startTime" onblur="IsValidTime(this.value,this.id)" onkeyup="mask(this.value,this,'2',':');" validate="Start Time,string,yes">
<label>End Time<span>*</span> </label>  <input type="text" name="endTime" id="endTime" onblur="IsValidTime(this.value,this.id)" onkeyup="mask(this.value,this,'2',':');" validate="End time,string,yes">
<label>Time taken<span>*</span> </label>  <input type="text" name="timeTaken" id="timeTaken" validate="Time taken,int,yes">
</div>
<table border="0" cellspacing="0" cellpadding="0">
	...
	<input type="hidden" name="<%=APPOINTMENT_ID%>" id="appointmentId<%=inc%>"
		...
		<td><input type="text" size=8 id="minDays<%=inc%>"
			...
			<%-- <td><input type="text" size=8 id="timeTaken<%=inc%>"
			name="timeTaken" MAXLENGTH="2" autocomplete="off" validate="Min no. of Days,num,no" /></td> --%>
	</tr>
...
<input name="" type="button" class="button" value="Submit" 
	onclick="if(validateDatefield() && checkShowButton()){<%=url%>}" /> 
	</div>
	<div id="updateId" style="display: none">
<input name="" type="button" class="button" value="Update"
	onclick="if(validateDatefield() && checkShowButton()){ submitForm('appSetup','appointment?method=updateAppointmentSetup');}" /> 
	</div>
	...
	for(var k=0;k<totalRow;k++){
		if(document.getElementById('TokenStart'+k).value!="" || document.getElementById('TokenInterval'+k).value!="" 
			|| document.getElementById('TotalToken'+k).value!="" || document.getElementById('TotalOnlineToken'+k).value!="")
			   {
			     if(document.getElementById('maxDays'+k).value=="" || document.getElementById('minDays'+k).value=="")
				   {
			    	 alert("Please fill al the values for row "+parseInt(k+1));
			    	 return false;
				   }
			   }
	}
