...
<input
				...
				<input
					...
					<input type="hidden" name="sss1" class="button" value="BACK" onClick="submitForm('searchDefectiveDrug','stores?method=showProformaBEntryJsp');" />
					<input type="hidden" name="sss1" align="right" class="buttonBig2" value="SENT FOR COMMAND APPROVAL" onclick="approvalFun();"/>
...
<script type="text/javascript" language="javascript">
		...
		data_header[0][0] = "Proforma SI No."
		data_header[0][1] = "data";
		data_header[0][2] = "5%";
		data_header[0][3] = "<%=RADIO_FOR_TABLE%>"

		data_header[1] = new Array;
		data_header[1][0] = "Proforma Date"
		data_header[1][1] = "data";
		data_header[1][2] = "5%";
		data_header[1][3] = "<%= ENTRY_NO%>"

		data_header[2] = new Array;
		data_header[2][0] = "Cmd HQ"
		data_header[2][1] = "data";
		data_header[2][2] = "5%";
		data_header[2][3] = "<%= ENTRY_DATE %>";

		data_header[3] = new Array;
		data_header[3][0] = "Accts Section"
		...
		Iterator itrrr=gridProformaBnoList.iterator();
			...
			data_arr[<%= counter%>][1] = "<%=storeDefectiveDrugM.getProformaNo()%>"
			data_arr[<%= counter%>][2] = "<%= formatter.format(storeDefectiveDrugM.getProformaDate())%>"

			<%if(storeDefectiveDrugM.getStatus().equals("s")){%>
			data_arr[<%= counter%>][3]="Not Present"
			data_arr[<%= counter%>][4]="Not Present"
			<%}%>
			<%if(storeDefectiveDrugM.getStatus().equals("n")){%>
			data_arr[<%= counter%>][3]="Pending for approval"
			data_arr[<%= counter%>][4]="Not Present"
			<%}%>
			<%if(storeDefectiveDrugM.getStatus().equals("n")){%>
			data_arr[<%= counter%>][3]="Pending for approval"
			data_arr[<%= counter%>][4]="Not Present"
			<%}%>
			<%if(storeDefectiveDrugM.getStatus().equals("r")){%>
			data_arr[<%= counter%>][3]="Not approved"
			data_arr[<%= counter%>][4]="Not Present"
			<%}%>
			<%if(storeDefectiveDrugM.getStatus().equals("y")){%>
			data_arr[<%= counter%>][3]="Approved"
			data_arr[<%= counter%>][4]="Pending for approval"
			<%}%>
			<%if(storeDefectiveDrugM.getStatus().equals("c")){%>
			data_arr[<%= counter%>][3]="Approved"
			data_arr[<%= counter%>][4]="Not approved"
			<%}%>
			<%if(storeDefectiveDrugM.getStatus().equals("p")){%>
			data_arr[<%= counter%>][3]="Approved"
			data_arr[<%= counter%>][4]="Approved"
			<%}%>
		<% counter++;
			}
		%>
