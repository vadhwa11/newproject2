...
<input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /><label>Transaction Type Name</label> <input
	type="radio" class="radioAuto" name="<%=SELECTED_RADIO %>" value="2" /> <input type="text"
	id="searchField" name="<%= SEARCH_FIELD%>" value=""
	validate="TransactionType Code,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'systemRelatedMaster?method=searchTransactionType')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','systemRelatedMaster?method=searchTransactionType','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_transaction_type"></div></form>
</div>
<div class="Clear"></div>
</div>
<div class="Clear"></div>
		...
		if(searchTransactionTypeList.size()>0)
		 {
			...
			{
	%> 
<h4><a href="systemRelatedMaster?method=showTransactionTypeJsp">Show All
Records</a></h4> <%
			}
		 }
	if(searchTransactionTypeList.size()==0 && map.get("search") != null)
	  {
%> <h4><a href="systemRelatedMaster?method=showTransactionTypeJsp">Show
All Records</a></h4> <%
   }
...
<form name="transactionType" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasTransactionType">
<input type="hidden" name="<%=POJO_PROPERTY_NAME %>"
	value="TransactionTypeName"> <input type="hidden" name="title"
	value="TransactionType"> <input type="hidden"
	name="<%=JSP_NAME %>" value="transactionType"> <input
	type="hidden" name="pojoPropertyCode" value="TransactionTypeCode">


<div class="Clear"></div>
<div class="Block">
<label> Transaction Type Code <span>*</span> </label> <input id="codeId"
	type="text" name="<%= CODE%>" value=""
	validate="TransactionType Code,string,yes" 
	MAXLENGTH="8" / tabindex=1> <label > Transaction Type Name <span>*</span> </label> <input type="text"
	name="<%= SEARCH_NAME %>" value=""
	validate="TransactionType Name,string,yes" 
	MAXLENGTH="30" / tabindex=1
	onkeypress="return submitenter(this,event,'systemRelatedMaster?method=addTransactionType')">
<script>
