...
<h6> General Contingent Bill Reimbursement Bill</h6>
<form name="contBillReim" target="_blank" action="" method="post">
...
<input 	type="text" class="calDate" id="fromDate" name="<%=FROM_DATE %>" value="" validate="From Date,date,no" MAXLENGTH="10" tabindex="1" readonly="readonly" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.contBillReim.<%=FROM_DATE%>,event)" />
	...
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.contBillReim.<%=TO_DATE%>,event)" />
...
<div class="bottom"><input type="button" name="OK" value="OK" class="button"
	onClick="if(testFromToDate())if(checkBlankSearch())submitForm('contBillReim','/hms/hms/mediClaim?method=printContingentBillRemReport','validateFromToDate');" />
...
<script type="text/javascript">
	...
	obj1 = eval(document.contBillReim.fromDate)
	obj2 = eval(document.contBillReim.toDate)
