...
<div class="Block">
<label>Authorizer Code</label><input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /> <label>Authorizer
Name</label> <input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO %>" value="2" />  <input type="text" id="searchField" name="<%= SEARCH_FIELD%>"
	value="" validate="Authorizer Code,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=searchAuthorizer')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchAuthorizer','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_authorizer"></div></form>
...
<form name="authorizer" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasAuthorizer"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="AuthorizerName">
<input type="hidden" name="title" value="Authorizer"> <input
	type="hidden" name="<%=JSP_NAME %>" value="authorizer"> <input
	type="hidden" name="pojoPropertyCode" value="AuthorizerCode"> 

<div class="Clear"></div>
<div class="Block">
<label> Authorizer Code <span>*</span></label> <input id="codeId" type="text"
	name="<%= CODE%>" value="" validate="Authorizer Code,string,yes"
	 MAXLENGTH="8" tabindex=1 /> <label>Authorizer
Name <span>*</span></label> <input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="Authorizer Name,string,yes" 
	MAXLENGTH="30" tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addAuthorizer')" />
<script>
...
<div class="Clear"></div>
 <input type="button" name="add" id="addbutton"
	...
	onClick="submitForm('authorizer','hospital?method=addAuthorizer');"
	accesskey="a" tabindex=1 /> <input type="button" name="edit"
	id="editbutton" value="Update" class="button"
	onClick="submitForm('authorizer','hospital?method=editAuthorizer')"
	accesskey="u" tabindex=1 /> <input type="button" name="Delete"
	id="deletebutton" value="Activate" class="button"
	onClick="submitForm('authorizer','hospital?method=deleteAuthorizer&flag='+this.value)"
...
data_arr[<%= counter%>][3] = "<%= masAuthorizer.getLastChgBy() %>"
data_arr[<%= counter%>][4] = "<%= HMSUtil.convertDateToStringWithoutTime(masAuthorizer.getLastChgDate()) %>"
data_arr[<%= counter%>][5] = "<%= masAuthorizer.getLastChgTime() %>"
