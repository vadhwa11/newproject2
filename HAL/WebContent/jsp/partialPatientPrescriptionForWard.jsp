...
<input type="button" name="search" id="addbutton"	onclick="if(validateDates()){submitForm('searchPrescription','/hms/hms/opd?method=showPartialPrescriptionForWardJsp');}"	value="Search" class="button" accesskey="a" />
...
<form name="partialPrescriptionForWard" method="post" action="">
	...
	int  counter=0;
	 if (prescriptionList != null && prescriptionList.size() > 0) {
		/*   for(PatientPrescriptionDetails prescriptionDetails : prescriptionList)
		 {
		 PatientPrescriptionHeader patientPresc = prescriptionDetails.getPrescription();  */
...
<%}%>
	...
	//data_arr[<%= counter%>][12] = "<%=patientPresc.getHin().getSex().getAdministrativeSexName()%>"
	<%
	if(patientPresc.getInpatient() != null && patientPresc.getInpatient().getDoctor() !=null){ %>
	
	data_arr[<%= counter%>][12] = "<%=patientPresc.getEmp().getFirstName()+" "+patientPresc.getEmp().getLastName()%>"
	<%}else{%>
	data_arr[<%= counter%>][12] =" "
	<%}%>
	data_arr[<%= counter%>][13] = "<%=patientPresc.getInpatient().getDepartment().getDepartmentName()%>"
	data_arr[<%= counter%>][14] = "<%=patientPresc.getInpatient().getBed().getBedNo()%>"
	<%
	counter++;
	 } 
}
...
formName = "partialPrescriptionForWard"
start = 0
