...
<div class="Clear"></div>
<label class="NoWidth" style="padding-left: 10px;">Service Type:</label>
<%
				if(dgResultEntryHeader.getPatient().getServiceType() != null){
			%> <label class="value"><%=dgResultEntryHeader.getPatient().getServiceType().getServiceTypeName()%></label>
...
Status:</label> <%if(dgResultEntryHeader.getPatient().getServiceStatus() != null){
			%> <label class="value"><%= dgResultEntryHeader.getPatient().getServiceStatus().getServiceStatusName()%></label>
...
<%}else{ %> <label class="value">-</label> <% }%>
<div class="Clear"></div>
<label class="NoWidth" style="padding-left: 10px;">Ser. Per.
Name:</label> <%
...
<%}else{ %> <label class="value">-</label> <% }%> <label class="NoWidth">Rank:</label>
<%
			if(dgResultEntryHeader.getPatient().getRank() != null){
			%> <label class="value"><%=dgResultEntryHeader.getPatient().getRank().getRankName()%></label>
<%} else{ %> <label class="value">-</label> <% }%> <label class="NoWidth">Unit</label>
<%
if(dgResultEntryHeader.getPatient().getUnit() != null){
%> <label class="value"><%= dgResultEntryHeader.getPatient().getUnit().getUnitName()%></label>
<%} else{ %> <label class="value">-</label> <% }%> <label class="NoWidth">Trade</label>
<%
if(dgResultEntryHeader.getPatient().getTrade() != null){
%> <label class="value"><%=  dgResultEntryHeader.getPatient().getTrade().getTradeName()%></label>
<%} else{ %> <label class="value">-</label> <% }%>
<div class="Clear"></div>
...
<%}else{ %> <label class="valuenoWidth">-</label> <%} %> <label
	class="noWidth">Result Time:</label> <label class="valuenoWidth"><%=dgResultEntryHeader.getResultTime() %></label>
...
<%} %> <input type="hidden" name="<%=RESULT_NO %>"
	...
	class="noWidth">Result Validated Date:</label> <label
	class="valuemedium"><%=date%></label> <label class="noWidth">Result
Validated Time:</label> <label class="valuemedium"><%=time%></label> <label
	class="noWidth"><span>*</span>Result Validated By</label> <select
...
<table border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<th scope="col">Validate </br>
		<input type="checkbox" name="checkall" value="Validate All"
		...
		/**second if **/		if(!dgDetail.getResult().contains("-")){
			...
			<%}else { %> <input name="<%=ADDITIONAL_REMARKS %>" type="text"
				value="" onkeyup="chkLength(this,100);" /> <%} %> <input
			...
			<input name="<%=RESULT %>" id="<%=RESULT %>"
				value="<%=dgDetail.getResult() %>" /> <%} }if(dgDetail.getNormal().getMaxNormalValue()!= null && !dgDetail.getNormal().getMaxNormalValue().equals("") ){
		   			if(Integer.parseInt(dgDetail.getResult()) <= Integer.parseInt(dgDetail.getNormal().getMaxNormalValue())&& Integer.parseInt(dgDetail.getResult()) >= Integer.parseInt(dgDetail.getNormal().getMinNormalValue())){
				   		%> <input name="<%= RESULT %>" id="<%= RESULT %>"
				class="highlight"
				style="color: #000000; font-weight: normal; width: 150px;"
				value="<%=dgDetail.getResult() %>" /> <%}else{%> <input
				name="<%= RESULT %>" id="<%= RESULT %>" class="highlight"
				value="<%=dgDetail.getResult() %>" /> <%}	} %> <!--  else if third if  -->
			<%}else{ %> <input name="<%=RESULT %>" id="<%=RESULT %>"
