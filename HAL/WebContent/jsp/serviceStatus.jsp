...
<div class="Block">
<label class="noWidth">Service Status Code</label><input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /> <label class="noWidth">Service
Status Name</label> <input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO %>" value="2" />  <input type="text" id="searchField"
	name="<%= SEARCH_FIELD%>" value=""
	validate="Service Status Code,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'hospitalRelated?method=searchServiceStatus')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchServiceStatus','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_service_status"></div></form>
</div>
<div class="Clear"></div>
</div>
<div class="Clear"></div>
		...
		if(searchServiceStatusList.size()>0)
		 {
			...
			{
	%> 
<h4><a href="hospital?method=showServiceStatusJsp">Show All Records</a></h4> <%
			}
		 }
...
<form name="serviceStatus" method="post" action=""><input
...
<div class="Clear"></div>
<div class="Block">
<label>  Service Status Code<span>*</span> </label>
<input type="text" name="<%= CODE%>" value="" validate="Service Status Code,string,yes" MAXLENGTH="8" / tabindex=1>
<label> Service Status Name <span>*</span></label>
<input type="text"	name="<%= SEARCH_NAME %>" value=""	validate="Service Status Name,string,yes" 	MAXLENGTH="30" / tabindex=1 onkeypress="return submitenter(this,event,'hospitalRelated?method=addServiceStatus')">
<script>
...
<div class="Clear"></div>
 <input type="button" name="add" id="addbutton"
	...
	onClick="submitForm('serviceStatus','hospital?method=addServiceStatus');"
	accesskey="a" tabindex=1 /> <input type="button" name="edit"
	id="editbutton" value="Update" class="button"
	onClick="submitForm('serviceStatus','hospital?method=editServiceStatus')"
	accesskey="u" tabindex=1 /> <input type="button" name="Delete"
	id="deletebutton" value="Activate" class="button"
	onClick="submitForm('serviceStatus','hospital?method=deleteServiceStatus&flag='+this.value)"
