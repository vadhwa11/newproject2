...
<div class="clear"></div>
<div class="cmntable"> 
<table width="100%" border="0" cellspacing="0" cellpadding="5"	id="voucherDetails" class="cmntable">
	<tr>
		<th scope="col"></th>
	...
	<td id="slId<%=i %>"> <input  type="text"  readonly="readonly" name="<%=SUB_LEDGER_CODE_BANK%><%=i%>" id="subledId<%=i%>" value="" size="12" /></td>
	...
	<td> <input id="drAmountId<%=i %>" type="text"  size="10" name="<%=DR_AMOUNT%><%=i%>" value="" validate="Dr Amount,float,yes"   onblur="totalDrCrAmount('dr');"  MAXLENGTH="12" tabindex=1 /></td>
	<td> <input id="crAmountId<%=i %>" type="text"  size="10" name="<%=CR_AMOUNT%><%=i%>" value="" disabled="disabled" validate="Cr Amount,float,no" onblur="totalDrCrAmount('cr');"  MAXLENGTH="12" tabindex=1 /></td>
	<td><input type="button" name="add" value="" class="buttonAdd" onclick="addRow();" tabindex="1" /></td>
	</tr>
	<tr class="background">
		<td colspan=5 class="right">Total</td>
		<td><input id="totalDrAmountId" type="text" size="10" class="readOnlySmall" readonly="readonly" name="<%=TOTAL_DR_AMOUNT %>" value="" validate="Total DrAmount,float,no"   MAXLENGTH="15" tabindex=1 /></td>
		<td colspan=2><input id="totalCrAmountId" size="10" type="text" class="readOnly" readonly="readonly" name="<%=TOTAL_CR_AMOUNT %>" value="" validate="Total CrAmount,float,no"   MAXLENGTH="15" tabindex=1 /></td>
	</tr>
	</table>
	</div>
	<div class="clear"></div>
...
<div class="clear"></div>
<input type="button" name="save" id="addbutton" value="Submit" class="button" onClick="if(checkTotalAmount()){submitForm('journalVoucher','account?method=submitOpeningBalance','validateNarration','validateTotalDrCr')};" accesskey="a" tabindex=1 />
...
function validateAccountName( strValue,inc,tdid ) {
 	...
 	if(strValue != "")
	{
		...
		//var id = strValue.substring(idx1,idx2);
		    if(id!="")
		    {
		    	submitProtoAjaxWithDivNameWithoutFormData('journalVoucher','account?method=getSubLedgerForAccountOP&accName='+encodeURIComponent(strValue)+'&rowVal='+inc,tdid+inc);
		    }
	}
...
function changeField(val,inc){
     ...
     if(val == 'cr'){
         ...
         document.getElementById('crAmountId'+inc).setAttribute('validate','Cr Amount,float,yes');
         document.getElementById('drAmountId'+inc).setAttribute('validate','Dr Amount,float,no');
         ...
         document.getElementById('drAmountId'+inc).setAttribute('validate','Dr Amount,float,yes');
         document.getElementById('crAmountId'+inc).setAttribute('validate','Cr Amount,float,no');
	...
	function checkTotalAmount(){
		...
		var tAmountdr = document.getElementById("totalDrAmountId").value;
		var tAmountcr = document.getElementById("totalCrAmountId").value;
		
		if(isNaN(tAmountdr) || isNaN(tAmountcr)){
