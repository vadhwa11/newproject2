...
<div class="Block">
 <input type="hidden" id="dgOrderhdId" name="dgOrderhdId" value="<%= dgOrderhdId%>" />
<input	type="hidden" id="hinId" value="<%= hinId%>" />
 <input type="hidden"	id="admissionNumber" value="<%= admissionNumber%>" /> 
 <label>Req.Date</label> <label class="value"> <%= date%> </label>
...
<table width="100%" colspan="7" id="chargeDetails" cellpadding="0"	cellspacing="0">
	...
	<tbody>
		...
		<tr>
			...
			<td width="5%"><input type="text" size="2" value="<%=inc%>"	tabindex="1" name="<%=SR_NO%>" readonly="readonly" /></td>
			<td width="95%"><input type="text" tabindex="1" size="80" align="right" name="<%=CHARGE_CODE%>" id="chargeCode<%=inc%>"	onblur="if(fillSrNo('<%=inc %>')){checkForChargeCode(this.value, '<%=inc %>');}" />
    ...
    <input type="hidden" value="" name="<%=CHARGE_CODE_ID%>" id="chargeCodeId<%=inc %>" />
    <input type="hidden" id="chargeName<%=inc%>" style="width: 80%" name="<%=CHARGE_NAME%>"	value="" readonly="readonly" /></td>
	<input type="hidden" id="qty<%=inc%>" name="<%=QUANTITY %>" value="" validate="Qty,int,no" MAXLENGTH="3" />
	</td>
		</tr>
	...
	<tbody>
		...
		<tr>
			<td><input type="checkbox" id="acceptedCheck<%=templateCount %>"
				 name="check<%=templateCount %>" value="y"
...
<div class="clear"></div>
<input type="button" tabindex="1" class="button" onclick="addRow();" value="Add " align="right" />
...
<input type="hidden" name="serviceNo" id="serviceNo" value="<%=visit.getHin().getServiceNo() %>" />
