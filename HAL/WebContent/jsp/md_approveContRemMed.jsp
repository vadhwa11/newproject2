...
<%@page import="jkt.hms.masters.business.MdContigentMedicalBillDt"%><script src="/hms/jsp/js/proto.js" type="text/javascript"></script>
...
%>
	...
	if (request.getAttribute("map") != null) {
		...
		if(specialInvHdList != null && specialInvHdList.size()>0){
			specialInvestigationHd = (MdContigentMedicalBillHd) specialInvHdList.get(0);
...
<input type="hidden"
 ...
 <label>Entry No.</label> <input id="orderNoId" type=hidden
	name="<%=ENTRY_NO %>" value="<%=specialInvestigationHd.getEntryNo() %>" title="Entry No" /> <label
	class="value"><%=specialInvestigationHd.getEntryNo()%></label> <input type="hidden"
	id="contHdId" name="contHdId"
	value="<%= specialInvestigationHd.getId()%>" />
	
 <label>Entry Date<span>*</span></label>
 <%if(specialInvestigationHd.getEntryDate() !=null){ %>
 <input type="text" class="calDate" id="fromDateId"
	name="<%=ENTRY_DATE %>" value="<%=HMSUtil.changeDateToddMMyyyy(specialInvestigationHd.getEntryDate()) %>" readonly="readonly"
	MAXLENGTH="30" validate="Entry Date,date,yes" /> 
 <%}else{ %> <input type="text" class="calDate" id="fromDateId"
	...
	<tbody>
		...
		<%
			...
			if (detailsMap.get("masInvestigationDtList") != null) {
				specialInvestigationDtList = (List<MdContigentMedicalBillDt>) detailsMap.get("masInvestigationDtList");
			}
			if (specialInvestigationDtList != null
					&& specialInvestigationDtList.size() > 0) {
				for (MdContigentMedicalBillDt specialInvestigationDt : specialInvestigationDtList) {
					inc++;
		%>
...
<%if(specialInvestigationHd.getBillNo() !=null){ %>
<input id="billNo" type="text" name="<%= BILL_NO%>" value="<%=specialInvestigationHd.getBillNo() %>"
	validate="Bill No,string,yes" MAXLENGTH="15" tabindex=1 /> 
	<%}else{ %>
	<input id="billNo" type="text" name="<%= BILL_NO%>" value=""
	validate="Bill No,string,yes" MAXLENGTH="15" tabindex=1 /> 
	<%} %>
	<label>
Bill Date <span>*</span></label> 
<%if(specialInvestigationHd.getBillDate() !=null){ %>
<input type="text" class="calDate" id="fromDateId"
	name="<%=BILL_DATE %>" value="<%=HMSUtil.changeDateToddMMyyyy(specialInvestigationHd.getBillDate()) %>" readonly="readonly"
	MAXLENGTH="30" validate="Bill Date,date,yes" />
	<%}else{ %>
	<input type="text" class="calDate" id="fromDateId"
...
<%if(specialInvestigationHd.getAmount() !=null){ %>
 <input id="amount" type="text"
	name="<%= AMOUNT%>" value="<%=specialInvestigationHd.getAmount() %>" validate="Amount,float,yes" MAXLENGTH="8"
	tabindex=1 onblur="fillAmount();" /> 
...
<label class="large"> Amt. Qualifying of reimbursement of (Rs.)<span>*</span> </label> 
<%if(specialInvestigationHd.getQualifyingAmount() !=null){ %>
<input id="qualifyingAmt" type="text" class="large"
	name="<%= QUALIFYING_RS%>" value="<%=specialInvestigationHd.getQualifyingAmount() %>"
	validate="Amount Qualifying of reimbursement of Rs.,float,yes"
	MAXLENGTH="8" tabindex=1
	onblur="checkQualifyingAmt();AmountToReceived();" /> <%}else{ %>
	<input id="qualifyingAmt" type="text" class="large"
...
<label class="large"> Amount Payable to<span>*</span></label> <%
	String fullName=specialInvestigationHd.getHin().getServiceNo()+" ";
	if (specialInvestigationHd.getHin().getRank() != null) {
	...
	%>
	<%if(specialInvestigationHd.getPayableTo() !=null){ %>
	<input id="payAmount" type="text" class="large"
	name="<%= PAYABLE_TO%>" value="<%=specialInvestigationHd.getPayableTo()%>"
	validate="Amount Payable,string,yes" MAXLENGTH=50 " tabindex=1 /><%}else{ %>
	 <input id="payAmount" type="text" class="large"
	name="<%= PAYABLE_TO%>" value="<%=fullName%>"
	validate="Amount Payable,string,yes" MAXLENGTH=50 " tabindex=1 /><%} %>
<div class="Clear"></div></div>
<div class="Block">
<div class="Clear"></div>
<label class="large"> Name of pay account office in which individual IRLAs are maintained <span>*</span></label>
<%if(specialInvestigationHd.getAccountOfficer() !=null){ %>
 <input id="payName" type="text"
	class="large" name="<%= NAME_PAY_OFFICER%>" value="<%=specialInvestigationHd.getAccountOfficer() %>"
...
<%if(specialInvestigationHd.getSubmissionDate() !=null){ %>
 <input type="text"
	class="date" id="fromDateId" name="<%=SUB_DATE %>"
	value="<%=HMSUtil.changeDateToddMMyyyy(specialInvestigationHd.getSubmissionDate()) %>" readonly="readonly" MAXLENGTH="30"
	validate="Date Of Submission,date,yes" />
<%}else{ %>
 <input type="text"
	class="date" id="fromDateId" name="<%=SUB_DATE %>"
...
<%if(specialInvestigationHd.getReceivedRs() !=null){ %>
<input id="receivedRs" type="text"
	name="<%= RECEIVED_RS%>" value="<%=specialInvestigationHd.getReceivedRs() %>" validate="Received Rs,float,no"
	MAXLENGTH="8" tabindex=1 />
<%}else{ %>
<input id="receivedRs" type="text"
	...
	<%
		if (authorityList != null) {
			for (MdMasAuthority masAuthority : authorityList) {
				if(specialInvestigationHd.getFwtTo() !=null){
					if(specialInvestigationHd.getFwtTo().getId() == masAuthority.getId()){
	%>
	<option value="<%=masAuthority.getId ()%>" selected="selected"><%=masAuthority.getAuthorityName()%></option>
	<%}else{ %>
...
<label> FWT Date<span>*</span></label>
<%if(specialInvestigationHd.getFwtDate() !=null){ %>
<input type="text"
	class="calDate" id="fromDateId" name="<%=FWT_DATE %>"
	value="<%=HMSUtil.changeDateToddMMyyyy(specialInvestigationHd.getFwtDate()) %>" readonly="readonly" MAXLENGTH="30"
	validate="FWT Date:,date,yes" />
<%}else{ %>
 <input type="text"
	class="calDate" id="fromDateId" name="<%=FWT_DATE %>"
	value="<%=currentDate %>" readonly="readonly" MAXLENGTH="30"
...
<input type="button" class="button" value="Submit"
	onclick="if(checkFilledRow())submitForm('contingentBill','mediClaim?method=validateContingentBill');"
	align="right" /> 
