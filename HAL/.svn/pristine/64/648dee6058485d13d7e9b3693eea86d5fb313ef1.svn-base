...
<form name="searchDefectiveDrug1" method="post" action="">
...
<input
				...
				<input
					...
					<input type="button" name="sss1" class="button" value="BACK" onClick="submitForm('searchDefectiveDrug1','stores?method=showDefectiveDrugJsp');" />
					</div>
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
...
<script type="text/javascript" language="javascript">
		...
		data_header[1][0] = "Sample Entry No."
		...
		Iterator itrrr=gridDefectiveDrugHeaderList.iterator();
		          ...
		          while(itrrr.hasNext())
		           {           
		        	  StoreSampleTestingEntry  storeDefectiveDrugM = (StoreSampleTestingEntry)itrrr.next(); 
		             %>
			...
			data_arr[<%= counter%>][2] = "<%=storeDefectiveDrugM.getDefectNo()%>"
			data_arr[<%= counter%>][3]="<%=HMSUtil.convertDateToStringWithoutTime(storeDefectiveDrugM.getDefectiveDate())%>"
