...
<input type="button" name="add1" id="addbutton1" value="Search" class="button" onClick="submitForm('searchPrescription','opd?method=showPendingNIPPrescriptionJsp');" accesskey="a" tabindex=1 />
</div> --%>
...
<form name="pendingNIPPrescription" method="post" action="">
...
data_header[13][0] = "Approved By"
data_header[13][1] = "data";
data_header[13][2] = "8%";
data_header[13][3] = "approvedBy";
...
<%}%>
	...
	<%if(patientPresc.getNipStatus().equalsIgnoreCase("A")){%>
		data_arr[<%= counter%>][13] ="Approved"
	<%}else
	{%>
	data_arr[<%= counter%>][13] ="Pending"
	
<%}
	
 	
	String approvedBy="";
 		if(patientPresc.getNipApprovedBy() != null){
 			
 			if(patientPresc.getNipApprovedBy().getEmployee().getFirstName() !=null){
 				approvedBy += patientPresc.getNipApprovedBy().getEmployee().getFirstName();
 				}
 				if(patientPresc.getNipApprovedBy().getEmployee().getMiddleName()!=null){
 					approvedBy += " "+patientPresc.getNipApprovedBy().getEmployee().getMiddleName();
 				}
 				if(patientPresc.getNipApprovedBy().getEmployee().getLastName()!=null){
 					approvedBy += " "+patientPresc.getNipApprovedBy().getEmployee().getLastName();
 				}
 				
 		
 		%>
 		data_arr[<%= counter%>][14] = "<%=approvedBy%>"
 		<%}else{%>
 		data_arr[<%= counter%>][14] = "-"
 		<%}
	counter++;
...
formName = "pendingNIPPrescription"
start = 0
