...
<%
	System.out.println("this is new indent to depot");
	Map map = new HashMap();
...
<label>Contact No.</label>
	...
	<input type="text" name="<%=CONTACT_NUMBER %>" id="tellNo" value="<%=storeIndentM.getTellNo()%>" maxlength="12" />
	<%}else{ %>
	<input type="text" name="<%=CONTACT_NUMBER %>" id="tellNo" value="" maxlength="12"/>
	<%} %>
...
<table width="100%" colspan="7" id="indentDetails" border="0" cellpadding="0" cellspacing="0">
	...
	<tbody>
 	   ...
 	   int incTemp2=inc+8;
 	   System.out.println("this is indent size"+gridIndentTList.size());
		...
		<% inc++;
 		  ...
 		  %>
 		  <input type="hidden" size="2" value="<%=inc%>"  name="gridLen" readonly="readonly" />
...
<div class="clear"></div>
<input type="button"	class="button" value="Next" 	onclick="if(checkForNext()){submitForm('indentGrid','stores?method=addNextOrSubmitIndentToDepotOld&buttonName=next');}" />
<input type="button" name="sss" class="button" value="Update"	onclick="if( checkForSubmit()){submitForm('indentGrid','stores?method=addNextOrSubmitIndentToDepotOld&buttonName=submit');}" />
<input type="button" name="sss" class="button" value="Delete"	onclick="submitForm('indentGrid','stores?method=deleteGridItemsForIndentDepo');" />
...
function jsSubmit()
{
		...
		if( checkSaveSubmit()){
			submitForm('indentGrid','stores?method=addNextOrSubmitIndentToDepotOld&buttonName=close');
			}
		//}
}
