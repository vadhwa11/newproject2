...
<%
	...
	if (map.get("medicalBoardOpinionList") != null) {
		medicalBoardOpinionList = (List) map.get("medicalBoardOpinionList");
	}
...
<%
int  counter=0;
if (medicalBoardOpinionList != null && medicalBoardOpinionList.size() > 0) {
for(MasMedicalExaminationReportOnEntry masexam : medicalBoardOpinionList){
%>
	   ...
	   String ma_status="";
		if(masexam.getVisit().getPriority()!=null){
			if(masexam.getVisit().getPriority()==4){
				...
				ma_status="Rejected By Approving Authority";
