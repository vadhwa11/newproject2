...
<h3 class="menuheader expandable openheader">FWC ManAgement</h3>
<ul class="categoryitems">
	...
	Booking </a></li>
	<li><a
		href="fwc?method=showFwcTemplateDepartmentWiseJsp&visitId=<%=visit.getId()%>">Fwc
	...
	<li><a
		href="fwc?method=showPatientPreviousVisitForPrescriptionReport&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&deptId=<%=visit.getDepartment().getId()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>">Previous
	Prescription</a></li>
	<li><a
...
<%
	if (opdPatientDetails.getReturnfromHospital() != null
...
<%
	if (opdPatientDetails.getHospName() != null) {
%> 
...
<%
	if (opdPatientDetails.getDoa() != null) {
%> 
...
<%
	if (opdPatientDetails.getDod() != null) {
%> 
...
if(opdPatientDetails.getDisposal()!=null){
	disposal=opdPatientDetails.getDisposal();
}
String dignosis="";
if(opdPatientDetails.getDisposal()!=null){
	disposal=opdPatientDetails.getDisposal();
...
<%
 	if (opdPatientHistory.getFamilyPastHistory() != null) {
...
%> <input name="weight" tabindex="1" type="text" class="auto" size="5" 	readonly="readonly" value="<%=opdPatientDetails.getVweight()%>" /> <label class="small">kg</label> 
<%
 	} else {
 %> <input name="weight" tabindex="1" type="text"	class="auto" size="5"  readonly="readonly" value="" /> <label class="small">kg</label> 
<%
 	}
 %> 
...
<%
 ...
 <label  class="medium">Temp</label>
<%
	if (opdPatientDetails.getTemperature() != null) {
%> 
...
%> 
 ...
 %>  <input	name="bp" id="bp" type="text" tabindex="1" class="auto" size="5" readonly="readonly" value="<%=opdPatientDetails.getBp()%>"/> <label	class="small">mm/hg</label>
<%
	} else {
%>  <input	name="bp" id="bp" type="text" tabindex="1" class="auto" size="5"  readonly="readonly" value=""/> <label	class="small">mm/hg</label>
<%
	}
%> 
 ...
 <%
 	if (opdPatientDetails.getRr() != null) {
 %><input	name="rr" id="rr" type="text" tabindex="1" class="small" readonly="readonly" value="<%=opdPatientDetails.getRr()%>" /> <label	class="smallAuto">/min</label>
<%
	} else {
%> <input	name="rr" id="rr" type="text" tabindex="1" class="small" readonly="readonly" value="" /> <label	class="smallAuto">/min</label>
<%
	}
%> 
 ...
 <%
	  	if (opdPatientDetails.getSystamicExam() != null) {
...
<table border="0" align="center" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<script type="text/javascript" language="javascript" charset="utf-8">
			  new Ajax.Autocompleter(document.getElementById('chargeCodeName<%=inc%>'),'ac2update2','fwc?method=getInvestigationListForAutoComplete',{parameters:'requiredField=chargeCodeName1'});
			</script></td>
...
<label>Additional Advice</label> <%
 	if (opdPatientHistory.getPresentAdvice() != null) {
 %>
...
<div class="clear"></div>
<label>Refered To MH</label>
<%
System.out.println("MH-->"+opdPatientDetails.getMhRun());
	if(opdPatientDetails.getMhRun()!=null)
	{
...
<%} %>
<div class="clear"></div>
<label>Disposal</label> <select name="disposal" id="disposal" size="0" disabled="disabled" class="med">
	...
	<option>LD</option>
    	...
    	<%}%>
</script> <label>Days</label> 
<%
 	if (opdPatientDetails.getDays() != null) {
 %>

<input name="days" type="text" class="auto" size="48" value="<%=opdPatientDetails.getDays()%>" readonly="readonly" tabindex="1" maxlength="2" />
...
<input type="hidden"	name="hinNoForReport" id="hinNoForReport" value="<%=visit.getHin().getHinNo()%>" />
 --%>
<input type="button" name="printReport" id="print" 	onclick="submitFormForPrescriptionReportNew();" value="Case Sheet"	class="buttonBig" accesskey="a" /> 
<%--
...
function goBack(){
	...
	document.ViewScreen.action="fwc?method=showPatientPreviousVisitForViewScreen&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&deptId=<%=visit.getDepartment().getId()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>&token=<%=token%>";
  document.ViewScreen.submit();

} 
function showPreviousVisit(visitNo){
	if(visitNo >=1){
		document.ViewScreen.action="/hms/hms/fwc?method=getPatientFwcDetails&flag=prev&hinId=<%=visit.getHin().getId()%>&deptId=<%=visit.getDepartment().getId()%>&visitNumber=<%=visit.getVisitNo()%>&token=<%=token%>";
		  document.ViewScreen.submit();
	}else{
...
document.ViewScreen.action="/hms/hms/fwc?method=getPatientFwcDetails&flag=next&hinId=<%=visit.getHin().getId()%>&deptId=<%=visit.getDepartment().getId()%>&visitNumber=<%=visit.getVisitNo()%>&token=<%=token%>";
document.ViewScreen.submit();

}
function openPopupProcedureAdviceWindow(visitId,hinId,doctorId)
{
 var url="/hms/hms/fwc?method=showProcedureListJsp&visitId="+visitId+"&hinId="+hinId+"&doctorId="+doctorId+"&flag=";
...
{
 var url="/hms/hms/fwc?method=showDetentionListJsp&visitId="+visitId+"&hinId="+hinId+"&doctorId="+doctorId+"&flag=";
 newwindow=window.open(url,'name',"left=2,top=100,height=300,width=1010,status=1,scrollbars=1,resizable=0");
...
function submitFormForPrescriptionReportNew(){
	submitForm('ViewScreen','/hms/hms/fwc?method=opdMedicalCaseSheetPrint');
}
function submitFormForPrescriptionReport(serviceNo,visitNo,hinNo){
	//submitForm('ViewScreen','/hms/hms/fwc?method=opdMedicalCaseSheetPrint');
	
	var url="/hms/hms/fwc?method=opdMedicalCaseSheetPrint&serviceNoForReport="+serviceNo+"&visitNumberForReport="+visitNo+"&hinNoForReport="+hinNo;
	 newwindow=window.open(url,'name',"left=2,top=100,height=300,width=1010,status=1,scrollbars=1,resizable=0");
		 
	//document.ViewScreen.action="/hms/hms/fwc?method=opdMedicalCaseSheetPrint";
	//document.ViewScreen.submit();
}
