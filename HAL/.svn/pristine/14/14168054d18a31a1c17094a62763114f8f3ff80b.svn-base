...
<script type="text/javascript">
  ...
  obj.action = "/hms/hms/stores?method=dailyIssueSummryReportDayWise";
  obj.submit();
  }
</script>
...
<div class="titleBg">
<h2>Daily FAC Issue Report Division Wise</h2>
</div>
<div class="Block">
<label class="medium">Issue Date<span>*</span></label> 
...
<label class="medium">Division<span>*</span></label> 
<select id="divisionId" name="divisionId" validate="Division,string,yes">
...
<input type="text" name="<%=TO_DATE%>" value="<%=currentDate %>"  MAXLENGTH="10" class="calDate" placeholder="DD/MM/YYYY" validate="To Date,date,yes" onkeyup="mask(this.value,this,'2,5','/');" />
 --%>
 <%-- 
<label class="medium">Pharmacist<span></span></label> 
<select	name="pharmacistId" id="pharmacistId" tabindex=1>
	<option value="0">Select</option>
	<%
					for (Users user :pharmacistList ) {
					String name="",m_name="",l_name="";
			         ...
			         if(user.getEmployee().getMiddleName()!=null)
			        	 name=name+" "+user.getEmployee().getMiddleName();
...
<input type="button" name="add1" id="addbutton1" value="Print" class="button" onClick="submitForm('dailyIssueSummry','stores?method=generateDailyIssueSummryDayWise');" accesskey="a" tabindex=1 />  
