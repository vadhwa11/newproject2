...
<label>Patient Status Name</label>  
...
<input type="button" name="search" value="Search" class="button" onclick="submitForm('search','hospital?method=searchPatientStatus','checkSearch')"
	tabindex=1 /> 
	<%--- Report Button   --%> <input type="button"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_patient_status"></div></form>
</div>
<div class="Clear"></div>
</div>
<div class="Clear"></div>
...
<% 
  if(searchPatientStatusList.size()>0)
   ...
   {
 %> 
<h4><a href="hospital?method=showPatientStatusJsp">Show All Records</a></h4> <%
   }
   }
...
<form name="patientStatus" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasPatientStatus">
<input type="hidden" name="<%=POJO_PROPERTY_NAME %>"
	value="PatientStatusName"> <input type="hidden"
	name="<%=JSP_NAME %>" value="patientStatus"> <input
...
<input id="codeId" type="text"	name="<%= CODE%>" value="" validate="PatientStatus Code,string,yes" 	 MAXLENGTH="8" / tabindex=1>
 <label class="noWidth">Patient Status Name <span>*</span></label> <input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="PatientStatus Name,string,yes" 
	MAXLENGTH="30" / tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addPatientStatus')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masPatientStatus.getLastChgBy() %>"
