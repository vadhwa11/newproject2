...
if(patientList.size()>0)
{
		...
		if(patientList.get(0).getUnit().getUnitName()!= null)
		{
...
<%
		...
		{
			int hinIdRider=patient.getId();
			%>
				<input type="hidden" name="hinIdRider" value=<%=hinIdRider%>  />
		  <%} %>
<div id="DivPillion">
<!--<label>Service No. </label>
<input type="text" name="serviceNoRider"  validate="Service No,alphanumeric,yes"  id="serviceNoRider" value="<%=serviceNo %>"
    onblur="getPatientDetailsRider(this.value);"  />
