...
<h6>Cancellation of Allotment Order for Officer</h6>
<form name="relegationProcess" method="post" action="">
<div class="Clear"></div>


<div class="blockTitle">Cancellation of Allotment Order for
Officer</div>
...
<label><span>*</span> Cancellation No.</label> <label
	class="valueNoWidth"><%=cancelNo %></label> <label><span>*</span>
Cancellation Date</label> <label class="valueNoWidth"><%=date%></label>
...
<table border="0" cellspacing="0" cellpadding="0">
   ...
   %>
	<tr>
		...
		<td><label name="<%=SMQ_NAME %>" id="<%=SMQ_NAME %>"><%= cancel.getSmq().getSmqName() %></label>
		<input type="hidden" name="<%=SMQ_ID %>" id="<%=SMQ_ID %>"
			value="<%=cancel.getSmq().getId() %>" /></td>

		<td>
		<%if(cancel.getAllotmentType().equals("1")){ %> <label
			name="<%=TYPE_OF_ALLOTMENT %>" id="<%=TYPE_OF_ALLOTMENT %>">Permanent</label>
		<%}else if(cancel.getAllotmentType().equals("2")){ %> <label
			name="<%=TYPE_OF_ALLOTMENT %>" id="<%=TYPE_OF_ALLOTMENT %>">Temporary</label>
		<%}else if(cancel.getAllotmentType().equals("3")){ %> <label
			name="<%=TYPE_OF_ALLOTMENT %>" id="<%=TYPE_OF_ALLOTMENT %>">Reimbursed</label>
		<%}else if(cancel.getAllotmentType().equals("4")){ %> <label
			name="<%=TYPE_OF_ALLOTMENT %>" id="<%=TYPE_OF_ALLOTMENT %>">MEO</label>
...
<div class="blockFrame"><input type="hidden" name="counter"
	id="counter" value="<%=inc %>" /> <label class="common">Remarks</label>
<textarea value="" class="large" name="<%=REMARKS %>" id="<%=REMARKS %>"
...
<div class="bottom"><input type="button" class="button"
	value="Submit"
	onclick="if(checkSubmit()){submitForm('relegationProcess','accom?method=submitCancellationForOfficer');}"
	...
	onclick="submitFormForButton('relegationProcess','accom?method=cancelForOfficer')"
	tabindex="1" />
<div class="division"></div>
