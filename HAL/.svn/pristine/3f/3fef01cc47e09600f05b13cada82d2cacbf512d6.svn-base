...
<input type="radio" class="radioAuto"
	...
	<label>CT Code</label> <input type="radio"
	name="<%=SELECTED_RADIO %>" value="2" class="radioAuto" /> <label>CT
Name</label> <input type="text" id="searchField" name="<%= SEARCH_FIELD%>"
	value="" validate="CT Code,string,no" MAXLENGTH="8" tabindex=1 /> <input
	type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchCaseType','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_case_type"></div></form>
</div>
</div>
...
<form name="caseType" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasCaseType"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="CaseTypeName">
<input type="hidden" name="title" value="CaseType"> <input
	type="hidden" name="<%=JSP_NAME %>" value="caseType"> <input
	type="hidden" name="pojoPropertyCode" value="CaseTypeCode"> <br>


<div class="Block"><label> CT Code <span>*</span></label> <input id="codeId" type="text"
	name="<%= CODE%>" value="" validate="CT Code,string,yes"
	class="textbox_size20" MAXLENGTH="8" / tabindex=1> <label
	> CT Name <span>*</span></label> <input
	type="text" name="<%= SEARCH_NAME %>" value=""
	validate="CT Name,string,yes" class="textbox_size20" MAXLENGTH="30"
	/ tabindex=1> <script>
...
data_arr[<%= counter%>][4] = "<%= HMSUtil.convertDateToStringWithoutTime(masCaseType.getLastChgDate()) %>"
