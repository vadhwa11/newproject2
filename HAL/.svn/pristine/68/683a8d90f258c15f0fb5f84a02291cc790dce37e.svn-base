...
<div class="titleBg"><h2>Acknowledgement Of MR</h2></div>
<% }else{%>
<div class="titleBg"><h2>Acknowledgement Of MR</h2></div>
<% }%>
...
<select id="issueNo"
	...
	<option value="<%=storeIssueM.getId()%>"
		<%=HMSUtil.isSelected(storeIssueM.getId().toString(),box.getString(ISSUE_NO)) %>><%=storeIssueM.getRequestNo().getDemandNo()%></option>
	     <% }
					//}
				//}
		%>
...
<input type="button" name="add" id="addbutton" value="Add Stock"
...
<script language="Javascript">
	...
	function ackconfirm()
	{
	  ...
	  {
	      submitForm('ack','stores?method=addAck');
			return true;
	   }else{
		  return false;
       }
	...
	function getIssue(form)
	{
		...
		submitForm(form,'stores?method=createGridIssueData&<%=ISSUE_NO%>='+issueNo+"&currPage=1");
	}
