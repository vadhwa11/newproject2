...
<form name="departmentWiseSummary" target="_blank" action=""
	method="post">
<div class="blockFrame"><label><span>*</span>From Date</label> <input
	...
	onClick="setdate('<%=currentDate %>',document.departmentWiseSummary.<%=FROM_DATE%>,event)" />
...
<label><span>*</span>To Date</label> <input type="text" class="calDate"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.departmentWiseSummary.<%=TO_DATE%>,event)" />

<label>Collection Center</label> <select id="collectionCenterId"
	name="<%=COLLECTION_CENTER_ID %>">
	<option value="0">Select</option>
	<%
							for (DgCollectionCenter collectionCenter : collectionCenterList) {
						%>
	<option value="<%=collectionCenter.getId() %>"><%=collectionCenter.getCollectionCenterName()%></option>
	<%
						}
					%>
</select></div>
...
<div class="bottom"><input type="button" name="OK" value="OK"
	class="button"
	onClick="submitForm('departmentWiseSummary','lab?method=generateDepartmentWiseSummary','validateFromToDate');" />
...
function validateFromToDate(){
	...
	obj1 = eval(document.departmentWiseSummary.fromDate)
	obj2 = eval(document.departmentWiseSummary.toDate)
