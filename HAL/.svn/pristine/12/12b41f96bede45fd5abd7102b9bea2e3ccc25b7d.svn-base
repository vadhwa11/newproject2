...
<label>Allot.Order File No</label> <input type="text"
	...
	onkeypress="return submitenter(this,event,'accom?method=searchCancelForOfficer')"
	tabindex="1" /> <script>
			document.smqSearch.<%=ALLOTMENT_NO%>.focus();
			</script> <label>Service No</label></label> <input type="text" name="<%=SERVICE_NO %>"
	value="" MAXLENGTH="15"
	onkeypress="return submitenter(this,event,'accom?method=searchCancelForOfficer')"
	tabindex="1" /> <label>Service Person Name</label> <input type="text"
...
<input type="button" name="submit" id="addbutton"
	onclick="submitForm('smqSearch','/hms/hms/accom?method=searchCancelForOfficer','checkFromNTodata');"
...
<form name="cancelOfficer" method="post" action=""><script
