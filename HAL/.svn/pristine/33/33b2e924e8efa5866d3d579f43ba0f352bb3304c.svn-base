...
<%
	...
	List<StoreGrnReturnT> storeReturnTList= new ArrayList();
	try {
		if(map.get("storeReturnTList")!=null){
			storeReturnTList=(List<StoreGrnReturnT>)map.get("storeReturnTList");
		}
	} catch (Exception exp) {
			exp.printStackTrace();
	}
		...
		<%
		    ...
		    int  i=0;
	          while(iterator.hasNext()){           
	        	  StoreGrnReturnT tObj = (StoreGrnReturnT) iterator.next();
	        	  
	        	  StoreGrnReturnM mObj = (StoreGrnReturnM)tObj.getGrnReturn();
	        	  ...
	        	  Date returnDate = tObj.getGrnReturn().getReturnDate();
	        	  String returnDateInString = HMSUtil.changeDateToddMMyyyy(returnDate);
	        	 BigDecimal costPrice=new BigDecimal(""+tObj.getUnitRate());
...
function checkForDelete(formName){
	...
	if(document.getElementById('parent').checked){
		obj.action = "/hms/hms/stores?method=deleteStockDetailsVendorReturn";
		obj.submit();
		return true;
	}else{
