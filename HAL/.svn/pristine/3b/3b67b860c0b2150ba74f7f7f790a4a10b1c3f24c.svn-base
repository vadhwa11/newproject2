...
if(map.get("bookingList") != null){
	patientDetailList=(List)map.get("bookingList");
	...
	if(patientDetailList!=null && patientDetailList.size()>0){
	    ...
	    if(otBooking.getOtBookingDt()!=null)
	    {
			...
			for(OtBookingDt otDt :OtBookingDtSet){
					if(otDt.getStatus() !=null && !otDt.getStatus().equalsIgnoreCase("n") && !otDt.getStatus().equalsIgnoreCase("c"))
					{
						if(count++ >1)
						procedure += " | ";
...
<div class="titleBg">
<h2>Post Anesthesia Procedure Note</h2>
</div>
<div class="clear"></div>
<%-- <div class="Block">
...
<!--Block One Starts-->
	...
	<div class="Block">
		...
		<%-- <label>Requisition No.</label> 
          <input type="text" readonly="readonly" value="<%=otBooking.getOpdSurseryHeader().getOrderNo()%>" />
...
<input type="hidden" name="<%=RequestConstants.VISIT_ID %>" id="visitId" value="<%=otBooking.getVisit()!=null?otBooking.getVisit().getId():"0" %>" />
<input type="hidden" name="<%=RequestConstants.INPATIENT_ID %>"  value="<%=otBooking.getInpatient()!=null?otBooking.getInpatient().getId():"0" %>"  />
 <%	if (otBooking.getVisit() != null) {%>
 <input name="visitId" type="hidden" value="<%=opdSurgeryHeader.getVisit()!=null?opdSurgeryHeader.getVisit().getId():"0"%>" /> <%	} else if (otBooking.getInpatient() != null) {
 %> <input name="inpatientId" type="hidden" value="<%=otBooking.getInpatient()!=null?otBooking.getInpatient().getId():"0"%>" /> <%
 	} %>
...
<%-- <label>Surgery Date</label> <input type="text" readonly="readonly" value="<%=otBooking.getOpdSurseryHeader()!=null?HMSUtil.convertDateToStringWithoutTime(otBooking.getSurgeryDate()):"" %>" /><label>Requisition No.</label> 
...
<div class="popupTextDiv">Current OPD/IPD/OT History</div>	 --%>
       <div class="floatRight"> <a href="#" onclick="getTodayAllPrescriptionPopup('<%=hinId%>','y','OT');">Current Medication</a>
...
<input type="button" class="button" tabindex="3" name=""
value="+" onclick="openWindow('/hms/hms/opd?method=viewOtPostOperative&remarks=<%=otBooking.getAdditionalNotes()!=null?otBooking.getAdditionalNotes():""%>')" />
<div class="popupTextDiv">Surgery Notes</div>

<%} %>

<div class="clear"></div>
<label>Overall assessment/GC</label>
 <textarea name="overall_assessment" cols="0" rows="0" class="large"	id="overall_assessment" 
	maxlength="50"></textarea>
<div class="clear"></div>

<label >Airway</label>
<textarea name="airway" cols="0" rows="0" class="large" maxlength="50"  ></textarea><div class="clear"></div> 

<label>Level of Consciousness/GCS</label> <input name="gcs" type="text" maxlength="25"  />
<label>Drowsy </label> <input name="drowsy" type="text" maxlength="25"  />
<label>Awake</label> <input name="awake" type="text" maxlength="25"  /><div class="clear"></div>
<label>Circulation</label> <input name="circulation" type="text" maxlength="25"  />
<label>HR</label> <input name="hr" type="text" maxlength="25"  />
<label>BP</label> <input name="bp" type="text" maxlength="25"  /><div class="clear"></div>
<label>CVS</label> <input name="cvs" type="text" maxlength="25"  />
<label>RS</label> <input name="rs" type="text" maxlength="25"  />
<label>Spo2 on room Level</label> <input name="spo" type="text" maxlength="25"  /><div class="clear"></div>
<label >Special Observation</label><textarea name="special_observation" cols="0" rows="0" class="large" maxlength="50"  ></textarea><div class="clear"></div> 
...
<table border="0" align="center" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<th scope="col">Post OP Instruction</th>
		<th>Add</th>
		<th>Delete</th>
	</tr>
	<%int inc=1;
    ...
    {
    	%>
    	<input type="text" name="" id="" value="<%=opInstruction.getInstruction()%>" size="150" readonly="readonly">
    	<%
    }
	       %>	      
	      </td>
...
<%-- <table border="0" align="center" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<%int inc=1; %> <input type="text" value="" tabindex="1"
			id="opInstruction1" size="43" name="opInstruction1" maxlength="70"
			 onblur="fillMemberDetails(this.value,<%=inc%>)"/>
		<td>
		  	<input type="button"
			class="button" alt="Add" value="Add" onclick="addRowOpInstruction();"
		...
		<input type="hidden" value="1" name="opHiddenValue" id="opHiddenValue" />
</table> --%>

<div class="clear"></div>
<h4>Post Medication</h4><div class="clear"></div>
...
<div class="paddingTop15"></div>
<div class="Block"><label>Post additional Advice</label> 
<textarea name="remarks"  id="remarks" cols="0" rows="0" class="large" 
...
<input type="button" name="Submit" class="button" value="Submit"
	onclick="if(checkSubmitNomenclature()){submitForm ('otBooking','ot?method=submitOtPostAnesthesiaProcedure');}" />
	
<input name="back" type="button" class="button"	value="Back" onclick="submitForm ('otBooking','ot?method=showOtPostAnaesthesiaPatientSearchJsp')" />
