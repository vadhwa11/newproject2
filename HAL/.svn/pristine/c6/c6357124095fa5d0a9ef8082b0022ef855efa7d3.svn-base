...
<%
	...
	if(map.get("pensionForm8EntryList") != null){
		pensionForm8EntryList=(List)map.get("pensionForm8EntryList");
	}
	
	MasPersonnelDetails masPersonnelDetails=masPersonnelDetailsList.get(0);	
	PensionForm8Entry pensionForm8Entry=pensionForm8EntryList.get(0);
...
<form name="updateForm8Entry" method="post">
<h6>Update Form 8 Entry</h6>
<div class="Clear"></div>
...
<label>File No. </label> <input name="fileNo" id="fileNo"
	value="<%=pensionForm8Entry.getFileNo() %>" type="text" maxlength="15" />

<label>Date</label> <%if(pensionForm8Entry.getDate()!= null && !pensionForm8Entry.getDate().equals("")){ %>
<input type="text" name="date"
	value="<%=HMSUtil.changeDateToddMMyyyy(pensionForm8Entry.getDate()) %>"
	id="date" /> <%}else{ %> <input type="text" name="date" value=""
	id="date" /> <%} %> <img src="/hms/jsp/images/cal.gif" width="16"
	height="16" border="0" validate="Pick a date"
	onClick="setdate('',document.updateForm8Entry.date,event)" />
<div class="Clear"></div>
<div class="division"></div>
...
house building or conveyance advance</label> <input type="text" id="balance"
	name="balance" value="<%=pensionForm8Entry.getBalanceOfHouse()%>"
	maxlength="7" validate="Balance,num,no"
...
payment of pay and allowance including leave salary</label> <input type="text"
	id="overPayment" name="overPayment"
	value="<%=pensionForm8Entry.getOverPayment() %>" maxlength="7"
...
deductible at source under the Income Tax act[1961](43 of 1961)</label> <input
	type="text" id="taxDeductible" name="taxDeductible"
	value="<%=pensionForm8Entry.getIncomeTaxDeductible() %>" maxlength="7"
...
licence fee for occupation of govt.accomodation</label> <input type="text"
	id="arrears" name="arrears" maxlength="7"
	value="<%=pensionForm8Entry.getArrearsOfLicenceFee() %>"
...
permissible period of two month beyond the date of retirement</label> <input
	...
	value="<%=pensionForm8Entry.getAmountOfLicenceFee() %>"
	validate="Amount Of Licence Fee,num,no"
	onblur="calculateTotalForForm8()" maxlength="7" />
<div class="Clear"></div>
...
assessed dues and the nature there of</label> <input type="text" id="otherDues"
	name="otherDues" validate="Other Assessed Dues,num,no"
	value="<%=pensionForm8Entry.getOtherAssessedDues() %>" maxlength="7"
	onblur="calculateTotalForForm8()" />
...
gratuity to be with held for the adjustment of un-assessed dues if any</label> <input
	...
	maxlength="7" value="<%=pensionForm8Entry.getAmountOfGratuity() %>"
	validate="Amount Of Gratuity,num,no" onblur="calculateTotalForForm8()" />
...
<label class="small"></lablel><label class="large3">Total</label> <input
	...
	value="<%=pensionForm8Entry.getTotal() %>" readonly /></div>

<div class="bottom">
<div class="division"></div>
<input name="personnelId" type="hidden"
	value="<%=masPersonnelDetails.getId() %>" /> <input name="form8Id"
	type="hidden" value="<%=pensionForm8Entry.getId() %>" /> <input
	...
	onclick="submitForm('updateForm8Entry','pension?method=updateForm8Entry')" />
<input name="Button" type="button" class="button" value="Back"
	onclick="submitForm('updateForm8Entry','pension?method=showUpdatePersonnelSearchForForm8Jsp')" />
<div class="division"></div>
