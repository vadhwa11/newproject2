...
<input	type="text" name="Prepared"  tabindex=1  id="Prepared" value="<% out.print(ApprovalRemarks);%>" readonly/> --%>
	...
	if(Status.equalsIgnoreCase("O") || Status.equalsIgnoreCase("U"))
	{
		%>
			...
			<input class="buttonDel" type="button" tabindex="1"  onclick="removeRow_FromDatabaseRC('MRGrid','RcRequestDetails',<%out.print(requestHeaderId); %>);" value=""><img id='imgRemoveDetails' src='/hms/jsp/images/saving.gif' style="display:none;" />
		<%
	}
...
<table id="MRGrid">
 	...
 	int counter =1;
		...
		<td id="SRNO"><input size="2" type="text" id="tdSRNO" value="<%out.print(counter); %>" readonly /></td>
      ...
      <td> <input  size="15" type="text" onchange ="if(isNumber(event)){checkRC(<%out.print(detailsList.getId()); %>)}"   id="txtRequiredQty<%out.print(detailsList.getId()); %>" name="txtRequiredQty<%out.print(detailsList.getId()); %>"  validate="Required Qty,string,yes" value="<%out.print(detailsList.getReqQty()); %>" />
      ...
      <td> <input  size="15" type="text"     id="txtWPStock<%out.print(detailsList.getId()); %>" name="txtWPStock<%out.print(detailsList.getId()); %>" value="<%out.print(detailsList.getWpStock()); %>"  readonly /></td>
       <td> <input  size="15" type="text"  id="txtCDStock<%out.print(detailsList.getId()); %>" name="txtCDStock<%out.print(detailsList.getId()); %>" value="<%out.print(detailsList.getCdStock()); %>" readonly /></td>
        ...
        <td> <textarea size ="20" class="large"  id="from<%out.print(detailsList.getId()); %>" class="small"  name="from<%out.print(detailsList.getId()); %>"  validate="From,string,no" value=""   tabindex=1 style="width: 250px; height: 50px;" ><%out.print(detailsList.getFromDepartments()!=null?detailsList.getFromDepartments():""); %></textarea></td>
...
<div class="clear"></div>
	...
	{
		%>
			<input type="button" name="Save"  class="button" value="Save" onclick="saveMR('MR','stores?method=updateSavedRC&requestHeaderId=<%out.print(requestHeaderId); %>&appId=A1523');"/>
			<input type="button" name="Submit"  class="button" value="Submit" onclick="submitMR('MR','stores?method=updateSavedRC&requestHeaderId=<%out.print(requestHeaderId); %>&appId=A1523');"/>
		<%
	}
	
	%>
	<input type="button" name="add" id="addbutton" value="Print" class="button" onClick="submitForm('MR','/hms/hms/stores?method=generateRCReport&requestHeaderId=<%=requestHeaderId %>');"  tabindex=1 />
	...
	<div class="division"></div>
		...
		function addRow(tableId)
		{
			...
			var SRNO = $j("tr:last").find("#tdSRNO").val();
			SRNO = SRNO*1;
			SRNO=SRNO+1;
			...
			var tableHtml ="";
			  ...
			  tableHtml= tableHtml +("<td><input size='2' id='tdSRNO' type=\"text\" value="+SRNO+" readonly /></td><td><input type='text' name='itemCode"+i+"' id='itemCode"+i+"' validate='Mat Code,String,no' size='12'/><input type='hidden' name='ItemId"+i+"'  id='ItemId"+i+"' value=''/></td>");
			  ...
			  tableHtml= tableHtml +("<td> <input  type=\"text\"  size=\"15\" class='medium3' id='txtRequiredQty"+i+"' name='txtRequiredQty"+i+"'  validate=\"Required Qty,TwoDigitAfterDecimal,yes\"  maxlength='10' onChange=\"if(isNumber(event)){checkRC("+i+")}\" /></td>");
