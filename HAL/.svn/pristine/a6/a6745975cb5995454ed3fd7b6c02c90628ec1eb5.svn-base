...
<br>
<br>
<%for(StoreIndentT storeIndentT :storeIndentTList ){ %> <label
	class="bodytextB_blue"><font id="error"></font>Unit Rate</label> <input
	type="text" name="<%=RequestConstants.UNIT_RATE %>"
...
<input type="text" name="<%=RequestConstants.TOTAL_COST%>"
	...
	class="textbox_size20" MAXLENGTH="8" / validate="Total Cost,num,yes">
...
Receipt Date</label> <%
	...
	tabindex=1 validate="Last Receipt Date,date,yes" /> <img
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date" class="calender" tabindex="1"
	onClick="setdate('<%=currentDate%>',document.windowForm.<%= RequestConstants.LAST_RECP_DATE%>,event)" />
...
<input type="button" name="add" id="addbutton" value="Submit"
	...
	accesskey="a" tabindex=1 /> <input type="hidden"
	name="<%=RequestConstants.STATUS %>" value="" /> <input type="hidden"
	name="" value="" /></form>
</div>
