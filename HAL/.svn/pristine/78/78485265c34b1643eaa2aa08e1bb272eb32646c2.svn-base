...
<%--
<div>
<input	name="investigationTemplate" type="button"	value="Previous Visits" tabindex="1" class="buttonBig2" onClick="submitForm('mbAppAuthForm16','opd?method=showPatientPreviousVisitForViewScreen&link=medicalExam&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>&token=<%=visit.getTokenNo()%>');"/>
<input	name="investigationTemplate" type="button"	value="Previous Medical Exams" tabindex="1" class="buttonBig2" onClick="submitForm('mbAppAuthForm16','medicalExam?method=getPrevMedExamFromHIC&serviceNo=<%=visit.getHin().getServiceNo() %>');" />
<input	name="investigationTemplate" type="button"	value="Previous Medical Boards" tabindex="1" class="buttonBig2" onClick="submitForm('mbAppAuthForm16','medicalExam?method=getPrevMedBoardFromHIC&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>&serviceNo=<%=visit.getHin().getServiceNo() %>');" />
<input	name="investigationTemplate" type="button"	value="Previous Hospitalizations" tabindex="1" class="buttonBig2" onClick="submitForm('mbAppAuthForm16','opd?method=showPatientPreviousVisitForHospitality&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>');" />
</div> --%>
<div>
...
<div class="clear"></div>
<form name="mbAppAuthForm16" action="" method="post">
<!--Block One Starts-->
<%
int medExamId = 0;
int visitId=0;
...
<label>Place </label>
<input tabindex="1" class="auto" size="50" type="text" maxlength="20"  id="" name="<%=PLACE %>"  value="<%=((String)session.getAttribute("hospitalName")) %>"
	onKeyUp="limitText(this,20);" readonly="readonly" />
<%}%>
<div class="clear"></div>
<label>Date Of Discharge</label>
<%if(medExamObj.getDateOfDischarge() !=null){ %>
<input tabindex="1" type="text"	name="dateOfDischarge" class="" maxlength="10"  class="auto" size="27"
...
<%if(medExamObj.getId()==null)
{%>
	 <input tabindex="1" type="text"	readonly="readonly"  name="<%=REPORTED_DATE %>" class="" saxlength="10"  class="auto"
		onKeyUp="mask(this.value,this,'2,5','/');" value="<%=date %>"
		validate="Reported Date,date,no" />
<% }else if(medExamObj.getDateOfReporting() !=null){%>
	 <input tabindex="1" type="text"	readonly="readonly"  name="<%=REPORTED_DATE %>" class="" maxlength="10" disabled="disabled"
		onKeyUp="mask(this.value,this,'2,5','/');" value="<%=HMSUtil.convertDateToStringWithoutTime(medExamObj.getDateOfReporting()) %>"
		validate="Reported Date,date,no" />

<% }else{%>
 <input tabindex="1" type="text"  readonly="readonly"	name="<%=REPORTED_DATE %>" class="" maxlength="10"  class="auto"
		onKeyUp="mask(this.value,this,'2,5','/');" value="<%=date %>"
		validate="Reported Date,date,no" />
<%} %>
...
<td width="10%"><input tabindex="1" type="text"	name="<%=P_NO+inc1 %>" readonly="readonly"  maxlength="10" value="<%=setMedicalExam.getPno() %>"/></td>
<% }else{%>
...
<%
	...
	//mbAppAuthForm16.out.println("this is else jsp");
inc1=1;%>
<tr>
...
<TH>Where Treated</TH>
<TH colspan="2" >Approximate Date and Periods Treated</TH>
...
for(MasMedicalExaminationDetail setMedicalExam:medExamObj.getMasmedicaldetail()){
	...
	if(setMedicalExam.getParticular()!=null && setMedicalExam.getParticular().equalsIgnoreCase("particular")
			&& setMedicalExam.getBeforeDisability().equalsIgnoreCase("n")){
		inc11=inc11+1;
	%>
...
<td width="10%"><input tabindex="1" size="2" type="text" readonly="readonly" name="<%=SIRIAL_NO1+inc11 %>" maxlength="3" /></td>
 ...
 %>
<td width="10%"><input tabindex="1" type="text"	readonly="readonly"  name="<%=ILLNESS+inc11 %>" maxlength="10" value="<%=disabilityStr %>"/></td>
  <% }else{%>
...
<input type="text"	readonly="readonly" tabindex="1" name="<%=APPROXIMATE_DATE+inc11 %>" id="approximatedate" size="11" value="<%=HMSUtil.convertDateToStringWithoutTime(setMedicalExam.getApproximatedate1())%>"	validate="DOB,String,no" maxlength="30"
	 />
  <% }else{%>
...
<input type="text"	readonly="readonly" tabindex="1" name="<%=APPROXIMATE_DATE2+inc11 %>" id="approximatedate2" size="11" value="<%=HMSUtil.convertDateToStringWithoutTime(setMedicalExam.getApproximatedate2())%>"	validate="DOB,String,no" maxlength="30"
	 />
  <% }else{%>
...
<%if(setMedicalExam.getTreated()!=null){ %>
<td width="10%">
<input tabindex="1" type="text" readonly="readonly" name="<%=TREATED%>1<%=incBefore %>" maxlength="10" value="<%=setMedicalExam.getTreated() %>"/></td>
<% }else{%>
<td width="10%">
<input tabindex="1" type="text"	readonly="readonly" name="<%=TREATED%>1<%=incBefore %>" maxlength="10" /></td>
<% }%>
</TR>
...
<td width="10%"><input tabindex="1" type="text"	readonly="readonly"  name="<%=ILLNESS%>1<%=incBefore %>" maxlength="10" /></td>
<td width="10%">
<input	tabindex="1" name="<%=PARTICULAR_DATE%>1<%=incBefore %>" size="11" 	maxlength="10" id="particulardate1<%=incBefore %>" value="<%=date %>"
	onKeyUp="mask(this.value,this,'2,5','/');" validate="First Started on,date,no"/>
...
<input tabindex="1" name="Button" type="button" class="buttonBig" value="Commanding Officer" onClick="openPopupForCommandingOfficer(<%=medExamId %>)" />
...
<div class="Block">
<label>Remarks</label>
<input type="text" name="<%=AUTH_REMARKS %>" maxlength="99" id="authRemarks" validate="Remarks,mtachar,no"/>

<label>Signed By</label>
<input type="text" name="<%=SIGNED_BY %>" value="<%=signedBy %>" readonly="readonly"validate="Signed By,mtachar,no"/>
...
<input tabindex="1" name="Button" type="button" class="button" value="Validate" onClick="submitForm('mbAppAuthForm16','medicalBoard?method=validateAppAuthForm16&data=validate');"/>
...
<input type="button" name="viewCaseSheet" value="Medical Case Sheet" class="buttonBig2" onclick="submitForm('mbAppAuthForm16','/hms/hms/medicalBoard?method=printMedicalCaseSheet')" />
...
function checkReject()
{
	...
	if(authRemarks!=""){
		submitForm('mbAppAuthForm16','medicalBoard?method=rejectMBForm16Entry&rejectStatus=mr');
	}else{
...
function submitForInvestigationPrintOut()
{
	...
	if(orderNo!=null && orderNo!="" && hinId!=null && hinId!=""){
		submitFormForButton('mbAppAuthForm16','/hms/hms/investigation?method=printResultValidationLab&parent='+orderNo+'&hinId='+hinId+'&flag='+flag);
		}
}
