...
%>
<div class="titleBg">
<h2>ICD MainCategory Master</h2></div>
<div class="Clear"></div>
<div id="searcharea">
<div id="searchbar">
<form name="search" method="post" action="">
<div class="Block">
<label>ICD Main Category Code</label><input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /> <label>ICD Main Category Name</label> <input
	type="radio" class="radioAuto" name="<%=SELECTED_RADIO %>" value="2" />  <input
	type="text" id="searchField" name="<%= SEARCH_FIELD%>" value=""
	validate="ICD main Category Code,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=searchIcdMainCategory')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchIcdMainCategory','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_icd_main_category"></div></form>
</div>
<div class="Clear"></div>
</div>
<div class="Clear"></div>
...
<h4><a href="hospital?method=showIcdMainCategoryJsp">Show All Records</a></h4> <%
			}
		 }
...
<form name="icdMainCategory" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasIcdMainCategory">
<input type="hidden" name="<%=POJO_PROPERTY_NAME %>"
	value="IcdMaincategoryName"> <input type="hidden" name="title"
	value="IcdMainCategory"> <input type="hidden"
	name="<%=JSP_NAME %>" value="icdMainCategory"> <input
...
<div class="Clear"></div>
<div class="Block">
<label> ICD Main Category Code <span>*</span></label> <input id="codeId"
	type="text" name="<%= CODE%>" value=""
	validate="ICD MainCategory Code,string,yes" 
	MAXLENGTH="8" / tabindex=1> <label> ICD Main Category Name<span>*</span></label> <input type="text"
	name="<%= SEARCH_NAME %>" value=""
	validate="ICD MainCategory Name,string,yes" 
	MAXLENGTH="30" / tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addIcdMainCategory')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masIcdMainCategory.getLastChgBy() %>"
