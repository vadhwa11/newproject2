...
<label><span>*</span> Pool Code</label> <select id="<%=POOL_ID %>"
	...
	onblur="submitForm('allotAirmen','accom?method=getRecordsForAirMenVacation');"
	onchange="submitForm('allotAirmen','accom?method=getRecordsForAirMenVacation');">
...
<table border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<input type="hidden" name="<%=REGISTRATION_STATUS %>"
			...
			value="<%=accomAllot.getSmq().getId() %>" name="<%=SMQ_ID %>"
			id="<%=SMQ_ID %><%=inc %>" /></td>
		...
		<%}else{ %> <label id="<%=SMQ_NAME%><%=inc%>" size=8
			name="<%=SMQ_NAME %>">-</label> <%}%>
		</td>
...
<div class="bottom"><input type="button" class="button"
	value="Submit"
	onclick="if(checkSubmit()&& check()){submitForm('allotAirmen','accom?method=updateSmqVacation');}"
