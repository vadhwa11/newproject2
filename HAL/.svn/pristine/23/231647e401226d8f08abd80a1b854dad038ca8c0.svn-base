...
<input type="text" class="date" name="<%=RequestConstants.ISSUE_DATE%>" id="<%= RequestConstants.ISSUE_DATE %>" readonly="readonly"  value="<%=map.get("date") %>"  MAXLENGTH="50" />
	...
	<input type="radio" name="Issue" value="UIssue" id="unit"  class="radioAuto" maxlength="20" tabindex=1  checked ="true"/>
	 
			<select class="small2" name="departmentIdTemp" tabindex="1" id="departmentIdTemp" validate='Unit Name,num,Yes'  >
				<option value="">Select</option>
				<%for (MasUnit unit :unitList ) {	%>
					<option value=<%=unit.getId()%> ><%=unit.getUnitName()%></option>
				<%}	%>
...
<div class="clear"></div>
  ...
  <tbody>
	...
	<input type="button" class="button" tabindex="1" onclick="if(validateIssue()){submitForm('issueDispensaryForm','stores?method=submitIssueForIndentToUnit');}" name="Submit2" value="Save"  />
