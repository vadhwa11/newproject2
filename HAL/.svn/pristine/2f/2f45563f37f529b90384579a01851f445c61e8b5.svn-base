...
<%@page import="jkt.hms.masters.business.PensionEmoluments"%>
...
<h6>Personnel Search For Updating Form 8 Details</h6>
<div class="Clear"></div>
<%
...
<div class="Clear"></div>
<div class="blockFrame"><label class="bodytextB">Personnel
Name</label> <input type="text" name="personnelName" value=""
	class="textbox_size20" MAXLENGTH="30" /> <label class="bodytextB">Pass
No. </label> <input type="text" name="passNo" value="" class="textbox_size20"
	MAXLENGTH="30" />
...
<input type="button" name="Search"
	onclick="submitForm('personnelSearch','/hms/hms/pension?method=personnelSearchJsp&jspName=updateForm8');"
...
<form name="updatePersonnelsearchForForm8" method="post" action="">
...
<script type="text/javascript" language="javascript">
	...
	data_arr = new Array();
	   <%
	    ...
	    <%}%>
				...
				%>
				data_arr[<%= counter%>][7] = "<%=masPersonnelDetails.getDateOfBirth()%> "
				<%}else{%>
				data_arr[<%= counter%>][7] = ""
				<%}%>
				<%
				...
				%>
					data_arr[<%= counter%>][8] = "<%=masPersonnelDetails.getAppointmentDate()%>"
				<%}else{%>
					data_arr[<%= counter%>][8] = ""
				<%
					}
    ...
    formName = "updatePersonnelsearchForForm8"
