...
<script type="text/javascript">
  ...
  function fillValuesBalance(inc)
  {
    ...
    if(document.getElementById(qtyVarTemp+inc).value!="0"){
  					...
  					if(itemList.size()>0)
 						...
 						for (MasStoreItem masStoreItem:itemList){
 							...
 							if(masStoreItem.getItemConversion() != null){
 								itemUnitName = masStoreItem.getItemConversion().getItemUnitName();
 							}
 							}catch(Exception e){
 								itemUnitName = "-";
 							}
...
<script>
				...
				itemsArray1[<%=k%>][3] = "<%=itemUnitName%>";
         		itemsArray1[<%=k%>][4] = "<%=masStoreItem.getBrand().getBrandName()%>";
    			itemsArray1[<%=k%>][5] = "<%=masStoreItem.getBrand().getId()%>";
         		</script>
<%
          System.out.println("masStoreItem.getStoreItemBatchStocks()----"+masStoreItem.getStoreItemBatchStocks());
          	set=(Set<StoreItemBatchStock>)masStoreItem.getStoreItemBatchStocks();
          System.out.println("--------SET KA SIZE----------"+set.size());
			...
			for (MasStoreBrand masStoreBrand : brandList) 
			{
				if(masStoreBrand.getItem() != null){
								%>
									...
									brandArray[<%=m%>][3] = "<%=masStoreBrand.getManufacturer().getManufacturerName()%>";
								<%
								m++;
								}
				}
