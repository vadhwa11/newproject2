...
<!-- 			<th width="13%">Qty</br>on Loan</th> -->
 ...
 %></td>
			...
			<td><input type="text"  size="8" name="qtyIssued<%=inc%>" id="qtyIssued<%=inc%>" value="" readonly="readonly" onblur="checkQty(<%=inc%>)" validate="Issue Qty,no,no"/></td>
			<td><input type="text"  size="8" name="qtyAftrIssued<%=inc%>" id="qtyAftrIssued<%=inc%>" value="" /></td>
			<td><input type="checkbox" name="rc" id="rc<%=inc%>" value="<%=object[0].toString()%>" /></td>
...
/*   for(var i = 0;i<batchArray[rowVal].length;i++ ){
  alert("batchArray[rowVal][i]"+batchArray[rowVal][i].length);
	  e23.options[i+1] = new Option(batchArray[rowVal][i][0],batchArray[rowVal][i][1]);   
  ...
  e242.size = '10';
	 ...
	 // e6.value=document.getElementById('stockAvailable'+rowVal).value;
	  ...
	  var cellRight352 = row.insertCell(13);
	  var e352 = document.createElement('input');
	  e352.type = 'hidden';	 
	  e352.readOnly=true;
	  cellRight351.appendChild(e352); 
