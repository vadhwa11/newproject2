...
<label>Death Cause Name</label> <input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO %>" value="2" />  <input type="text" id="searchField"
	name="<%= SEARCH_FIELD%>" value="" validate="DeathCause Code,string,no"
	MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=searchDeathCause')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchDeathCause','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_death_cause"></div></form>
...
<form name="deathCause" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasDeathCause"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="DeathCauseName">
<input type="hidden" name="title" value="DeathCause"> <input
...
<div class="Clear"></div>
<div class="Block">
<label> Death Cause Code<span>*</span> </label> <input id="codeId" type="text"
	name="<%= CODE%>" value="" validate="Death Cause Code,string,yes"
	 MAXLENGTH="8" / tabindex=1> <label
	id=biglabel> Death
Cause Name<span>*</span></label> <input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="Death Cause Name,string,yes" 
	MAXLENGTH="30" / tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addDeathCause')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masDeathCause.getLastChgBy() %>"
