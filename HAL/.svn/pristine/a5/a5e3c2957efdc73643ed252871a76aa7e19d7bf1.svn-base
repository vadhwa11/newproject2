...
<form name="foodHandlerReport" target="_blank" method="post" action="">
<div class="Block">
...
<input	type="text" id="fromDateId" name="<%=FROM_DATE %>" 
	...
	class="calender"
	onClick="setdate('<%=currenDate %>',document.foodHandlerReport.<%=FROM_DATE%>,event)" />
...
<label> To Date<span>*</span> </label> <input type="text" id="ToDateId"
	...
	onClick="setdate('<%=currenDate %>',document.foodHandlerReport<%=TO_DATE%>,event)" />
<div class="Clear"></div>
</div>
<div class="Clear"></div>
<input type="button" name="OK" value="Print" class="button"
onClick="submitForm('foodHandlerReport','mis?method=printFoodHandlerReport');" />
