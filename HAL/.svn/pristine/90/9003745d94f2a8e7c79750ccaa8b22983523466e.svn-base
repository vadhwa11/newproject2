...
%>
	serverdate = '<%=dateCal+"/"+month+"/"+year%>'
		...
		if(map.get("detailsMapForOrder") != null){
			System.out.println("visitList "+visitList.size());
			detailsMapForOrder = (Map<String,Object>)map.get("detailsMapForOrder");
			if(detailsMapForOrder.get("visitList")!=null){
				visitList = (List<Visit>)detailsMapForOrder.get("visitList");
				System.out.println("visitList "+visitList.get(0));
			}
		}
		if(detailsMapForOrder.get("dgOrderhd") != null){
			dgOrderhd = (DgOrderhd)detailsMapForOrder.get("dgOrderhd");
			System.out.println("in dgOrderhd "+dgOrderhd.getOrderTime());
			session.setAttribute("dgOrderhd",dgOrderhd);
			...
			if(dgOrderhd.getOrderDate()!=null){
				orderDate=dgOrderhd.getOrderDate();
				System.out.println("in orderdate");
			}
			if(dgOrderhd.getOrderTime()!=null){
				orderTime=dgOrderhd.getOrderTime();
			}
			if(dgOrderhd.getHin().getServiceNo()!=null){
				serviceNo=dgOrderhd.getHin().getServiceNo();
			}
			if(dgOrderhd.getHin().getServiceNo()!=null){
				doctorId=dgOrderhd.getPrescribedBy().getId();
			}
			if(dgOrderhd.getClinicalNote()!=null){
				clinicalNote=dgOrderhd.getClinicalNote();
			}
			for(DgOrderdt dgOrderDt:dgOrderhd.getDgOrderdts()){
			box.put(MAIN_CHARGECODE_ID,dgOrderDt.getMainChargecode().getId());	
			//box.put(SUB_CHARGECODE_ID,dgOrderDt.getSubChargeid().getId());
			}
		...
		if(map.get("detailsMapForMainAndSubCharge")!=null){
			detailsMapForMainAndSubCharge=(Map<String, Object>)map.get("detailsMapForMainAndSubCharge");
		}
	
		
		if(detailsMapForMainAndSubCharge.get("mainChargeCodeList") != null){
			mainChargeCodeList = (List<MasMainChargecode>)detailsMapForMainAndSubCharge.get("mainChargeCodeList");
		}
		if(detailsMapForMainAndSubCharge.get("subChargeCodeList") != null){
			subChargeCodeList = (List<MasSubChargecode>)detailsMapForMainAndSubCharge.get("subChargeCodeList");
		}
		if(detailsMapForMainAndSubCharge.get("chargeList") != null){
			chargeList = (List)detailsMapForMainAndSubCharge.get("chargeList");
		}
...
<label>Order Number</label> <input id="fatherId" type=hidden
	name="<%=ORDER_NO %>" value="<%=orderNo %>" title="Order Number" /> <label
	class="value"> <%=orderSeqNo %> </label> <input type="hidden"
	id="MAIN_CHARGECODE_ID" name="<%=MAIN_CHARGECODE_ID %>"
	value="<%= box.getInt(MAIN_CHARGECODE_ID )%>" /> <input type="hidden"
...
Date</label> <label class="value"> <%= orderDate%> </label> <label>Order
Time</label> <label class="value"> <%= orderTime%> </label>
<div class="Clear"></div>

<script language="javascript">

populateSubCharge(document.getElementById("MAIN_CHARGECODE_ID").value,'investigationOrderBooking');
</script> <%
int counter=0;   
             Visit  visit = visitList.get(0);
%> <label>Service No</label> <%if(serviceNo!=null&&!serviceNo.equalsIgnoreCase("")){ %>
<label class="value"><%=serviceNo%></label> <% }else{%> <label
	class="value">-</label> <%} %> <label>HIN No.</label> <label
	class="value"><%=hinNo %></label> <%
					String middleName = "";
					String lastName = "";
...
<label class="common">Clinical Notes</label> <input id="clinicalNote"
	type="text" name="<%= CLINICAL_NOTE%>" value="<%=clinicalNote %>"
...
<table width="100%" colspan="7" id="chargeDetails" cellpadding="0"
 ...
 String chargeCodeNamewithId="";
    	...
    	List<DgOrderdt>dgOrderdtList=new ArrayList<DgOrderdt>(dgOrderhd.getDgOrderdts());
    	for(inc=1;inc<=10;inc++){
    		if(inc<=dgOrderdtList.size()){
    			dgOrderdt=dgOrderdtList.get(inc-1);
    			chargeCodeNamewithId=dgOrderdt.getChargeCode().getChargeCodeName()+"["+dgOrderdt.getChargeCode().getId()+"]";
    			chargeCodeName=dgOrderdt.getChargeCode().getChargeCodeName();
    			qnty=dgOrderdt.getOrderQty().toString();
		...
		<tr>
			...
			<td><input type="text" id="chargeName<%=inc%>"
				name="<%=CHARGE_NAME%>" value="<%= chargeCodeName%>"
				readonly="readonly" /></td>
			</td>
			<td><input type="text" id="qty<%=inc%>" name="<%=QUANTITY %>"
				value="<%= qnty%>" validate="Qty,int,no" MAXLENGTH="3" /></td>
		...
		<script type="text/javascript" language="javascript">
	if(<%=inc%><=<%=dgOrderdtList.size()%>){
	document.getElementById("mainChargeId<%=inc %>").value=<%=dgOrderdt.getMainChargecode().getId()%>
	document.getElementById("subChargeId<%=inc %>").value=<%=dgOrderdt.getSubChargeid().getId()%>
	
	document.getElementById("chargeCodeId<%=inc %>").value=<%=dgOrderdt.getChargeCode().getId()%>
	}
	else{
...
<div class="bottom"><input type="button" class="button"
	value="Update"
	onclick="if(checkFilledRow()){submitForm('investigationOrderBooking','lab?method=updateOrderBookingForInvestigation');}"
...
function checkFilledRow(){
	...
	if(document.getElementById('noOfRecords').value===0 || document.getElementById('noOfRecords').value ===""){
	  	if(document.getElementById("chargeCode1").value!=""){
	  	return true;
	  	}
