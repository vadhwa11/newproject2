...
<%}else{%>
		<% if (hinId !=0) { %>
			...
			submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showIPDPatientIssueGridPartial');
			
		<% 
		//System.out.println("(hinId !=0) ");
		}else if(patient.getId()!=null){ %>
		var wardId ='<%=wardId%>';
				submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showIPDPatientIssueGridPartial&wardId='+wardId);
	...
	/* 	var issueQty = document.getElementById("qtyIssued"+rowVal).value;
		...
		icdArray[2]="BMW";
		 ...
		 /*  var cellRight32 = row.insertCell(8);
		  ...
		  var e33 = document.createElement('input');
		  e33.type = 'text';
		  e33.name = 'totalPrescription' + iteration;
		  e33.id = 'totalPrescription' + iteration;
		  e33.size = '6';
		  e33.value=document.getElementById('totalPrescription'+rowVal).value;
		  e33.readOnly='readOnly';
		  cellRight33.appendChild(e33);
		...
		/*   if(document.getElementById('qtyIssued'+rowVal).value=="")
		   ...
		   */
		   ...
		   var cellRight36 = row.insertCell(8);
			  var e36 = document.createElement('input');
			  e36.type = 'text';
			  e36.name = 'totalIssued' + iteration;
			  e36.id = 'totalIssued' + iteration;
			  e36.value=document.getElementById('totalIssued'+rowVal).value;
			  e36.size = '6';	
			  e36.readOnly='readonly';
		  ...
		  var cellRight40 = row.insertCell(10);
		  ...
		  var cellRight36 = row.insertCell(12);
		  var e36 = document.createElement('input');
		  e36.type = 'text';
...
function fillPrescribeBy() 
{
		...
		if(confirm(msg)){
			...
			/* if(valCheckBox.length==0 && rcFlag)
				...
				if(rcFlag)
				{
				alert("Please Insert at least one Qty Issue");
				}
			else
				{
				...
				if(saveFlag)
				{
				submitForm('opdPatientIssue','stores?method=submitIPPatientStockDetailsPartial');
				}
				}
