...
<label> Unpack-Checked By<span>*</span></label>
<select
...
<select name="<%=MODE_OF_CONVEYANCE%>"	validate="Mode Of Conveyance,string,yes" tabindex=1>
 ...
 <label> Unpacked-Checked By</label> <select
	...
	if(masEmployee.getLastName() != null){
		...
		<option value="<%=masEmployee.getId()%>"><%=masEmployee.getRank()+" "+masEmployee.getFirstName()+" "+lastName+"("+masEmployee.getRank().getRankName()+")" %></option>
	<% } %>
</select>
...
<table width="98%" colspan="7" id="grnDetails" 	border="0" cellpadding="0" cellspacing="0">
		...
		<%
 		...
 		int noOfRows = 0;
		System.out.println("Size of -----gridLoanInTList----"+gridLoanInTList.size());
	   ...
	   <input type="hidden" name="gridSize" id="gridSize" value="<%=gridLoanInTList.size()%>"/>
		...
		<td>
		<input type="text" name="BRAND_GEN" id=""<%="BrandGen"+""+inc%>" value="<%=storeLoaninT.getItem().getBrandedGeneric()%>" tabindex="1"/>
		</td>
		...
		<%if(storeLoaninT.getItem().getBrand()!=null){%>
		<option value="">Select</option>
		<option value="<%=storeLoaninT.getItem().getBrand().getId()%>"><%=storeLoaninT.getItem().getBrand().getBrandName()%>
		</option>
		<%}else{%>
				<option value=""></option>
...
<td width="40%">
			<input type="text" value=""	name="<%=MANUFACTURING_DATE%>" size="8" id="<%=manufacturingDate+""+inc%>" MAXLENGTH="30"  readonly="readonly" tabindex="1"/>
...
<td width="40%">
			...
			<%} %>
				...
				<option value="<%=storeLoaninT.getItem().getItemConversion().getIntermediateUnit().getUnitName()%>"><%=storeLoaninT.getItem().getItemConversion().getIntermediateUnit().getUnitName()%></option>
				 ...
				 class="medcaption"
				name="mdq"  id="<%=mdq+""+inc%>" tabindex="2" onblur="calculationInCRV();" value="1" onfocus="calculateTotalCrvAmount(<%=inc%>);"/>
			</td>
