...
<%
	...
	ArrayList<MasPublisher> searchPublisherList = (ArrayList<MasPublisher>)map.get("searchPublisherList");
...
<div class="Clear"></div>
<h6>Publisher Master</h6>
<div class="Clear"></div>
<div class="header">
<div id="searcharea">
<div id="searchbar">
<form name="search" method="post" action=""><label>Publisher
Code</label> <input type="radio" name="<%=SELECTED_RADIO  %>" class="radio"
	value="1" checked="checked" /> <label>Publisher Name</label> <input
	...
	onkeypress="return submitenter(this,event,'lib?method=searchPublisher')" />
<input type="button" name="search" value="Search" class="cmnButton"
	onclick="submitForm('search','lib?method=searchPublisher','checkSearch')"
	tabindex=1 /></form>
</div>
</div>
<div class="Clear"></div>
...
<% 
		if(searchPublisherList.size()>0)
		 {
			String strForCode = (String)map.get("publisherCode");
			String strForCodeDescription = (String)map.get("publisherName");
			if(strForCode!= null && strForCode!= "" || strForCodeDescription!= null && strForCodeDescription!= "")
			{
	%>
<div class="Clear"></div>
<h5><a href="lib?method=showPublisherMaster">Show All Records</a></h5>
<div class="Clear"></div>
<%
			}
		 }
	 if(searchPublisherList.size()==0 && map.get("search") != null)
	  {
	 %><div class="Clear"></div>
<h5><a href="lib?method=showPublisherMaster">Show All Records</a></h5>
<div class="Clear"></div>
...
<form name="bookClass" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasPublisher"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="PublisherName">
<input type="hidden" name="title" value="Publisher"> <input
	type="hidden" name="<%=JSP_NAME %>" value="publisher"> <input
	type="hidden" name="pojoPropertyCode" value="PublisherCode">

<div class="division"></div>
<label class="common"><span>*</span> Publisher Code</label> <input
	id="codeId" type="text" name="<%= CODE%>" value=""
	validate="Publisher Code,string,yes" maxlength="10" tabindex=1 /> <label
	class="common"><span>*</span> Publisher Name</label> <input type="text"
	name="<%= SEARCH_NAME %>" id="<%= SEARCH_NAME %>" value=""
	validate="Publisher Name,string,yes" maxlength="30" tabindex=1 /> <script>
...
<label class="common">City</label> <select name="<%= DISTRICT_ID %>"
	...
	<%}%>
</select> <label class="common">State</label> <select name="<%= STATE_ID %>"
...
<div class="bottom"><input type="button" name="add" id="addbutton"
	...
	onClick="if(checkSpecialChar){submitForm('bookClass','lib?method=addPublisher');}"
	accesskey="a" tabindex=1 /> <input type="button" name="edit"
	id="editbutton" value="Update" class="button"
	onClick="if(checkSpecialChar()){submitForm('bookClass','lib?method=editPublisher');}"
	accesskey="u" tabindex=1 /> <input type="button" name="Delete"
	id="deletebutton" value="Activate" class="button"
	onClick="submitForm('bookClass','lib?method=deletePublisher&flag='+this.value)"
...
data_header[0][0] = "Publisher Code"
data_header[0][1] = "data";
data_header[0][2] = "25%";
data_header[0][3] = "<%= CODE%>"

data_header[1] = new Array;
data_header[1][0] = "Publisher Name"
...
<%
Iterator<MasPublisher> itr=searchPublisherList.iterator();
          int  counter=0;
          while(itr.hasNext())
           {
            MasPublisher  vendor = (MasPublisher)itr.next(); 
%>

data_arr[<%= counter%>] = new Array();
data_arr[<%= counter%>][0] = "<%= vendor.getId()%>"
data_arr[<%= counter%>][1] = "<%=vendor.getPublisherCode()%>"
data_arr[<%= counter%>][2] = "<%= vendor.getPublisherName()%>"
