...
<select name ="noteSheetNo" id="noteSheetNoId"   validate="NoteSheet No,string,yes" onchange ="GetPatientList('FILTER');">
...
<div class="clear" style="padding-top: 10px;"></div>
		...
		<div class="block">
				...
				<input type="button" name="yes" value="Dispatch"	class="button"	onclick="if(confirm('Are you sure want to dispatch '+document.getElementById('noteSheetNoId').value)){validateCheckbox()}" />
				<input class="button" name="yes" value="Generate Note Sheet" onclick="generateNoteSheet()" type="button">
...
<script language="javascript">
	...
	function GetPatientList(MODE) {
		...
		var url = "referral?method=getInvoiceWaitingListForPFD";
		var bClickable = true;
...
/*
	...
	/* 	var flag = false;
		...
		{ */
			...
			submitForm('getReferralWaitingList','/hms/hms/referral?method=approveFinanceAuditPFD');
...
/* 		} */
	...
	function generateNoteSheet()
	{
		...
		submitForm('getReferralWaitingList','/hms/hms/referral?method=showReferralReport&flag=Note_sheet_letter&consolidatedNo='+noteSheeNo);
	}
</script>
