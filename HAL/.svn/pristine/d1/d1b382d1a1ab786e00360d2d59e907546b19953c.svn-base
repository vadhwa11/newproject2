...
<div class="titleBg">
<h2>OPD Patient Issue With Loan Out</h2>
</div>
<div class="Clear"></div>
<div class="Block">
...
<input type="hidden" name="issueType" id="issueType" value="loanOut" tabindex="1" size="10"/>
...
<th width="13%">Stock Quantity In Stores</th>
<th width="13%">Item Issued</th>
<%if(deptId!=24){%>
<th width="13%">Loan Out Item </th>
<%}%>
</tr>
	</thead>
	<tbody>
...
<tr>
			...
			<%if(deptId!=24){%>
			<td width="13%">
			<input type="text" name="loanOutQty<%=i%>" id="loanOutQty<%=i%>" value="" tabindex="1" size="10"/>
			</td>
			<%}else{%>
			<td width="13%">
				<input type="hidden" name="loanOutQty<%=i%>" id="loanOutQty<%=i%>" value="0" tabindex="1" size="10"/>
			</td>
			<%}%>
...
///	function validateInteger( strValue ) {
	...
	function validatePage(formName,rowVal)
	{
        ...
        var totalLoanQty=0;
        for(i=0;i<parseInt(counter);i++){
        issLoanQtyVal=eval('document.'+formname+'.loanOutQty' + i + '.value')
	     if(issQtyVal != "")
	    	 totalLoanQty=totalLoanQty+parseInt(issLoanQtyVal)
		 else
			 totalLoanQty=totalLoanQty+parseInt(0)
	    }
	...
	function setTotalQtyInParent(){
		...
		for(var index = 0; index < parseInt(document.opdPatientStockDetailsForm.counter.value); index++ ){
			...
			var toLoanEval = "document.opdPatientStockDetailsForm.loanOutQty" + index;
			var qtyIssuedObj = eval(toEval);
			var qtyLoanIssuedObj=eval(toLoanEval);
			...
			if(qtyLoanIssuedObj.value != "")
			{

				loanSum = loanSum + parseInt(qtyLoanIssuedObj.value);
			}else
			{
		...
		//alert("The Sum is" + loanSum);
		toEval = "window.opener.document.getElementById(\'qtyIssued" + <%=rowVal%> + "\')";
		toLoanEval = "window.opener.document.getElementById(\'lotQty" + <%=rowVal%> + "\')";
		//alert("to Evaluate:+" + toLoanEval);
		//toEval = "window.opener.document.wardConsumption.qtyIssued";
		var totalQtyIssued = eval(toEval);
		var totalLoanQtyIssued = eval(toLoanEval);
		var totalQty;
		var totalLoanQty;
		...
		if(totalLoanQtyIssued.value != ""){
			totalLoanQty = parseInt(totalLoanQtyIssued.value)+parseInt(loanSum);
		}else{
			totalLoanQty = loanSum;
		}
		...
		if(window.opener.document.getElementById('qtyPending' + <%=rowVal%>)){
			if(window.opener.document.getElementById('qtyPrescribed' + <%=rowVal%>).value != "")
				...
				window.opener.document.getElementById('lotQty'+ <%=rowVal%>).value =totalLoanQty;
				window.opener.document.getElementById('lo'+ <%=rowVal%>).disabled = true;
