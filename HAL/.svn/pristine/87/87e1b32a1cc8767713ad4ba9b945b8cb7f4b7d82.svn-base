...
<%@page import="jkt.hms.masters.business.StoreFollowupDetails"%>
<%@page import="jkt.hms.util.HMSUtil"%>
...
<%
	...
	if (map.get("storeFollowupDetailsList") != null) {
		storeFollowupDetailsList = (List<StoreFollowupDetails>)map.get("storeFollowupDetailsList");
	}
...
<option value="0">Select</option>
			<%
					for (StoreDefectiveDrugM storeDefectiveDrugM :defectiveList ) {
				%>
...
<label class="value"><%=defectiveDrugMObj.getEntryNo() %></label>
<input type="hidden" name="<%= ENTRY_NO %>" value="<%=defectiveDrugMObj.getEntryNo()%>" readonly="readonly" />
<%}else{ %>
<input type="text"  name="<%= ENTRY_NO %>" value=""	validate="Entry Number ,String,yes" tabindex=1 /><%} %>
<label>Defect Date<span>*</span></label>
<% if(defectiveDrugMObj.getDocDate() != null){ %>
<label class="value"><%=HMSUtil.convertDateToStringWithoutTime(defectiveDrugMObj.getDocDate()) %></label>
<input type="hidden" name="<%=DOC_DATE %>" id="docDate" value="<%=defectiveDrugMObj.getDocDate() %>"/>
<%}else{ %>
<!--<label><%=currentDate %></label>-->
<input type="hidden" name="<%=DOC_DATE %>" id="docDate" value="<%=currentDate %>"/>
<%} %>

<label>Signal No.</label>
<% if(defectiveDrugMObj.getSignalNo() != null){ %>
<input type="text"  name="<%= SIGNAL_NO%>"  id="signalNo" value="<%=defectiveDrugMObj.getSignalNo() %>"/>
<%}else{ %>
<input type="text"  name="<%= SIGNAL_NO%>"  id="signalNo" value=""/>
<%} %>
<div class="Clear"></div>
<label>Disposal Instructions</label>
<% if(defectiveDrugMObj.getDisposalInstructions() != null){ %>
<input type="text"  name="<%= DISPOSAL_INSTRUCTIONS%>"  id="disposalInstruction" value="<%=defectiveDrugMObj.getDisposalInstructions() %>"/>
<%}else{ %>
<input type="text"  name="<%= DISPOSAL_INSTRUCTIONS%>"  id="disposalInstruction" value=""/>
<%} %>

<label>Remarks</label>
<% if(defectiveDrugMObj.getRemarks() != null){ %>
<textarea value="" name="<%=REMARKS %>" validate="Remarks ,String,no" tabindex=1 onpaste="return checkOnPaste(this)" oninput="return checkMaxLengthMoz(this)" onkeydown="return checkMaxLength(this)" onkeyup="finalCheck(this)" maxlength="200"/>
<%=defectiveDrugMObj.getRemarks()%>
...
<%} %>
	<!-- -
<label>To Place</label> <%if(defectiveDrugMObj != null){ %>
<textarea value="<%=defectiveDrugMObj.getToPlace()%>"
	name="<%=TO_PLACE %>" validate="To Place,String,no" tabindex=1
	onpaste="return checkOnPaste(this)"
	oninput="return checkMaxLengthMoz(this)"
	onkeydown="return checkMaxLength(this)" onkeyup="finalCheck(this)"
	style="width: 500px;" maxlength="250" />
</textarea> <%}else{ %> <textarea value="" name="<%=TO_PLACE %>"
...
</textarea> <%} %> <label>From Store</label> <input type="text"
	name="<%=FROM_STORE %>" value="Expendable Store" id="<%=FROM_STORE %>"
	readonly="readonly"  MAXLENGTH="18"/  > <br />
 -->
<%
	} %>
...
<table width="100%" colspan="7" id="purchaseDetails"
	...
	<thead>
		<tr>
			...
			<th width="5%" colspan="2" >Disposal</br>Date</th>
			<!-- -
			<td width="15%"><label valign="left" class="smalllabel">Authy
			</label></td>
			<td width="15%"><label valign="left" class="smalllabel">Disposal
			</label></td>
   ...
   %>
    ...
    %>
		<tr>
			...
			<td width="10%">
			<input type="text" value="<%=storeDefectiveDrugT.getItem().getItemConversion().getItemUnitName() %>"
				 readonly="readonly" size="5" name="<%=AV%>" id="<%=idAu%>" />
			...
			<td>
			<%if(storeDefectiveDrugT.getBrand().getId() !=null){ %>
		...
		<td width="10%">
			...
			<%} %>
				...
				<td width="10%">
				<%if(storeDefectiveDrugT.getBatchNo() !=null && storeDefectiveDrugT.getBatchNo().getId()!= null)
				{%>
			    <select name="<%=BATCH_ID %>"  id="<%=batchId%>"  onchange="getExpiryDateByAjax(this.value,<%=inc%>);" tabindex="1">
				<option value="0">Select</option>
				<option value="<%=storeDefectiveDrugT.getBatchNo().getId()%>" selected="selected" ><%=storeDefectiveDrugT.getBatchNo() %></option>
			</select>
			<input type="hidden"
				value="<%=storeDefectiveDrugT.getBatchNo().getId() %>" class="medcaption"
				name="<%=BATCH_ID %>" tabindex="2" id="<%=batchId%>" />
			<%}else{ %>
			
			  <select name="<%=BATCH_ID %>" tabindex="2" id="<%=batchId%>"	onchange="getExpiryDateByAjax(this.value,<%=inc%>);" tabindex="1">
				<option value="0">Select</option>
			</select>
<%} %>
			...
			<input type="text" value="<%=HMSUtil.convertDateToStringWithoutTime(storeDefectiveDrugT.getManufactureringDate()) %>" id="manufacturingDate<%=inc%>" name="<%=MANUFACTURING_DATE%>" size="8"  tabindex="1" />
			...
			<%if(storeDefectiveDrugT.getExpiryDate() != null){%>
			 <input type="text" size="8"	value="<%=HMSUtil.convertDateToStringWithoutTime(storeDefectiveDrugT.getExpiryDate()) %>"
				name="<%=EXPIRY_DATE %>" id="<%=expiryDate %>" /> <%}else{ %> <input
				type="text" value="0" size="8" name="<%=EXPIRY_DATE %>"
				id="<%=expiryDate%>" /> <%} %>
			</td>

			<td><input type="text"
				value="<%= storeDefectiveDrugT.getDefectQty()%>"  onblur="fillQuanForDefectiveDrug(<%=inc%>)"
				name="" id="<%=quanRecTemp%>"  size="5"/> <input type="hidden"
				value="<%=storeDefectiveDrugT.getDefectQty()%>"
				name="<%=QUANTITY_RECEIVED%>" id="<%=quanRec%>" />
				</td>
				<td>
				<%if(storeDefectiveDrugT.getDisposal()!=null){
				%>
				<select name="<%=DISPOSAL%>" tabindex="1" id="<%=disposal%>" class="small">
				<%if(storeDefectiveDrugT.getDisposal()=="y"){%>
				<option value="y" selected="selected">y</option>
				<%}else{%>
				<option value="y">y</option>
				<%}%>
				<%if(storeDefectiveDrugT.getDisposal()=="n"){%>
				<option value="n" selected="selected">y</option>
				<%}else{%>
	...
	<select name="<%=DISPOSAL%>" tabindex="1" id="<%=disposal%>" class="small">
				...
				<td>
				<%if(storeDefectiveDrugT.getDateOfDisposal()!=null){ %>
				<input type="text" size="8"	name="<%=DISPOSAL_DATE_D%>" value="<%=storeDefectiveDrugT.getDateOfDisposal() %> "	id="<%=disposalDate%>" readonly onblur=""	tabindex=1 />
				<%}else{ %>
				<input type="text" size="8"	name="<%=DISPOSAL_DATE_D%>" value=""	id="<%=disposalDate%>" readonly onblur=""	tabindex=1 />
				<%} %>
				</td>
				<td>
		...
		<tr>
			...
			<td width="10%"><input type="text" class="medcaption"
				name="<%=ITEM_CODE %>" size="10" readonly="readonly" id="<%=codeItem%>" /> <input
				type="hidden" size="2" value="0" class="smcaption"
				name="<%=ITEM_ID%>" id="<%=idItem%>" /></td>
			<td width="10%">
			<input type="text" value="" id="<%=nameItem%>"  size="30"
		  ...
		  new Ajax.Autocompleter('<%=nameItem%>','ac2update','stores?method=getItemListForDefectiveDrugsByAutocomplete',{parameters:'requiredField=<%=nameItem%>'});
			...
			<td width="10%"><input type="text" value="" size="5" readonly="readonly" name="<%=AV%>" id="<%=idAu%>" /></td>
			...
			<td width="10%">
			<input
				type="text" value="0" name="<%=EXPIRY_DATE %>"
				id="<%=expiryDate%>" size="8" />
			</td>
...
<input
				...
				<input
					...
					class="button"
					onClick="submitForm('modifyDefecetiveDrug','stores?method=updateDefectiveDrug');">
...
<table width="200px" colspan="7" id="grnFollow"
	...
	%>
		...
		<%
		int rowVal=1;
		for(StoreFollowupDetails storeFollowupDetails:storeFollowupDetailsList)
		{
