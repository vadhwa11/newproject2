...
if(errorMessage!=""){
	...
	var url="/hms/hms/stores?method=openItemBrandWindowJspToOtherUnits&itemId="+itemId+"&rowVal="+rowVal+"&issueNo="+issueNo+"&isssueDate="+isssueDate+"&departmentIdTemp="+departmentIdTemp+"&requestBy="+requestBy+"&approvedBy="+approvedBy+"&issuedBy="+issuedBy+"&docNo="+docNo+"&issueId="+issueId+"&nomenclature="+encodeURIComponent(nomenclature)+"&pvms="+pvms+"&qtyRequest="+qtyRequested;
...
function onChangeSourceOfSupply(sos)
{
	...
	//--comment by dipali if (sos == "o" || sos == "l")
	if (sos == "l")
	{
...
function calculationInCRV()
 {
    	...
    	for(rowVal=1;rowVal<=40;rowVal++)
		{
			...
			{
				alert(document.getElementById('idItem'+rowVal).value)
