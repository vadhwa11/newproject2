...
<script type="text/javascript">	
	function submitFormForPrescriptionReport(){
		...
		if(flag == false){
			...
			submitForm('opdPatientPrevVisitForViewScreen','/hms/hms/fwc?method=opdMedicalCaseSheetPrint');
			checkTargetForNo();
		}
	...
	function showPreviousVisit(hinId,deptId,visitNo,token){
       ...
       document.opdPatientPrevVisitForViewScreen.action="/hms/hms/fwc?method=getPatientOpdDetails&flag=current&hinId="+hinId+"&deptId="+deptId+"&visitNumber="+visitNo+"&token="+token+"&link=<%=link%>";
	...
	function backtoHome()
	{
		...
		<%}else{%>
		 document.opdPatientPrevVisitForViewScreen.action="fwc?method=showFWCMainJsp&visitId=<%=visitId%>&token=<%=token%>";
		 <%}%>
		document.opdPatientPrevVisitForViewScreen.submit();
