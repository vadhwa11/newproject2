...
<label>Ser. Per. Name</label> <input type="text"
	name="<%=S_FIRST_NAME %>" value="" MAXLENGTH="30" /> <label>Hin
No.</label> <input type="text" name="<%=HIN_NO %>" value="" MAXLENGTH="50" /> <label>Patient
...
<input type="button" name="submit" id="addbutton"
	onclick="submitForm('patientSearch','/hms/hms/investigation?method=searchPatientForReportCollection','checkFromNTodata');"
...
<form name="reportCollection" method="post" action=""><script
	type="text/javascript">
	formFields = [
	[0, "<%=RESULT_ID%>", "id"],[1,"<%=SAMPLE_NO%>"],[2,"smpcDate"],[3,"time"], [4,"serNo"],[5,"sPerName"], [6,"hin"], [7,"patName"], [8,"age"], [9,"sex"],[10,"pType"],[11,"orderBy"],[12,"doct"],[13,"<%=DIAGNOSIS_NO%>"]];
	 statusTd = 13;
	</script></form>
</div>
<div class="Clear"></div>
...
<script type="text/javascript" language="javascript">
	...
	data_header[12][0] = "Radio Id."
	<%}else{%>
	data_header[12][0] = "Diag.No."
	<%}%>
	data_header[12][1] = "hide";
	data_header[12][2] = "10%";
	data_header[12][3] = "<%=DIAGNOSIS_NO%>";
	...
	<% 	for(DgResultEntryHeader dgHeader : patientList){
		Patient patient = dgHeader.getPatient();
		...
		for(DgResultEntryDetail resultDetail :sampleDet){
			...
			System.out.print("DiagNo"+DiagNo);
		}
	%>
				...
				<%if(dgHeader.getPatient() != null){%>
				data_arr[<%= counter%>][8] = "<%=dgHeader.getPatient().getAge()%> "
				<%}else{%>
				data_arr[<%= counter%>][8] = "-"
				<%}%>
				
				<%if(dgHeader.getPatient().getSex() != null){%>
				data_arr[<%= counter%>][9] = "<%=dgHeader.getPatient().getSex().getAdministrativeSexName()%> "
				<%}else{%>
				data_arr[<%= counter%>][9] ="-"
				<%}%>
				
				<%if(dgHeader.getSampleCollectionHeader().getOrder() != null){%>
				data_arr[<%= counter%>][10] = "<%=dgHeader.getSampleCollectionHeader().getOrder().getPatientType()%> "
				<%}else{%>
				data_arr[<%= counter%>][10] = "-"
				<%}%>
				<%if(dgHeader.getSampleCollectionHeader().getOrder() != null){%>
				data_arr[<%= counter%>][11] = "<%=dgHeader.getSampleCollectionHeader().getOrder().getDepartment().getDepartmentName()%> "
				<%}else{%>
				data_arr[<%= counter%>][11] = "-"
				<%}%>
				<%if(dgHeader.getSampleCollectionHeader().getOrder().getPrescribedBy() != null){%>
				data_arr[<%= counter%>][12] = "<%=dgHeader.getSampleCollectionHeader().getOrder().getPrescribedBy().getFirstName()+" "+dgHeader.getSampleCollectionHeader().getOrder().getPrescribedBy().getMiddleName()+" "+dgHeader.getSampleCollectionHeader().getOrder().getPrescribedBy().getLastName()%> "
				<%}else{%>
				data_arr[<%= counter%>][12] = "-"
				<%}%>
				
				data_arr[<%= counter%>][13] = "<%=DiagNo%>"
			<%	
				}catch(Exception e){
					e.printStackTrace();
				}
