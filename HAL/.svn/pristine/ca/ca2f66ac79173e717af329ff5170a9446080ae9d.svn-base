...
value="<%=opdPatientDetails.getVisit().getHin().getServiceNo() %>" />

<div id="hinDiv">
<label>HIN</label> 
<input	type="text" name="<%=HIN_NO%>" value="<%=opdPatientDetails.getVisit().getHin().getHinNo() %>"	MAXLENGTH="30" onchange="submitProtoAjax('notifiableDisease','mis?method=getPatientDetailForMalaria','hinDiv')"/>
</div>


<input type="hidden" name="hinId" value="<%=opdPatientDetails.getVisit().getHin().getId() %>" />
...
<input type="text" readonly="readonly" name="patientName" value="<%=opdPatientDetails.getVisit().getHin().getPFirstName().concat(" ").concat(opdPatientDetails.getVisit().getHin().getPLastName())%>" tabindex="1" />

<label>Relation</label>
<input type="text" readonly="readonly" name="relation" value="<%=opdPatientDetails.getVisit().getHin().getRelation().getRelationName() %>" tabindex="1" />

<label>Rank</label>
<input type="text" name="rank" readonly="readonly" validate="Rank, number,no" id="rank/RateId" value="<%=opdPatientDetails.getVisit().getHin().getRank().getRankName() %>" tabindex="1" />

<div class="clear"></div>

<label>Name</label>
<input type="text" name="servicePersonName" id="nameId" readonly="readonly" validate="name, string,no" value="<%=opdPatientDetails.getVisit().getHin().getSFirstName().concat(" ").concat(opdPatientDetails.getVisit().getHin().getSLastName()) %>" tabindex="1" />

<!--<label>Relation</label>
<input type="text" name="relation" readonly="readonly" id="relation" value="" />
-->
<label>Age</label>
<input type="text" readonly="readonly" name="age" value="<%=opdPatientDetails.getVisit().getHin().getAge() %>" tabindex="1"  />

<label>Gender</label>
<input type="text" readonly="readonly" name="gender" value="<%=opdPatientDetails.getVisit().getHin().getSex().getAdministrativeSexName() %>" tabindex="1"  />

<div class="clear"></div>

<label>Unit</label>
<input type="text" name="unit" readonly="readonly" id="unit" value="<%=opdPatientDetails.getVisit().getHin().getUnit().getUnitName() %>" tabindex="1"  />
...
<textarea name="clinical" class="auto" cols="80" rows=""><%=a%></textarea>
<% }else{ %> 
...
<%
 ...
 if(masMedicalExamDetails.getPrincipal()!=null){
  ...
  %>
 <input type="hidden" tabindex="1" value="" id=""  name=""	maxlength="200" class="auto"  size="122" onblur="checkDisability(this.value);" />
...
<input	tabindex="1" type="text"  name="suspectedsourceofinfection"	class="auto" size="122" maxlength="200"  />
...
-->
<input	tabindex="1" type="text" maxlength="200" name="genaralRemarks"	class="auto" size="155" maxlength="50"  />

</div>
 	
<div class="Clear PaddingTop15"></div>
<input type="button" name="Submit11" id="addbutton"
	onclick="submitForm('notifiableDisease','/hms/hms/mis?method=submitNotifiableDiseaseWLJsp');"
