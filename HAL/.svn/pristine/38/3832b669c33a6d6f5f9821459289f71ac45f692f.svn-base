...
<input	tabindex="1" name="<%=APPROXIMATE_DATE+inc11 %>" id="approximatedate" class="date" value="<%=HMSUtil.convertDateToStringWithoutTime(setMedicalExam.getApproximatedate())%>"
	...
	onKeyUp="mask(this.value,this,'2,5','/');" /> 
  <% }else{%>
<input	tabindex="1" name="<%=APPROXIMATE_DATE+inc11 %>" id="approximatedate" value="<%=date %>" class="date" 	validate="DOB,date,no" maxlength="10"
	onKeyUp="mask(this.value,this,'2,5','/');" /> 
 <% }%>
 </td><td>
 <img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
...
<input tabindex="1" name="Button"	type="button" class="button" value="Forward"	onClick="submitForm('medicalBoardExaminationReleaseDischarge','medicalBoard?method=updateMedicalExamEntry&data=farwarded&Labresult=<%=Labresult.trim() %>');" />
