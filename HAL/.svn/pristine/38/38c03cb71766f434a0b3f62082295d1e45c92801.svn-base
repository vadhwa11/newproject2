...
<label>Remarks</label><textarea validate="Remarks,string,no" maxlength="500" name ="hr_remarks" id ="hr_remarks" /></textarea>
<div class="Clear"></div>
...
<div class="Clear"></div>
    <label>Bill Amount</label><input type="text"  maxlength="15" name ="impanelled_bill<%=divCountBilling%>" id ="impanelled_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getTotalBillAmt()!=null?referralPatientBilling.getTotalBillAmt():""%>'  readOnly="readOnly"/>

<label>Approved Bill Amount</label><input type="text"  maxlength="15" name ="hal_bill<%=divCountBilling%>" id ="hal_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getAdminBillAmt()!=null?referralPatientBilling.getAdminBillAmt():""%>'  readOnly="readOnly"/>
...
{
if(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getApprovalStatus().equals("RFA"))
		{
		%>
<label>Admin Approval Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getAdminApprovalDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getAdminApprovalDate()):""%></label></textarea>
<%}}%>
...
<label>HR Remarks</label><textarea class="large" onkeyup="auto_grow(this)" validate="Remarks,string,no" maxlength="500" name ="hr_remarks" id ="hr_remarks" /><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks():""%></textarea>
<div class="clear"></div> 
...
else
{
	%>
	<label>Admin Approval Date</label><label class="value"><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getAdminApprovalDate()!=null?HMSUtil.convertDateToStringWithoutTime(referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getAdminApprovalDate()):""%></label></textarea>
	<label>Remarks</label><textarea class="large" onkeyup="auto_grow(this)" validate="Remarks,string,no" maxlength="500" name ="hr_remarks" id ="hr_remarks" /><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks():""%></textarea>
	<%
}}
		%>
...
<div class="Block">
	...
	<%-- <input type="button" class="button" value="Referral Note " onClick="submitFormForButton('generateReferralLetterPage','referral?method=showInvoiceReport&referralPatientDetailsId=<%=referralPatientDetailsId%>&flag=<%=flag%>&referralTreatmentType=<%=referralTreatmentType%>');" align="left"
	onClick="" /> --%>
	<input type="button" name="yes" value="Print Covering Letter"	class="button"	onclick="submitForm('generateReferralLetterPage','/hms/hms/referral?method=showReferralReport&flag=covering_letter&referral_patient_header_id=<%=referralPatientHeaderId%>');" />
...
<input type="button" class="button" value="Approve " align="left"
	onClick="submitForm('generateReferralLetterPage','referral?method=submitInvoicePageForHRDivision');" />
<input type="button" class="button" value="Reject " align="left" onClick="submitForm('generateReferralLetterPage','referral?method=submitInvoicePageForHRDivision&flag=rejected');" />
	<%
	/* } */
}
else
...
<input type="button" class="button" value="Back" align="left"
	onClick="submitFormForButton('generateReferralLetterPage','referral?method=invoiceWaitingListForHRDivision');" />
