...
<form name="pendingPrescriptionForWard" method="post" action="">
...
<%
	int  counter=0;
	 if (prescriptionList != null && prescriptionList.size() > 0) {
 		for(Object[] patientPresc : prescriptionList)
 		{
 		 
 %>
	data_arr[<%= counter%>] = new Array();
	data_arr[<%= counter%>][0] = "<%=patientPresc[0]%>"
	data_arr[<%= counter%>][2] = "<%=patientPresc[1]%>"
		<%System.out.println(patientPresc[1]);%>
...
formName = "pendingPrescriptionForWard"
start = 0
