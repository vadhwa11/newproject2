...
<div id="hinDiv"><input type="hidden"
	...
	onblur="if(checkHinNo()){submitProtoAjax('opdPatientIssue','stores?method=showIPDPartialPatientIssueGrid');}" />
</div>
...
<%}else{ %> <label>Employee No.</label> <input type="text" id="serviceNo"
	...
	onblur="getHinNo('opdPatientIssue','stores?method=getHinForPatientIssue');submitProtoAjax('opdPatientIssue','stores?method=showIPDPartialPatientIssueGrid');"
...
String doc_name="";
if(prescriptionList.size()>0){	
if(prescriptionList.get(0).getInpatient() != null && prescriptionList.get(0).getInpatient().getDoctor()!=null){
	doc_name=prescriptionList.get(0).getInpatient().getDoctor().getFirstName()+" "+(prescriptionList.get(0).getInpatient().getDoctor().getLastName()!=null?prescriptionList.get(0).getInpatient().getDoctor().getLastName():"");
}
}
%>
...
<%}else{%>
		<% if (hinId !=0) { %>
			...
			submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showIPDPartialPatientIssueGrid');
			
		<% 
		//System.out.println("(hinId !=0) ");
		}else if(patient.getId()!=null){ %>
		var prescriptionId ='<%=prescriptionId%>';
				submitProtoAjax('opdPatientIssue','/hms/hms/stores?method=showIPDPartialPatientIssueGrid&prescriptionId='+prescriptionId);
...
*/
 ...
 function jsDisplay()
	...
	function generateRowPrescription(rowVal)
	 {
		var issueQty = parseInt(document.getElementById("qtyIssued"+rowVal).value);
		  var prevQtyIssued = 0;
		  if(document.getElementById('prevQtyIssued'+rowVal)!=null && document.getElementById('prevQtyIssued'+rowVal).value!=''){
			   prevQtyIssued=parseInt(document.getElementById('prevQtyIssued'+rowVal).value);
	      } 
		 
		  issueQty+= prevQtyIssued;
		var flag = false;
		...
		icdArray[2]="BMW";
		 ...
		 // e2.onblur = function(){checkForNomenclature(this.value, iteration,deptId,'prescription');};
		  ...
		  {	  
		   ...
		   e34.value=parseInt(document.getElementById('qtyPrescription'+rowVal).value)-(parseInt(document.getElementById('qtyIssued'+rowVal).value)+parseInt(document.getElementById('prevQtyIssued'+rowVal).value));
		  ...
		  var cellRight42 = row.insertCell(12);
		  var e42 = document.createElement('input');
		  e42.type = 'hidden';
		  e42.name = 'prevQtyIssued' + iteration;
		  e42.id = 'prevQtyIssued' + iteration;
		  e42.size = '6';
		  e42.readOnly=true;
	...
	function checkQty(rowVal)
	{
		...
		/*  if(document.getElementById('rcQty'+rowVal)!=null && document.getElementById('rcQty'+rowVal).value!=''){
		 ...
		 issueQty+=prevQtyIssued;
			if((issueQty) > parseInt(qtyPrescription.value))
			{
...
function fillPrescribeBy() 
{
		...
		/* for(i=1;i<trlength;i++){
			var issueQty = document.getElementById('qtyIssued'+i).value;
			if(issueQty==null || issueQty==''){
				if(flag){msg = 'Do You Want to Generat NIS of:\n---------------------------------------------\n'}
				flag = false;
				var temp = msg.search(document.getElementById('nomenclature'+i).value);
					if(temp ==-1)
						msg += document.getElementById('nomenclature'+i).value+'\n';
			}else{
				if(flag){msg = 'Do You Want to Generat NIS of:\n---------------------------------------------\n'}
				flag = false;
		...
		if(saveFlag){
			fillItemsInGrid();submitForm('opdPatientIssue','stores?method=submitIPPartialPatientStockDetails');
		}
}
