...
<div class="titleBg"><h2>OT Dashboard</h2></div>
<div class="clear"></div>
<!--<h4><%=deptName %></h4>
...
<%} %>
<input type="button" class="button" value="Search" onclick="submitForm('search','/hms/hms/ot?method=showOtDashboard')" />
</div>
</form>
...
while (myMonth==cal.get(Calendar.MONTH)) {
	...
	for(OtBooking booking:bookList)
	{
		...
		if(booking.getSurgeryStatus()!=null && ( booking.getSurgeryStatus().equalsIgnoreCase("y") || booking.getSurgeryStatus().equalsIgnoreCase("d")))
		...
		/* 	Set<OpdSurgeryDetail> opdSuregryDetailsSet = new HashSet<OpdSurgeryDetail>();
			...
			if(bokkingDetails.get(current)==null)
			{
				...
				bokkingDetails.put(current,  "Employee No:"+booking.getHin().getServiceNo()+ " | Name:"+booking.getHin().getPFirstName()+" | Age:"+HMSUtil.calculateAge(booking.getHin().getDateOfBirth())+"<br/>Department:"+booking.getDepartment().getDepartmentName()+" | Surgey Time: "+booking.getSurgeryStartTime() +"-"+booking.getSurgeryEndTime()+" <br/>Procedure:"+procedureName +"<br/>  OT:"+booking.getOt().getOtName()+"  |  Surgeon:"+surgeon+"<br/> Surgery Type: "+(booking.getInpatient()!=null?"Major":"Minor") +" | Surgery Status: "+surgeryStatus+"<br/>");
			...
			else
			{
				...
				bokkingDetails.put(current, bokkingDetails.get(current)+"</br>"+"Employee No:"+booking.getHin().getServiceNo()+ " | Name:"+booking.getHin().getPFirstName()+" | Age:"+HMSUtil.calculateAge(booking.getHin().getDateOfBirth())+"<br/>Department:"+booking.getDepartment().getDepartmentName()+" | Surgey Time: "+booking.getSurgeryStartTime() +"-"+booking.getSurgeryEndTime()+" <br/>Procedure:"+procedureName +"<br/> OT:"+booking.getOt().getOtName()+"  |  Surgeon:"+surgeon+"<br/> Surgery Type: "+(booking.getInpatient()!=null?"Major":"Minor")+" | Surgery Status: "+surgeryStatus+"<br/>");
			}
		}
	}	
...
while (myMonth==cal.get(Calendar.MONTH)) {
	...
	<%if(cal.getTime().before(cal1.getTime())){
		...
		int done = 0;
		 ...
		 for(OtBooking booking:bookList)
		{
			if((HMSUtil.convertDateToStringTypeDateOnly(booking.getSurgeryDate())).equals(current))
			{
				schedule++;
				if(booking.getSurgeryStatus().equals("y") || booking.getSurgeryStatus().equals("d"))
				{
					done++;
				}
			}
	...
	<%}else if(workingDays.contains(f.format(cal.getTime())) && bokkingDetails.get(current)!=null ){
	  ...
	  int done = 0;
		 for(OtBooking booking:bookList)
		{
			...
			{
				schedule++;
				if(booking.getSurgeryStatus().equals("y") || booking.getSurgeryStatus().equalsIgnoreCase("d"))
				{
					done++;
				}
			}
...
<%}/* else {
	 ...
	 for(OtBooking booking:bookList)
	{
		...
		{
			schedule++;
			if(booking.getSurgeryStatus().equals("y"))
			{
				done++;
			}
		}
...
%>
		<%-- 	<div rel="balloon<%=i %>" class="orange"> &nbsp;   <%=current%>
			   <br/> &nbsp; Scheduled:  <%=schedule %>
	  		   <br/> &nbsp; Done:  <%=done %>
			</div> --%>
	<%-- <%} %> --%>
