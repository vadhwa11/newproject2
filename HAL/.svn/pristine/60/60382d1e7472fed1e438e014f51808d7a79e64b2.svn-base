...
if(prescripionList.size() >0){
	...
	for(PatientPrescriptionHeader prescriptionHeader : prescripionList){
		tRow =1;
		String str = "";
		 if(prescriptionHeader.getPatientPrescriptionDetails().iterator().next().getOtStage()!=null){
...
<table border="0" align="center" cellpadding="0" cellspacing="0" id="opdgrid">
	...
	<%-- 	<%
		...
		<td><input type="checkbox" name="stop<%=tCount%><%=tRow%>" id="stop<%=tCount%><%=tRow%>" checked="true"  disabled="disabled"  value="<%=prescriptionDetails.getId()%>"/></td>
		<%
		}
		else
		{%>
