...
<h6> Pre Facto Sanction Cardic Army</h6>
<form name="preFacto" target="_blank" action="" method="post">
...
<input 	type="text" class="calDate" id="fromDate" name="<%=FROM_DATE %>" value="" validate="From Date,date,no" MAXLENGTH="10" tabindex="1" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.preFacto.<%=FROM_DATE%>,event)" />
	
	
	 <label>To Date</label>
	  <input 	type="text" class="calDate" id="toDate" name="<%=TO_DATE %>" value="" validate="To Date,date,no" MAXLENGTH="10" tabindex="1" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.preFacto.<%=TO_DATE%>,event)" />
...
<div class="bottom"><input type="button" name="OK" value="OK" class="button"
	onClick="if(testFromToDate())if(checkBlankSearch())submitForm('preFacto','/hms/hms/mediClaim?method=printPreFactoSanctionReport','validateFromToDate');" />
...
<script type="text/javascript">
	...
	obj1 = eval(document.preFacto.fromDate)
	obj2 = eval(document.preFacto.toDate)
