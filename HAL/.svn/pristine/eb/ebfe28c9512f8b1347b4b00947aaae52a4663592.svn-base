...
<div class="titleBg">
<h2>Work Order Register Report</h2>
</div>
<div class="Block">
...
<label>PVMS/NIV No <span>*</span></label>
<select name="<%=PVMS_NO%>" validate="PVMS/NIV No,String,yes">
<option value=0>Select</option>

<% for (StoreWorkOrderT masWorkOrder :searchMasStoreItemList) { %>

<option value=<%=masWorkOrder.getItem().getPvmsNo()%>><%=masWorkOrder.getItem().getPvmsNo()%></option>
...
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Generate Report" class="buttonBig" onClick="submitForm('workRegister','neStores?method=generateWorkRegisterReport');" accesskey="a" tabindex=1 />
<input type="button" name="clear" id="clearbutton" value="Clear" class="button"
	onClick="clearButton('workRegister');" accesskey="a" tabindex=1 />
