	...
	if(map.get("max") != null){
	 ...
	 if(map.get("message") != null)
	 {
		...
		for(StoreInternalIndentM header: mrHeader)
		{
			...
			FromDept = (header.getDepartment() != null?header.getDepartment().getDepartmentName():"");
			ToDept = (header.getToStore() != null?header.getToStore().getDepartmentName():"");
		}
...
%>
 ...
 <div class="titleBg">
<h2>Approval of MR <%out.print(message); %></h2>
</div>
...
<input	type="text" name="from"  tabindex=1  id="Prepared" value="<% out.print(FromDept);%>" readonly/>

<label>To Department</label>
<input	type="text" name="to"  tabindex=1  id="Prepared" value="<% out.print(ToDept);%>" readonly/>
...
<textarea  name="approvalRemarks" validate="Approval Remarks ,String,yes" tabindex=1 onpaste="return checkOnPaste(this)" oninput="return checkMaxLengthMoz(this)" onkeydown="return checkMaxLength(this)" onkeyup="finalCheck(this)" maxlength="200"/><%out.print(ApprovalRemarks); %></textarea> 
...
<div class="clear"></div>
	...
	if(Status.equalsIgnoreCase("s"))
	{
		%>
			...
			<input type="button" name="Submit"  class="button" value="Submit" onclick="submitMR('MR','stores?method=submitApprovalDetailsofMR&mrHeaderId=<%out.print(mrHeaderId); %>&appId=A1523');"/>
		<%
	}
	
	%>
	
	<input type="button" name="Submit"  class="button" value="Back" onclick="backtoList('stores?method=showMRApprovalList&selectedAppId=A1669');"/>
