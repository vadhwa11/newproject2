 ...
 * socIndent.jsp  
 * Purpose of the JSP -  This is showing SOC Indent.
 * @author  Deepali
 * @author  Mansi
...
<input type="text" name="<%=RequestConstants.INDENT_NO %>"
	value="<%=maxIndentNo%>" readonly="readonly" class="textbox_size20"
	MAXLENGTH="8"/  >
...
<input type="text" name="<%=RequestConstants.INDENT_DATE%>"
	readonly="readonly" value="<%=currentDate %>" class="textbox_size20"
	MAXLENGTH="30" />
<br />
...
<label class="bodytextB_blue"><font id="error"></font>Type of
Indent:</label>
...
<label class="bodytextB_blue"><font id="error"></font>NRS:</label>
<input type="text" name="<%= RequestConstants.NRS %>" value=""
	class="textbox_size20" MAXLENGTH="30" validate="NRS,num,yes" />
<input type="hidden" name="<%=RequestConstants.NO_OF_ROWS%>" id="rr"
	value="22" />
...
<div class="panelbar">
<div class="paneltext">
<div id="servicePerson" style="display: block;">Details of
patient:</div>
</div>
</div>
<label class="bodytextB_blue"><font id="error"></font>Details:</label> <textarea
	name="<%=RequestConstants.DETAIL_OF_PATIENT%>" cols="27" rows="2"></textarea>
<label class="bodytextB_blue"><font id="error"></font>Authority:</label>
<textarea name="<%=RequestConstants.AUTHORITY%>" cols="27" rows="2"></textarea>
<br />
<label class="bodytextB_blue"><font id="error"></font>Justification
of using NIV item:</label> <textarea name="<%=RequestConstants.JUSTIFICATION%>"
	rows="2" cols="25"></textarea> <label class="bodytextB_blue"><font
	id="error"></font>If PAC, General Detail justification:</label> <textarea
	name="<%=RequestConstants.GENERAL_DETAILS%>" rows="2" cols="30"></textarea>

<label class="bodytextB_blue"><font id="error"></font>*PAC Name:</label>
<textarea name="<%=RequestConstants.ADDRESS_OF%>" rows="2" cols="30"></textarea>
...
<input type="hidden" class="button" value="Add Row"
	...
	onclick="if(checkForNext()){submitForm('indentGrid','stores?method=nextIndentForSoc');}"
	...
	<tbody>
		...
		<tr>
			...
			<td width="3%"><input type="hidden" value="0"
				name="<%=RequestConstants.UNIT_RATE %>" id="pf1" /> <input
				type="hidden" value="0" name="<%=RequestConstants.BRAND_NAME %>"
				id="pf2" /> <input type="hidden" value="0"
				name="<%=RequestConstants.MANUFACTORED_BY %>" id="pf3" /> <input
				type="hidden" value="0" name="<%=RequestConstants.MARKETED_BY %>"
				id="pf4" /> <input type="hidden" value="0"
				name="<%=RequestConstants.DEPARTMENT_ID %>" id="pf5" /> <input
				type="hidden" value="0" name="<%=RequestConstants.TOTAL_COST %>"
				id="pf6" /> <input type="hidden" value="0"
				name="<%=RequestConstants.LAST_RECP_QTY %>" id="pf7" /> <input
				type="hidden" value="0" name="<%=RequestConstants.LAST_RECP_DATE %>"
				id="pf8" /> <input type="button" onclick="get_value();"
				name="Submit2" value="More" class="smbutton" /></td>
		</tr>
		<%   } %>
