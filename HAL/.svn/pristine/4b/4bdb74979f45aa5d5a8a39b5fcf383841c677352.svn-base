...
<div class="Block" id="billingList">
 <label><font color="red">Deduction From Salary</font></label><input type="text" style="margin-right: 0px;" validate="Deduction From Salary,string,nos" maxlength="19" name ="deductionFS" id ="deductionFS" value="0" />
 <div class="Clear"></div>
...
<label>Bill No<span>*</span></label><input type="text" validate="Bill No,string,yes" maxlength="100" name ="bill_no<%=divCountBilling%>" id ="bill_no<%=divCountBilling%>" value='<%=referralPatientBilling.getBillNo()!=null?referralPatientBilling.getBillNo():""%>' disabled="true"/>
<label>Bill Date<span>*</span></label>
<input id="bill_date<%=divCountBilling%>" disabled="true" validate="Bill Date,string,yes" placeholder="DD/MM/YYYY" name="bill_date<%=divCountBilling%>"    maxlength="30" type="text" value='<%=referralPatientBilling.getBillDate()!=null?HMSUtil.convertDateToStringTypeDateOnly(referralPatientBilling.getBillDate()):""%>'>
...
<%if(!referralPatientBilling.getReferralDetails().getReferralPatientHeader().getApprovalStatus().equals("RHD"))
		{
		%>
<input name="Button" type="button" class="buttonAdd" value="" onclick="addDivBilling();" tabindex="1" /> 			
<%-- <input type="button" name="delete" value="" class="buttonDel" onclick="removeDivBillingDB(<%=referralPatientBilling.getId() %>);" tabindex="1" /> --%>
<%}%>
<div class="Clear"></div>
    <label>Bill Amount<span>*</span></label><input type="text" disabled="true" validate="Bill Amount,float,yes" maxlength="15" name ="impanelled_bill<%=divCountBilling%>" id ="impanelled_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getTotalBillAmt()!=null?referralPatientBilling.getTotalBillAmt():""%>' />

<label>Approved Bill Amount<span>*</span></label><input type="text" disabled="true" validate="Approved Bill Amount,float,yes" maxlength="15" name ="hal_bill<%=divCountBilling%>" onchange="validateBill(<%=divCountBilling%>)" id ="hal_bill<%=divCountBilling%>" value='<%=referralPatientBilling.getAdminBillAmt()!=null?referralPatientBilling.getAdminBillAmt():""%>' />
<%-- <input type="checkbox" style="margin-right: 0px;" disabled="true" validate="Include in clarification Amount,string,nos" maxlength="15" name ="clarri_bill<%=divCountBilling%>" id ="clarri_bill<%=divCountBilling%>" value='y' /><label>Include In Clarification</label> --%>
	<div class="Clear"></div>
	<%if(referralPatientBilling.getReferralDetails().getReferralPatientHeader().getApprovalStatus().equals("RHD"))
		{
		%>
...
<label>Remarks</label><textarea validate="Remarks,string,no" disabled="disabled" class="large" onkeyup="auto_grow(this)" maxlength="500" name ="admin_remarks<%=divCountBilling%>" id ="admin_remarks<%=divCountBilling%>" /><%=referralPatientBilling.getAdminRemarks()!=null?referralPatientBilling.getAdminRemarks():""%></textarea>

 <label>Referral Number <span>*</span></label>	
<select id="referralNoId<%=divCountBilling%>" name="referralNoId<%=divCountBilling%>"  validate="Referral Number,string,yes" disabled="true">
			<option value="0">Select</option>
			<%
...
<label>Remarks</label><textarea validate="Remarks,string,no" disabled="disabled" class="large" onkeyup="auto_grow(this)" maxlength="500" name ="admin_remarks<%=divCountBilling%>" id ="admin_remarks<%=divCountBilling%>" /><%=referralPatientBilling.getAdminRemarks()!=null?referralPatientBilling.getAdminRemarks():""%></textarea>
 ...
 <label>Referral Number <span>*</span></label>	
<select id="referralNoId<%=divCountBilling%>" name="referralNoId<%=divCountBilling%>"  validate="Referral Number,string,yes" disabled="disabled">
			<option value="0">Select</option>
			<%
...
{%>
	<label>Hr Remarks</label><textarea validate="Remarks,string,no" disabled="disabled" class="large" onkeyup="auto_grow(this)" maxlength="500"  readOnly="readOnly"/><%=referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks()!=null?referralPatientBillingList.get(0).getReferralDetails().getReferralPatientHeader().getHrRemarks():""%></textarea>
	<% 
}
...
<input type="text" validate="Bill No,string,yes" disabled="disabled" maxlength="100" name ="bill_no<%=divCountBilling%>" id ="bill_no<%=divCountBilling%>"  />

<label>Bill Date<span>*</span></label>
<input id="bill_date<%=divCountBilling%>" class="calDate" disabled="disabled" validate="Bill Date,date,yes" placeholder="DD/MM/YYYY" onkeyup="mask(this.value,this,'2,5','/');" name="bill_date<%=divCountBilling%>"  value="" tabindex="1" maxlength="30" type="text">
...
<div class="Clear"></div>
    <label>Bill Amount<span>*</span></label><input type="text" disabled="disabled" validate="Bill Amount,float,yes" maxlength="15" name ="impanelled_bill<%=divCountBilling%>" id ="impanelled_bill<%=divCountBilling%>"  />

<label>Approved Bill Amount<span>*</span></label><input type="text" disabled="disabled" validate="Approved Bill Amount,float,yes" maxlength="15" name ="hal_bill<%=divCountBilling%>" id ="hal_bill<%=divCountBilling%>" onchange="validateBill(<%=divCountBilling%>)" />
<%-- <input type="checkbox" style="margin-right: 0px;" disabled="disabled" validate="Include in clarification Amount,string,nos" maxlength="15" name ="clarri_bill<%=divCountBilling%>" id ="clarri_bill<%=divCountBilling%>" value='y' /><label>Include In Clarification</label> --%>	
<div class="Clear"></div>
<label>Remarks</label><textarea validate="Remarks,string,no" class="large" disabled="disabled" onkeyup="auto_grow(this)" maxlength="500" name ="admin_remarks<%=divCountBilling%>" id ="admin_remarks<%=divCountBilling%>" /></textarea>

 <label>Referral Number <span>*</span></label>	
<select id="referralNoId<%=divCountBilling%>" name="referralNoId<%=divCountBilling%>" disabled="disabled" validate="Referral Number,string,yes">
			<option value="0">Select</option>
			<%
...
<input type="button" class="button" value="Approve " align="left"
	onClick="submitForm('generateReferralLetterPage','referral?method=submitInvoicePageAlreadySend', 'billNoValidation');" />
<input type="button" class="button" value="Back" align="left"
	onClick="submitFormForButton('generateReferralLetterPage','referral?method=invoiceWaitingListAlreadySend');" />
...
function removeDivBillingDB(Id)
{
var divCount = parseInt(document.getElementById("divCountBilling").value);
...
{		
	DeleteFromDatabase_AddRemoveGrid(Id,divCount);	   
...
function DeleteFromDatabase_AddRemoveGrid(tableId,divCount)
{	
	var data = "Id="+tableId;
	...
	for(var i = 1; i<=divCount; i++)
		{
		 ...
		 if(document.getElementById("clarri_bill"+i).checked)
			 {
			 flag = true;
			 }
		}
