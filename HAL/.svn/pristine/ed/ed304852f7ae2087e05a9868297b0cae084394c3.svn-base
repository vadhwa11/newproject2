...
<li><a href="fwc?method=showPatientAllergicDrugsJsp&visitId=<%=visit.getId() %>">Patient Allergic Drugs</a></li>
<li><a href="fwc?method=showUploadingDocumentsJsp&visitId=<%=visit.getId() %>">Show Uploading documents </a></li>
<li><a href="fwc?method=getPatientDetailsForOPDOrderBooking&visitId=<%=visit.getId() %>">Order Booking </a></li>
<li><a href="fwc?method=showOpdTemplateDepartmentWiseJsp&visitId=<%=visit.getId() %>">Opd Template Department Wise</a></li>
<li><a href="fwc?method=showSurgeryRequisitionJsp&visitId=<%=visit.getId() %>">Surgery Requisition Form</a></li>-->
<!--  <li><a href="adt?method=showAdmissionJsp">Admitted Patient</a></li> -->
<!-- <li><a href="fwc?method=showPatientPreviousVisitForPrescriptionReport&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&deptId=<%=visit.getDepartment().getId()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>" >Prev Prescription</a></li>
<li><a href="fwc?method=showPatientPreviousVisitForInvestigationReport&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&deptId=<%=visit.getDepartment().getId()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>" >Prev Investigation</a></li>
...
%>	
	...
	<input name="Back" type="button" tabindex="1"align="right" onclick="submitForm('opdMain','/hms/hms/fwc?method=showUpdatePatientPrescribtionJsp')" class="button" value="Back" />
<div class="clear"></div>
	<div class="division"></div>
	<div class="clear"></div>
  ...
  function validateBpValue(obj){
	...
	function addRow(){
	 ...
	 // e0.innerHTML = iteration+':'
	  ...
	  new Ajax.Autocompleter('nomenclature'+iteration,'ac2update1','fwc?method=getItemListForAutoCompleteItem',{parameters:'requiredField=nomenclature'+iteration});
	   //alert("name--"+e0.name)
...
var xmlHttp;
  try {
    ...
    xmlHttp.send(null);
	 ...
	 // e0.innerHTML = iteration+':'
	  ...
	  e0.size = '65'
	  cellRight0.appendChild(e0);
		new Ajax.Autocompleter('chargeCodeName'+iteration,'ac2update2','fwc?method=getInvestigationListForAutoComplete',{parameters:'requiredField=chargeCodeName'+iteration});
  ...
  <script type="text/javascript">
     ...
     function submitDetails(){
        ...
        document.opdMain.action="hms/hms/fwc?method=submitFWCPatientDetails";
        document.opdMain.submit();
        document.opdMain.action="fwc?method=showEntJsp&visitId=<%=visit.getId() %>"
        document.opdMain.submit();
     ...
     function showPreviousVisit(visitNo){
        ...
        document.opdMain.action="/hms/hms/fwc?method=showPatientPreviousVisit&hinId=<%=visit.getHin().getId()%>&deptId=<%=visit.getDepartment().getId()%>&visitNo=<%=visit.getVisitNo() %>";
        document.opdMain.submit();
        }else{
          alert("This Is Patient's first Visit.")
        }
	...
	function getDoctorList(){
		...
		submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=responseForDoctarsList&referredDepartmentId='+indexes+'','referredDoctarsIdDiv');
		//submitProtoAjaxforOpdMain('opdMain','/hms/hms/fwc?method=responseForDoctarsList&referredDepartmentId='+indexes+'');
	}
  ...
  function copyPrevPrescriptionTempate(visitNo,hinId){
	    ...
	    submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=getPatientPreviousPrescriptionForCopy&&visitNo='+visitNo+'&hinId='+hinId,'testDiv');
  }
  ...
  function copyPrevInvestigationTempate(visitNo,hinId){
		...
		var hdb = document.getElementById('hdb').value;
	    submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=getPatientPreviousInvestigationForCopy&&visitNo='+visitNo+'&hinId='+hinId,'gridview');
  }
 ...
 function getListForTreatment(val){
 	if(val=='investigationDiv'){
		submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=getListForTreatment&flag=investigation',val);
	}else if(val=='treatmentDiv'){
		submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=getListForTreatment&flag=treatment',val);
	}
 ...
 function checkForChargeCode(val,inc,chargeCodeTdDiv){
	...
	if(val != ""){
		...
		//submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=fillChargeCodeForInvestigation&chargeCodeNAmeForAjax='+ tempChargeCodeString+'&rowVal=1',chargeCodeTdDiv);
	...
	function showHideDrugTemplateCombo(valueOfTemplate){
		if(checkTemplateId(valueOfTemplate)){
			...
			submitProtoAjax('opdMain','/hms/hms/fwc?method=showGridInMainJsp','testDiv');
		}
	}
	function showHideInvestigationTemplateCombo(valueOfTemplate){
		if(checkTemplateId(valueOfTemplate)){
			...
			submitProtoAjaxWithDivName('opdMain','/hms/hms/fwc?method=showGridForInvestigation','gridview');
		}
	} 
	...
	function submitOPDMainForm(){  			
		if(validateFieldValuesForMainSubmit()){
			if(validateFrequency()){
				submitForm('opdMain','fwc?method=submitFWCPatientDetailsChange&flag=opd');			
			}
		}
	}
