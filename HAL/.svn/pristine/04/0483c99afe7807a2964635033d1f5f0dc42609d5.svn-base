...
poolName = document.getElementById('poolName'+rowVal).value;
 ...
 var url="/hms/hms/accom?method=showAllotmentDetailsOfficer&smqId="+document.getElementById('smqId'+rowVal).value+'&vacationNo='+document.getElementById('vacationNo'+rowVal).value+'&vacationDate='+document.getElementById('vacationDate'+rowVal).value+'&registrationId='+document.getElementById('registrationId'+rowVal).value+'&allotmentId='+document.getElementById('allotmentId'+rowVal).value+'&poolId='+document.getElementById('poolId'+rowVal).value+'&poolName='+document.getElementById('poolName'+rowVal).value;
   popwindow(url);
   }
 }  
...
<h6>ALLOTMENT/VACATION OF SMQs for Officer</h6>
<div class="Clear"></div>
<div class="Clear"></div>
...
<label><span>*</span> SMQs</label> <select id="<%=SMQ_ID %>"
	...
	onblur="submitFormForButton('allotAirmen','accom?method=getRecordsAllotVacatOfficer');"
	onchange="submitFormForButton('allotAirmen','accom?method=getRecordsAllotVacatOfficer');">
