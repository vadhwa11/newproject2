...
<%
		...
		if(patientMap.get("patientList") != null){
			patientList= (List<MdBillMovement>)patientMap.get("patientList");
		}
...
<label>Patient Last Name</label> <input id="pLName" type="text"
	name="<%=P_LAST_NAME%>" value="" MAXLENGTH="30" /> <label>Sr.
Per. First Name</label> <input id="sPerFName" type="text"
	name="<%=S_FIRST_NAME %>" value="" MAXLENGTH="30" /> <label>Sr.
...
<label>Rank</label> <select id="rankId" name="<%=RANK_ID %>">
	...
	onclick="if(checkBlankSearch()){submitForm('patientSearch','/hms/hms/mediClaim?method=showPatientDetailsForCardicTracking');}"
...
<form name="searchCardicClaimTracking" method="post" action=""><script
...
<script type="text/javascript" language="javascript">
	...
	<% 	for(MdBillMovement billMovement : patientList){
	%>
	  		data_arr[<%= counter%>] = new Array();
			data_arr[<%= counter%>][0] = <%= billMovement.getId()%>
			data_arr[<%= counter%>][1] = "<%=billMovement.getHin().getServiceNo()%>"
			data_arr[<%= counter%>][2] = "<%=billMovement.getHin().getHinNo()%>"
			<%
					String middleName = "";
					String lastName = "";
					if(billMovement.getHin().getPMiddleName() != null){
						middleName = billMovement.getHin().getPMiddleName();
					}
					if(billMovement.getHin().getPLastName() != null){
						lastName = billMovement.getHin().getPLastName();
					}
					
					%>
			data_arr[<%= counter%>][3] = "<%=billMovement.getHin().getPFirstName()%> <%=middleName%> <%=lastName%>"
			data_arr[<%= counter%>][4] = "<%=billMovement.getHin().getRank().getRankName()%> "
			<%
			if(billMovement.getHin().getSFirstName() != null  && !(billMovement.getHin().getSFirstName().equals(""))){
				
				String sMiddleName = "";
				String sLastName = "";
				if(billMovement.getHin().getSMiddleName() != null){
					sMiddleName = billMovement.getHin().getSMiddleName();
				}
				if(billMovement.getHin().getSLastName() != null){
					sLastName = billMovement.getHin().getSLastName();
				}
				String sName = billMovement.getHin().getSFirstName()+" "+sMiddleName+" "+sLastName;
			%>
    ...
    formName = "searchCardicClaimTracking"
	start = 0
