...
<h6>Compatibility Register</h6>
<form name="compatibilityRegister" target="_blank" action=""
	method="post">
<div class="blockFrame"><label><span>*</span>From Date</label> <input
	...
	onClick="setdate('<%=currentDate %>',document.compatibilityRegister.<%=FROM_DATE%>,event)" />
...
<label><span>*</span>To Date</label> <input type="text" class="calDate"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.compatibilityRegister.<%=TO_DATE%>,event)" />

</div>
<div class="Clear"></div>
<div class="bottom"><input type="button" name="OK" value="OK"
	class="button"
	onClick="submitForm('compatibilityRegister','bloodBank?method=generatePatientDiagnosticRegister','validateFromToDate');" />
...
function validateFromToDate(){
	...
	obj1 = eval(document.compatibilityRegister.fromDate)
	obj2 = eval(document.compatibilityRegister.toDate)
