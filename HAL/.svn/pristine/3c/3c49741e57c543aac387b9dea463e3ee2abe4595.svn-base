...
<%
	...
	System.out.print("------------------------ In Jsp departmentList "+departmentList.size());
	...
	System.out.print("------------------------ In Jsp approvedByEmployeeList "+approvedByEmployeeList.size());
	...
	System.out.print("------------------------ In Jsp requestByList "+requestByEmployeeList.size());
	...
	if(map.get("noDetailRecords")!=null){
		...
		System.out.print("------------------------ In Jsp noDetailRecords "+noDetailRecords);
	}
...
Demand No: </label> <input type="text" name="<%=DEMAND_NO %>"
	value="<%=maxDemandNo%>" readonly="readonly" class="textbox_size20"
	MAXLENGTH="8"/  > <span class="bodytextB_blue">Demand
...
<span class="bodytextB_blue">From Store/Dept/Ward:</span> <select
...
<span class="bodytextB_blue">To Store:</span> <select
...
Demand No: </label> <input type="text" name="<%=DEMAND_NO %>" value="<%=max%>"
	...
	name="<%=DEMAND_DATE %>" value="<%=date%>" class="textbox_size20"
	readonly="readonly" tabindex=3 /> <br>
<span class="bodytextB_blue">From Store/Dept/Ward:</span> <select
	name="<%=FORM_STORE_DEPT_WARD_DEPARTMENT_ID_DEPENDENT_INDENT%>"
	...
	<tbody>
		...
		var obj = actb(document.getElementById('<%=nameItem%>'),nameArray);
			...
			<td width="10%"><input type="text" value="" class="medcaption"
				tabindex=1 name="<%=QTY_IN_MMF_TEMP%>" tabindex="2"
				id="<%=mmfVarTemp%>" readonly="readonly"
				validate="Qty In MMF,num,no" onblur="fillValues(<%=inc%>);" /> <input
				type="hidden" value="0" class="medcaption" name="<%=QTY_IN_MMF%>"
