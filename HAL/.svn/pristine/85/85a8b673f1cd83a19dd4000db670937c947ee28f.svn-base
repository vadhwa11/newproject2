...
<%
	...
	List<HrOrderlySgtDutyEntry>hrorderlysgtdutyentry=new ArrayList<HrOrderlySgtDutyEntry>();
	...
	if (map.get("hrorderlysgtdutyentry")!=null)
	{
		hrorderlysgtdutyentry = (List)map.get("hrorderlysgtdutyentry");
	}
	if (map.get("SgtDutyHoliDay")!=null)
	{
		SgtDutyHoliDay = (List)map.get("SgtDutyHoliDay");
	}
%>
...
{ 
	var msg="";
			 for(var i = 0; i < document.getElementsByName('employeeToBeAdded').length; i++){
              ...
              { 
              	var dutyFrom1=document.getElementsByName('nextDutyDate')[i].value;
              	...
              	if(dutyFrom1!="")
				{
					...
					dutyFrom = new Date(dutyFrom1.substring(6),(dutyFrom1.substring(3,5) - 1) ,dutyFrom1.substring(0,2));
...
function GoPage() {	
	...
	submitForm('guardDutyAddForm','hrOrderly?method=getGridDataForOrderlyEmployeeAdd');
}
...
function jsAdd()
{
		if (validateButton())
		{
			...
			{
				submitForm('guardDutyAddForm','hrOrderly?method=AddOrderlyDutyEntry');
...
function jsSearch()
{
	...
	{
		guardDutyAddForm.method="post";
		submitForm('guardDutyAddForm','hrOrderly?method=searchOrderlyDutyEntry');
	}
	else
	...
	function jsClose()
	{
	  window.opener.location.href = "hrOrderly?method=showOrderlyDutyRecords&toDate="+guardDutyAddForm.toDate.value+"&fromDate="+guardDutyAddForm.fromDate.value;
...
function getLastDutyDetails(empData)
    {
 		...
 		submitProtoAjaxWithDivName('guardDutyAddForm','/hms/hms/hrOrderly?method=getOrderlyEmployeeLastDutyDetails&empData='+empData,'DutyDetail');
    }
...
<%
		...
		%>
<%if(SgtDutyHoliDay!=null && SgtDutyHoliDay.size()>0){
		  int id=0;	
		  int counter=0;
		    for(HrOrderlySgtDutyEntry sgtDutyForHoliday:SgtDutyHoliDay){
		    	if(sgtDutyForHoliday.getEmp().getId()!=id){
		    		id=sgtDutyForHoliday.getEmp().getId();%>
<script>
		    		holiday[<%=counter%>]=new Array();
		    		holiday[<%=counter%>][0]=<%=id %>;
		    		
		    		</script>
<%if(sgtDutyForHoliday.getHolidayStatus().equals("y")){%>
