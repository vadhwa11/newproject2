...
<h6>Occupancy / Vacation Return for Airmen</h6>
<div class="Clear"></div>
...
<label>Allotment No</label> <input type="text" name="<%=ALLOTMENT_NO %>"
	...
	onkeypress="return submitenter(this,event,'accom?method=searchOccVacAirmen')" />
...
<div class="Clear"></div>
<input type="button"
	onclick="submitFormForButton('smqSearch','/hms/hms/accom?method=searchOccVacAirmen','checkFromNTodata');"
...
<table width="100%" colspan="7" id="chargeDetails" cellpadding="0"
	...
	<tr>
		...
		<td><input type="text" class="calDate"
			...
			value="<%=currentDate %>" readonly="readonly" MAXLENGTH="30" /> <img
			src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
			validate="Pick a date" id="date<%=inc %>"
...
<input type="hidden" name="counter" id="counter" value="<%=inc %>" /> <input
	...
	onClick="if(checkSubmit()){submitForm('grid','accom?method=submitRecordsForOccVacAirmen');}"
