...
<form name="investigationUploadDoc" method="post" enctype="multipart/form-data">
		...
		if (document.getElementById('attachment'+i).value!="")
		{
			...
			if(filename.lastIndexOf(".doc")==-1){
				...
				document.getElementById('attachment'+i).value="";
					...
					invest_id=<%=investId%>;
					hin_no='<%=hinNo%>';
					if( validateMetaCharacters(hinId) && validateMetaCharacters(masExam_Id) && validateMetaCharacters(visit_id) && validateMetaCharacters(hin_no)  &&  validateMetaCharacters(inc) &&  validateMetaCharacters(invest_id)){
					submitForm('investigationUploadDoc','medicalExam?method=submitUploadDocumentsInvestigation'+fileNames +'&hin_id='+ hin_id+'&hin_no='+hin_no+'&masExam_Id='+masExam_Id+'&invest_id='+invest_id+'&visitId='+visit_id+'&uploadCount='+inc);
					}
				}
...
function viewPatientDocuments()
{
   ...
   document.investigationUploadDoc.method="post";
   if( validateMetaCharacters(hin_id) && validateMetaCharacters(masExam_Id) && validateMetaCharacters(visit_id)   &&  validateMetaCharacters(invest_id)){
