...
<h6> Entitlement Card For Families</h6>
<form name="cardFamilies" target="_blank" action="" method="post">
...
<input 	type="text" class="calDate" id="fromDate" name="<%=FROM_DATE %>" value="" validate="From Date,date,no" MAXLENGTH="10" tabindex="1" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.cardFamilies.<%=FROM_DATE%>,event)" />
	
	
	 <label>To Date</label>
	  <input type="text" class="calDate" id="toDate" name="<%=TO_DATE %>" value="" validate="To Date,date,no" MAXLENGTH="10" tabindex="1" /> 
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.cardFamilies.<%=TO_DATE%>,event)" />
...
<div class="bottom"><input type="button" name="OK" value="OK" class="button"
	onClick="if(testFromToDate())if(checkBlankSearch())submitForm('cardFamilies','/hms/hms/mediClaim?method=printEntitlementCardFamReport','validateFromToDate');" />
...
<script type="text/javascript">
	...
	obj1 = eval(document.cardFamilies.fromDate)
	obj2 = eval(document.cardFamilies.toDate)
