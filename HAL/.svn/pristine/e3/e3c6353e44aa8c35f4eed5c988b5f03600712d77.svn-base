...
<!--By Vishnu  -->
<title>Pending Surgery Consent Document</title>
...
<div class="clear"></div>
<div class="titleBg">
<h2>Pending Surgery Consent Document</h2>
</div>
<div class="clear"></div>
...
<input type="button" name="Search" value="Search" class="button"
	onClick="submitForm('search','/hms/hms/ot?method=showOtConsentDetails');" />
...
<form name="pendingForSurgeryConsentForm" method="post" action="">
...
<%
 ...
 %>
	...
	data_arr[<%= counter%>][2] = "<%=otBooking.getHin().getServiceNo()%>"
	<%
...
data_arr[<%= counter%>][4] = "<%=otBooking.getHin().getRelation().getNewRelationName()%>"
	data_arr[<%= counter%>][5] = "<%=otBooking.getHin().getAge()%>"
	data_arr[<%= counter%>][6] = "<%=otBooking.getHin().getSex().getAdministrativeSexName()%>"
	     ...
	     <%if(otBooking.getInpatient()!=null){ %>
	     data_arr[<%= counter%>][8] = "<%= HMSUtil.convertDateToStringWithoutTime(otBooking.getInpatient().getDateOfAddmission())%>"
         <%}else{ %>
         data_arr[<%= counter%>][8] = "-"
         <%} %>
         
         <%if(otBooking.getInpatient()!= null  && otBooking.getInpatient().getVisit()!= null ){ %>
         data_arr[<%= counter%>][9] = "<%=otBooking.getInpatient().getVisit().getPriority()%>"
         <%}else{ %>
         data_arr[<%= counter%>][9] = "-"
         <%} %>
...
formName = "pendingForSurgeryConsentForm"
start = 0
