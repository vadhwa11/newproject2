...
<div class="Clear"></div>
<label style="width: 50px; padding-left: 35px; font-size: 11px;">Sr
No. </label>
<label style="padding-left: 15px; width: 90px; font-size: 11px;">Service
</label>
...
<label class="value" style="padding-left: 40px; width: 60px;"><%=dgResultEntryDetailForSingleResult.getInvestigation().getInvestigationName() %>
<%}else { %> <label class="value" style="padding-left: 40px; width: 60px;">-</label>
<%} %> <% 
    try{
    	if(dgResultEntryDetailForSingleResult.getInvestigation().getMinNormalValue() != null
    		...
    		if(result1 <= maxValue && result1 >= minValue ){ %> <label
	class="value" style="width: 80px; padding-left: 70px;"><%=result%></label>
<% }else{ %> <label class="value"
	style="width: 80px; font-weight: bold; padding-left: 70px;"><%=result %></label>
<% }
    		
    		%> <% }else{ %> <label class="value"
	style="width: 80px; padding-left: 70px;"><%=dgResultEntryDetailForSingleResult.getResult()%></label>
<% } 
    }catch(Exception exception){ %> <label class="value"
	style="width: 80px; padding-left: 70px;"><%=dgResultEntryDetailForSingleResult.getResult()%></label>
...
<% }else{ %> <label class="value" style="width: 60px; padding-left: 10px;">-</label>
<% } %> <% if(dgResultEntryDetailForSingleResult.getInvestigation().getMinNormalValue() != null
    		&& dgResultEntryDetailForSingleResult.getInvestigation().getMaxNormalValue() != null){ 
    		...
    		%> <label class="value" style="width: 170px;"><%=minValue%>
- <%=maxValue %> </label> <% }else{ %> <label class="value" style="width: 170px;">-</label>
<% } %>