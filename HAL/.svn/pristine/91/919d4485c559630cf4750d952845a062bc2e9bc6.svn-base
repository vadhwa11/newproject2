...
%>
<label>HIN </label> 
<select name="<%=HIN_ID%>"	validate="Hin,String,no" onchange="submitProtoAjax('patientAppointment','/hms/hms/adt?method=getHinDetailsForAppointment')">
	...
	<% 
	     	...
	     	{ 
				...
				%>
	<option value="<%=patient.getId()%>"><%=patient.getHinNo()%>
	</option>
	<% } 
			} 
	     	%>
