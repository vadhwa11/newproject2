...
<form name="icdCausegrp" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasIcdcausegrp"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="IcdCauseName">
<input type="hidden" name="title" value="IcdCausegrp"> <input
	type="hidden" name="<%=JSP_NAME %>" value="icdCausegrp"> <input
	type="hidden" name="pojoPropertyCode" value="IcdCauseCode"> 


<div class="Clear"></div>
<div class="Block">
<label> Cause Code <span>*</span></label> <input id="codeId" type="text"
	name="<%= CODE%>" value="" validate="Cause Code,string,yes"
	 MAXLENGTH="8" tabindex=1> 
	 <label
	id=biglabel>Cause Name<span>*</span></label> <input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="Cause Name,string,yes"  MAXLENGTH="250"
	tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addIcdCause')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masIcdcausegrp.getLastChgBy() %>"
