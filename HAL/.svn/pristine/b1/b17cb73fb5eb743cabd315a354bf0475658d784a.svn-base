...
<h6>Patient Wise Films</h6>
<form name="patientWiseFilms" target="_blank" action="" method="post">
<div class="blockFrame"><label><span>*</span>From Date</label> <input
	...
	onClick="setdate('<%=currentDate %>',document.patientWiseFilms.<%=FROM_DATE%>,event)" />
...
<label><span>*</span>To Date</label> <input type="text" class="calDate"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.patientWiseFilms.<%=TO_DATE%>,event)" />
<div class="Clear"></div>
<label>Sub Group:</label> <select name="<%= SUB_CHARGECODE_ID %>"
	onchange="populateInvestigationName(this.value,'patientWiseFilms');"
	tabindex=1>
...
<div class="Clear"></div>
<label>Summary</label> <input type="radio" class="radio"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /> <label>&nbsp;</label>
<label>Detail</label> <input type="radio" class="radio"
	name="<%=SELECTED_RADIO  %>" value="2" /> <script
...
<div class="bottom"><input type="button" name="OK" value="OK"
	class="button"
	onClick="submitForm('patientWiseFilms','lab?method=generatePatientWiseFilms','validateFromToDate');" />
...
<script type="text/javascript" language="javascript">
	...
	function validateFromToDate(){
		...
		obj1 = eval(document.patientWiseFilms.fromDate)
		obj2 = eval(document.patientWiseFilms.toDate)
