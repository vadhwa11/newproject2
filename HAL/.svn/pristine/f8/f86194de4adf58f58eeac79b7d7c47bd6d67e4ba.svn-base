...
<ul class="categoryitems">
	...
	<li><a
		href="opd?method=viewOpdTemplateDepartmentWisePreNext&hinId=<%=visit.getHin().getId()%>&flag=current&<%=VISIT_NUMBER %>=<%=visit.getVisitNo() %>&visitNoForJsp=<%=visitNoForJsp %>&viewScreen=yes&deptId=<%=visit.getDepartment().getId() %>">Opd
...
<input name="consultationTime" type="hidden"
	...
	align="right" class="button" value="Back"
