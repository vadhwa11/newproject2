...
<%
	...
	if(session.getAttribute("deptId") != null){
		...
		// add by javed khan			
			...
			System.out.println(""+IndentDate1);
			System.out.println(IndentDate1.substring(0,2));
		       System.out.println("t "+IndentDate1.substring(3,5));
		       System.out.println("tII "+IndentDate1.substring(6,10));
			
				IndentDate=IndentDate1.substring(0,2)+"/"+IndentDate1.substring(3,5)+"/"
								+IndentDate1.substring(6,10);
				System.out.println(">>>>>>>>>>>>>> "+IndentDate);
			}
...
// add by javed khan
			...
			if(map.get("depart")!=null){
				...
				System.out.println("javed depart_id >>>>   "+depart_id+">>>>>>>>>>");
			}
...
<input type="text" class="date" name="<%=RequestConstants.ISSUE_DATE%>" id="<%= RequestConstants.ISSUE_DATE %>" readonly="readonly" value="<%=date %>"  MAXLENGTH="50" />
 ...
 <%if(deptId!=35){ %>
<label><span>*</span>Issue To</label>
...
<%} %>
<%if(deptId==35){ %>
<input type="radio" name="Issue" value="DIssue" id="depart" class="radioAuto" maxlength="30" tabindex=1 checked ="true" onClick="getUnit(this.value)"/>
	...
	<input type="radio" name="Issue" value="UIssue" id="unit"  class="radioAuto" maxlength="20" tabindex=1 onClick="getUnit(this.value)"/>
	 <label class="medium">Units</label> 
	 <%if(depart_id==0){ %>
...
<select name="<%= RequestConstants.ISSUED_BY%>" tabindex="1" id="issuedBy" validate="Issued By,String,yes" onchange="if(testForAdjustLoanOut()&&cheackForSelect())submitForm('issueDispensaryForm','stores?method=searchIndentDetailsWithoutBar');">
					<option value="">Select</option>
...
<input type="hidden" name="<%=RequestConstants.INDENT_ID %>" value="<%=indentId%>" id="hdb" />
  ...
  <tbody>
    ...
    //	int tempVar=((pageNo-1)*20)+1;
 	...
 	inc=((pageNo-1)*indentTList.size())+1;
 	 ...
 	 int incTemp2=inc+indentTList.size();
		...
		{
		 Object[] object = (Object[]) iterator.next();
		 System.out.println("item in yu jso>>>>. "+object[0].toString());
 		  tempVar--;
	...
	if(object[7] != null){
		...
		System.out.println("-qtyIssueRequest-- "+qtyIssueRequest);
	}
	...
	if(object[8] != null){
		...
		System.out.println("-qtyIssueDB-- "+qtyIssueDB);
	}
%>
		...
		<td>
	      ...
	      <% //} --%>
			...
			<%}%>
				...
				<td width="8%"> <input type="button" name="delete" value="" class="buttonDel" onclick="removeRow('indentDetails','hdb',this);" tabindex="1" /></td>
		</tr>
		<% inc++;}  }
...
<input type="button" class="button" tabindex="1" onclick="if(validateIssue()){submitForm('issueDispensaryForm','stores?method=submitIssueForIndentwithoutbar');}" name="Submit2" value="Save"  />
...
function addRowIssue(rowVal)
{
	  ...
	  e1.size='10';
	  //alert(document.getElementById('codeItem'+rowVal).value)
	      ...
	      if(xmlHttp.readyState==4){
		     ...
		     var pvms  = item.getElementsByTagName("pvms")[0];
		      ...
		      // var barcodeNo=item.getElementsByTagName("barCodeNo")[0];
		       ...
		       //alert("batchId<<<<"+bID.childNodes[0].nodeValue)
		     //  var cost=item.getElementsByTagName("costPrice")[0];
		      // alert("cost >>>>>> "+cost.childNodes[0].nodeValue)
