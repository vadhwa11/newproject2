...
<%
	...
	if (map.get("hrorderlyGuardDutyEntry")!=null)
	{
		hrorderlyGuardDutyEntry = (List)map.get("hrorderlyGuardDutyEntry");
	}
	if (map.get("GuardDutyHoliDay")!=null)
	{
		GuardDutyHoliDay = (List)map.get("GuardDutyHoliDay");
	}
...
function jsSearch()
{
	...
	{
		guardDutyAddForm.method="post";
		submitForm('guardDutyAddForm','hrOrderly?method=searchGuardDutyEntry');
	}
	else
    ...
    function getLastDutyDetails(empData)
    {
 		...
 		submitProtoAjaxWithDivName('guardDutyAddForm','/hms/hms/hrOrderly?method=getGuardEmployeeLastDutyDetails&empData='+empData,'DutyDetail');
    }
...
<%} %> <%		int k=0;
		...
		%> <%if(GuardDutyHoliDay!=null && GuardDutyHoliDay.size()>0){
		  int id=0;	
		  int counter=0;
		    for(HrorderlyGuardDutyEntry guardDutyForHoliday:GuardDutyHoliDay){
		    	if(guardDutyForHoliday.getEmp().getId()!=id){
		    		id=guardDutyForHoliday.getEmp().getId();%> <script>
		    		holiday[<%=counter%>]=new Array();
		    		holiday[<%=counter%>][0]=<%=id %>;
		    		
		    		</script> <%if(guardDutyForHoliday.getHolidayStatus().equals("y")){%> <script>holiday[<%=counter%>][1]=1;</script>
<%}else{%> <script>holiday[<%=counter%>][1]=0;</script> <%	}}
		    	counter++;
		    }}
		%> <%		int i=0;
...
<table align="center" width="100%" colspan="7" cellpadding="0"
	...
	<tbody>
		<%
			    ...
			    %>
		<tr>
			<td><label><%=masEmployee.getServiceNo()%></label></td>
			<td><label><%=masEmployee.getRank().getRankName()%></label></td>
			<td><a
				href="javascript:getLastDutyDetails(<%=masEmployee.getId()%>)"><%=masEmployee.getFirstName()+" "+ masEmployee.getMiddleName()+" "+ masEmployee.getLastName()%></a></td>
			<td style="width: 30px;"><input type="text" name="nextDutyDate"
				id="nextDutyDate<%=slno%>" onfocus="checkForNextDutyDate();"
