...
<div class="titleBg"><h2>Previous Investigation View</h2></div>
<div class="Clear"></div>
...
<script	type="text/javascript" language="javascript">
		...
		data_header[13][0] = "Disposal"
		data_header[13][1] = "data";
		data_header[13][2] = "10%";
		data_header[13][3] = "<%=RequestConstants.DISPOSAL_ID %>";

		data_header[14] = new Array;
		data_header[14][0] = "Diag No."
		data_header[14][1] = "data";
		data_header[14][2] = "10%";
		data_header[14][3] = "diagNo";
		data_arr = new Array();
		...
		%>
			...
			data_arr[<%= i%>][0] ="<%=visit.getId()%>"
				...
				data_arr[<%= i%>][1] = '<input type="radio" class="radiogrid" name="parent" value="<%=visit.getHin().getServiceNo()%>@<%=visit.getVisitNo()%>@<%=visit.getHin().getHinNo()%>@<%=visit.getId()%>@<%=visit.getHin().getId()%>@<%=investigationHeader.getId()%>" id="parent" onclick="fillVisitNo(this)"/>'
		...
		data_arr[<%= i%>][14] = "<%=disposal%>"	
		data_arr[<%= i%>][15] = "<%=diagNo%>"
		<% 	
			i++;
		  }
      	  } 
...
function openInvestigationDetails(){
	...
	if(flag == false){
		...
		submitProtoAjaxWithDivName('opdPatientPrevVisitForReport','/hms/hms/fwc?method=showPatientPreviousInvestigation','testP');
	...
	function submitFormForPrescriptionReport(){
		...
		if(flag == false){
			...
			submitForm('opdPatientPrevVisitForReport','/hms/hms/fwc?method=showPatientInvestigationReport');
			checkTargetForNo();
		}
