...
<%
		...
		if (map.get("complaintRegisterList") != null) {
			masComplaintRegister = (MasComplaintRegister)complaintRegisterList.get(0);
...
<label>Department </label> <select name="<%= DEPARTMENT_ID %>" id="dept"
	...
	onclick="submitForm('search','complaint?method=searchWorkNotCompletion')"
	tabindex=1 /></form>
...
<h5><a href="/hms/hms/complaint?method=showWorkNotCompletionJsp">Show
All Records</a></h5>
<%
			}
			}
...
<div class="Clear"></div>
<form name="workNotCompletion" method="post" action=""><input
...
<label><span>*</span> C Adm O's Remark</label> <textarea
	name="<%=REMARKS %>" value="" validate="Remarks,string,no"
	class="large" onKeyDown="limitText(this,200);" tabindex=1
	onkeypress="return submitenter(this,event,'complaint?method=addCAdmRemark')"> </textarea>

<input type="hidden" name="complaintId" id="complaintId" value="" />

<div class="Clear"></div>
<label>Commandant Remarks</label> <textarea
	name="<%=COMMANDENT_REMARKS %>" readonly="readonly" value=""
	class="large" onKeyDown="limitText(this,200);" tabindex=1
	onkeypress="return submitenter(this,event,'complaint?method=addCAdmRemark')"> </textarea>
...
<div class="bottom"><input type="button" name="add" id="edited"
	...
	onClick="submitForm('workNotCompletion','complaint?method=addCAdmRemark')"
...
function checkd()
{
var SDate = document.workNotCompletion.<%= WORK_COMPLETION_DATE%>.value;
var EDate = document.workNotCompletion.<%=COMPLAINT_DATE%>.value;
