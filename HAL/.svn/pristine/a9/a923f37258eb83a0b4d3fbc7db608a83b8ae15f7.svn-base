...
function checkForReturnDispensary(val,a,inc)
{
	   ...
	   // alert("checkForDefectiveDrugs"+pvms);
	    ...
	    ajaxFunctionForAutoCompleteInReturn('returnSearch','stores?method=fillItemsForDefectiveDrugs&pvmsNo=' + pvms , inc);
...
int pageNo=1;
	...
	try {
		...
		if(map.get("returnNo") != null){
			returnNo = ""+map.get("returnNo");
		}
		//out.println("returnNo   "+returnNo);
		
		returnNo = (String)map.get("returnNo");
		//out.print("returnNo="+returnNo);
 ...
 <select name="<%=ISSUE_RETURN_ID%>">
			...
			<%
						...
						Iterator iterator = searchList.iterator();
			    			...
			    			StoreGrnReturnM mObj = (StoreGrnReturnM) iterator.next();
		...
		<input type="image" class="button" value=""
			onClick="submitForm('search','stores?method=searchReturnToDispensary&pageNo=<%=pageNo-1%>&deptId=<%=deptId%>&buttonFlag=<%=buttonFlag%>');" />
			 ...
			 <input type="button" name="sss" class="button" value="Print" onClick="if(validateMetaCharacters(this.value)){submitForm('depCiv','stores?method=printVendorReturnJsp&deptId=<%=deptId%>')};" />
</form>
</div>
</form>
...
<select	name="vendorName" id="vendorName" validate="Vendor Name,String,yes" onchange="getPoList();">
	<option value="0">Select</option>
	<%
	for (MasStoreSupplier masSupplier : supplierList) {
	%>
	<option value="<%=masSupplier.getId() %>"><%out.print(masSupplier.getSupplierName()!=null?masSupplier.getSupplierName():"");%> </option>
	<%
	}
	%>
	</select>
...
<input type="button" class="button" value="Submit"	onclick="submitForm('returnSearch','stores?method=submitVendorReturnDetails');" />
...
<input type="button" name="sss" class="button" value="Reset" onclick="submitFormForButton('returnSearch','stores?method=showVendorReturnJsp');" /> 
<div class="clear"></div>
<div class="division"></div>
</form>
<script type="text/javascript">
...
function getPoList()
{
	...
	if(vendorName !=0)
		{
			submitProtoAjaxWithDivName('depCiv','/hms/hms/stores?method=getPOListbasedonYear&vendorName='+vendorName,'divPOList');
		}
