...
<div class="titleBg"><h2>Patient Medicine Issue</h2></div>
...
<!-- 			<th width="13%">Qty</br>on Loan</th> -->
            <th width="13%">Qty</br>Remaining</th>
			<th width="13%">Qty</br>Issue</th>
		...
		<tr>
		 ...
		 <input type="hidden" name="batchNoValue<%=inc%>" id="batchNoValue<%=inc%>" size="10" readonly="readonly" value="" />
			...
			BatchList = (List<Object[]>) mapbatch.get(object[0]);
								...
								I = object[0];
			%>
			<select name="batchNo<%=inc%>" id="batchNo<%=inc%>"  class="small3" onchange="if(document.getElementById('batchNo<%=inc%>').value != 0){if(cheackForBatch(<%=inc%>)){getDataForBarBatchStockId(this.value,<%=inc%>)}}" >
										...
										BigDecimal qtyIssueRequest = new BigDecimal(0);
														...
														if (object[8] != null) {
															qtyIssueDB = new BigDecimal(object[8]
																	.toString());
															System.out.println("ggg"+object[0]);
															System.out.println("ggg"+object[1]);
															System.out.println("ggg"+object[2]);
															System.out.println("ggg"+object[3]);
															System.out.println("ggg"+object[4]);
															System.out.println("ggg"+object[5]);
															System.out.println("ggg"+object[6]);
															System.out.println("ggg"+object[7]);
															System.out.println("ggg"+object[8]);
															System.out.println("ggg"+object[9]);
															System.out.println("ggg"+object[10]);
															System.out.println("ggg"+object[11]);
														}
									%>
			<td width="10%">
	
			<input type="text" tabindex="1" size="8"
				value="<%=qtyIssueRequest.intValue()%>" readonly="readonly"
				name="qtyRequested" id="qRequested<%=inc%>" />
...
<!-- 	<tr>
	...
	<input type="button" class="button" tabindex="1" onclick="if(validateIssue() && qtyValidation()){submitForm('issueDispensaryForm','stores?method=submitIssueForIndentForWard');}" name="Submit2" value="Submit"  />
...
%>
<!-- <input type="button" name="sss" class="button" value="SEARCH" onclick="getSearchBlock();" />  -->
<input type="button" name="sss" class="button" value="Request REPORT" onclick="generateMRReport()" /> 
...
function qtyValidation(){
	...
	for(var i=1;i<=obj.length;i++){
		var issue = 0;
		if(document.getElementById("batchNo"+i).value!='0' && document.getElementById("qtyIssued"+i).value=='')
			flag = false;
	}
	if(flag==false)
...
function generateRowIssueCIV(rowVal)
{
 ...
 // for(var i = 0;i<li;i++ ){
  ...
  e242.size = '10';
	  ...
	  e6.value=parseInt(document.getElementById('stockAvailable'+rowVal).value);
	  e6.size = '8';
	 // e6.value=document.getElementById('stockAvailable'+rowVal).value;
	  ...
	  var cellRight341 = row.insertCell(10);
	  var e341 = document.createElement('input');
	  e341.type = 'text';
	  e341.name = 'qRequested';;
	  e341.id = 'qRequested'+ iteration;;
	  e341.size = '8';
	  e341.value = parseInt(document.getElementById('qRequested'+rowVal).value);
...
function cheackForBatch(rowVal){
	...
	for(i=1;i<rowVal;i++){
		...
		alert(rowVal+" --i- "+document.getElementById("batchNo"+rowVal).value) */
