...
<h6>Stock Ledger</h6>
<form name="stockLedger" target="_blank" action="" method="post">
<div class="blockFrame"><label><span>*</span> From Date</label> <input
	...
	onClick="setdate('<%=currentDate %>',document.stockLedger.<%=FROM_DATE%>,event)" />
...
<label><span>*</span> To Date</label> <input type="text" class="calDate"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.stockLedger.<%=TO_DATE%>,event)" />

</div>
<div class="Clear"></div>
<div class="bottom"><input type="button" name="OK" value="OK"
	class="button"
	onClick="submitForm('stockLedger','bloodBank?method=printStockLedger','validateFromToDate');" />
...
function validateFromToDate(){
	...
	obj1 = eval(document.stockLedger.fromDate)
	obj2 = eval(document.stockLedger.toDate)
