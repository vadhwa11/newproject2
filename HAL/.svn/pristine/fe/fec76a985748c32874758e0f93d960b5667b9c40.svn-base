...
<%
	...
	try {
			...
			if(map.get("dmiArray") != null){
				dmiArray = (ArrayList<Object[]>)map.get("dmiArray");
			}
...
<!-- 			<th width="10%">Barcode</th> -->
			...
			<th width="5%">Route</th> -->
			<th width="2%">Total </br>Prescribed</th>
			<th width="2%">Qty </br>Prescribed</th>
			
			
			<th width="5%">Qty Issued</th>
			<th width="5%">Qty Issue</th>
<!-- 			<th width="8%">NIS</th> -->
	...
	if(loopCount == presList.size())
	{
		...
		Iterator itr= listOfItemsStock.iterator();
			...
			BigDecimal totalIssued = new BigDecimal(0.0);
			System.out.println("dmiArray"+dmiArray.size());
			for(Object[] dmi: dmiArray) {
				System.out.println("prescriptionDetails.getItem().getId()"+prescriptionDetails.getItem().getId().toString());
				System.out.println("dmi[0]"+dmi[0].toString());
				
				if(dmi[0].toString().equals(prescriptionDetails.getItem().getId().toString()))
				{
					totalIssued = (BigDecimal)dmi[1];
					
				}
			}
			
			BigDecimal totalPresQt = new BigDecimal(totalPresQty).setScale(2, BigDecimal.ROUND_HALF_UP);
			System.out.println("totalPresQt"+totalPresQt);
			System.out.println("totalIssued"+totalIssued);
			if(totalPresQt.compareTo(totalIssued)!=0)
			{
...
<%}%>
			...
			<td width="5%"><input type="text" size="6" value="<%=totalPresQt%>" name="totalPrescription<%=i%>" readonly="readonly" id="totalPrescription<%=i%>" /></td>
			<td width="5%"><input type="text" size="6" value="<%=totalPresQt.subtract(totalIssued)%>" name="qtyPrescription<%=i%>" readonly="readonly" id="qtyPrescription<%=i%>" /></td>
			<td width="5%"><input type="text" size="6" value="<%=totalIssued%>" name="totalIssued<%=i%>" readonly="readonly" id="totalIssued<%=i%>" /></td>				
...
<%-- 		<td width="7%"><input type="checkbox" name="nis<%=i%>" id="nis<%=i%>"   /></td> --%>
		...
		<%-- <input type="hidden" name="counter" id="listsize"	value="<%=presList.size() %>" /> --%>
		</table>
		<div class="Clear"></div>
		</div>
		<div class="Clear"></div>
...
<input type="hidden" name="counter" id="listsize"	value="<%=(i-1)%>" />
