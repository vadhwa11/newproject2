...
function add()
{
	...
	{
		var url="/hms/hms/hrRelated?method=showGuardDutyAddJsp&entryNo="+guardDuty.entryNo.value+"&entryDate="+guardDuty.entryDate.value+"&dutyId="+guardDuty.dutyId.value+"&toDate="+guardDuty.toDate.value+"&fromDate="+guardDuty.fromDate.value;
...
function GoPage() {	
	...
	submitForm('guardDuty','hrRelated?method=getGridDataForGuardEmployee');
}
...
function jsUpdate()
{
		...
		{
		guardDuty.method="post";
		submitForm('guardDuty','hrRelated?method=updateGuardDutyEntry','checkForNextDutyDate');
		}
		else
...
function jsDisplay() {
	...
	submitForm('searchPanel','hrRelated?method=searchGuardDutyData&searchEntryNo='+searchPanel.searchEntryNo.value+'&searchEntryDate='+searchPanel.searchEntryDate.value);
}  
...
<%
	...
	value="<%=box.get(TO_DATE)%>" MAXLENGTH="30" readonly="readonly" />
<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%	String msg="";
				...
				%>
		<tr>
			<td width="10%"><%=hrGuardDutyEntry.getEmp().getServiceNo()%></td>
			<td width="40%"><%=hrGuardDutyEntry.getEmp().getRank().getRankName()%></td>
			<td width="12%"><%=msg%></td>
			<td width="40%"><%=HMSUtil.convertDateToStringWithoutTime(hrGuardDutyEntry.getDutyDate())%></td>
			<td style="width: 60px"><input type="text" value=""
			...
			<td style="width: 140px;"><Select name="Location"
				...
				<%for(MasDepartment masdepartment:masdepartmentList){ 
					if(hrGuardDutyEntry.getDepartmentId().getId().equals(masdepartment.getId())){
					%>
			...
			<td style="width: 140px"><Select name="dutyTime"
				id="dutyTime<%=iFirstRow%>" style="width: 140px">
	...
	<tbody>
		...
		<tr>
			...
			<td width="12%"><input type="text"
				...
				readonly="readonly" /></td>
			<td width="40%"><input type="text"
				value="<%=HMSUtil.convertDateToStringWithoutTime(hrGuardDutyEntry.getDutyDate())%>"
...
<div class="bottom"><input type="button" class="button"
	...
	onClick="if (validateButton()){submitForm('guardDuty','hrRelated?method=deleteGuardDutyEntry');}else{alert('Pl. Select Employee to delete.');}"
