...
<script language="javascript">
	...
	submitForm('attachPhoto','stores?method=showChallanExlJsp&filename='+filename);
	}
	}
...
<%
		...
		if(map.get("preQty")!=null){
			preQty=(Map)map.get("preQty");
		}
		...
		System.out.print(preQty.entrySet());
		...
		List<MasStoreItem> itemList = new ArrayList<MasStoreItem>();
				...
				{
					netAmuont=(String)map.get("netAmuont");
				}
				Object [][]obj=null;
				if(map.get("obj")!=null){
				...
				String BillAmt="";
				if(obj[3][7]!=null){
					BillNoAct1=obj[3][7].toString();
				} 
				if(obj[4][1]!=null){
					BillDate=obj[4][1].toString();
				}
				if(obj[4][4]!=null){
					BillAmt=obj[4][4].toString();
				}
				String BillNoAct="";
				try{
				if(BillNoAct1!=null && BillNoAct1!=""){
				
				BillNoAct=BillNoAct1;
				}
				}catch(Exception e){}
				...
				ArrayList itemQtyDistList=new ArrayList();
		%>
<%
...
<input type="file" name="<%=UPLOAD_FILENAME %>" id="exlfileNameId"  style="height: 28px; width: 250px;   margin-right: 35PX " class="browse">
...
<label>SO No.<span>*</span></label> <select
...
<label >Amount Pending for Proforma B Entry</label>
		<input  type="text" name="amtForProB" id="amt_For_ProB"  value="<%=netAmuont%>"/> 
		<div class="clear"></div>
			<label >SO Date</label>
 	<input type="text" name="<%=LOANIN_DATE%>"	value="<%=currentDate %>"  class="" readonly="readonly" MAXLENGTH="30" />
<!--<div class="clear"></div>
...
<%if(storeGrnObj != null){ %> <input
	type="text" name="<%=CHALLAN_NO %>"  id="<%=INVOICE_NO%>"
	value="<%=storeGrnObj.getInvoiceNo()%>"
	validate="Invoice No, String, no" maxlength="30" tabindex=1 /> <%}

else{ %>
<input type="text" name="<%=CHALLAN_NO %>" id="<%=INVOICE_NO%>" value="<%=BillNoAct%>"
	validate="Invoice No,String,no" MAXLENGTH="30" tabindex=1 /> <%} %> 
	
	
	<label>Challan
Date</label> <input type="text" class="date" name="<%=CHALLAN_DATE%>" readonly
...
</select> --> <label>Unpacked-Checked By<span>*</span></label> 
<select
...
<table >
	<thead>
		<tr>
			...
			<th width="9%">A/U</th>
			<!-- Added By Ritu Date 26 June 2012 -->
			<th>S B/G</th>	
			<th>S Brand</th>	
			<th>S Manufacturer</th>	
	...
	<tbody>
		...
		double netAmoutCal=0.0;
			...
			if(obj!=null)
				...
				for(int i=7;i<l;i++){%>
				    ...
				    System.out.println("TPPPPPPPPPP---"+i);
				    System.out.println("TPPPPPPPPPPPPPPPP---"+brand[0]);
				    if(brand.length>0)
				    {
				        actBrand=brand[0].split(":");
				    }
				    System.out.println("actBrand---"+actBrand[0]);
				    String qtySupply=obj[i][10].toString();
				    String qtyDemanded=preQty.get(item[0]).toString();
				    System.out.println(qtySupply);
				    System.out.println(qtyDemanded);
				    String qtySplit[]=qtyDemanded.split(",");
				    System.out.println("this is qty length"+qtySplit.length);
				    System.out.println(pre.get(item[0]).toString());
				    if(qtySplit.length==1){
					inc = inc+1;
			        %>
		 <tr>
			<td width="10%"><input type="text" value="<%=inc%>" class="smcaption" name="<%=SR_NO%>" readonly="readonly"  size="3"/></td>
			<%
			%>
			<td width="10%">
			<input type="hidden" value="<%=item[0]%>" class="medcaption" id="<%=idItem+""+inc%>" name="<%=ITEM_ID%>" readonly="readonly" />
			<input type="text" value="<%=item[1]%>" class="medcaption" id="<%=codeItem+""+inc%>"  name="<%=ITEM_CODE %>" readonly="readonly"  size="10"/></td>
			<td width="55%"><input type="text" value='<%=obj[i][2]%>' class="bigcaption" id="<%=nameItem+""+inc%>" name="<%=nameItem%>"   readonly="readonly"  size="50"/></td>
			<td width="25%"><input type="text" value="" id=""<%=barCodeId+""+inc%>" name="barCodeNo"  class="medcaption" size="10"/></td>
			
			<td width="25%"><input type="text" value="<%=obj[i][4]%>" name="<%=BG%>"  id="<%=bg+""+inc%>"  class="medcaption"readonly="readonly" size="1"/></td>
			
			<td width="25%">
			<%if(actBrand!=null ){%>
			<input type="text" value="<%=actBrand[0]%>" id="" name="<%=BRAND%>"  class="medcaption"readonly="readonly" size="10"/>
			<%}else{%>
			<input type="text" value="" id="" name="<%=BRAND%>"  class="medcaption"readonly="readonly" size="10"/>
			<%}%>
			<%if(brand.length>1){%>
			<input type="hidden" value="<%=brand[1]%>" id="<%=brandId+""+inc%>" name="<%=BRAND_ID%>"  class="medcaption"readonly="readonly"  size="10"/>
			<%}else{%>
			<input type="hidden" value="" id="<%=brandId+""+inc%>" name="<%=BRAND_ID%>"  class="medcaption"readonly="readonly"  size="10"/>
			<%}%>
			</td>
			<td width="25%">
				...
				<%}else{%>
				<input type="text" value="<%=manu[0]%>" id="" name="<%=MANUFACTURER_NAME%>" class="medcaption"readonly="readonly"  size="15"/>
				<%}%>
				<%if(manu.length>1){%>
				<input type="hidden" value="<%=manu[1]%>" id=<%=manufacturerId+""+inc%> name="<%=MANUFACTURER_ID %>" class="medcaption"readonly="readonly"  size="15"/>
				<%}else{%>	
				<input type="hidden" value="" id=<%=manufacturerId+""+inc%> name="<%=MANUFACTURER_ID %>" class="medcaption"readonly="readonly"  size="15"/>
				<%}%>		
			</td>
			<td width="25%"><input type="text" value="<%=obj[i][3]%>" id="<%=idAu+""+inc%>" name="<%=AV%>" class="medcaption" readonly="readonly" size="5"/></td>
		<!-- Added By Ritu Date 26 June 2012 -->
			<td width="25%"><input type="text" value="<%=obj[i][11]%>" name="suppliedBG"  id="suppliedBG<%=inc%>"  class="medcaption"readonly="readonly" size="1"/></td>
		
			<%if(obj[i][12].equals("na")){%>
			<td width="25%"><input type="text" value="" name="suppliedBrand"  id="suppliedBrand<%=inc%>"  class="medcaption"readonly="readonly" size="5"/></td>
			<%}else{%>
			<td width="25%"><input type="text" value="<%=obj[i][12]%>" name="suppliedBrand"  id="suppliedBrand<%=inc%>"  class="medcaption"readonly="readonly" size="5"/></td>
			<%}%>
			
			<%if(obj[i][13].equals("na")){%>
				<td width="25%"><input type="text" value="" name="suppliedManufacturer"  id="suppliedManufacturer<%=inc%>"  class="medcaption"readonly="readonly" size="10"/></td>
		<%}else{%>
				<td width="25%"><input type="text" value="<%=obj[i][13]%>" name="suppliedManufacturer"  id="suppliedManufacturer<%=inc%>"  class="medcaption"readonly="readonly" size="8"/></td>
		<%}%>
		
		
		<td width="25%"><input type="text" value="<%=obj[i][10]%>" name="suppliedQty"  id="suppliedQty<%=inc%>"  class="medcaption"readonly="readonly" size="8"/></td>
		
		
		<!-- End -->
			<td width="25%"><input type="text" value="<%=obj[i][15]%>" id="<%=batchNo+""+inc%>" name="<%=BATCH_NO %>"  class="medcaption"readonly="readonly"  size="10"/></td>
			<%
			String toManDate="";
			String toExpDate="";
			if(obj[i][16]!=null){
		         toManDate=obj[i][16].toString();
				StringBuffer s1 = new StringBuffer(toManDate);
				if(toManDate.length()==8){
				toManDate=s1.insert(6,"20").toString();
				}else if(toManDate.length()==7){
					s1=s1.insert(5,"20");
					toManDate=s1.insert(3,"0").toString();
				}else{
					s1=s1.insert(4,"20");
					s1=s1.insert(2,"0");
					toManDate=s1.insert(0,"0").toString();
				}
			}
			if(obj[i][18]!=null){
				toExpDate=obj[i][18].toString();
				StringBuffer s1 = new StringBuffer(toExpDate);
				if(toExpDate.length()==8){
					toExpDate=s1.insert(6,"20").toString();
					}else if(toExpDate.length()==7){
						s1=s1.insert(5,"20");
						toExpDate=s1.insert(3,"0").toString();
					}else{
						s1=s1.insert(4,"20");
						s1=s1.insert(2,"0");
						toExpDate=s1.insert(0,"0").toString();
					}
			}
			%>
			
			
			<td width="25%"><input type="text" value="<%=toManDate%>" id="<%=manufacturingDate+""+inc%>" name="<%=MANUFACTURING_DATE%>" class="medcaption"readonly="readonly"  size="9"/></td>
			
			<td width="0%"><input type="hidden" value="<%=obj[i][16]%>"  class="medcaption"readonly="readonly" /></td>
			
			
			
			
			<td width="25%"><input type="text" value="<%=toExpDate%>" id="<%=expiryDate+""+inc %>" name="<%=EXPIRY_DATE%>"  class="medcaption"readonly="readonly"  size="9"/></td>
			<td width="0%"><input type="hidden" value="<%=obj[i][18]%>" name="<%=AU%>" class="medcaption"readonly="readonly" size="5"/></td>
			<td width="25%"><input type="text" value="<%=obj[i][14]%>" name="<%=QUANTITY_RECEIVED %>" size="8" tabindex="1" id="<%=quanRec+""+inc%>"	onblur="calculationInCRV();" tabindex="1" class="medcaption"readonly="readonly" class="medcaption"  size="6"/></td>
			<td width="25%"><input type="text" value="<%=obj[i][8]%>" name="dispenseType" id=<%=dispenseType+""+inc%> class="medcaption"readonly="readonly" size="10" /></td>
			<td width="25%"><input type="text" value="<%=obj[i][9]%>" name="mdq" id="<%=mdq+""+inc%>" tabindex="1" class="medcaption"readonly="readonly" size="7"/></td>
			<td width="25%"><input type="text" value="<%=obj[i][20]%>" name="ratePerMdq" size="8" id="<%=ratePerMdq+""+inc%>" tabindex="1" onblur="calculationInCRV();" tabindex="1" class="medcaption"readonly="readonly" size="6"/></td>
			<td width="25%"><input type="text" value="<%=obj[i][21]%>" name="<%=DISCOUNT_PERCENTAGE%>" size="5" tabindex="1"	id="<%=discountVar+""+inc%>" onblur="calculationInCRV();" class="medcaption"readonly="readonly"  size="8"/>
			<%
			double d=0.0;
			if(obj[i][21]!=null){
			double j=Double.parseDouble(obj[i][21].toString());
			if(obj[i][10]!=null && obj[i][20]!=null)
			{
				double TotalAmt2=Double.parseDouble(obj[i][14].toString())*Double.parseDouble(obj[i][20].toString());
				TotalAmt2=(TotalAmt2*j)/100.0;
				d=TotalAmt2;
			}
		   ...
		   // d=j;
			...
			%>
			<input type="hidden" value="<%=d%>" name="discountAmount"	id="<%=discountAmount+""+inc %>" tabindex="1" size="8"/></td>
			
			<td width="25%"><input type="text" value="<%=obj[i][22]%>" name="<%=TAX_PERCENT %>" tabindex="1" id="<%=taxVar+""+inc%>" onblur="calculationInCRV();" size="6"/></td>
			<td width="25%">
			<input type="hidden" value="0"	 name="<%= COST_PRICE %>" id="<%=costPrice+""+inc%>" tabindex="1"/>
			<input type="text"  value="<%=obj[i][23]%>"  name="<%=AMOUNT%>" size="8"	id="<%=amtVar+""+inc%>" readonly="readonly" tabindex="1"  size="8"/>
			<%
			...
			if(obj[i][23]!=null){
				netAmoutCal=netAmoutCal+Double.parseDouble(obj[i][23].toString());
			}
		...
		if(checkItemList.contains(item[0]))
		{
			System.out.println("this is duduplicate item"+item[0]);
			Resuffel="yes";
			for(int h=0;h<qtySplit.length;h++){
				double temReQTY=Double.parseDouble(itemQtyDistList.get(h).toString());
				double qtyReDemandCheck=Double.parseDouble(qtySplit[h]);
				double qtyReSuffel=qtyReDemandCheck-temReQTY;
				xyz.add(qtyReSuffel);
			}
			itemQtyDistList=new ArrayList();
		}
		
		checkItemList.add(item[0]);
		
	   double qtySupplyCheckTemp=Double.parseDouble(obj[i][14].toString());
       for(int k=0;k<qtySplit.length;k++){
    	   String PatientId=pre.get(item[0]).toString();
    	   String PatientIdCheck[]=PatientId.split(",");
    
    	  	double qtySupplyCheck=Double.parseDouble(qtySupply);
    	  	
    	  	
    	  	double qtyDemandCheck=Double.parseDouble(qtySplit[k]);
    	  	
    		
    	  	double qtyRecivetCheck=0.0;
    	  	double lastQty=qtySupplyCheck;
    	  	qtySupplyCheck=qtySupplyCheck-qtyDemandCheck;
    	  	if(qtyDemandCheck>0.0){
    	  		qtyRecivetCheck=qtyDemandCheck;
    	  	}else{
    	  		qtyRecivetCheck=lastQty;
    	  	}
    	  	
    		System.out.println("this is qty supplied check"+qtySupplyCheckTemp);
    		double qtyDemandCheckTemp=0.0;
    		if(Resuffel.equalsIgnoreCase("yes")){
    			qtyDemandCheckTemp=Double.parseDouble(xyz.get(k).toString());
    		}else{
    			qtyDemandCheckTemp=Double.parseDouble(qtySplit[k]);
    		}    	  	
    	  	System.out.println("this is qty demanded check"+qtyDemandCheckTemp);
    	  	double qtyRecivetCheckTemp=0.0;
    	  	double lastQtyTemp=qtySupplyCheckTemp;
    	  	qtySupplyCheckTemp=qtySupplyCheckTemp-qtyDemandCheckTemp;
    	  	System.out.println("this is my check sum>>>>>>>>>>>>>>>>>>>>>>>>>"+qtySupplyCheckTemp);
    	  	System.out.println("this is my check >>>>>>>>>>>>>>>>>>>>>>>>>"+lastQtyTemp);

    	  	if(qtySupplyCheckTemp>0.0){
    	  		qtyRecivetCheckTemp=qtyDemandCheckTemp;
    	  	}else{
    	  		if(lastQtyTemp>0.0){
    	  		qtyRecivetCheckTemp=lastQtyTemp;
    	  		}else{
    	  			qtyRecivetCheckTemp=0.0;
    	  		}
    	  	}
    	  	
    	  	
    	  	itemQtyDistList.add(qtyRecivetCheckTemp);
    	  	
    	  	
    	  	
    	  	System.out.println("this is my check >>>>>>>>>>>>>>>>>>>>>>>>>-----------"+qtyRecivetCheckTemp+"this is itemqtydistcheck"+itemQtyDistList);
...
<td width="10%">
<input type="hidden" value="<%=item[0]%>" class="medcaption" id="<%=idItem+""+inc%>" name="<%=ITEM_ID%>" readonly="readonly" />
<input type="text" value="<%=item[1]%>" class="medcaption" id="<%=codeItem+""+inc%>"  name="<%=ITEM_CODE %>" readonly="readonly"  size="8"/></td>
<td width="55%"><input type="text" value='<%=obj[i][2]%>' class="bigcaption" id="<%=nameItem+""+inc%>" name="<%=nameItem%>"   readonly="readonly"  size="50"/></td>
<td width="25%"><input type="text" value="" id=""<%=barCodeId+""+inc%>" name="barCodeNo"  class="medcaption" size="10"/></td>

<td width="25%"><input type="text" value="<%=obj[i][4]%>" name="<%=BG%>"  id="<%=bg+""+inc%>" name="<%=BRAND_ID%>"  class="medcaption"readonly="readonly" size="5"/></td>
...
<td width="25%"><input type="text" value="<%=obj[i][3]%>" id="<%=idAu+""+inc%>" name="<%=AV%>" class="medcaption" readonly="readonly" size="5"/></td>
<!-- Added By Ritu Date 26 June 2012 -->
<td width="25%"><input type="text" value="<%=obj[i][11]%>" name="suppliedBG"  id="suppliedBG<%=inc%>"  class="medcaption"readonly="readonly" size="5"/></td>
<td width="25%"><input type="text" value="<%=obj[i][13]%>" name="suppliedBrand"  id="suppliedBrand<%=inc%>"  class="medcaption"readonly="readonly" size="5"/></td>
<td width="25%"><input type="text" value="<%=obj[i][12]%>" name="suppliedManufacturer"  id="suppliedManufacturer<%=inc%>"  class="medcaption"readonly="readonly" size="5"/></td>
		
		
		<td width="25%"><input type="text" value="<%=qtyRecivetCheck%>" name="suppliedQty"  id="suppliedQty<%=inc%>"  class="medcaption"readonly="readonly" size="5"/></td>
		
		
		<!-- End -->

<td width="25%"><input type="text" value="<%=obj[i][15]%>" id="<%=batchNo+""+inc%>" name="<%=BATCH_NO %>"  class="medcaption"readonly="readonly"  size="15"/></td>
<%
String toManDate="";
String toExpDate="";
if(obj[i][16]!=null){
...
<td width="25%"><input type="text" value="<%=toManDate%>" id="<%=manufacturingDate+""+inc%>" name="<%=MANUFACTURING_DATE%>" class="medcaption"readonly="readonly"  size="11"/></td>
...
<td width="25%"><input type="text" value="<%=toExpDate%>" id="<%=expiryDate+""+inc %>" name="<%=EXPIRY_DATE%>"  class="medcaption"readonly="readonly"  size="11"/></td>
<td width="0%"><input type="hidden" value="<%=obj[i][18]%>" name="<%=AU%>" class="medcaption"readonly="readonly" size="5"/></td>
<td width="25%"><input type="text" value="<%=qtyRecivetCheckTemp%>" name="<%=QUANTITY_RECEIVED %>" size="8" tabindex="1" id="<%=quanRec+""+inc%>"	onblur="calculationInCRV();" tabindex="1" class="medcaption"readonly="readonly" class="medcaption"  size="8"/></td>
<td width="25%"><input type="text" value="<%=obj[i][8]%>" name="dispenseType" id=<%=dispenseType+""+inc%> class="medcaption"readonly="readonly" /></td>
<td width="25%"><input type="text" value="<%=obj[i][9]%>" name="mdq" id="<%=mdq+""+inc%>" tabindex="1" class="medcaption"readonly="readonly" size="8"/></td>
<td width="25%"><input type="text" value="<%=obj[i][20]%>" name="ratePerMdq" size="8" id="<%=ratePerMdq+""+inc%>" tabindex="1" onblur="calculationInCRV();" tabindex="1" class="medcaption"readonly="readonly" size="8"/></td>
<td width="25%"><input type="text" value="<%=obj[i][21]%>" name="<%=DISCOUNT_PERCENTAGE%>" size="5" tabindex="1"	id="<%=discountVar+""+inc%>" onblur="calculationInCRV();" class="medcaption"readonly="readonly"  size="8"/>
<%
double d=0.0;
if(obj[i][21]!=null){
...
if(obj[i][10]!=null && obj[i][20]!=null)
{
	double TotalAmt2=qtyRecivetCheckTemp*Double.parseDouble(obj[i][20].toString());
	TotalAmt2=(TotalAmt2*j)/100.0;
	d=TotalAmt2;
	System.out.println("this is discount amount"+d);
}
//d=j;
...
<td width="25%"><input type="text" value="<%=obj[i][22]%>" name="<%=TAX_PERCENT %>" tabindex="1" id="<%=taxVar+""+inc%>" onblur="calculationInCRV();" size="8"/></td>
<td width="25%">
<%
double f=0.0;
double unitRate=0.0;
...
if(obj[i][20]!=null){
	unitRate=Double.parseDouble(obj[i][20].toString());	
}
if(obj[i][22]!=null){
	taxTemp=Double.parseDouble(obj[i][22].toString());	
}

if(qtyRecivetCheckTemp>0.0 && unitRate>0.0){
      totalAmount= qtyRecivetCheckTemp*unitRate;
if(d>0.0){
	// double disAmount=totalAmount*(d/100);
	 totalAmount=totalAmount-d;
	 }
if(taxTemp>0.0){
	 double taxAmount1=totalAmount*(taxTemp/100);
	 totalAmount=totalAmount+taxAmount1;
}
}
%>
<input type="hidden" value="0"	 name="<%= COST_PRICE %>" id="<%=costPrice+""+inc%>" tabindex="1"/>
<input type="text"  value="<%=totalAmount%>"  name="<%=AMOUNT%>" size="8"	id="<%=amtVar+""+inc%>" readonly="readonly" tabindex="1"  size="8"/>
<%
...
if(obj[i][23]!=null){
	netAmoutCal=netAmoutCal+totalAmount;
}
%>
<input type="hidden" name="loanInItem" value="Yes" id="loanInItem" />
 	 <%if(PatientIdCheck[k].equals("na")){%>
<input type="hidden" name="PreId" id="<%="PreId"+""+inc%>" value="0"/>
<%}else{
%>
<input type="hidden" name="PreId" id="<%="PreId"+""+inc%>" value="<%=PatientIdCheck[k]%>"/>
<%}%></td>
<!--
 <td><input type="checkbox" name="check_one" id="chk" value="" /></td>
-->
...
//}
%>
<input type="text" id="total_amount" name="actualGrnValue" value="<%=df.format(netAmoutCal)%>" readonly="readonly" tabindex="1"/>
</div>
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
<input type="button" name="sss" align="right" class="button"
	value="Submit" onclick="submitForm('loanGrid','stores?method=submitLoanIn');"/>
<div class="clear"></div>
<div class="division"></div>
<div class="clear"></div>
