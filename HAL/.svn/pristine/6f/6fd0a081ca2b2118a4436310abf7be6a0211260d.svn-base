...
function showReport()
{
if (PNCForm.<%=TENDER_NO%>.value=="" || PNCForm.<%=TENDER_SUPPLIER_GROUP_ID %>.value=="")
{
...
{
//PNCForm.<%=TENDER_SUPPLIER_GROUP_ID%>.options.length=1;
PNCForm.method="post";
submitForm('PNCForm','tender?method=showChartPostPNCJsp&groupId='+PNCForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value);
}
</script>

<form name="PNCForm"><input type="hidden" name="hospitalId"
...
<h6>Tender Comparative Post PNC Data</h6>
<div class="Clear"></div>
<div class="blockTitle">Tender Comp. Post PNC</div>
