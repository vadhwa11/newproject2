...
<h4><%=deptName%></h4>
<div class="Clear"></div>
<%if(waitingCount !=0){ %> <label class="noWidth"><span><%=waitingCount%>
Patients are waiting for Acknowledgement</span></label> <%} %>
<div class="Clear"></div>
...
<%for(UserButtonRights  userButtonRights : userRightsList){
		...
		%><!--
		<input type="button"	name="<%=userButtonRights.getButton().getButtonName() %>"	class="<%=userButtonRights.getButton().getClassName() %>" onClick="submitForm('patientList','<%=userButtonRights.getButton().getUrl() %>','<%=formulaUsed%>');" />
--><%}else{%>
<!--<input type="button"	name="<%=userButtonRights.getButton().getButtonName() %>"	class="<%=userButtonRights.getButton().getClassName() %>" align="left"	onClick="submitForm('patientList','<%=userButtonRights.getButton().getUrl() %>','');" />
...
<input type="image"	src="/hms/jsp/images/buttonIcons/nursingEntry.jpg" class="button"	onClick="submitForm('patientList','ipd?method=showNursingCareEntryJsp');"	/>  
<input type="image"	src="/hms/jsp/images/buttonIcons/clinicalChart.jpg" class="button"	onClick="submitForm('patientList','ipd?method=showNursingClinicalChartJsp');"	/>  
...
<input type="image" src="/hms/jsp/images/buttonIcons/monthlyIndent.jpg" class="button" onClick="submitForm('patientList','stores?method=showDepartmentIndentJsp');"	/>  
	...
	<input type="button" class="sildilButt" value=" " align="left"     onClick="submitForm('patientList','ipd?method=showSilDilJsp','validateRadio');" />	
...
<script type="text/javascript" language="javascript">
		...
		%>
			...
			String ptName ="";
			   ...
			   {
				   try{
				    ptName=patientHin.getPFirstName()+" "+patientHin.getPMiddleName()+" "+patientHin.getPLastName();
			...
			<%
			String str ="";
			System.out.print("patientHin.getServiceNo().length() "+patientHin.getServiceNo().length());
