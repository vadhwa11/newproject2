...
<%@page import="jkt.hms.masters.business.StoreLoanoutExpendT"%><script type="text/javascript" src="/hms/jsp/js/autoComplete1.js"></script>
...
<%	
	Map map = new HashMap();
		...
		List<StoreLoanoutExpendT> searchList = new ArrayList<StoreLoanoutExpendT>();
		System.out.println("ssssssss > "+map.get("searchList"));
		try{
			searchList=(List)map.get("StoreLoanoutExpendTList");
...
<script type="text/javascript" language="javascript">
		...
		//Iterator itr=objectList.iterator();
		          ...
		          int a=0;
		         // while(itr.hasNext())
		        	 for(StoreLoanoutExpendT slet: searchList)
		           {           
...
<input type="hidden" name="Modify" type="submit" value="Modify" 	class="button" onClick="if(requiredFields()){submitForm('searchCiv','stores?method=searchIssueCiv');}">
<script type="text/javascript">
