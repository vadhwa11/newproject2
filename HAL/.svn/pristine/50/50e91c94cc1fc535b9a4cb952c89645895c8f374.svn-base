...
<div class="titleBg">
<h2>Stock Status Report- Stores,CD and WP </h2>
</div>
...
<input type="button" name="add" id="addbutton" value="Search" class="button" onClick="GetStockReport1('FILTER');" accesskey="a" tabindex=1 />


<input type="reset" name="Reset" id="reset" value="Show All" class="button" onclick="GetStockReport('ALL');" accesskey="r" /> 
...
<div id="divSearchResult">
  	...
  	<table id="tblSearchResultsHeader" class="tblSearchResultsHeader" cellspacing="0" cellpadding="0" border="0">
			<tr>				
				...
				<th id="th4" style='width: 120px;'>	Stores Stock</th>	
				<th id="th4" style='width: 100px;'>	Dispensary Stock</th>				
				<th id="th5" style='width: 150px;'>Ward Pharmacy Stock</th>			
			</tr>
			<tbody id="tblListOfStockReport">
			</tbody>
			</table>			
...
$j(document).ready(function()
		...
		function GetStockReport1(MODE)
		{
			nPageNo = 1;
			GetStockReport(MODE)
		}
		
function GetStockReport(MODE)
{
	...
	var url = "stores?method=getStockDetailsForDepartment";
	var bClickable = false;
	GetJsonData('tblListOfStockReport',data,url,bClickable);
}
...
function makeTable(jsonData)
{
	...
	for(i=0;i<jsonData.length;i++)
		{			
			...
			htmlTable = htmlTable +"<td style='width: 120px;'>"+jsonData[i].Stores+"</td>";
			htmlTable = htmlTable +"<td style='width: 120px;'>"+jsonData[i].CD+"</td>";
			htmlTable = htmlTable +"<td style='width: 150px;'>"+jsonData[i].WP+"</td>";
	...
	$j("#tblListOfStockReport").html(htmlTable);	
