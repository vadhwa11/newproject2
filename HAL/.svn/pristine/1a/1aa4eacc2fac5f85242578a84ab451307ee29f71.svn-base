...
<script>
	...
	if (TenderComPriorPNCForm.<%=TENDER_NO%>.value=="" || TenderComPriorPNCForm.<%=TENDER_SUPPLIER_GROUP_ID %>.value=="")
	{
	alert('Pl. Check Your Inputs (Tender No & Group)');
	return;
	}
	
	TenderComPriorPNCForm.method="post";
	submitForm('TenderComPriorPNCForm','tender?method=printReportChartPriorPNCJsp');
	}

	function onChangeTender()
	{
	//TenderComPriorPNCForm.<%=TENDER_SUPPLIER_GROUP_ID%>.options.length=1;
	TenderComPriorPNCForm.method="post";
	submitForm('TenderComPriorPNCForm','tender?method=showChartPriorPNCJsp&groupId='+TenderComPriorPNCForm.<%=TENDER_SUPPLIER_GROUP_ID%>.value);
	}
</script>

<form name="TenderComPriorPNCForm"><input type="hidden"
...
<h6>Tender Comparative Prior PNC Data</h6>
<div class="Clear"></div>
<div class="blockTitle">Tender Comp. Prior PNC</div>
...
<div class="Clear"></div>
<input type="button" name="softcopy" onClick="printReport()"
