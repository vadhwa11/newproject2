...
<%
	     if (visitList!=null && visitList.size() > 0 )
	     	{
	     %> <select name="visitList" id="visitNoReport" onchange="return getVisit(this.value)"	>
	...
	<%
	    ...
	    String visitdate="";
	   		...
	   		%>
	<option value="<%=patientPrescriptionHeader.getVisit().getId()+"@"+patientPrescriptionHeader.getStatus()%>"><%=patientPrescriptionHeader.getVisit().getVisitNo()+ " (" + visitdate+"  -  "+ department+")" %>
	</option>
	<% }
	     	 %>
</select> <%} else{
