...
<form name="getReferralWaitingList" method="post" action="">
    <h4><%=message %></h4>
	<h2>Invoice for Division Already Send</h2>

	<div class="Block" style="padding-top: 20px;">
	
	<label>Emp No</label>

<input type="text" name ="empNo" id="empNo" value="" onchange="GetPatientList1('FILTER');">

<label>Referral No</label>

<input type="text" name ="referralNo" id="referralNo" value="" onchange="GetPatientList1('FILTER');">
...
<select name ="divisionId" id="divisionId"  onchange="GetPatientList1('FILTER');">
<option value="0">Select</option>
	<%
		...
		<div id="divSearchResult">
			...
			<table id="tblSearchResultsHeader" class="tblSearchResultsHeader"
				...
				<tr>
					...
					<th id="th4" style='width: 120px;'>Treatment Type</th>
           ...
           <!--  <input type="button" name="yes" value="Print Covering Letter"	class="button"	onclick="submitForm('getReferralWaitingList','/hms/hms/referral?method=showInvoiceReport&flag=covering_letter');" /> -->
...
<script language="javascript">
	...
	function GetPatientList(MODE) {
		...
		var referralNo = $j("#referralNo").val();
		...
		if (MODE == 'ALL') {
			...
			var data = "PN=" + nPageNo + "&impHospitalId=" + impHospitalId+"&empNo="+empNo+"&rankCategoryId="+rankCategoryId+"&divisionId="+divisionId+"&referralNo="+referralNo;
		}
		var url = "referral?method=getInvoiceWaitingListAlreadySend";
		var bClickable = true;
	...
	function executeClickEvent(Id)
	 ...
	 window.location="referral?method=generateInvoicePageAlreadySend&opdId="+Id;
	 }
