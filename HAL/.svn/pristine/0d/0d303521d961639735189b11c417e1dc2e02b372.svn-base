...
<%
	...
	if (map.get("patientInvestigationList") != null) {
		patientInvestigationdetails = (List) map
				.get("patientInvestigationList");
...
<%
	if (patientInvestigationdetails.size() > 0) {
		PatientInvestigationDetails pInvestigationdetails = patientInvestigationdetails
				.get(0);
		Visit visit = pInvestigationdetails.getInvestigationHeader()
				.getVisit();
%>
<div class="Block">
...
<%
		...
		if (patientInvestigationdetails != null
			...
			for (PatientInvestigationDetails patientInvestigation : patientInvestigationdetails) {
				...
				investigationName = patientInvestigation
						...
						+ "]";
				first.put(investigationName, patientInvestigation
						.getChargeCode().getId());
				third.put(investigationName, patientInvestigation
						.getId());
				int visitId=0;
				if(patientInvestigation.getInvestigationHeader().getVisit()!=null){
					visitId=patientInvestigation.getInvestigationHeader().getVisit().getId();
					fourth.put(visitId, patientInvestigation.getInvestigationHeader().getVisit().getId());
				}
				String val = "";
				String val1 = "";
				int investigationId = 0;
				patientInvestigation.getInvestigationHeader()
						.getVisit().getId();
				
				/*
				if (resultList.size() > 0 && inc1 <= resultList.size()) {
					DgResultEntryHeader dgEH = (DgResultEntryHeader) resultList
							.get(inc1 - 1);
					for (DgResultEntryDetail dgre : dgEH
							.getDgResultEntryDetails()) {
						if (dgre.getSubInvestigation() != null)
							val1 = dgre.getSubInvestigation()
									.getSubInvestigationName();
						if (!dgre.getResultType().equalsIgnoreCase("t")) {
							val = val + "," + val1 + ":"
									+ dgre.getResult();
						}
						investigationId = dgre.getInvestigation()
								.getId();
			...
			/*for (Iterator i = new InvestigationDetailByInvestigationId()
					.sortByValue(first).iterator(); i.hasNext();) {
				...
				*/
				for (PatientInvestigationDetails patientInvestigation : patientInvestigationdetails) {
					...
					chargeCodeId = patientInvestigation.getChargeCode().getId();
	...
	<td><%=inc%></td>
			...
			readonly="readonly" size="70" value="<%=patientInvestigation.getChargeCode().getChargeCodeName() %>"/> 
	<input type="hidden" value="<%=patientInvestigation.getChargeCode().getId() %>"  id="chargeCodeId<%=inc%>" name="chargeCodeId<%=inc%>" />
	<input type="hidden" value="<%=orderNo %>"  id="orderNo<%=inc%>" name="orderNo<%=inc%>" />
			...
			tabindex="1" value="" /></td>
			 	<%if(resultList.size()>0){
					for (Iterator iterator = resultList.iterator(); iterator.hasNext();) {
						...
						DgResultEntryHeader dgResultEntryHeader=(DgResultEntryHeader)object[0];
						DgSampleCollectionHeader dgSampleCollectionHeader=(DgSampleCollectionHeader)object[1];
						if(dgResultEntryHeader.getDgMasInvestigation().getChargeCode().getId()==chargeCodeId){
						Visit visit2=(Visit)object[2];
						if(dgResultEntryHeader.getDgMasInvestigation().getConfidential() != null
								&& !dgResultEntryHeader.getDgMasInvestigation().getConfidential().equals("")
								&& !dgResultEntryHeader.getDgMasInvestigation().getConfidential().equalsIgnoreCase("n")){
							confidential = "y";
						}else{
							confidential = "n";
						}
						if(dgResultEntryHeader.getDgMasInvestigation().getInvestigationType() != null){
							resultType = dgResultEntryHeader.getDgMasInvestigation().getInvestigationType();
						}
						Set<DgResultEntryDetail>dgResultEntryDetailSet = new HashSet<DgResultEntryDetail>();
						dgResultEntryDetailSet = dgResultEntryHeader.getDgResultEntryDetails();
						for(DgResultEntryDetail dgResultDetail : dgResultEntryDetailSet){
							dgResultEntryDetailId =dgResultDetail.getId(); 
						}
						System.out.println("dgResultEntryDetailId===="+dgResultEntryDetailId);
						
					//	dgResultEntryDetailId = dgResultEntryDetail.getId();
						resultid = dgResultEntryHeader.getId();
...
<%-- 	
	...
	<%
		...
		if(resultList.size()>0){
			for (Iterator iterator = resultList.iterator(); iterator.hasNext();) {
				...
				DgSampleCollectionHeader dgSampleCollectionHeader=(DgSampleCollectionHeader)object[1];
				Visit visit2=(Visit)object[2];
				if(dgSampleCollectionHeader.getOrder()!=null){
					%>
					<td><input type="text" name="resultTest<%=inc%>"
			tabindex="1" value="<%=dgSampleCollectionHeader.getOrder().getOrderNo() %>" /></td>
					<%
				}
			}
		}else{
	...
	--%>
	<td>
	<input  id="Result<%=inc%>" type="button" tabindex="1" name="Result<%=inc%>"  value="Result" class="button" onclick="clearTestDivDown('<%=resultid%>',<%=dgResultEntryDetailId%>,'<%=resultType%>','<%=orderStatus%>','<%=confidential%>');" />
...
/*function openPopUp()
{
	var chargeCodeId = document.getElementById('chargeCodeId').value;
	var orderNo = document.getElementById('orderNo').value;
		window.open('opd?method=showInvestigationResult&orderNo='+orderNo+'&chargeCodeId="+chargeCodeId,'mywindow','location=1,status=1,scrollbars=1,top=210,left=30,width=960,height=300');
