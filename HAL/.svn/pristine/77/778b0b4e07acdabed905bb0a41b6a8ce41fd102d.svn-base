...
<input type="button" name="add" style="display: inline" value="Add" class="button"	id="addbutton" onClick="submitForm('brand','personnel?method=addBrand');"	accesskey="a" tabindex=1 />
	...
	<input type="reset" name="Reset" value="Reset" class="button"  onClick="submitForm('brand','personnel?method=showBrandJsp')" accesskey="r" />
...
data_arr[<%= counter%>][3] = "<%= masStoreBrand.getLastChgBy() %>"
<%if(masStoreBrand.getLastChgDate()!=null){%>
data_arr[<%= counter%>][4] = "<%= HMSUtil.convertDateToStringWithoutTime(masStoreBrand.getLastChgDate()) %>"
<%}%>
data_arr[<%= counter%>][5] = "<%= masStoreBrand.getLastChgTime() %>"
