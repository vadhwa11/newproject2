...
<%-- 	  <label>Treatment Advice Name</label>    
					 <input type="text" id="searchField" name="<%= SEARCH_NAME%>" value=""  validate="Treatment Advice,string,no"   tabindex=1 />
                    <input type="button" name="search" value="Search" class="button" onclick="submitForm('search','opdMaster?method=searchTreatmentAdvice','checkSearch')" tabindex=1  />
                    - Report Button  
                    
<input	type="hidden" name="templateCode" value="TA"/>
...
<input type="button" name="search" value="Search" class="button" onclick="submitForm('search','opdMaster?method=searchTreatmentAdvice','checkSearch')" tabindex=1  />
<input type="button" name="Report" value="Generate Report" class="buttonBig" onClick="submitForm('search','hospital?method=generateReportForGeneralMastersPatientFamilyHistory');"	accesskey="g" tabindex=1 /> 
<input type="hidden" name="<%=JASPER_FILE_NAME%>" value="Mas_Treatment_Advice"/>
</form>
                    </div>
...
<div class="Clear"></div>
   ...
   <h4> <a href="opdMaster?method=showTreatmentAdviceJsp">Show All Records</a></h4>
	<%
			}
		 }
  ...
  <input type="hidden" name="<%=JSP_NAME %>" value = "treatmentAdvice">
  <input type="hidden" name="pojoPropertyName" value = "PatientPresentComplaintName">
<div class="Clear"></div>	  		
 	<div class="Block">

	  	
  		<label class="auto" >Treatment Advice Name <span>*</span> </label>
...
<div class="Clear"></div>
			...
			<div class="Clear"></div>
			<input type="button" name="add" id="addbutton" value="Add" class="button" onClick="submitForm('unit','opdMaster?method=addTreatmentAdvice');" accesskey="a" tabindex=1/>
			<input type="button" name="edit" id="editbutton" value="Update" class="button" onClick="submitForm('unit','opdMaster?method=editTreatmentAdvice')" accesskey="u" tabindex=1 />
			<input type="button" name="Delete" id="deletebutton" value="Activate" class="button" onClick="submitForm('unit','opdMaster?method=deleteTreatmentAdvice&flag='+this.value)" accesskey="d" tabindex=1/>		
...
data_header[0][0] = "Treatment Advice Name "
