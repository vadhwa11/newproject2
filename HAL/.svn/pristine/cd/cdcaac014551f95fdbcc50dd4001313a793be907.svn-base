...
<div class="Block">
<label>Complaint Code</label><input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO  %>" value="1" checked="checked" /> <label>Complaint
Name</label> <input type="radio" class="radioAuto"
	name="<%=SELECTED_RADIO %>" value="2" />  <input type="text" id="searchField" name="<%= SEARCH_FIELD%>"
	value="" validate="Complaint Code,string,no" MAXLENGTH="8" tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=searchComplaint')" />
<input type="button" name="search" value="Search" class="button"
	onclick="submitForm('search','hospital?method=searchComplaint','checkSearch')"
	...
	name="<%=JASPER_FILE_NAME%>" value="Mas_complaint"></div></form>
</div>
<div class="Clear"></div>
</div>
<div class="Clear"></div>
...
<form name="complaint" method="post" action=""><input
	type="hidden" name="<%= POJO_NAME %>" value="MasComplaint"> <input
	type="hidden" name="<%=POJO_PROPERTY_NAME %>" value="ComplaintName">
<input type="hidden" name="title" value="Complaint"> <input
...
<div class="Clear"></div>
<div class="Block">
<label>Complaint Code <span>*</span> </label> <input id="codeId" type="text"
	name="<%= CODE%>" value="" validate="Complaint Code,string,yes"
	 MAXLENGTH="8" / tabindex=1> <label> Complaint Name <span>*</span> </label>
<input type="text" name="<%= SEARCH_NAME %>" value=""
	validate="Complaint Name,string,yes" 
	MAXLENGTH="30" / tabindex=1
	onkeypress="return submitenter(this,event,'hospital?method=addComplaint')">
<script>
...
data_arr[<%= counter%>][3] = "<%= masComplaint.getLastChgBy() %>"
