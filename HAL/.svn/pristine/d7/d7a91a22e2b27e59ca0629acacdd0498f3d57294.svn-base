...
<input type="button" name="submit" id="addbutton"
	onclick="submitForm('patientSearch','/hms/hms/lab?method=searchPatientForOrderUpdate','check()');"
	value="Search" class="CmnButton" accesskey="a" /> <jsp:include
	page="searchResultBlock.jsp" />
<div class="division"></div>
<div id="searchresults" tabindex="2">
<div id="searchtable" tabindex="2"></div>
<form name="updateOrder" method="post" action=""></form>
...
data_header[11][0] = "VisitId"
data_header[11][1] = "hide";
data_header[11][2] = "15%";
data_header[11][3] = "visitId";
data_arr = new Array();
	<%
	    int  counter=0;
	...
	<% 	
		for(DgOrderhd dgOrderhd: patientDetailList){
			...
			%>
			 ...
			 <%   try{
					if(patient.getSFirstName() != null  && !(patient.getSFirstName().equals(""))){
						...
						data_arr[<%= counter%>][12] = "<%=dgOrderhd.getVisit().getId()%> "
			...
			if(deptType.equalsIgnoreCase("RADIO")){ 
				for(DgOrderdt orderDt : set){
					if(orderDt.getMainChargecode().getDepartment().getDepartmentType().getDepartmentTypeCode().equalsIgnoreCase("RADIO")){
						flag = "radio";
					}
				}
