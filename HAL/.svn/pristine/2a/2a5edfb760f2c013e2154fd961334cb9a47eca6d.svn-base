...
public class AgendaPointUpdateSearchDataServiceImpl extends HibernateDaoSupport
		implements AgendaPointUpdateSearchDataService {
	...
	public Map<String, Object> showAgendaPointSearchJsp() {
		...
		agendaPointsList = getHibernateTemplate()
				.find(
						"from jkt.hms.masters.business.MasAgendaPointForWorkServices as mr where mr.Status = 'a'");
		//System.out.println("size" + agendaPointsList.size());
		// TODO Need to check
	...
	public Map<String, Object> searchAgendaPointSearch(
		...
		try {
			...
			crit = crit.add(Restrictions.eq("Status", "a"));
			if (fromDate != null) {
