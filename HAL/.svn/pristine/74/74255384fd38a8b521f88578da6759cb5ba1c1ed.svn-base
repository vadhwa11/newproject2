...
<%@page import="jkt.hms.masters.business.TherapyHeader"%>
...
<%
	...
	String fromDate = (String)utilMap.get("currentDate");
	String toDate = (String)utilMap.get("currentDate");

    if(map.get("FromDateId")!=null)
	{
    	
    	 fromDate = (String)map.get("FromDateId");
    	 //toDate = (String)map.get("ToDateId");
	}
...
System.out.println("departmentCodeForPhychiatrist "+departmentCodeForPhychiatrist +"de "+departmentCode +"fg "+departmentCodeForPhychiatrist.equalsIgnoreCase(departmentCode));
if(pendingNursingList!=null && pendingNursingList.size()>0){ 
	
%>
<h4><%=departmentCodeForPhychiatrist.equalsIgnoreCase(pendingNursingList.get(0).getDepartment().getDepartmentCode())?"Therapy":dentalDepartmentCode.equalsIgnoreCase(pendingNursingList.get(0).getDepartment().getDepartmentCode())?"Dental Procedure":"" %> Waiting List</h4>
<%} 

System.out.println("dsf"+dentalDepartmentCode.equalsIgnoreCase(departmentCode) +" departmentCode="+departmentCode +"jj"+dentalDepartmentCode);
System.out.println("dsf"+departmentCodeForPhychiatrist.equalsIgnoreCase(departmentCode) +" departmentCode="+departmentCode +"jj"+dentalDepartmentCode);
%>

<%if((departmentCodeForPhychiatrist.equalsIgnoreCase(departmentCode)) || (dentalDepartmentCode.equalsIgnoreCase(departmentCode))){%>
<form name="therapy_Waiting_List" method="post" action="">
...
<img src="/hms/jsp/images/cal.gif"	width="16" height="16" border="0" validate="Pick a date"
	onclick="setdate('<%=date%>',document.therapy_Waiting_List.FromDateId,event);" />

<%-- <label>To Date</label>
<input type="text" id="ToDateId"name="ToDateId" class="dateTextSmall" value="<%=toDate%>" readonly="readonly">
<img src="/hms/jsp/images/cal.gif"	width="16" height="16" border="0" validate="Pick a date"	
	onclick="setdate('<%=date%>',document.therapy_Waiting_List.ToDateId,event);" />
</div>  --%>
<!-- <input type="button" name="search" value="Search" class="button"
onClick="if(datevalidation()){submitForm('therapy_Waiting_List','/hms/hms/opd?method=getOPNursingCareWaitingList');}" /> -->
	<!-- onClick="if(datevalidation()){submitForm('op_nursingcare_Waiting_List','/hms/hms/opd?method=getPendingInjectionList');}" /> -->
	 <%if(departmentCodeForPhychiatrist.equalsIgnoreCase(departmentCode)){%>
     <input type="button" name="search" value="Search" class="button"  onClick="submitForm('therapy_Waiting_List','/hms/hms/opd?method=getTherapyWaitingList');" />

 <%}else if(dentalDepartmentCode.equalsIgnoreCase(departmentCode)){%>
  <input type="button" name="search" value="Search" class="button"  onClick="submitForm('therapy_Waiting_List','/hms/hms/opd?method=getDentalProcedureWaitingList');" />
 <%} %>
</form>
<%} %>
</div>
...
<div class="Blockdiv1"  id="searchtable" class="small" tabindex=2></div>
</div>
<div id="edited"></div>
...
/* 	data_header[8] = new Array;
	...
	/* 	if(patientHin.getPatientCategory()!=null){
		...
		if(visit.getDoctor()!=null){
			...
			if(visit.getDoctor().getDepartment()!=null && visit.getDepartment().getDepartmentName()!=null)
		...
		priority=String.valueOf(visit.getPriority());
		
		Date therapyDate =null;
		if(visit.getTherapyHeaders()!=null && visit.getTherapyHeaders().size() >0)
			  therapyDate = ((TherapyHeader)(visit.getTherapyHeaders().toArray()[0])).getProcedureDate();
		...
		data_arr[<%= i%>][4] = "<%=HMSUtil.convertDateToStringWithoutTime(therapyDate)%>" 
		data_arr[<%= i%>][5] = "<%=patientName%> " 
		data_arr[<%= i%>][6]="<%=age%>"
		data_arr[<%= i%>][7] = "<%=sex%>"
		data_arr[<%= i%>][8] = "<%=doctorName%>"
		data_arr[<%= i%>][9] = "<%=departmentName%>"
