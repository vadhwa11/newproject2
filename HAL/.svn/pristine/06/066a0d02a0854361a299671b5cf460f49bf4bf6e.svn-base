...
function add()
{
	...
	{
		var url="/hms/hms/hrOrderly?method=showOfficerDutyAddJsp&entryNo="+guardDuty.entryNo.value+"&entryDate="+guardDuty.entryDate.value+"&dutyId="+guardDuty.dutyId.value+"&toDate="+guardDuty.toDate.value+"&fromDate="+guardDuty.fromDate.value;
...
function GoPage() {	
	...
	submitForm('guardDuty','hrOrderly?method=getGridDataForOfficerEmployee');
}
...
function jsUpdate()
{
		...
		{
		guardDuty.method="post";
		submitForm('guardDuty','hrOrderly?method=updateOfficerDutyEntry','checkForNextDutyDate');
		}
		else
...
function jsDisplay() {
	...
	submitForm('searchPanel','hrOrderly?method=searchOfficerDutyData&searchEntryNo='+searchPanel.searchEntryNo.value+'&searchEntryDate='+searchPanel.searchEntryDate.value);
}  
...
<h6>Orderly Officer Duty Entry(CPL & Below)</h6>
<div class="Clear"></div>
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%
			    	int iFirstRow=0;
			    for(HrorderlyOfficerDutyEntry hrorderlyOfficerDutyEntry:dutyEntryList)
			    { 
			    	iFirstRow++;
			    %>
		<tr>
...
<table width="98%" colspan="7" id="tblSample" cellpadding="0"
	...
	<tbody>
		...
		<%
			    	int iFirstRow=0;
			    for(HrorderlyOfficerDutyEntry hrorderlyOfficerDutyEntry:searchGuardDutyList)
			    { 
			    	iFirstRow++;
			    %>
		<tr>
