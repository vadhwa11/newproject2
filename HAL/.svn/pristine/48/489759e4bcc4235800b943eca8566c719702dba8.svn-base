...
function GoPage() {	
	var pgno = parseInt(wardDutyAddForm.gopage.value);
	var totalPages = parseInt(wardDutyAddForm.totalPages.value);
	if (pgno < 1 || pgno > totalPages)
	{
	alert('Invalid Page No!.....');
	return;
	}  
	wardDutyAddForm.pageno.value = pgno; 
	wardDutyAddForm.method="post";
	submitForm('wardDutyAddForm','hrRelated?method=getGridDataForWardEmployeeAdd');
}

function goNext()
{
 var pgno = parseInt(wardDutyAddForm.pageno.value)+1;
 
 if (pgno > wardDutyAddForm.totalPages.value)
 {
...
{
	if (wardDutyAddForm.<%=EMPLOYEE_TO_BE_ADDED%>.length)
	{
			 for(var i = 0; i < wardDutyAddForm.<%=EMPLOYEE_TO_BE_ADDED%>.length; i++)
			 {
			  if (wardDutyAddForm.<%=EMPLOYEE_TO_BE_ADDED%>[i].checked == true)
             		return true;
			 }
	}
	else
	{
		if (wardDutyAddForm.<%=EMPLOYEE_TO_BE_ADDED%>.checked == true)
			return true;
	}
	return false;
}
...
function jsAdd()
{
		if (validateButton())
		{
			...
			{
				submitForm('wardDutyAddForm','hrRelated?method=AddWardDutyEntry');
...
function jsSearch()
{
	if(wardDutyAddForm.searchServiceNo.value!="" || wardDutyAddForm.searchName.value!="" )
	{
		wardDutyAddForm.method="post";
		submitForm('wardDutyAddForm','hrRelated?method=searchWardDutyEntry');
	}
	else
	...
	function jsClose()
	{
	  window.opener.location.href = "hrRelated?method=showWardDutyRecords&toDate="+wardDutyAddForm.toDate.value+"&fromDate="+wardDutyAddForm.fromDate.value;
    ...
    {
    	wardDutyAddForm.method="post";
 		submitForm('wardDutyAddForm','hrRelated?method=getWardEmployeeLastDutyDetails&empData='+empData);
    }
...
<h6>Employee Addition for Ward Duty</h6>
<div class="Clear"></div>
<form name="wardDutyAddForm"><input type="hidden"
...
<table align="center" width="100%" colspan="3" border="0"
	...
	<tbody>
		...
		<tr>
			...
			<td><%=hrWardDutyEntry.getDutyTime().getDutyFromTime()%>-<%=hrWardDutyEntry.getDutyTime().getDutyToTime()%></td>
		</tr>
		<%}
	...
	<tbody>
		<%
			    if(employeeLeaveList!=null && employeeLeaveList.size()>0){
			    	for(HrorderlyLeaveApplication hrLeaveMaintenance:employeeLeaveList){
			    	if(hrLeaveMaintenance.getLeaveFromDate().after(HMSUtil.convertStringTypeDateToDateType((String)utilMap.get("currentDate")))){
			    %>
		<tr>
			<td><%=HMSUtil.convertDateToStringWithoutTime(hrLeaveMaintenance.getLeaveFromDate()) %></td>
