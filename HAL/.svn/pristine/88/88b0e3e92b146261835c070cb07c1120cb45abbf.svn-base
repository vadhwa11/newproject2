...
<%
	...
	if(session.getAttribute("userName") != null){
	 ...
	 {
	 searchMeScaleList = (List<StoreMeScaleDetails>)map.get("searchMeScaleList");
	 }
...
<div class="titleBg"><h2>AF SCALE ENTRY</h2></div>
<div class="clear"></div>
...
<%if(searchMeScaleList.size()>0){
	if(searchMeScaleList.get(0).getMeScale().getMeScale()!=null)
	{
	%>
	<input type="hidden" name="headerId" value="<%=searchMeScaleList.get(0).getMeScale().getId()%>">
<label >AF Scale No.<span>*</span></label>
<input id="codeId" type="text" name="<%= ME_SCALE_NUMBER%>" value="<%=searchMeScaleList.get(0).getMeScale().getMeScale()%>" validate="AF Scale No,alphanumeric,yes"  MAXLENGTH="30"x tabindex=1 />
<%}else{ %>
<label >AF Scale No.<span>*</span></label>
...
<%if(searchMeScaleList.size()>0){
	if(searchMeScaleList.get(0).getMeScale().getMeScaleDescription()!=null)
	{
	%>
<label ><span>*</span> AF Scale Description </label>
<input type="text" name="<%= ME_SCALE_DESCRIPTION %>" value="<%=searchMeScaleList.get(0).getMeScale().getMeScaleDescription()%>" validate="AF Scale Description,alphanumeric,yes"  MAXLENGTH="50" tabindex=1)" />
<%}else{%>
<label ><span>*</span> AF Scale Description </label>
...
<div class="clear"></div>
<input type="button" name="add" id="addbutton" value="Update" class="button" onClick="if(checkForItemDetailsGrid()){submitForm('meScale','pharmacy?method=updateMeScale');}" accesskey="a" tabindex=1/>
...
<table border="0" align="center" cellpadding="0" cellspacing="0" id="grid">
	...
	{
	int incr=1;
	for(int i=0;i<searchMeScaleList.size();i++)
	{
	%>
	<input type="hidden" name="detailId" id="detailId" value="<%=searchMeScaleList.get(i).getId()%>"/>
<tr>
		<input type="hidden" name="gridSize" id="gridSize" value="<%=searchMeScaleList.size()%>"/>
		
	<td>
		
	    <input type="text" value="<%=searchMeScaleList.get(i).getSerialNo()%>" tabindex="1" id="allergyTypeId<%=incr%>" size="5"  name=<%=SR_NO%> />
	    
	<td width="10%">
	     <input type="text"  name="<%=ITEM_CODE %>" tabindex="1" size="10" id="codeItem<%=incr%>" value="<%=searchMeScaleList.get(i).getItem().getPvmsNo() %>" />
	      <input type="hidden"  name="idItem" value="<%=searchMeScaleList.get(i).getItem().getId()%>" tabindex="1" id="idItem<%=incr%>"/>
      
      </td>
      
      </td>
		<td width="17%">
      	<input type="text" size="30" value="<%=searchMeScaleList.get(i).getItem().getNomenclature()%>" 	id="nameItem<%=incr%>" onblur="checkForMeScale(this.value, 'nameItem1','1');"  name="nameItem" tabindex="1" />
			<div id="ac2update" style="display:none;" class="autocomplete"></div>
			<script type="text/javascript" language="javascript" charset="utf-8">
			new Ajax.Autocompleter('nameItem<%=incr%>','ac2update','nonExp?method=getItemListForMeScale',{parameters:'requiredField=nameItem'});
			</script>
		</td> 
     
      <td width="10%">
      <input type="text" value="<%=searchMeScaleList.get(i).getItem().getItemConversion().getIssueUnit().getUnitName()%>"  size="10" readonly="readonly" name="<%=AV%>" id="idAu<%=incr%>" tabindex="1" validate="A/U ,String,no"/>
      </td>
     
	
	<%if(searchMeScaleList.get(i).getQty().toString()!=""){ %>
	<td><input type="text"  value="<%=searchMeScaleList.get(i).getQty()%>" name="qty" tabindex="1" id="qty"  size="10"	maxlength="3"  /></td>
	<%}else{ %>
