...
<div>
<!--<input	name="investigationTemplate" type="button"	value="Previous Visits" tabindex="1" class="buttonBig2" onClick="submitForm('medicalExamPrimaryExtnAA','opd?method=showPatientPreviousVisitForViewScreen&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>&token=<%=visit.getTokenNo()%>');" />
<input	name="investigationTemplate" type="button"	value="Previous Medical Exams" tabindex="1" class="buttonBig2" onClick="submitForm('medicalExamPrimaryExtnAA','medicalExam?method=getPrevMedExamFromHIC&serviceNo=<%=visit.getHin().getServiceNo() %>');" />
<input	name="investigationTemplate" type="button"	value="Previous Medical Boards" tabindex="1" class="buttonBig2" onClick="submitForm('medicalExamPrimaryExtnAA','medicalExam?method=getPrevMedBoardFromHIC&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>&serviceNo=<%=visit.getHin().getServiceNo() %>');" />
<input	name="investigationTemplate" type="button"	value="Previous Hospitalizations" tabindex="1" class="buttonBig2" onClick="submitForm('medicalExamPrimaryExtnAA','opd?method=showPatientPreviousVisitForHospitality&visitId=<%=visit.getId()%>&visitNo=<%=visit.getVisitNo()%>&<%=HIN_ID%>=<%=visit.getHin().getId()%>');" />
-->
...
<form name="medicalExamPrimaryExtnAA" action="" method="post">
<!--Block One Starts-->
<%
int medExamId = 0;
...
<div class="clear"></div>
 ...
 %>
  <input tabindex="1" name="<%=DATE_OF_BIRTH %>"  value="<%= HMSUtil.convertDateToStringWithoutTime(visit.getHin().getDateOfBirth()) %>" maxlength="10"
 ...
 <%
 String tradeName = "";
 float serviceYears = 0.0f;
 String servicePeriod = "";
 if(visit.getHin().getServiceYears()!= null)
 {
	 serviceYears=visit.getHin().getServiceYears(); 
 }
 ...
 <input	type="text" readonly="readonly" value="<%= serviceYears+" "+servicePeriod%>" name="<%=TOTAL_SERVICE%>"	tabindex="2" maxlength="20" />
...
<% if(medExamObj.getEvidenceOfTrachoma()!=null){%>
 <textarea rows="" cols=""
	name="<%=ANY_EVIDENCE_OF_TRACHOMA %>" disabled="disabled" class="large" 
 ...
 <% }else{%>
<textarea rows="" cols="" name="<%=ANY_EVIDENCE_OF_TRACHOMA %>" class="large" disabled="disabled"  
onkeyup="chkLength(this,30);" onpaste="return checkOnPaste(this)"	oninput="return checkMaxLengthMoz(this)"	onKeyDown="return checkMaxLength(this)" ></textarea>
 <% }%>
<div class="clear"></div>
...
<table width="50%" border="0" cellspacing="0" cellpadding="0">
	...
	<td>
		...
		<%}else{ %>
		<input tabindex="1" value="600" type="text" readonly="readonly" maxlength="20" class="auto" id="hrfw" name="<%=HEARING_R_F_W %>" onkeyup="isNumber1(this)"	maxlength="6" onblur="checkForWiegth(this.value,id);" />
	...
	<td>
		...
		<%}else{ %>
		<input tabindex="1" value="600" readonly="readonly" type="text" maxlength="20" class="auto" id="hlfw" name="<%=HEARING_L_F_W %>" onkeyup="isNumber1(this)"	maxlength="6" onblur="checkForWiegth(this.value,id);" />
	...
	<td>
		...
		<%}else{ %>
		<input tabindex="1" value="600" readonly="readonly" type="text" maxlength="20" class="auto" size="15" id="bothfw"	name="<%=HEARING_BOTH_FW %>" onkeyup="isNumber1(this)" maxlength="6" onblur="checkForWiegth(this.value,id);" />
...
<input tabindex="1" type="text" id="height" class="date" size="" readonly="readonly"	name="<%=HEIGHT_WITHOUT_SHOOSE %>" onkeyup="isNumber1(this)" value="<%=medExamObj.getHeight() %>"
	maxlength="6" onblur="checkForWiegth(this.value,id);;calculateBMI();" /><label class="unit">cm</label>

 <% }else{%>
<input tabindex="1" type="text"  id="height" readonly="readonly"	class="date"	size=""  name="<%=HEIGHT_WITHOUT_SHOOSE %>" onkeyup="isNumber1(this)"
	maxlength="6" onblur="checkForWiegth(this.value,id);calculateBMI();" /><label class="unit">cm</label>
...
<input tabindex="1" type="text" readonly="readonly"  id="weight" class="date" size=""	name="<%=ACTUAL_WEIGHT %>" maxlength="6" value="<%=medExamObj.getWeight() %>"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);" /><label class="unit">kg</label>

 <% }else{%>
<input tabindex="1" type="text" readonly="readonly" id="weight" class="date" size="" 	name="<%=ACTUAL_WEIGHT %>" maxlength="6"
	onKeyUp="limitText(this,6);" onblur="checkForWiegth(this.value,id);"/><label class="unit">kg</label>
 <% }%>
...
<input tabindex="1" type="text" readonly="readonly" id="fullExpiration" name="fullExpiration" maxlength="20" value="<%=medExamObj.getFullExpiration() %>"
	onKeyUp="limitText(this,20);" class="auto" size="17"  /><label class="auto">cm</label>
<%}else{ %>
<input tabindex="1" type="text" readonly="readonly" id="fullExpiration" name="fullExpiration" maxlength="20"
	onKeyUp="limitText(this,20);" class="auto" size="17"  /><label class="auto">cm</label>

<%} %>
	<label class="auto">Range of Expansion</label>
...
<input tabindex="1" type="text" readonly="readonly" id="<%=RANGE_EXPANSION %>" name="<%=RANGE_EXPANSION %>" maxlength="5" value="<%=medExamObj.getRangeofexpansion() %>"
	onKeyUp="limitText(this,6);" class="auto" size="17" /><label class="auto">cm</label>
...
<input tabindex="1" type="text" readonly="readonly" id="<%=RANGE_EXPANSION %>" name="<%=RANGE_EXPANSION %>" maxlength="5"
 ...
 <% }%>
	...
	onKeyUp="limitText(this,20);" class="auto" size="16"  />
...
<input tabindex="1" type="text" readonly="readonly" id="albumin" name="<%=ALBUMIN %>" maxlength="20" value="<%=medExamObj.getAlbumin() %>"
	onKeyUp="limitText(this,6);" class="" size="20"  />
...
<input tabindex="1" type="text" readonly="readonly" id="sugar" name="<%=SUGAR %>" maxlength="20" value="<%=medExamObj.getSugar() %>"
	onKeyUp="limitText(this,20);" class="date" size=""  />

 <% }else{%>
<input tabindex="1" type="text" readonly="readonly" id="sugar" name="<%=SUGAR %>" maxlength="20" value="ND"	onKeyUp="limitText(this,20);"  class="date" size="" />
...
<input tabindex="1" type="text" readonly="readonly" id="otherAbnormalities" name="otherAbnormalities" maxlength="20" value="<%=medExamObj.getAnyOtheAbnormalities() %>"
	onKeyUp="limitText(this,20);" class="" size="20"  />

 <% }else{%>
<input tabindex="1" type="text" readonly="readonly" id="otherAbnormalities" name="otherAbnormalities" maxlength="20" value="Nil"	onKeyUp="limitText(this,20);"  class="" size="20" />
 <% }%>
...
<input tabindex="1" type="text" readonly="readonly" id="anyEvidenceOfSkin" name="anyEvidenceOfSkin" maxlength="100" value="<%=medExamObj.getAnyEvidenceOfSkin() %>"
	onKeyUp="limitText(this,100);" class="auto" size="74"  />

 <% }else{%>
<input tabindex="1" type="text" readonly="readonly"  id="anyEvidenceOfSkin" name="anyEvidenceOfSkin" maxlength="100"  value="Nil"	onKeyUp="limitText(this,100);"  class="auto" size="74" />
 <% }%>
</div>
</div>
<div class="clear paddingTop15"></div>
...
<label>Pulse</label>
 <% if(medExamObj.getPulseRates()!=null){%>
  <input tabindex="1" type="text" readonly="readonly" name="<%=PULSE_RATES%>" class="date" size="" maxlength="20"  value="<%=medExamObj.getPulseRates() %>"/><label class="unit">/min</label>
 <% }else{%>
 <input tabindex="1" type="text" readonly="readonly" name="<%=PULSE_RATES%>" class="date" size="" maxlength="20" /><label class="unit">/min</label>
 <% }%>

 <label>BP</label>
<% if(medExamObj.getBp()!=null){%>
  <input tabindex="1" type="text" readonly="readonly" name="<%=BP1%>" class="date" size="" maxlength="20"  value="<%=medExamObj.getBp() %>"/><label class="unit">mm Hg</label>
 <% }else{%>
 <input tabindex="1" type="text" readonly="readonly" name="<%=BP1%>" class="date" size="" maxlength="20" /><label class="unit">mm Hg</label>
 <% }%>
...
<%} %>
	...
	onKeyUp="isNumber(this);" maxlength="2" class="autoArial" size="29" />
...
<input	tabindex="1" type="text" readonly="readonly" name="<%=CONDITION_OF_GUMS %>" value="<%=medExamObj.getConditionOfGums() %>"
	class="autoArial" size="29" onchange="return CheckAlpha(event);"  id="txtAlpha" maxlength="2" validate="Condition Of Gums,Alphabetic,Yes" />
...
<input name="allergies" type="text" tabindex="1" class="auto" value="<%=visit.getHin().getDrugAllergies() %>" maxlength="60" id="allergies" size="92"  readonly="readonly"/>
<%}else{ %>
<input name="allergies" type="text" tabindex="1" class="auto" value="" maxlength="60" id="allergies" size="92"readonly="readonly" />
<%} %>
</div>
<div class="clear paddingTop15"></div>
...
<textarea rows="" cols="35" disabled="disabled" class="auto" name="<%=DEFECTS_NOT_TO_CAUSE_REJECTION %>" onkeyup="chkLength(this,200);" tabindex="1"
...
<textarea rows="" cols="35" disabled="disabled" class="auto" name="<%=FOUND_FIT_IN_CATEGORY %>" onkeyup="chkLength(this,200);" tabindex="1"
...
<label >Remarks</label>
<% if(medExamObj.getPerAuthRemarks()!=null){%>
<textarea cols="20" rows="2" tabindex="1" maxlength="100" name="paRemarks" tabindex="1" readonly="readonly" onkeyup="chkLength(this,100);"
...
<input tabindex="1" name="Button"	type="button" class="button" value="VALIDATE"	onClick="submitForm('medicalExamPrimaryExtnAA','medicalExam?method=validateMedExamApprovingAuthority');" />
<input tabindex="1" name="Button"	type="button" class="button" value="REJECT"	onClick="if(checkRemarksAA()){submitForm('medicalExamPrimaryExtnAA','medicalExam?method=rejectMedExamAA');}" />
  ...
  function showHideInvestigationTemplateCombo(valueOfTemplate){
		...
		if(checkTemplateId(valueOfTemplate)){
		 ...
		 // 	document.getElementById("copyPrevInvestigationTemplateDiv").style.display='none';
				...
				submitProtoAjaxWithDivName('medicalExamPrimaryExtnAA','/hms/hms/opd?method=showGridForInvestigation','gridview');
...
function removeRowForInvestigation()
{
 ...
 function copyPrevInvestigationTempate(visitNo,hinId){
	    ...
	    submitProtoAjaxWithDivName('medicalExamPrimaryExtnAA','/hms/hms/opd?method=getPatientPreviousInvestigationForCopy&&visitNo='+visitNo+'&hinId='+hinId,'gridview');
}

 function getListForTreatment(val){
	 	if(val=='investigationDiv'){
			submitProtoAjaxWithDivName('medicalExamPrimaryExtnAA','/hms/hms/opd?method=getListForTreatment&flag=investigation',val);
		}
	 }
 function submitdata()
	{
  ...
  if(charge=="")
  {
   alert("Please Select Test Name");
  	...
  	submitForm('medicalExamPrimaryExtnAA','medicalExam?method=addMedicalExaminationBoardAnnual2A');
  }
	}
