...
<h6>Blood Issue Register</h6>
<form name="bloodIssueRegister" target="_blank" action="" method="post">
<div class="blockFrame"><label><span>*</span>From Date</label>
	<input type="text" class="calDate" id="lastDateId" name="<%=FROM_DATE %>" value="<%=currentDate %>"  validate="From Date,date,no" MAXLENGTH="10"/>
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.bloodIssueRegister.<%=FROM_DATE%>,event)"/>
	<label><span>*</span>To Date</label>
	<input type="text" class="calDate" id="lastDateId" name="<%=TO_DATE %>" value="<%=currentDate %>"  validate="To Date,date,no" MAXLENGTH="10"/>
	<img src="/hms/jsp/images/cal.gif" width="16" height="16" border="0" validate="Pick a date" onClick="setdate('<%=currentDate %>',document.bloodIssueRegister.<%=TO_DATE%>,event)"/></div>
<div class="Clear"></div>
