...
<div class="bottom"><input type="button" class="cmnButton"
	value="Submit"
	onclick="if(checkFilledRow()){submitForm('investigationOrderBooking','fwc?method=submitOrderBookingForInvestigation');}"
...
function clearAllFields(inc){
	...
	function  accepted(chkObj, inc) {
		...
		if(checkTemplateId(this.value)){
			submitProtoAjaxWithDivNameByNaresh('investigationOrderBooking','/hms/hms/fwc?method=showGridForOrderBooking','gridview');
		}
	}
	...
	function submitForBack(){
		...
		submitForm('investigationOrderBooking','fwc?method=showFWCMainJsp&visitId=<%=visitList.get(0).getId()%>&deptId=<%=deptId%>');
	}
</script></form>
</div>
