...
<h6>Total Order Booked Report</h6>
...
<div class="blockFrame"><label><span>*</span>From Date</label> <input
	...
	onClick="setdate('<%=currentDate %>',document.totalOrderBooked.<%=FROM_DATE%>,event)" />
...
<label><span>*</span>To Date</label> <input type="text" class="calDate"
	...
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.totalOrderBooked.<%=TO_DATE%>,event)" />
<div class="Clear"></div>
<label>Ward</label> <select name="<%=WARD_ID%>" id="<%=WARD_ID%>">
	<option value="0">Select</option>
	<%for(MasDepartment department : wardList){ %>
...
<div class="bottom"><input type="button" name="OK" value="OK"
	class="button"
	onClick="submitForm('totalOrderBooked','lab?method=showTotalOrderBookedReport','validateFromToDate');" />
