...
<%
	...
	System.out.println("u r in view patient issue jsp");
...
<div class="titleBg"><h2>View Stock Details</h2></div>
<div class="clear"></div>
...
<script type="text/javascript" language="javascript">
		...
		data_header[6][0] = "Quantity Issued"
		...
		data_header[8][0] = "Date Of Expiry"
		    ...
		    int  i=0;
		          while(iterator.hasNext())
		           {    
		        	  ...
		        	  String issueNo=storeIpIssueM.getIpIssueNo();
		...
		%>
			...
			<%
				...
				{
			%>
			data_arr[<%= i%>][2] = "<%=issueDateInString%>"
			<%
				}else{
			%>
			data_arr[<%= i%>][2] = ""
			...
			data_arr[<%= i%>][9] = "<%=expiryDateInString%>"
            <%
			   }else{
            %> 			
			data_arr[<%= i%>][9] = ""
