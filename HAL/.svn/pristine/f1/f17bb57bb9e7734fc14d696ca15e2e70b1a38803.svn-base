...
<h6>Journal/ Book Issue Register</h6>
<form name="bookIssueReg" target="_blank" action="" method="post">
...
<select name="<%= TYPE %>" id="<%=TYPE %>" tabindex=1
	...
	class="calDate" validate="From Date,date,yes" id="fromDateId"
	name="<%=FROM_DATE %>" value="<%=currentDate %>" readonly="readonly"
	MAXLENGTH="30" /> <img src="/hms/jsp/images/cal.gif" width="16"
	height="16" border="0" validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.bookIssueReg.<%=FROM_DATE%>,event)" />

<label> To Date</label> <input type="text" class="calDate"
	validate="From Date,date,yes" id="fromDateId" name="<%=TO_DATE %>"
	value="<%=currentDate %>" readonly="readonly" MAXLENGTH="30" /> <img
	src="/hms/jsp/images/cal.gif" width="16" height="16" border="0"
	validate="Pick a date"
	onClick="setdate('<%=currentDate %>',document.bookIssueReg.<%=TO_DATE%>,event)" />
...
<div class="bottom"><input type="button" name="OK" value="OK"
	class="button"
	onClick="submitForm('bookIssueReg','lib?method=generateIssueRegisterReport','validateFromToDate');" />
...
<script type="text/javascript" language="javascript">
	...
	function validateFromToDate(){
		...
		obj1 = eval(document.bookIssueReg.fromDate)
		obj2 = eval(document.bookIssueReg.toDate)
