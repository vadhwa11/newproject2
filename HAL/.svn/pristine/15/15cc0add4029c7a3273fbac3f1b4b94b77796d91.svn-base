...
<div class="bottom"><input type="button" name="yes" value="Yes"
	class="button"
	onclick="submitForm('message','/hms/hms/fwc?method=printSurgeryRequisitionForInPatient&orderNo=<%=orderNo %>','checkTargetForYes');" />
<input type="button" name="no" class="button" value="No"
	onclick="submitForm('message','/hms/hms/fwc?method=showFWCMainJsp&deptId=<%=deptId %>&visitId=<%=visitId %>','checkTargetForNo');" />
