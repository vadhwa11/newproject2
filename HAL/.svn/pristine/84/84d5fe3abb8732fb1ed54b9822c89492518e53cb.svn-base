...
<div class="titleBg"><h2>RC Issue</h2></div>
...
<%-- <select  name="<%=REQUEST_NO%>" tabindex="1" validate="Indent No. ,string,no" id="requestNo"  onchange="submitForm('issueDispensaryForm','stores?method=searchIndentDetails');">
	<option value="0">Select Request No</option>
	<%
...
<input type="text" name="requestNoDis"  id="requestNoDis" value="<%=indentTList.get(0)[9]%>" readOnly="readonly"/>
<input type="hidden" name="<%=REQUEST_NO%>"  id="requestNo" value="<%=indentTList.get(0)[5]%>" readOnly="readonly"/>
</div>

 <label>Request Date</label>
<input type="text" name="requestDate"  id="requestDate" value="<%=HMSUtil.convertDateToStringTypeDateOnly((Date)indentTList.get(0)[10])%>" readOnly="readonly"/>
...
<div id="testDivR">
     <label>Requested By<span>*</span>
     </label><label class="value"  ><%=indentTList.get(0)[14] + " " + indentTList.get(0)[15]%></label>
<input type="hidden" name="<%=REQUEST_BY%>" id="requestBy" validate="Request By,string,no" value="<%=indentTList.get(0)[11]%>"/>
<input type="hidden" name="<%=REQUEST_BY_NAME%>" id="requestByName" validate="Request By,string,no" value="<%=indentTList.get(0)[14]%>"/>
</div>
<div class="clear"></div>
...
<%for(MasDepartment masDept:deptList) { if(masDept.getId().equals(indentTList.get(0)[4])){%>
...
<select name="<%=RequestConstants.ISSUED_BY%>" tabindex="1" id="issuedBy" validate="Issued By,String,yes" >
					<%
						if (issuedBy == 0) {
					%>
...
<!-- 			<th width="13%">Qty</br>on Loan</th> -->
		 ...
		 <input type="hidden" name="tt6" id="quantityToIssueOverAll<%=inc%>" value="" readonly="readonly"/>
			...
			<%
				List lis = new ArrayList();
								...
								I = object[0];
			%>
			<select name="batchNo<%=inc%>" id="batchNo<%=inc%>"  class="small3" onchange="if(cheackForBatch(<%=inc%>)){getDataForBarcode(this.value,<%=inc%>)}" >
 ...
 %><input type="hidden" name="list<%=inc%>" id="list<%=inc%>" size="10" readonly="readonly" value="<%=lis1.size()%>" />
 ...
 %></td>
			...
			<td width="8%"><input type="text"  size="8" name="qtyAftrIssued<%=inc%>" id="qtyAftrIssued<%=inc%>" value="" readonly="readonly"/>
...
<!-- 	<tr>
	...
	<input type="button" class="button" tabindex="1" onclick="if(qtyValidation()){submitForm('issueDispensaryForm','stores?method=submitIssueForIndentForRC');}" name="Submit2" value="Submit"  />
 ...
 // for(var i = 0;i<li;i++ ){
  ...
  e242.size = '10';
	 ...
	 // e6.value=document.getElementById('stockAvailable'+rowVal).value;
	  ...
	  var cellRight351 = row.insertCell(13);
	 ...
	 /*  var cellRight10 = row.insertCell(15); */
