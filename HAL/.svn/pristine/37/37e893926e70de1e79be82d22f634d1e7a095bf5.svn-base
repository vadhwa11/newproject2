...
<td>
		...
		<input type="checkbox"	id ='selectedCharge<%=inc%>' value="<%=inc%>" name="selectedChrage" class="radioCheckCol2" onclick = "calculateTotalChargeAmount(<%=inc%>);"/></td>

		<td><input type="text" name="chargeCode<%=inc%>"	id="chargeCode<%=inc%>" style="width:250px"
			value="<%=masChargeCode.getChargeCodeName() %>[<%=masChargeCode.getChargeCodeCode() %>]"
		...
		<input type="hidden" id="chargeId<%=inc %>"
			...
			value="<%=masChargeCode.getId()%>" /></td>

		<td style = "display:none"><input type="text" size="5" id="qty<%=inc%>"name="<%=QUANTITY %><%=inc%>"
			 value="1" validate="Qty,int,no" maxlength="5" readonly="readonly"
			onblur="if(validateQtyForBilling(this.value,this.id,<%=inc %>)){calculateNetAmount(<%=inc %>);calculateTotalAmt();};"/></td>
			

			  
		<td  id="rateVal<%=inc %>">
		<input type="text" size="12" value="<%=masChargeCode.getCharge() %>" id="rate<%=inc%>" name="<%=RATE%><%=inc%>"
...
<%BigDecimal payableAmt = new BigDecimal(0);%>
 ...
 <input type="hidden" value="<%=empId%>" name="billTakenBy" id="cnsltDocId"/> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td><input type="text" name="<%=AMOUNT_RECEIVED %><%=i%>" 
			...
			maxlength="9"
			onblur="if(validateAmount(this.value,<%=i %>)); checkAmountToPay(<%=i %>);"/>
...
<input type="button" tabindex="1" class="button" name="Submit11"	value="Submit"	onclick="if(checkFilledRow()){if(validateFieldsOnSubmitForOtherPatients()){if(checkCollectedAmt()){submitForm('billServicing','billing?method=submitBillServicingDetailsOtherPatients','validatePaymentAmt','validateChequeAndCreditCardDate');}}}"
	align="right" /> 
...
%> <input type="button" tabindex="1" class="buttonBig" name="Submit12"
	...
	align="right" /> <%} %>  --%>
<input type="button" class="button" value="Back" onclick="submitFormCancel('billServicing','billing?method=showPendingBillOtherPatientJsp')" />
