...
<%
	...
	if(map.get("dgResultdetailSenList") != null){
		...
		System.out.print(dgResultdetailSenList.size());
	}
	try{
...
<%}else{ %> <label class="valuemedium">-</label> <% }%> <label>Service
No.</label> <%
...
Time</label> <label class="value"><%=dgresultHeader.getResultTime()%></label> <label>Result
Prepared By:</label> <%if(dgresultHeader.getEmployee() != null) {%> <label
...
<div class="Clear"></div>
<label>Result Validated Date</label> <label class="valuemedium"><%=HMSUtil.changeDateToddMMyyyy(dgresultHeader.getVerifiedOn())%></label>
...
for (MasEmployee masEmp : prescribedByList){
	...
	if(masEmp.getMiddleName()!=null){
		doctorMiddleName = masEmp.getMiddleName();	
	}
	if(masEmp.getLastName()!=null){
		doctorLastName = masEmp.getLastName();						
	}
	System.out.println("ID :"+masEmp.getId());

	if (dgResultEntryHeader.getPrescribedBy() != null && dgResultEntryHeader.getPrescribedBy().getId().equals(masEmp.getId())) {
	%>
	<option value=<%=masEmp.getId()%> selected="selected"><%=masEmp.getFirstName()+" "+doctorMiddleName+" "+doctorLastName%></option>
	<%
	}else {
	%>
	<option value=<%=masEmp.getId()%>><%=masEmp.getFirstName()+" "+doctorMiddleName+" "+doctorLastName%></option>
	<%
	}
}
