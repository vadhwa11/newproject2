...
System.out.println("reportType "+dailyIssueItem.get("objectList"));
List objectList = new ArrayList();
	...
	if (map.get("deptList") != null) {
		...
		System.out.println("deptList in jsp "+deptList.size());

	}
	if (dailyIssueItem.get("deptList") != null) {
		deptList = (List<MasDepartment>)dailyIssueItem.get("deptList");
		System.out.println("deptList dailyIssueItem in jsp "+deptList.size());
...
<%} %>
	...
	<label> Unit</label>
	<select id="unit" name="unit"   onchange=""  MAXLENGTH="25" tabindex=1>
	<option value="0">Select</option>
				...
				<input type="button" name="add" id="addbutton" value="Display" class="button" onClick="submitForm('proformaBApproval','stores?method=getPendingProformaForApproval');" accesskey="a" tabindex=1 />
...
<td width="5%"><input type="radio" name="pId"  id="pId" value="<%=sph.getId() %>" />
